function inlineIcon () {
  this.process((parent, target, attrs) => {
      ...
      if (attrs.set === 'plenty') {
          ...
          console.log(`asciidoctor: WARNING: 'set=plenty' is deprecated. Please use 'terra:' instead of 'icon:'.`)
      } else if (attrs.set === 'material') {
          const materialIconColour = attrs.role || '';
          theIcon = `<span class="icon ${materialIconColour}"><i class="material-icons">${icon}</i></span>`;
          console.log(`asciidoctor: WARNING: 'set=material' is deprecated. Please use 'material:' instead of 'icon:'.`)
      } else {
