...
* Mit Rollen gruppierst du Benutzer:innen-Konten und ordnest dieselben Berechtigungen zu mehreren Benutzer:innen-Konten.
...
Im Menü *Einrichtung » Kontverwaltung » Sicherheit* stellst du die Anforderungen ein, die Benutzer:innen erfüllen müssen, wenn sie ein neues Passwort vergeben.
...
|Nach Ablauf des Gültigkeitszeitraums in Tagen wird das Benutzer:innen-Konto gesperrt. Wenn diese Funktion nicht gewünscht ist, lass den Regler bei Null stehen.
...
Wenn diese Funktion nicht gewünscht ist, lass den Regler bei Null stehen.
|====
...
|Aktivieren, um das Benutzer:innen-Konto zu deaktvieren. Deaktivierte Personen können sich nicht mehr in das System einloggen.
...
| *Signatur*
| Text, der deinen Nachrichten und Kommentaren im Ticketsystem automatisch als Signatur angefügt wird.
|====

[#40]
=== Eigner aktivieren

Ob und in welchen Bereichen das Benutzer:innen-Konto als Eigner verfügbar sein soll, wird über bestimmte Berechtigungen im erweiterten Modus der Rollen bestimmt. Wenn du zum Beispiel eine Rolle erstellst, die Eigner-Rechte für den Wareneingang verfügt musst du im erweiterten Modus *Warenbestände* oder gezielter *Warenbestände » Wareneingänge* als Berechtigung freigeben. Welche Berechtigungen welche Eignerrechte aktiviert findest du unter <<tabelle-eignerrechte>>. Wenn das Benutzer:innenkonto über Adminrechte verfügt, verfügt es auch über alle Eignerrechte.
...
. Öffne eine Rolle.
. Wechsel auf den erweiterten Modus (material:toggle_on[]).
. Aktiviere die Bereiche, in denen die Rolle als Eigner verfügbar sein soll.
. *Speichere* (icon:save[]) die Einstellungen.

[[tabelle-eignerrechte]]
.Notwendige Berechtigungen für unterschiedliche Eignerrechte
[cols="1,3"]
|====
|Eignerrecht |Notwendige Berechtigung
...
Unter dem plenty p und deinem Profilbild findest du die *persönlichen Einstellungen*. Hier nimmst du Einstellungen vor, die nur für das im System angemeldete Benutzer:innen-Konto gelten. Deshalb ist das Tab für dich auch nur in deinem eigenen Benutzer:innen-Konto sichtbar. Die persönlichen Einstellungen unterteilen sich in die Tabs xref:willkommen:system-personalisieren.adoc#70[Allgemein], und xref:willkommen:system-personalisieren.adoc#80[Shortcuts] die nur dich persönlich betreffen.
...
| *Angefangene Box-Kommisionierungen zurücksetzen*
...
In plentymarkets werden Rechte über Rollen an Benutzer:innen vergeben. Dies beinhaltet Menü- und Plugin-Sichtbarkeiten sowie REST-API-Berechtigungen. Ein:e Benutzer:in hat also immer die Rechte, die von der Rolle zugewiesen werden. Die einzige Ausnahme dabei bilden Admins, die unabhängig von ihrer Rolle, alle Rechte im System besitzen.
Im Menü *Einrichtung » Kontoverwaltung » Rollen* können neue Rollen erstellt werden oder existierende Rollen angepasst werden. Beim Erstellen oder Bearbeiten einer Rolle kann im Menüpunkt *Zugriffsbeschränkung* die Auftragsherkünfte freigegeben werden, die für die Benutzer:innen sichtbar sein sollen. +
...
Die Tabelle der Benutzerrechte gliedert sich in vier Spalten in denen Rechte vergeben werden können durch das Anhaken einer Checkbox: *Betrachten*, *Bearbeiten*, *Erstellen*, *Löschen*. Zwei dieser vier Arten ändern im erweiterten Modus ihre Namen, aus *Betrachten* wird *Lesen* und aus *Bearbeiten* wird *Aktualisieren*.

[#106]
=== Einfacher und erweiterteter Modus

Beim Erstellen und Bearbeiten von Rollen gibt es zwei Modi, den einfachen und den erweiterten. Welcher Modus für welche Rolle relevant ist, kommt auf die Rechte an, die der Rolle zugewiesen werden sollen. Der einfache Modus und erweiterter Modus sind verknüpft. Wenn du also Menü-Sichtbarkeiten im einfachen Modus auswählst werden automatisch Berechtigungen im erweiterten Modus automatisch aktiv sein. Diese Verknüpfung existier allerdings nur diese Richtung, wenn du also im erweiterten Modus Berechtigungen vergibst werden keine Menü- oer Plug-In-Sichtbarkeiten automatisch aktiv sein.

**Einfacher Modus** = Im einfachen Modus kannst du die Sichtbarkeiten für Menüs und Plug-Ins vergeben. Die Tabelle des einfachen Modus ist genauso aufgebaut wie die Navigation und bildet zudem individuelle Plug-Ins ab.

**Erweiterter Modus** = Im erweiterteten Modus vergibst du ausschließlich Berechtigungen.

[#110]
=== Rollen vergeben

Eine Rolle vergibst du an ein Konto wie folgt:

. Öffne das Menü *Einrichtung » Kontoverwaltung » Konten*.
→ Die Übersicht wird geöffnet.
. Wähle ein Konto.
→ Die Konto-Detailansicht wird geöffnet.
. Wähle im Reiter *Rechte* das Dropdown-Menü *Zugewiesene Rollen*
→ Das Dropdown-Menü klappt aus.
. Wähle eine vorhandene Rolle oder erstelle eine neue auf *Neue Rolle erstellen ...* (material:add[]).

Sobald du die Rolle an das Konto vergeben hast, erhält das Konto die dazugehörigen Rechte.
...
. Öffne das Menü *Einrichtung » Kontoverwaltung » Rollen*. +
→ Die Übersicht wird geöffnet.
. Klicke auf material:search[], um alle Rollen zu sehen.
. Wähle eine Rolle. +
→ Der Einfache Modus mit den Rechten wird geöffnet.
...
. Klicke auf *Suchen* (icon:search[]), um nach Rollen zu suchen. +
→ Alle vorhandenen Rollen werden angezeigt.
. Wähle eine Rolle. +
→ Die Übersicht wird geöffnet.
. Wähle das Tab *Rollendefinierte Ansichten*. +
...
Unter *Einrichtung » Kontoverwaltung » Rollen » [Rolle auswählen]* legst du fest, welche Bereiche und Einstellungen die Person mit der jeweiligen Rolle sehen kann, wenn die Person ein Menü öffnet. Bei fehlender Berechtigung ist zum Beispiel ein Lager oder sind Aufträge in einem Status für diese Person nicht sichtbar.
...
Im erweiterten Modus legst du fest, welche Berechtigungen ein Benutzer:innen-Konto für die REST-API erhält. Benutzer:innen-Konten des Zugangs *Admin* verfügen über alle Berechtigungen.

[.instruction]
Menü- und REST-API-Rechte verwalten:

. Öffne das Menü *Einrichtung » Kontoverwaltung » Rollen*.
. Öffne eine Rolle oder erstelle eine neue.
. Wechsel auf *Erweitert* (material:toggle_on[]).
→ Die Tabelle des erweiterten Modus wird angezeigt.
. Vergib die Berechtigungen in dem jeweiligen Bereich durch Anklicken der Checkboxen. Aktiviere komplette Bereiche oder klappe Bereiche auf, um einzelne Berechtigungen zu aktivieren, die dem Benutzer:innen-Konto zur Verfügung stehen sollen.
. *Speichere* (icon:save[]) die Einstellungen.

[#125]
=== App-Berechtigungen aktivieren

Unter dem Punkt *Mobile* im einfachen Modus der Rollenerstellung bzw. -bearbeitung legst du fest, welche Bereiche und Kennzahlen für die Rolle mit de in der plentymarkets App sichtbar sein sollen.
...
Nachdem du eine Rolle erstellt hast, wird sie zur Bearbeitung geöffnet und der Rollenübersicht hinzugefügt, so dass du die Rolle auch später wieder öffnen und bearbeiten kannst. Rollen werden in der Übersicht nach ID gelistet. Jede Rolle gliedert sich in vier Tabs. Im Tab *Einstellungen* lässt sich der Name der Rolle ändern. Im Tab *Rechte* ordnest du der Rolle im Einfachen Modus Menü-Sichtbarkeiten sowie Plugin-Sichtbarkeiten zu, im Erweiterten Modus die REST-API Berechtigungen. In den *Rollendefinierten Ansichten* lassen sich die verfügbaren My-View-Ansichten für eine Rolle anpassen. Es ist automatisch immer die Standardansicht ausgewählt. Unter *Zugrifssrechte* lassen sich die Ressouren aktivieren, auf die eine Rolle Zugriff haben soll. Details zu den Berechtigungen findest du hier auf der Seite im Kapitel xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#105[Rechte verwalten].

Rollen können von Benutzer:innen des Zugangs *Admin* erstellt werden oder von Nutzer:innen, die eine Rolle mit den notwendigen Rechten hat.
...
. Klicke auf das Dropdown-Menü *Zugewiesene Rollen*
...
. Öffne das Menü *Einrichtung » Kontoverwaltung » Rollen*.
. Wähle eine Rolle.
. Klicke in der Zeile der Rolle auf material:more_vert[]. +
→ Ein Optionsfenster wird angezeigt.
. Wähle *Rolle duplizieren*. +
→ Die Rolle wird kopiert und mit dem Namen *Kopie von bisheriger Rolle* erstellt .

[#60]
== Benutzer:innen-Konto suchen

Filter helfen dir bei der Suche nach Benutzer:innen-Konten. Ohne Filter werden in der Übersicht alle Benutzer:innen-Konten angezeigt. Folgende Elemente und Funktionen sind im Menü *Konten* verfügbar.
