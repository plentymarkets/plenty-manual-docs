...
| *Signatur*
| Text, der Nachrichten und Kommentaren des:der Benutzer:in im Ticketsystem automatisch als Signatur angefügt wird.
|====

[#40]
=== Eigner aktivieren

Ob und in welchen Bereichen das Benutzer:innen-Konto als Eigner verfügbar sein soll, wird über bestimmte Berechtigungen im erweiterten Modus der Rollen bestimmt. Wenn du zum Beispiel eine Rolle erstellst, die Eignerrechte für den Wareneingang haben soll, musst du im erweiterten Modus *Warenbestände* oder gezielter *Warenbestände » Wareneingänge* als Berechtigung vergeben. Welche Berechtigungen welche Eignerrechte aktiviert, findest du unter <<#tabelle-eignerrechte>>. Wenn das Benutzer:innenkonto über Administratorrechte verfügt, verfügt es auch über alle Eignerrechte.
...
**Erweiterter Modus** = Im erweiterteten Modus vergibst du Berechtigungen. Das sind REST-API-Rechte, die für das Laden und Schreiben von Daten benötigt werden.
...
Wenn die zulässige Anzahl gleichzeitig eingeloggter Benutzer:innen-Konten bereits erreicht oder überschritten ist, können nicht eingeloggte Administratoren die Session-Verwaltung über einen Link in der eingeblendeten Fehlermeldung erreichen.
