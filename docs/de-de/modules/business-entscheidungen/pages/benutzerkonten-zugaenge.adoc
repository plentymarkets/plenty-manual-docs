...
. Öffne eine Rolle.
. Wechsele in den erweiterten Modus (material:toggle_on[role=skyBlue]).
...
. Öffne das Menü *Einrichtung » Kontoverwaltung » Konten*.
. Wähle (material:check_box[role=skyBlue]) die zu bearbeitenden Benutzer:innen.
...
Die Tabelle der Benutzerrechte gliedert sich in vier Spalten. In diesen Spalten kannst du Rechte vergeben, indem du die Optionen *Betrachten*, *Bearbeiten*, *Erstellen* oder *Löschen* aktivierst. Zwei dieser vier Optionen ändern im erweiterten Modus ihre Namen: Aus *Betrachten* wird *Lesen* und aus *Bearbeiten* wird *Aktualisieren*.

[#106]
=== Einfacher und erweiterteter Modus

Beim Erstellen und Bearbeiten von Rollen gibt es zwei Modi: Den einfachen und den erweiterten Modus. Welcher Modus für welche Rolle relevant ist, kommt auf die Rechte an, die der Rolle zugewiesen werden sollen. Der einfache Modus und der erweiterte Modus sind miteinander verknüpft. Wenn du also Menü-Sichtbarkeiten im einfachen Modus aktivierst, sind diese Berechtigungen auch im erweiterten Modus automatisch aktiv. Diese Verknüpfung existiert allerdings nur in diese Richtung. Wenn du also im erweiterten Modus Berechtigungen vergibst, werden keine Menü- oder Plug-In-Sichtbarkeiten automatisch aktiviert.

**Einfacher Modus** = Im einfachen Modus kannst du die Sichtbarkeiten für Menüs und Plugins vergeben. Die Tabelle des einfachen Modus ist genauso aufgebaut wie die Navigation und bildet zudem individuelle Plugins ab.
...
*Alle* = Personen mit dieser Rolle haben Zugriff auf alle Lager. +
...
Rollen können von Benutzer:innen des Zugangs *Admin* erstellt werden oder von Nutzer:innen, die eine Rolle mit den notwendigen Rechten haben.
