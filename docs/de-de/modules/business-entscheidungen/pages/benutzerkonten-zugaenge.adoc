...
[#87]
=== Einladungsstatus von Benutzer:innen-Konten

Im Menü *Einrichtung » Kontoverwaltung » Konten* siehst du welche Benutzer:nnen-Konten eine Einladung für das System bekommen haben und wie der Einladungsstatus ist.

image::invitation-status-de.png[width=600,height=480]

Die unterschiedlichen Farben in der Spalte *Einladungsstatus* bedeuten folgendes: 

kein Symbol: die Einladung wurde noch nicht verschickt.
schwarz = die Einladung wurde verschickt und wurde noch nicht angenommen.
Akzeptiert (material:lens[role:green]): = die Einladung wurde angenommen.
