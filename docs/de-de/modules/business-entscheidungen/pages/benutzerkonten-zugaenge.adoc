...
====
CUL steht für Concurrent-User-Lizenzmodell. Über die Lizenz wird festgelegt, wie viele Benutzer:innen-Konten gleichzeitig auf das plentysystems Backend, die REST-API und die plentysystems App zugreifen dürfen. Wir limitieren also nur die gleichzeitige Nutzung und nicht die Anzahl der Benutzer:innen-Konten, die du erstellen kannst.
====
...
Unter dem Punkt *plentysystems App / plentyPOS* im einfachen Modus der Rollen legst du fest, welche Bereiche und Kennzahlen für die Rolle in der plentysystems App sichtbar sein sollen.
...
. Öffne das Menü *Einrichtung » Kontoverwaltung » Rollen*.
. Öffne eine Rolle.
. Öffne im einfachen Modus den Punkt *plentysystems App / plentyPOS*.
...
Abhängig von deiner plentysystems Edition ist die Anzahl der Benutzer:innen-Konten, die gleichzeitig in plentysystems eingeloggt sein dürfen, möglicherweise beschränkt. Dieses Limit gilt für das plentysystems Backend, die REST-API und die plentysystems App.
