...
IMPORTANT: Beim Anlegen neuer Kennzahlen über das Dashboard ist es nicht möglich, <<#option-targetvalue, Zielwerte>> für diese Kennzahlen einzugeben. Diese Einstellungen kannst du jedoch im Nachhinein im Menü *Einrichtung » plentyBI » Kennzahlen* für die entsprechende Kennzahl ergänzen.
--
====
...
| Mit dieser Einstellung kannst du eine individuell konfigurierte Option zum Menü *Aktionen* (icon:ellipsis-h[rotate=90]) auf der Kachel hinzufügen. Du kannst auch bestimmen, unter welchen Voraussetzungen diese Option angezeigt wird und was geschieht, wenn Nutzer:innen darauf klicken. +
...
. Klicke auf *Aktionen* (icon:ellipsis-h[rotate=90]). +
→ Weitere Optionen werden angezeigt. Beachte die Informationen in <<table-tile-data>>.
...
*_Wichtig:_* Um diese Funktion nutzen zu können, musst du über einen Google Account verfügen und im Google Tabellen-Tool bereits eine leere Tabelle angelegt haben. Wähle nach dem Klick auf die Option *In Google Tabellen exportieren* aus, welche Inhalte des Dashboards exportiert werden sollen. Im nächsten Schritt gibst du die URL zur Google Tabelle ein. Gib auch eine *Zielzelle* an, um den Bereich der Tabelle festzulegen, der als Startpunkt für die resultierende Datentabelle gelten soll. Du wirst nach dieser Eingabe aufgefordert, dich mit einem Google-Konto anzumelden, bevor der Export gestartet werden kann. +
...
| *Ziel*
| Soll der in der Kennzahl <<#option-targetvalue, festgelegte Zielwert>> in der Zeile angezeigt werden? Wähle zwischen den Optionen *Anzeigen* und *Nicht anzeigen*.
...
| *Individuelle Shortcuts*
| Mit dieser Einstellung kannst du eine individuell konfigurierte Option zum Menü *Aktionen* (icon:ellipsis-h[rotate=90]) an der Zeile hinzufügen. Du kannst auch bestimmen, unter welchen Voraussetzungen diese Option angezeigt wird und was geschieht, wenn Nutzer:innen darauf klicken. +
Klicke auf (icon:icon-settings[role=darkGrey]), um einen oder mehrere Shortcuts hinzuzufügen.

| *Link zu detaillierter Dashboard-Ansicht*
| Wähle, welche bereits angelegte Dashboard-Ansicht geöffnet werden soll, wenn du am rechten Rand der Zeile auf *Aktionen* (icon:ellipsis-h[rotate=90]) klickst und die Option *Details zur Zeile* klickst. Wählst du die Option *Keine*, wird beim Klick auf *Details zur Zeile* eine vom System vorgegebene Standard-Ansicht geöffnet. Diese enthält weitere Details zum Ergebnis und den Daten der Kennzahl, die als Grundlage für die Zeile gewählt wurde.
...
[#805]
==== Daten aus Zeilen einsehen oder exportieren

Möchtest du auf Zeilen dargestellte Daten genauer einsehen oder exportieren? Dann gehe wie unten beschrieben vor.
...
. Klicke auf *Aktionen* (icon:ellipsis-h[rotate=90]). +
→ Weitere Optionen werden angezeigt. Beachte die Informationen in <<table-row-data>>.
...
| *In Google Tabellen exportieren*
| Nutze diese Option, um die Daten der Zeile in eine Google Tabelle zu exportieren. +
*_Wichtig:_* Um diese Funktion nutzen zu können, musst du über einen Google Account verfügen und im Google Tabellen-Tool bereits eine leere Tabelle angelegt haben. Wähle nach dem Klick auf die Option *In Google Tabellen exportieren* aus, welche Inhalte des Dashboards exportiert werden sollen. Im nächsten Schritt gibst du die URL zur Google Tabelle ein. Gib auch eine *Zielzelle* an, um den Bereich der Tabelle festzulegen, der als Startpunkt für die resultierende Datentabelle gelten soll. Du wirst nach dieser Eingabe aufgefordert, dich mit einem Google-Konto anzumelden, bevor der Export gestartet werden kann. +
*_Beispiel:_* Mit der Eingabe `A1` startet der Export in der linken oberen Ecke der Google Tabelle.
...
| *Ziel*
| Soll der in der Kennzahl <<#option-targetvalue, festgelegte Zielwert>> im Diagramm angezeigt werden? Wähle zwischen den Optionen *Anzeigen* und *Nicht anzeigen*.
...
[#750]
==== Daten zu Diagrammen einsehen und exportieren
...
. Klicke an der rechten oberen Ecke des Diagramms auf (icon:ellipsis-h[rotate=90]). +
