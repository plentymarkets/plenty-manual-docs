...
====
Die Kennzahlkonfiguration *Zahlungen (Live)* ist eine so genannte Standard-Kennzahlkonfiguration. Das bedeutet, dass sie in plentysystems bereits mit einigen Voreinstellungen eingerichtet ist. Du musst also nicht zwingend eine zusätzliche Kennzahlkonfiguration erstellen. Öffne das Menü *Einrichtung » plentyBI » Kennzahlkonfigurationen* und klicke auf die Kennzahlkonfiguration *Zahlungen (Live)*, die von `plentymarkets` erstellt wurde. Passe bei Bedarf die voreingestellten Optionen an und ergänze die gewünschten <<#400, Filter>>.
====
