...
====
Möchtest du die reine Retourenquote errechnen lassen? Dann muss beim Erstellen der Kennzahlkonfiguration zwingend nach dem Auftragstyp *Retoure* gefiltert werden. Wenn du diesen Filter nicht setzt, fließen auch Gutschriften mit in die Berechnung ein, sodass falsche Ergebnisse entstehen.
====
