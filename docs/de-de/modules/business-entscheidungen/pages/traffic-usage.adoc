= Traffic-Verbrauch
:keywords: Verbrauch, Verbrauchsdaten, Traffic-Verbrauch, Traffic
:author: team-bi
:description: Diese Seite erläutert die Berechnungen, Dimensionen und Filter, die dir für die Kennzahl Traffic-Verbrauch zur Verfügung stehen.

Das Datenvolumen und der zulässige Traffic sind je nach gebuchter plentysystems Edition begrenzt. Überschreitungen werden separat in Rechnung gestellt. plentysystems bietet dir daher die Möglichkeit, deinen Verbrauch jederzeit nachzuvollziehen.

:kennzahl: Traffic-Verbrauch

include::partial$kennzahlen-einleitung.adoc[]

[TIP]
.Was ist Traffic?
====
Wünscht du dir weitere Informationen zum Thema oder bist nicht sicher, welche Aktionen so genannten Traffic verursachen? Schau dir die Handbuchseite xref:business-entscheidungen:hosting-daten.adoc#100[Verbrauchsdaten] an.
====

[IMPORTANT]
.Standard-Kennzahlkonfiguration nutzen
====
Die Kennzahlkonfiguration *Traffic-Verbrauch* ist eine so genannte Standard-Kennzahlkonfiguration. Das bedeutet, dass sie in deinem plentysystems System bereits mit einigen Voreinstellungen eingerichtet ist. Du musst also nicht zwingend eine zusätzliche Kennzahlkonfiguration erstellen. Öffne das Menü *Einrichtung » plentyBI » Kennzahlkonfigurationen* und klicke auf die Kennzahlkonfiguration *Traffic-Verbrauch*, die von `plentymarkets` erstellt wurde. Passe bei Bedarf die voreingestellten Optionen an und ergänze die gewünschten <<#300, Filter>>.
====

[#100]
== plentyBI Edition

Dies ist eine kostenlose Kennzahl.
Du kannst die Kennzahl verwenden, ohne vorher eine plentyBI Edition gebucht zu haben.

Beachte jedoch, dass _andere Funktionen und Kennzahlen_ des plentyBI-Tools kostenpflichtig sind.

Buche ganz einfach die passende plentyBI Edition mit Hilfe des plentysystems Konfigurators.

* *_Neukunden_*: Buche die passende plentyBI Edition link:https://get.plentymarkets.com/[während du plentysystems bestellst^].
* *_Bestandskunden_*: Buche die passende plentyBI Edition xref:business-entscheidungen:dein-vertrag.adoc#30[im "Mein Konto"-Bereich].

[#200]
== Dimension

=== Erstellungsdatum

:kennzahl: Der Traffic-Verbrauch
:das-bedeutet: Die Verbrauchsdaten werden also nach dem Datum gruppiert, an dem der Verbrauch angefallen ist.

include::partial$erstellungsdatum.adoc[]

[#300]
== Filter

include::partial$filter-einleitung.adoc[]

=== Erstellungsdatum

Verwende diesen Filter, um den Träffic-Verbrauch anhand des Datums oder des Zeitraums zu gruppieren, in dem der Verbrauch angefallen ist.

Wähle einen Vergleichsoperatoren aus und wähle dann rechts das gewünschte Datum oder einen Zeitraum.

//TODO: Praxisbeispiel (mit Kachel) hinzufügen
