...
:description: Mit dem Datenformat-Creator orders kannst du Rohdaten Aufträgen exportieren und in der Cloud speichern. Diese Seite erläutert die technischen Spezifikationen und verfügbaren Datenspalten des Datenformats.
...
|order status
|Status des Auftrags im Dezimalformat. Je nach den im System eingerichteten Status werden Zahlen mit bis zu 4 Nachkommastellen ausgegeben. Die IDs der Standardstatus finden Sie auf der Handbuchseite xref:daten:interne-IDs.adoc#120[Interne IDs in plentymarkets].
...
Weitere mögliche Werte für Auftragsherkünfte finden Sie auf der Handbuchseite xref:daten:interne-IDs.adoc#90[Interne IDs in plentymarkets].
