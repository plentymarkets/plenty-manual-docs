...
:lang: de
:description: Erfahre, wie du mit Lagerumbuchungen in deinem plentymarkets System arbeitest.
...
:author: team-stock, team-order-payment
...
| *Standard-Status*
|Wähle einen Standard-Status, der beim Anlegen einer Umbuchung vorausgewählt sein soll. Wie du Status für Umbuchungen anlegst erfährst du im Kapitel <<warenwirtschaft/umbuchungen-vornehmen#200, Status für Umbuchungen anlegen>>.
...
|Aktiviere diese Option, damit sie für die Artikelsuche in der Umbuchung vorausgewählt ist. Angezeigt werden in den Suchergebnissen dann nur <<artikel/webshop/checkliste-artikel-anzeige#100, aktivierte Artikel>>.
...
Möchtest du das Dokument *PO Lieferschein* individueller zu gestalten, zum Beispiel das Logo deiner Firma oder zusätzliche Informationen darauf abbilden, dann gehe gemäß der Anleitungen auf der Handbuchseite zum Thema <<auftraege/auftragsdokumente#300, Dokumentenvorlage einrichten>> vor.
...
.Eigner auswählen
====
Fehlt der gewünschte Eigner, liegt das daran, dass diese:r Benutzer:in nicht als Eigner aktiviert wurde. Eine Anleitung zum Aktivieren der Eigner findest du auf der Handbuchseite <<business-entscheidungen/benutzerkonten-zugaenge#40, Benutzer:innen-Konten und Zugänge>>. Prüfe außerdem die oben beschriebenen <<#400, Standardeinstellungen>> zu Eigner.
====

[#700]
=== Auftragspositionen hinzufügen

Im nächsten Schritt fügst du die Auftragspositionen hinzu, die von einem Lager in ein anderes Lager umgebucht werden sollen. Wende die Suche (icon:search[set=material]) an, um die gewünschten Auftragspositionen anhand der vorhandenen Artikel mit Bestand zu finden.
...
. Klicke im Bereich *Auftragspositionen* auf *Hinzufügen* (icon:add[set=material]). +
...
. Klicke auf den *Zum Warenkorb hinzufügen* (icon:add_shopping_cart[set=material]). +
...
Im Feld *Artikelname* kannst du die Bezeichnung für den Artikel anpassen. Die Änderungen gelten nur für die Umbuchung, in der sie vorgenommen wurden. Möchtest du die <<artikel/einstellungen#, Artikeleinstellungen>> generell anpassen, kannst du dies im Menü *Artikel » Artikel bearbeiten* im entsprechenden Tab machen.
...
.Umbuchung löschen
====
Umbuchungen können in der Detailansicht gelöscht werden, solange die Umbuchung noch nicht durch Setzen des Bestelldatums ausgelöst wurde. Klicke zum Löschen auf *Bestellung löschen* (icon:delete[set=material]) rechts oberhalb der Infoboxen.
====
...
. Klicke auf *Dokument hinzufügen* (icon:add[set=material]).
...
. Klicke auf *Warenbewegungen anlegen* (icon:execute[set=plenty]). +
...
. Klicke auf *Hinzufügen* (icon:add[set=material]). +
...
Beim Öffnen des Menüs *Waren » Bestellwesen » Bestellung* werden dir alle Umbuchungen und <<warenwirtschaft/nachbestellungen-verwalten/nachbestellungen-vornehmen#, Nachbestellungen>> in einer Übersichtstabelle angezeigt. Wende die Suche (icon:search[set=material]) an, um die angezeigten Umbuchungen und Nachbestellungen eingrenzen.
