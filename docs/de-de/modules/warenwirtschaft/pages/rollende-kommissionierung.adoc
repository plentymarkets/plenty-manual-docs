...
====
Bei Bedarf kannst du Artikel gezielt von der rollenden Kommissionierung ausschließen. Das kann zum Beispiel sinnvoll sein, wenn es nicht möglich ist, Etiketten mit Barcodes auf diesen Artikeln anzubringen. Aktiviere dazu die Option *Von Pickliste ausschließen* im Menü *Artikel » Artikel » [Variante öffnen] » Element: Versand » Dropdown-Liste: Kommissionierung*.
====
