...
→ Eine neue Ansicht wird geöffnet. Klicke auf *Suchen*, um eine Suche nach geeigneten Lagerorten auszuführen. Da den Kommissionierwagen nur Lageorte vom Typ *Box* zugeordnet werden können, ist der Suchfilter entsprechend voreingestellt und kann nicht verändert werden. +
...
. Anstatt <<#900, die Anzahl der Boxen manuell festzulegen>>, scannst du nun den Barcode, der an deinem Kommissionierwagen angebracht ist. +
→ Im Hintergrund wird der Kommissionierwagen identifiziert. Somit wählt plentyWarehouse automatisch genau die Anzahl an Boxen aus, die auf dem Kommissionierwagen vorhanden ist.
