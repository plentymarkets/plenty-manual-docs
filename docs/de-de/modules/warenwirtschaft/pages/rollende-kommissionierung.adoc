...
====
Für die rollende Kommissionierung mit plentyWarehouse empfehlen wir dir, dass du pro Lagerort und <<#400, Box>> ein Label erstellt und am passenden Lagerort angebracht hast. Während der rollenden Kommissionierng scannst du den Barcode auf dem Label mit deinem Gerät, um so die passenden Lagerorte und Box-Lagerorte auszuwählen.
...
*Nicht aktiviert* (icon:toggle-off[role="darkGrey"]) = Lagerorte und Boxen können manuell ausgewählt werden, indem du auf sie tippst.
