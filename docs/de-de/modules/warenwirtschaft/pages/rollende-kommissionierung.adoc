...
Zunächst folgt eine <<#100, kurze Beschreibung der rollenden Kommissionierung>>, die den Ablauf erklärt. Danach werden die <<#700, Einstellungen>> beschrieben, die du im Voraus vornehmen solltest. Diese Einstellungen greifen dann später, wenn du die <<#800, Funktion nutzt>>.
