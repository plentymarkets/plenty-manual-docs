...
Bevor eine Inventur mit plentyWarehouse durchgeführt wird, muss sichergestellt sein, dass alle Benutzer:innen ohne Rolle vom Typ *Administrator* über die entsprechenden Berechtigungen verfügen. Nur so können die gezählten Bestände erfasst und sowohl in plentyWarehouse als auch im plentymarkets Backend korrekt gespeichert werden.
