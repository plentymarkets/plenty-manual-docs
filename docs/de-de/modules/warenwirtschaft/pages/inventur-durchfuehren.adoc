...
[IMPORTANT]
.Wichtiger Hinweis zu Lagerorten mit Bestand 0
====
Beachte, dass Lagerorte unter bestimmten Bedingungen nicht im Zählprotokoll-Export aufgeführt werden. Genauer gesagt ist dies der Fall, wenn auf einem Lagerort keinerlei Artikel vorhanden sind und dementsprechend während der Inventur ein Bestand von 0 für diesen Lagerort erfasst wird. Dieses Verhalten ist rechtskonform und kein Systemfehler.
====
