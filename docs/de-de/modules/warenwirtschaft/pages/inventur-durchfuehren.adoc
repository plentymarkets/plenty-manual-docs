...
|Mit dieser Einstellung kannst du bestimmen, ab wie vielen gescannten oder über das Mengenfeld erfassten Exemplaren einer Variante der Mengendialog geöffnet wird. Anschließend kannst du im Eingabefeld für die Menge die gesamte Anzahl der gezählten Variante eingeben und sie somit für die Inventur erfassen. +
*_Beispiel:_* Wenn du hier den Wert *5* auswählst, wird nach dem Erfassen des fünften Exemplars der Variante automatisch der Mengendialog geöffnet.
...
Bevor eine Inventur mit plentyWarehouse durchgeführt wird, muss sichergestellt sein, dass alle Benutzer:innen ohne Admin-Konto über die entsprechenden Berechtigungen verfügen. Nur so können die gezählten Bestände erfasst und sowohl in plentyWarehouse als auch im plentymarkets Backend korrekt gespeichert werden.
