...
. Tippe auf *Inventur* (image:warenwirtschaft:icon_stocktaking.png[width=14, height=14]).
...
*Aktiviert* (icon:toggle-on[role="green"]) = Der Export wird um zwei Spalten erweitert, die jeweils den Einkaufspreis und den gleitenden Durchschnitss-Einkaufspreis der Variante beinhalten. Hintergrundinformationen zu diesen zwei Arten von Einkaufspreisen findest du auf der Handbuchseite xref:warenwirtschaft:wareneingaenge-verwalten.adoc#250[Wareneingänge verwalten]. +
