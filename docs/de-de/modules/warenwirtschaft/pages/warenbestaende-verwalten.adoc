...
Barcodes legst du im Menü xref:artikel:artikel-verwalten.adoc#630[Einrichtung » Artikel » Barcode] an und anschließend xref:artikel:artikel-verwalten.adoc#250[verknüpfst du Barcodes] mit deinen Varianten.
...
| **Bestellen**
| **Ja** = Erlaubt Nachbestellungen aus der Bestandssuche heraus anzulegen. Die Artikel müssen für das Anlegen von Nachbestellungen mit Lieferant:innen verknüpft sein. Die Nachbestellung wird über **Bestellung erstellen** über der Ergebnisliste eingeblendet. Im Kapitel xref:warenwirtschaft:vornehmen.adoc#[Nachbestellungen anlegen] wird die genaue Vorgehensweise in Schritten erklärt. +
...
Du kannst Warenbestandsdaten importieren oder exportieren und so z.B. eine Inventur vornehmen. Das Datenformat xref:daten:stock.adoc#[Stock] enthält die meisten Daten über deine Warenbestände. Datenformate, die im Zusammenhang mit deinen Beständen nützlich sein können, sind:

* xref:daten:stock.adoc#[Stock],
* xref:daten:variationsupplier.adoc#[VariationSupplier],
* xref:daten:variationwarehouse.adoc#[VariationWarehouse] und
* xref:daten:warehouse.adoc#[Warehouse]
