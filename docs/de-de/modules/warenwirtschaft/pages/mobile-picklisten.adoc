...
Die Pickliste ist ein wichtiges Dokumente für dein Lagerpersonal, denn sie enthält alle Informationen darüber, wie viele Artikel von welchen Lagerorten geholt werden müssen, um einen Auftrag abzuwickeln.

Mit plentyWarehouse kann dein Lagerpersonal Picklisten, die xref:auftraege:pickliste.adoc#300[in deinem plentymarkets System erstellt] wurden, bequem auf mobilen Geräten suchen, öffnen und bearbeiten.
...
Stelle zuerst sicher, dass alle benötigten Picklisten bereits im Menü *Aufträge » Fulfillment* im Backend deines plentymarkets Systems erzeugt wurden. Weitere Informationen dazu findest du auf der Handbuchseite xref:auftraege:pickliste.adoc#[Picklisten]. 
...
====
Alternativ zur oben beschriebenen Vorgehensweise kannst du auf das Barcode-Symbol (icon:barcode[set=material]) am oberen Bildschirmrand tippen. Scanne dann den Barcode einer bereits ausgedruckten Pickliste, um diese in plentyWarehouse zu öffnen. 
====
...
Für jeden Artikel wird neben weiteren Informationen der Lagerort angegeben, auf dem er sich befindet, sowie die Menge des Artikels, die gepickt werden muss.
...
Sobald alle Artikel gepickt wurden, erscheint die Meldung *Alle Artikel gepickt*. Tippe auf icon:done[set=material], um die Pickliste abzuschließen.

[NOTE]
.Shortcuts in der Artikelansicht
====
Für jeden Artikel auf der Pickliste stehen weitere Shortcuts zur Verfügung. Dieese kannst du nutzen, um während der Arbeit mit der Pickliste unkompliziert weitere Aktionen durchzuführen. <<table-pick-list-shortcuts>> enthält weitere Informationen zu den verfügbaren Shortcuts.
====
...
. Tippe oben rechts auf icon:more_vert[set=material].
. Gib eine Wagenkennung ein. +
*_Oder:_* Tippe auf icon:barcode[set=material], um einen QR- oder Barcode zu scannen.
...
//Verwendung in Prozessen: So kann das Lagerpersonal am Packplatz diese Wagenkennung abscannen, um die Pickliste in den Prozessen zu laden und weiter zu bearbeiten. Dazu wurde in der Aktion Picklisten-Suche die Einstellung Wagenkennung hinzugefügt. 