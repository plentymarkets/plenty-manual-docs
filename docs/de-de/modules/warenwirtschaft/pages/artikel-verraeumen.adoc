...
Bevor das Verräumen mit plentyWarehouse durchgeführt wird, muss sichergestellt sein, dass alle alle Benutzer:innen ohne Rolle vom Typ *Administrator* über die entsprechenden Berechtigungen verfügen. Nur so können die Artikel korrekt verräumt werden und die Bestandsänderungen im plentymarkets Backend gespeichert werden.
