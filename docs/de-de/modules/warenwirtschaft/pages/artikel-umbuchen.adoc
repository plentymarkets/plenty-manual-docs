...
Bevor eine Umbuchung mit plentyWarehouse durchgeführt wird, muss sichergestellt sein, dass alle Benutzer:innen ohne Rolle vom Typ *Administrator* über die entsprechenden Berechtigungen verfügen. Nur so kann die Umbuchung korrekt abgewickelt werden und die Bestandsänderungen im plentymarkets Backend gespeichert werden.
