...
:position: 30
:id: A5QIHYV
:url: warenwirtschaft/plentywarehouse/artikel-umbuchen
...
. Tippe auf *Umbuchung* (icon:sync_alt[set=material]).
...
*_Tipp:_* Tippe und halte (icon:drag_handle[set=material]) für jedes aktivierte Kriterium, um die Reihenfolge der angezeigten Lagerortvorschläge anzupassen. +
...
*_Beispiel:_* Deaktiviere (icon:toggle-off[role="darkGrey"]) den Zweck *Retoure*, damit retournierte Artikel nicht fälschlicherweise an Lagerorte mit diesem Zweck umgebucht werden und fälschlicherweise mit retournierten Artikeln vermischt werden.
...
Bevor eine Umbuchung mit plentyWarehouse durchgeführt wird, muss sichergestellt sein, dass alle Benutzer:innen über die entsprechenden Berechtigungen verfügen. Nur so kann die Umbuchung korrekt abgewickelt werden und die Bestandsänderungen im plentymarkets Backend gespeichert werden.
