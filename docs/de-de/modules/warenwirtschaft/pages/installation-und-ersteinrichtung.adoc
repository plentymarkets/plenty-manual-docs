...
→ Das Einstellungsmenü öffnet sich.
. Öffne die einzelnen Bereiche, um für jede Funktion genauere Einstellungen zur Sprachausgabe vorzunehmen. Beachte die Informationen in Beachte dazu die Erläuterungen in <<table-pw-tts-settings>>.
...
| *Sprachausgabe*
|
|Aktiviere (icon:toggle-on[role="green"]) diese Option, um während der Nutzung der Funktionen von plentyWarehouse eine zusätzliche Sprachausgabe zu erhalten. Die Sprachausgabe enthält Warnungen, Anweisungen und Bestätigungshinweise. Je nach Ablauf und Anwendungsfall kann dies eine nützliche Hilfe für dein Lagerpersonal sein. +
Über die Einstellung *Information zum Artikel* legst du je nach deinen Anforderungen fest, welche Artikelinformationen in der Sprachausgabe enthalten sein sollen. Wenn du diese Einstellung nicht anpasst, ist standardmäßig die Option *Variantenname* vorausgewählt. Wähle alternativ eine der Optionen *Artikelname*, *Variantenname* oder *Varianten-ID* aus. Sollen keine Artikelinformationen ausgegeben werden, wähle die Option *Keine Auswahl*. +
*_Hinweis:_* Diese Einstellung greift für die Sprachausgabe aller Funktionen von plentyWarehouse.
