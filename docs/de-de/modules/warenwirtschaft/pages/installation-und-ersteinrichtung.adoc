...
| *plentyBase* 
|image:warenwirtschaft:icon_plentybase.png[width=24, height=24]
...
| [#intable-other-settings]*Sonstiges* 
|material:settings[]
...
. Öffne die einzelnen Bereiche (icon:chevron-right[role="darkGrey"]), um pro Funktion die gewünschten Einstellungen vorzunehmen. Beachte dazu die Erläuterungen in <<table-pw-tts-settings>>.
...
*Detailliert* = Die Bestätigung enthält so genaue Informationen wie möglich. *_Beispiel:_* Wurde ein Artikel erfolgreich umgebucht, enthält die Sprachausgabe Informationen zum Artikel sowie den Namen des Ziel- und Quellagerorts. +
*Kurz* = Die Bestätigung wird kurz gehalten. *_Beispiel:_* Wurde ein Artikel erfolgreich umgebucht, werden Informationen zum Artikel sowie zu betroffenen Lagerorten nicht genannt. +
*Aus* = Für die gewählte Funktion werden keine Bestätigungen über die Sprachausgabe ausgegeben.

| *Anweisung*
| Wie detailliet sollen die Informationen sein, wenn Benutzer:innen über die Sprachausgabe Anweisungen erhalten? +
*Detailliert* = Die Anweisung enthält so genaue Informationen wie möglich. *_Beispiel:_* Während der rollenden Kommissionierung wird genau benannt, welcher Artikel von welchem Lagerort entnommen werden muss. +
*Kurz* = Die Anweisung wird kurz gehalten. *_Beispiel:_* Während der rollenden Kommissionierung werden keine genaueren Informationen zum Artikel oder zu betroffenen Lagerorten genannt. +
*Aus* = Für die gewählte Funktion werden keine Bestätigungen über die Sprachausgabe ausgegeben.
