...
→ Du bleibst angemeldet, bis du dich xref:app:installation.adoc#1600[sperrst]. Du kannst die App also schließen und öffnen, ohne dich erneut anmelden zu müssen.
...
* Das Menüsymbol (icon:bars[role="blue"]) oben links. Tippe auf dieses Symbol, um in das Hauptmenü zu gelangen. Von hier aus kannst du deine xref:app:lagerverwaltung.adoc#[Warenwirtschaft im Lager nutzen>>. Oder du kannst in deinem <<pos#, stationären Laden kassieren].
...
* Das Benutzersymbol (icon:user[role="blue"]). Dieses Symbol zeigt an, welches Benutzerkonto gerade angemeldet ist. Tippe auf dieses Symbol, um den xref:app:installation.adoc#1500[dich gänzlich auszuloggen]. +
