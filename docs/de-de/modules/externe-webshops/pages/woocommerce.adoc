...
* Beim *Auftragsimport*: Die WooCommerce *DHL post number* importiert als *CRM » Kontante » Betroffener Kontat » Adressen Adresse » Typ: Postnummer*
* Der *pro-Einheit-Basispreis* der Variante. Dieser Preis wird folgenderweise generiert: +
*_Wichtig:_* _Der exportierte Basispreis berechnet sich aus dem WooCommerce-Verkaufspreis, der der Variante zugeordnet ist. Wenn an deiner Variante auch ein Aktionspreis für WooCommerce definiert ist, wird auch dafür ein Basispreis berechnet und exportiert. +
** *Größe*: Meter, Zentimeter und Millimeter werden beim Export in Meter umgerechnet. Zoll werden in Zoll ausgedrückt.
** *Gewicht*: Kilogramm, Gramm und Milligramm werden beim Export in Kilogramm umgerechnet. Unzen werden in Unzen ausgedrückt.
** *Volumen*: Liter und Milliliter *_bis 250ml_* werden beim Export in Millilitern ausgedrückt. *_Ab 250ml_* wird der exportierte Wert in Litern ausgedrückt.

| Niedrigsten Preis der letzten 30 Tage über Germanized senden
| material:check_box_outline_blank[] = Der Preis nicht gesendet. +
material:check_box[] = Wenn du den *Slug-Wert für Preishinweis 'Niedrigster Preis der letzten 30 Tage'* korrekt eingerichtet hast, wird der niedrigste Preis der letzten 30 Tage zu WooCommerce exportiert.
...
| *Slug-Wert für Preishinweis 'Niedrigster Preis der letzten 30 Tage'*
...
. Gib einen Wert in das benutzerdefinierte Platzhalterfeld ("custom slug") ein, zum Beispiel `lowest30days`.

Nun speicherst du den Wert des benutzerdefinierten Platzhalterfelds in deinem plentymarkets System:

. Öffne das Menü *Einrichtung » Märkte » WooCommerce » Tab: Einstellungen*.
. Navigiere zum Bereich *Germanized*.
. Gib den Wert für das benutzerdefinierte Platzhalterfeld in das Feld *Slug-Wert für Preishinweis 'Niedrigster Preis der letzten 30 Tage'* ein. +
