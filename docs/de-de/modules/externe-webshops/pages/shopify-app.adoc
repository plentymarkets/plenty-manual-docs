...
. Öffne dein Shopify-Backend.
. Klicke in der Navigationsleiste links auf *Apps hinzufügen*.
. Gib in die eingeblendete Suchleiste *Apps* den Suchbegriff `plentymarkets` ein. +
→ Du wirst aufgefordert, den Begriff im Shopify App Store zu suchen. +
*_Tipp:_* Wenn die plentymarkets App in deinem Backend gefunden wird, hast du die App schon installiert. Dann kannst du die Installation überspringen.
. Benutze die Eingabetaste oder klicke auf die Lupe, um den Shopify App Store zu öffnen. +
→ Der Shopify App Store wird in einem neuen Browsertab geöffnet.
. Klicke auf die plentymarkets App. +
*_Oder:_* Öffne die plentymarkets App direkt im link:https://apps.shopify.com/plentymarkets-1?locale=de[Shopify App Store].
. Klicke auf *App hinzufügen*. +
→ Du wirst zurück in dein Shopify-Backend geleitet.
. Klicke auf *App installieren*. +
...
→ Du kannst einen Mandanten verbinden und den Datenaustausch aktivieren.

[#BL4N7F]
=== Mandanten verbinden und Datenaustausch einstellen

[.instruction]
Mandanten verbinden und Datenaustausch einstellen:

. Aktiviere für den Mandanten, den du mit diesem Shopify-Konto verbinden möchtest, ganz rechts den Toggle *Verbunden*. +
*_Wichtig:_* Du kannst nur einen Mandanten verbinden. +
→ Für den aktivierten Mandanten werden die Optionen zum Datenaustausch wählbar.
. Aktiviere die gewünschten Optionen für den Datenaustausch. Beachte <<#QVFOWVID>>. +
*_Wichtig:_* Preise, Bestand und Artikel können nur in eine Richtung gesendet werden. Welche Richtung du wählst hängt davon ab, ob du Artikeldaten in plentymarkets oder in Shopify pflegst. Verschiedene Szenarien findest du in den Tabs unterhalb dieser Beschreibung.
. Speichere die Einstellungen. +
*_Wichtig:_* Der Datenaustausch wird erst gestartet, nachdem du Shopify auch im plentymarkets Backend eingerichtet hast.
...
! *Aufträge*
! Aktivieren, um Aufträge alle 15 Minuten in plentymarkets zu iportieren.

! *Kunden* 
! Aktivieren, um Kundendaten einmal täglich in plentymarkets zu iportieren.
...
//tag::sync-direction[]
Preise, Bestände und Artikeldaten können nur in eine Richtung gesendet werden. Welche Richtung du wählst hängt davon ab, ob du Artikeldaten in plentymarkets oder in Shopify pflegst. Hier findest du die häufigsten Szenarien.
...
*_Wichtig:_* Damit deine Varianten zu Shopify exportiert werden, müssen der Mandant und die Herkunft Shopify aktiviert werden. Wie das geht, erfährst du im Kapitel <<#XV8ANTMR, Varianten vorbereiten>>.
...
*_Tipp:_* Du musst die Option *Möchtest du Artikel aus Shopify importieren* deaktivieren, um die neuen Option aktivieren zu können.
!===
//end::sync-direction[]
--
