...
*_Hinweis:_* Wenn du einen oder mehrere Artikel auf Shopify gelöscht hast, um sie noch einmal zu exportieren, bereinigt der erste Export nur die Plugin-Datenbank. Erst der zweite Export speichert den Artikel wieder auf Shopify.
...
. Öffne das Menü *Einrichtung » Märkte » Shopify-App » Bestandsexport*.
. Wähle einen Webshop.
. Wähle aus der Dropdown-Liste *xxx* die Option *BereinigungsdatumXXX*.
