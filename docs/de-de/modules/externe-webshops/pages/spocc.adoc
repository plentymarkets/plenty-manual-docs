...
Im besten Fall sind Verkaufspreis und Zielpreis gleich, sodass der Händler den Artikel regulär verkauft. Der "retailPrice" wird in den meisten Shops nur dann in rot angezeigt, wenn er unterhalb des "targetPrice" liegt.
 Der "recommendedRetailPrice" wird meist nur dann angezeigt, wenn der "targetPrice" untwerhalb des "recommendedRetailPrice" liegt und somit einen Hauspreis des Händlers darstellen soll.
