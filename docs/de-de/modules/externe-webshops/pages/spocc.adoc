...
| *Barcode as SPoCC identifier*
| Bei Neuanlage von Varianten wird der eindeutige SPoCC-Identifier als Variantennummer gespeichert. Dieser Identifier setzt sich aus der achstelligen ECC-Variantennummer und der dreistelligen Größennummer zusammen. Damit aber auch vorhandene Varianten korrekt zugeordnet werden können, benötigst du einen weiteren Barcode-Typen, in dem diesen Identifier für alle Varianten gespeichert werden kann. 
...
*_Wichtig:_* Bei Retouren erwartet SPoCC grundsätzlich die Angabe eines Retourengrunds. Wenn du für einen plentymarkets Retourengrund keinen SPoCC-Retourengrund zugeordnet hast, wird deshalb standardmäßig der Retourengrund *[1] No reason* an SPoCC übertragen.

| *SPoCC-Retourengründe*
| Die Dropdown-Liste enthält deine Retourengründe in SPoCC. Wähle einen Retourengrund aus der Dropdown-Liste, der zu dem Grund passt, den du in der Spalte *plentymarkets Retourengründe* gewählt hast.
...
* Nach dem ersten Import werden nur noch Teilimporte durchgeführt, die auf den zuvor gewählten <<#80, Importeinstellungen>> basieren.
...
| ???
| Importiert EAN, Bilder und Eigenschaften von Varianten, die mit dem Tag gekennzeichnet sind, das du in der Einstellung *???* gewählt hast.
...
* <<#5454333207, SPoCC: Bestand reservieren>>
* <<#0414974358, SPoCC: Versand an SPoCC melden>>
...
| Aktion
| Plugins > SPoCCConnector order export??? +
...
:table-event-procedure: <<#table-event-procedure-one-two>>

include::_includes:partial$event-procedure.adoc[]

[[table-event-procedure-one-two]]
[cols="1,2"]
|===
...
| Aktion
| Plugins > SPoCCConnector order is paid??? +
...
[#0414974358]
=== SPoCC über xxxx informieren

Informiere SPoCC automatisch darüber, dass der Auftrag versandbereit ist. Diese Ereignisaktion sollte ausgelöst werden, wenn die Versanddokumente am Auftrag vorhanden sind.
...
| Aktion
| Plugins > SPoCCConnector order shipping document export??? +
...
Informiere SPoCC automatisch darüber, dass eine Retoure  Auftrag versandbereit ist. Diese Ereignisaktion sollte ausgelöst werden, wenn die Versanddokumente am Auftrag vorhanden sind.
...
| Aktion
| Plugins > SPoCCConnector return order export??? +
→ In SPoCC wird der Auftrag in die SPoCC-Status-ID 18 verschoben. +
→ Der Retourengrund wird übertragen. Wenn du für einen plentymarkets Retourengrund keinen SPoCC-Retourengrund zugeordnet hast, wird standardmäßig der Retourengrund *[1] No reason* an SPoCC übertragen.
