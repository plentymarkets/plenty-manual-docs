...
[discrete]
[#260]
=== Benutzerrechte für Standard-Benutzer:innen vergeben

Benutzer:innen mit Standard-Zugang sehen nur bestimmte Bereiche und Menüs in plentymarkets. Somit können sie nur eingeschränkt im System arbeiten. Sollen Mitarbeiter:innen mit diesem Zugang das Menü *plentymarkets Logo (Start) » Kalender* sehen können, muss eine Person mit *Administrator*-Zugang eine Rolle mit der aktivierten  Sichtbarkeit *Start » Kalendar* für Standard-Benutzer:innen vergeben.

[.instruction]
Rolle für Standard-Benutzer:innen vergeben:

. Öffne das Menü *Einrichtung » Kontoverwaltung » Konten*.
. Nutze die Suchfunktion (material:search[role="blue"]) und öffne das zu bearbeitende Konto.
. Weise dem Konto die passende Rolle unter *zugewiesene Rollen* zu. +
→ Oder: erstelle vorher eine Rolle mit den *Start » Kalender* Sichtbarkeiten im Einfachen Modus.
. *Speichere* (material:save[]) die Einstellungen.
...
Gehe wie im Folgenden beschrieben vor, um einen Termin zu erstellen. Abhängig von deinen Zugriffsrechten kannst du auch Termine für andere Benutzer:innen erstellen. Du kannst nur Termine für Benutzer:innen erstellen, für die du Schreibrechte hast. Benutzer:innen, für die du nicht über Schreibrechte verfügst, stehen beim Erstellen eines Termins nicht zur Auswahl.
