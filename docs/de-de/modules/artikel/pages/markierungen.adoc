...
[.collapseBox]
.*Anwendungsbeispiel: Backend-Übersicht*
--
...
[.collapseBox]
.*Anwendungsbeispiel: Schlagwörter im plentyShop*
--
...
[.collapseBox]
.*Anwendungsbeispiel: Artikelliste im plentyShop*
--
...
. xref:artikel:markierungen.adoc#500[Erstelle] die benötigten Tags, z.B. "Bestseller-Schulranzen".
Wähle dabei (icon:check-square[role="blue"]) den Bereich *Variante* und den entsprechenden Mandanten.
. xref:artikel:markierungen.adoc#600[Verknüpfe] das Tag mit den passenden Varianten, z.B. mit den 6 beliebtesten Schulranzen.

---

*_Artikelliste im plentyShop_*:

. Öffne das Menü *CMS » ShopBuilder » [Artikel-Kategorie öffnen] » [Inhalt öffnen]*.
...
Beispiel: Gib hier die ID des Tags "Bestseller-Schulranzen" ein.
...
Mit plentymarkets kannst du jedem deiner Artikel eine oder zwei Markierungen zuweisen.
Die Markierungen helfen dir, den Überblick über deine Artikel im Backend zu behalten.
Du kannst sogar gezielt nach Artikeln im Backend suchen, die eine bestimmte Markierung oder Kombination von Markierungen haben.

[discrete]
==== Ablauf

* Wirf einen Blick auf die Markierungen, die in plentymarkets enthalten sind, und benenne sie bei Bedarf um.
* Versehe deine Artikel mit den Markierungen.
* Suche nach Artikeln im Backend anhand ihrer Markierungen.
...
//. Öffne das Menü *Artikel » Artikel-UI*.
//. Klicke auf *Filter* (icon:tune[set=material, role="darkGrey"]) und wähle aus den Dropdown-Listen xref:artikel:artikelsuche.adoc#100[Markierung 1 und 2] die passende Kombination von Markierungen.
//. Klicke auf icon:search[role="blue"] *Suchen*. +
//→ Alle Artikel mit dieser Kombination von Markierungen werden angezeigt.

[discrete]
==== Ich sehe die Markierung nicht in der Übersicht
...
//. Wähle die Spalten *Markierung 1* und *Markierung 2* (icon:check-square[role="blue"]).
//. Bewege deinen Mauszeiger über das Symbol icon:drag_indicator[set=material, role="darkGrey"]. +
