...
//ToDo - beim schreiben ist es noch nicht möglich in die neue Artikel-UI eine andere Hauptvariante zu wählen, es geht nur in die alte UI - wenn die neue UI als standard wird, dann prüfen ob das inzwischen geht und die include-Datei entsprechend anpassen
