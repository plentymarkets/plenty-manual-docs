...
. Klicke auf icon:plus[role="darkGrey"] und wähle die Art des Artikels, den du erstellen möchtest.
Beachte, dass diese Einstellung nicht mehr geändert werden kann, nachdem du den Artikel erstellt hast.
* *Standard-Artikel*: Ein normaler Artikel.
In den meisten Fällen wirst du wahrscheinlich einen Standard-Artikel erstellen.
Diese Seite beschreibt den Vorgang.
* <<Multipack-Artikel>>:
