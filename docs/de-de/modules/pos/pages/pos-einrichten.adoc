...
. Öffne das Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Buchhaltung*.
. Scrolle nach unten, bis du die Bereiche *Kassenvorfälle Einlage* und *Kassenvorfälle Entnahme* siehst.
. Klicke in einem der beiden Bereiche auf *Hinzufügen* (icon:plus[role="darkGrey"]), je nachdem, welchen Zweck das Buchungskonto erfüllen soll.
→ Ein leerer Eintrag wird angelegt.
...
*_Tipp:_* Klicke und halte (material:drag_indicator[role=darkGrey]), um die Einträge neu anzuordnen. Damit bestimmst du die Reihenfolge, in der die Vorfälle in der App angezeigt werden.
. *Speichere* (icon:save[role="darkGrey"]) die Einstellungen. +
