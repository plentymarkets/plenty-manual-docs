...
| *Standardkunde*
|Für Kassenaufträge <<#95, erstellten Standardkunden>> wählen, dem über diese Kasse generierte Aufträge zugeordnet werden sollen.
...
|Wählen, ob das <<glossar#grand-total, Grand Total>> im Tagesabschlussbericht aufgeführt werden soll. +
Nicht aktiviert (Standard) = Das Grand Total wird nicht im Tagesabschlussbericht aufgeführt. *_Hinweis:_* In diesem Fall kann das Grand Total nur von POS-Benutzer:innen mit Admin-Rechten am POS selbst eingesehen werden. +
...
|Wählen, ob Rabatt-Codes, die im Rahmen von <<glossar#aktionsgutschein, Aktionsgutscheinen>> eingelöst wurden, einzeln im Tagesabschlussbericht aufgeführt werden sollen. +
...
|Eine der Optionen *Varianten-ID*, *Variantennummer*, *Attribute* oder *Ext. Varianten-ID* aktivieren, um auf
dem Beleg die entsprechende Zusatzinformation zum verkauften Artikel auszugeben. Die Information wird unterhalb des Artikelnamens auf dem Kassenbeleg angezeigt. Die Option *Keine* aktivieren, um keine Zusatzinformation anzuzeigen.

| **Bild**
|Option aktivieren und PNG-Datei wählen, die auf Belegen der Kasse ausgeben werden soll. Die Datei muss vorher im Menü **CMS » Bilder-Galerie** hochgeladen werden.
...
Im Falle einer xref:pos:pos-kassenbenutzer.adoc#175[Retoure] oder eines <<pos/pos-online-bestellungen#, Click & Collect-Auftrags>> bringen Kund:innen typischerweise ihre Kaufbelege mit in den Laden. Wurde ein Barcode auf dem Beleg gedruckt, muss das Kassenpersonal lediglich xref:pos:pos-kassenbenutzer.adoc#520[den Barcode scannen], um den entsprechenden Auftrag aufzurufen. +
...
| **Auftragsherkunft**
|plentyPOS-Kasse(n) als Auftragsherkunft wählen.
...
| **Herkunft**
|Eine oder mehrere plentyPOS Auftragsherkünfte aktivieren, mit denen der Verkaufspreis verknüpft wird.
|====
...
| **Land**
|Land aus der Dropdown-Liste wählen, in dem die Kasse eingesetzt wird. Standardeinstellung richtet sich nach der Option **Land** im Menü xref:willkommen:schnelleinstieg-basiseinstellungen.adoc#100[Einrichtung » Einstellungen » Stammdaten].
...
| **Nutzung**
| **Einzelbestellungen**, xref:auftraege:scheduler.adoc#[Abonnements] oder beides wählen.

| **Mindestbestellwert**
|Keinen Wert eingeben, da Mehrzweckgutscheine wie Zahlungsmittel zu behandeln sind und keinem Mindestbestellwert unterliegen dürfen.
...
| **Nicht erlauben** wählen, da plentyPOS nicht an Preisportale angebunden ist.
...
| **Artikel**
|Keinen Wert eingeben, da Mehrzweckgutscheine wie Zahlungsmittel zu behandeln sind und nicht auf bestimmte Artikel beschränkt werden dürfen.
...
| *Nutzung*
| *Einzelbestellungen* oder xref:auftraege:scheduler.adoc#[Abonnements] oder beides wählen.
...
| **Preis**
|Wählen, ob die Auftragspositionen in Brutto- oder Nettopreisen angezeigt werden sollen.

| **Fiskalisierung**
| IP-Adresse der xref:pos:pos-rechtssicherheit.adoc#500[technischen Sicherheitseinrichtung], die mit der Kasse gekoppelt ist.

| **Bondrucker**
|IP-Adresse des Bondruckers eingeben. +
*_Tipp:_* Auf **Testbeleg drucken** tippen, um einen Testbeleg zu drucken.
...
(nur neue Kassen mit Standort Österreich)
|Wählen, um den xref:pos:pos-rechtssicherheit.adoc#1600[Startbeleg  zu erstellen] und so die manipulationssichere Sicherheitseinrichtung für die Kasse zu aktivieren. Nachdem der Startbeleg erstellt wurde, kann das Standortland der Kasse nicht mehr geändert werden.

| **Endbeleg erstellen** +
(nur Kassen mit Standort Österreich und aktivierter Sicherheitseinrichtung)
|Wählen, um xref:pos:pos-rechtssicherheit.adoc#2300[den Endbeleg zu erstellen] und so die manipulationssichere Sicherheitseinrichtung für die Kasse zu deaktivieren. Nachdem der Endbeleg erstellt wurde, kann die Kasse nicht mehr verwendet werden.
|====
...
Wie du Etikettenvorlagen erstellst und Artikeletiketten in der Artikelsuche der plentymarkets App druckst, erfährst du auf der Handbuchseite <<app/funktionen/artikelsuche#, Artikel-, Kunden- und Auftragssuche>>.
...
* <<pos/pos-online-bestellungen#900, Online-Bestellungen (Click & Collect)>> laden und kassieren
...
| *Suchen*
|Ohne Filtereinstellungen auf *Suchen* (icon:search[role="blue"]) klicken, um alle Kassenvorgänge anzuzeigen. +
icon:map-signs[] *_Oder:_* Die Suchergebnisse durch Filtereinstellungen eingrenzen und auf *Suchen* (icon:search[role="blue"]) klicken. Die gefundenen Kassenvorgänge werden im Tab *Kassenjournal* aufgelistet.
