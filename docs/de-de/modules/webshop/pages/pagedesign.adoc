...
* xref:omni-channel:cms-syntax.adoc#webdesign-pagedesign-link-picalikesearch[Link_PicalikeSearch] — Liefert eine URL zur picalike-Bildersuche. Die Artikelbild-Position muss angegeben werden. Außerdem muss die picalike-Suche im Menü *Einrichtung » Mandant » Global » Dienste » picalike* aktiviert werden.
...
* xref:omni-channel:cms-syntax.adoc#webdesign-pagedesign-link-store[Link_Store] — Liefert die URL eines Mandanten (Shops). Die IDs werden im Menü *Einrichtung » Mandant » Mandant wählen » Einstellungen* unter *Webstore ID* angezeigt. Der Standard-Webshop hat die ID *0*, Mandanten erhalten ab ID *1* fortlaufende IDs.
