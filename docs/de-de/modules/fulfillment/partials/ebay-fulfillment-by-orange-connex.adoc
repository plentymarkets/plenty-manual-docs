...
* Prüfe die Auftragseinstellungen und passe abhängig von der Einstellung die xref:warenwirtschaft:lager-einrichten.adoc#200[Lagerprioritäten] oder das xref:auftraege:grundeinstellungen.adoc#200[Hauptlager] an den Varianten an.
...
Gib den Postausgangsserver / SMTP-Server und den Port ein und wähle die Verschlüsselungsmethode aus der Liste. Diese Daten bekommst du von deinem E-Mail-Anbieter.
...
Während der Einrichtung ist der Live-Modus deaktiviert. Das heißt, ausgehende E-Mails werden nur an die oben eingegebene E-Mail-Adresse gesendet, damit du den E-Mail-Versand testen kannst. Wenn du mit dem Einrichten deines Systems fertig bist, solltest du den Live-Modus aktivieren, damit E-Mails an deine Kund:innen gesendet werden können.

|(material:check_box_outline_blank[])
|Wenn du kein Häkchen bei *Live-Modus aktivieren* setzt (material:check_box_outline_blank[]), ist der Testmodus aktiviert. + Alle E-Mails werden nur an die eingegebene E-Mail-Adresse gesendet. So kannst du vor dem Live-Betrieb die korrekte Funktionsweise prüfen. +
