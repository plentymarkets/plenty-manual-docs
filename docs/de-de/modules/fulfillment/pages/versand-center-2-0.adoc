...
:description: Erfahre, wie du das neue Versand-Center in plentymarkets nutzt.
...
Das Menü *Aufträge » Versand-Center* ist wie folgt aufgebaut:
...
--
Du findest die folgenden Neuerungen im Versand-Center, die es vorher im "alten" Versand-Center nicht gab:
...
icon:bullhorn[role="orange"] Du möchtest wissen, welche Features und Funktionen als nächstes im Versand-Center zur Verfügung stehen? Dann schau einfach in diesem Forumsbeitrag nach:
--
////
...
Über die Übersicht im Versand-Center kannst du alle Aufträge in einer Liste öffnen oder du wählst Filter (material:tune[]), um die Suchergebnisse einzugrenzen. Standardmäßig ist der Datumsbereich der letzten 4 Wochen gesetzt.
...
*_Hinweis:_* In der Übersicht wird vor dem xref:auftraege:order-statuses.adoc#[Auftragsstatus] die Farbe, die du im Menü *Einrichtung » Aufträge » Auftragsstatus* für die jeweiligen Status gespeichert hast, angezeigt. +
...
Damit du einen häufig verwendeten Filter nicht jedes Mal beim Öffnen des Versand-Centers erst aus der Liste deiner <<#gespeicherte-filter-anwenden, gespeicherten Filter>> wählen musst, kannst du einen erstellten Filter als Standard festlegen. Jedes Mal, wenn du das Versand-Center öffnest, wird dieser Filter dann also automatisch ausgeführt.
...
In der Übersicht des Versand-Centers kannst du dir die folgenden Spalten anzeigen lassen:
...
[NOTE]
Nach der Retourenanmeldung wird standardmäßig eine Auftragsnotiz erstellt, die den Versanddienstleister und die Sendungsnummer der Retoure enthält. Diese Option kannst du unter *Einrichtung » Aufträge » Einstellung → Retouren* deaktivieren.
...
======
Als Alternative zur Auftragszusammenfassung eines Tages im Menü *Aufträge » Versand-Center* kannst du einen xref:daten:elastischer-export.adoc#[elastischen Export] durchführen. Erstelle dazu im Vorfeld ein Exportformat mit dem xref:daten:FormatDesigner.adoc#[FormatDesigner] vom Typ xref:daten:formatdesigner-auftraege.adoc#1100[Aufträge].
======
...
[NOTE]
Wenn mehrere Aufträge angemeldet werden sollen, entspricht die Reihenfolge, in der die Versandlabel gedruckt werden, der Reihenfolge, in der die Aufträge zuvor ausgewählt/angeklickt wurden. +
Wenn alle Aufträge ausgewählt werden, indem der Haken im entsprechenden Kästchen oben links gesetzt wird, wird beim Drucken die in der Liste bestehende Reihenfolge beibehalten.
...
Nachdem du die Retoure angemeldet hast, ist das Retourenlabel als PDF-Datei im Bereich *Retouren* des Auftrags zum Download verfügbar. Du meldest eine Retoure entweder über den Hauptauftrag oder über den Auftragstyp *Retoure* an. Weitere Informationen zum Auftragstyp *Retoure* findest du auf der Handbuchseite xref:auftraege:order-type-return.adoc#[Auftragstyp Retoure].
...
[#pakete-widget]
== Versandpakete in der Auftragsansicht bearbeiten

Auch in der UI eines Auftrags steht dir der Bereich Pakete zur Verfügung. Du kannst ihn über myView hinzufügen und seine Position bestimmen.

[[image-pakete-widget]]
.Bereich Pakete
image::fulfillment:pakete-widget.jpeg[width=640, height=360]

Der Bereich Pakete bietet dir die Funktionalitäten für Versandpakete aus dem Versand-Center: Mit einem Klick öffnet sich ein Overlay, in dem die Paketdaten angepasst werden können. 

[[table-edit-shipping-package-in-widget]]
.Versandpaket in der Auftragsansicht bearbeiten
[cols="1,3"]
|====
|Einstellung |Erläuterung

|icon:plus[]
|Mit einem Klick auf das Plus erstellst du weitere Pakete.

|material:text_snippet[]
|Lade deine Versandlabels herunter.

|material:print[]
|Durch einen Klick auf das Drucker-Symbol erhältst du eine Vorschau des/der Versandlabel(s).

|material:delete[]
|Lösche alle offenen Pakete.

|material:update[]
|Aktualisiere die Daten in diesem Widget.

|material:settings[]
|Konfiguriere die Spalten dieses Widgets.

|ID
|Klicke auf die Paket-ID, um die Paketinhaltsliste zu öffnen.

|Paketnummer
|Öffne die Sendungsverfolgung mit einem Klick auf die hinterlegte Paketnummer.

|material:more_vert[]
|Über das Drei-Punkte-Menü kannst du einzelne Pakete löschen.
...
Wenn du Versandpakete aus dem Versand-Center heraus bearbeitest, öffnet sich in einem neuen Tab die xref:fulfillment:paketinhaltsliste-testphase.adoc#[Paketinhaltsliste].
...
*_Wichtig:_* Die Anzahl der Versandpakete im Menü *Aufträge » Aufträge bearbeiten » [Auftrag öffnen]* im Tab *Einstellungen* hat keinen Einfluss auf die hier gespeicherten Pakete und wird bei Änderungen nicht angepasst. Ändere daher die Anzahl der Pakete ausschließlich im Versand-Center.
...
Weitere Informationen findest du im Bereich xref:auftraege:auftraege.adoc#[Aufträge] im Kapitel xref:auftraege:order-type-delivery-order.adoc#[Auftragstyp Lieferauftrag nutzen].
