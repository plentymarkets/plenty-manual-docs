...
image::fulfillment/assets/versand-center-2-0-menueaufbau.png[width=640, height=360]
...
Über die Übersicht im Versand-Center 2.0 kannst du alle Aufträge in einer Liste öffnen oder du wählst Filter (icon:filter[]), um die Suchergebnisse einzugrenzen. Standardmäßig ist der Datumsbereich der letzten 4 Wochen gesetzt.
...
* *_Möglichkeit 1:_* Klicke auf icon:search[set=material], um eine Liste aller Aufträge zu sehen.
...
*_Hinweis:_* In der Übersicht wird vor dem <<auftraege/auftraege-verwalten#1200, Auftragsstatus>> die Farbe, die du im Menü *Einrichtung » Aufträge » Status* für die jeweiligen Status gespeichert hast, angezeigt. +
...
|Wähle die Daten aus den Kalendern (icon:calendar[]), um nach Aufträgen in einem bestimmten Zeitraum zu suchen. 
...
. Klicke in die Zeile des Auftrags icon:shopping-cart[role="blue"] *Auftrags-ID*, um den Auftrag in der Detailansicht zu öffnen. 
. Nimm die gewünschten Änderungen vor.
. Klicke auf icon:times[] in der Zeile eines Auftrags, um den Auftrag wieder aus der Liste zu entfernen.
...
. Klicke in der Symbolleiste ganz oben auf *Aufträge anmelden* (icon:outgoing_items[set=plenty]). +
...
. Klicke in der Symbolleiste ganz oben auf *Polling* (icon:file_extension_csv[set=plenty]).
...
. Klicke in der Symbolleiste ganz oben auf *Retouren anmelden* (icon:order_return[set=plenty]). +
...
. Klicke in der Symbolleiste ganz oben auf *Zusammenfassung* (icon:order_note[set=plenty]).
. Wähle den Dienstleister aus der Dropdown-Liste. +
...
======
Als Alternative zur Auftragszusammenfassung eines Tages im Menü *Aufträge » Versand-Center 2.0* kannst du einen <<daten/daten-exportieren/elastischer-export#, elastischen Export>> durchführen. Erstelle dazu im Vorfeld ein Exportformat mit dem <<daten/daten-exportieren/FormatDesigner#, FormatDesigner>> vom Typ <<#daten/daten-exportieren/FormatDesigner/format-typen/auftraege#1100, Aufträge>>.
======
...
. Klicke in der Symbolleiste ganz oben auf *Versandlabels herunterladen* (icon:order_open[set=plenty]). +
...
. Klicke in der Symbolleiste ganz oben auf *Retourenlabels herunterladen* (icon:order_return_slip[set=plenty]). +
→ Die PDF-Datei wird generiert und du kannst sie auf deinem Computer speichern. +
...
*_Tipp:_* Wie du Retourenlabels über das Plugin *DHL Retoure Online* automatisch über eine Ereignisaktion oder einen Prozess generierst, ist auf der Handbuchseite <<fulfillment/versanddienstleister-plugins/plugin-dhl-retoure-online#optionen-retourenlabels-generieren, DHL Retoure Online>> beschrieben.
...
. Klicke in der Symbolleiste ganz oben auf *Aufträge zurücksetzen* (icon:reset[set=plenty]). +
...
image::fulfillment/assets/versand-center-funktionen-einzelner-auftrag.png[width=640, height=360]
...
Wenn der Auftrag erfolgreich angemeldet wurde, ist das Versandlabel im Bereich *Pakete* verfügbar. Mit einem Klick auf *Versandlabels herunterladen* (icon:order_open[set=plenty]) im Bereich *Pakete* des Auftrags kannst du das Label auf deinem Computer speichern und anschließend drucken.
...
Richte zunächst eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, die den Auftrag anmeldet und die Paketnummer zurückgibt.
...
Richte nun eine weitere <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, die auf den erfolgreich angemeldeten Auftrag reagiert und die E-Mail mit dem Versandlabel an deine Kund:innen versendet.
...
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Öffne den Auftrag.
. Klicke in der Symbolleiste ganz oben auf *Auftrag stornieren* (icon:times-circle[]). +
...
Nachdem du die Retoure angemeldet hast, ist das Retourenlabel als PDF-Datei im Bereich *Retouren* des Auftrags zum Download verfügbar. Du meldest eine Retoure entweder über den Hauptauftrag oder über den Auftragstyp *Retoure* an. Weitere Informationen zum Auftragstyp *Retoure* findest du auf der Handbuchseite <<auftraege/auftraege-verwalten#400, Aufträge verwalten>>.
...
Wenn die Retoure erfolgreich angemeldet wurde, ist das Retourenlabel im Bereich *Retouren* des Auftrags verfügbar. Mit einem Klick auf *Retourenlabel* (icon:order_open[set=plenty]) kannst du das Label auf deinem Computer speichern und es anschließend drucken.
...
image::fulfillment/assets/versand-center-retourenlabel-drucken.png[width=640, height=360]
...
Weitere Informationen zum Plugin *DHL Retoure Online* findest du auf der Handbuchseite <<fulfillment/versanddienstleister-plugins/plugin-dhl-retoure-online#, DHL Retoure Online>>. 
...
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Öffne den Auftrag.
. Klicke im Bereich *Pakete* auf *Exportlabels herunterladen* (icon:data_export[set=plenty]). +
→ Die PDF-Datei wird generiert und du kannst sie auf deinem Computer speichern.
...
Wenn du Versandpakete aus dem Versand-Center 2.0 heraus bearbeitest, öffnet sich in einem neuen Tab die <<auftraege/paketinhaltsliste#, Paketinhaltsliste>>. 
...
--
Wenn du einmal nicht weiterkommst, lohnt sich ein Blick in den umfangreichen Forumsbeitrag in unserer Fulfillment-Kategorie, in dem die häufigsten Fehler sowie deren Lösung aufgezeigt werden. Du findest den Beitrag link:https://forum.plentymarkets.com/t/fulfillment-faq/591262[hier^]{nbsp}icon:external-link[].
