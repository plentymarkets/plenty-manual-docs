...
Über die Übersicht im Versand-Center 2.0 kannst du alle Aufträge in einer Liste öffnen oder du wählst Filter (icon:tune[set=material]), um die Suchergebnisse einzugrenzen. Standardmäßig ist der Datumsbereich der letzten 4 Wochen gesetzt.
...
|Wähle die Daten aus den Kalendern (icon:today[set=material]), um nach Aufträgen in einem bestimmten Zeitraum zu suchen. 
...
======
Als Alternative zur Auftragszusammenfassung eines Tages im Menü *Aufträge » Versand-Center 2.0* kannst du einen xref:daten:elastischer-export.adoc#[elastischen Export] durchführen. Erstelle dazu im Vorfeld ein Exportformat mit dem xref:daten:FormatDesigner.adoc#[FormatDesigner] vom Typ <<#daten/daten-exportieren/FormatDesigner/format-typen/auftraege#1100, Aufträge>>.
======
...
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Öffne den Auftrag.
. Klicke in der Symbolleiste ganz oben auf *Auftrag stornieren* (icon:cancel[set=material]). +
