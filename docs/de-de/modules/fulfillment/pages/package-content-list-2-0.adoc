= Paketinhaltsliste 2.0
:keywords: Paketinhaltsliste, neue Paketinhaltsliste, Paket, Palette, Pakete packen, Schnelles Packen, Artikel verpacken, Pakete bearbeiten, Paletten bearbeiten, Artikel scannen
:description: Erfahre, wie du mit der Paketinhaltsliste 2.0 in plentymarkets arbeitest.
:author: team-payment-fulfillment

Auf dieser Handbuchseite erfährst du, wie du die Paketinhaltsliste 2.0 in plentymarkets verwendest. Die Paketinhaltsliste gibt Auskunft über den Inhalt von Paletten, Paketen und den darin enthaltenen Artikeln. Dabei entscheidest du, wie deine Pakete gepackt werden und kannst diese mit nur wenigen Klicks bequem verwalten. Du möchtest deine Paketinhaltsliste an deinen Fulfillment-Dienstleister übergeben? Kein Problem - nutze dazu den CSV-Export. Außerdem steht dir die neue Funktion <<#quick-packing, Schnelles Packen>> zur Verfügung, mit der du blitzschnell große Mengen von Artikeln auf Pakete verteilen kannst.

Das Menü *Aufträge » Paketinhaltsliste 2.0* ist wie folgt aufgebaut:

*[red]#Screenshot wird noch hinzugefügt#*
image::fulfillment:paketinhaltsliste-2-0-menueaufbau.png[width=640, height=360]

* *Navigation*: Hier navigierst du durch die einzelnen Bereiche des Menüs. Der aktuell ausgewählte Bereich (blaue Schrift) wird im Hauptfenster rechts angezeigt. Klicke auf icon:bars[], um die Navigationsleiste ein- und auszublenden. +
* *Toolbar*: Hier kannst du nach Aufträgen <<#search-orders, suchen>>, die <<#spalten-konfigurieren, Spalten konfigurieren>>, die Ergebnisse pro Seite, die du angezeigt haben möchtest, einstellen und durch die Seiten mit den Aufträgen klicken. +
* *Hauptfenster*: Hier wird der aktuell geöffnete Bereich angezeigt.
** *Übersicht*: Wenn du eine Suche ausführst, siehst du die Ergebnisse in einer Übersicht. Die Übersicht liefert auf einen Blick Informationen zu den gefundenen Aufträgen. Zum Beispiel siehst du den Auftragstyp, die Anzahl der Artikel im Auftrag oder den Status des Auftrags.
** *Detailansicht*: Wenn du einen Auftrag öffnest, werden die <#work-with-pallets, Paletten>> und <<#work-with-packages, Pakete>> sowie Details zu den Inhalten im Hauptfenster angezeigt.

[discrete]
[#new-features]
=== Neue Features

[.collapseBox]
.Neue Features im Vergleich zur "alten" Paketinhaltsliste
--
Du findest die folgenden Neuerungen in der Paketinhaltsliste 2.0, die es vorher in der "alten" Paketinhaltsliste nicht gab:

* Pakete können nun auch ohne Palette zu einem Auftrag hinzugefügt werden.
* Dank Gruppenfunktionen kannst du verschiedene Aktionen für mehrere Pakete oder Paletten auf einmal ausführen.
* Die Funktion <<#use-quick-packing, Schnelles Packen>> ist neu hinzugekommen und ist besonders bei großen Aufträgen hilfreich, da du damit viele Pakete auf einmal packen kannst.
* Dank einer Verknüpfung zu xref:maerkte:amazon-fba-nutzen.adoc#[Amazon FBA] kannst du das Versenden deiner Ware auch dem <<#use-amazon-fba-service, FBA-Service>> überlassen.
* Du kannst deine Auftragsinhalte nun als CSV-Datei exportieren.
--

[#assign-user-rights]
== Rechte für Backend-Benutzer:innen vergeben

:rights-purpose: mit der Paketinhaltsliste 2.0 arbeiten können
:right-one: pass:quotes[*Aufträge > Paketinhaltsliste 2.0*]

include::_includes:page$rechte.adoc[]

[#search-orders]
== Aufträge suchen

In der Übersicht der Paketinhaltsliste 2.0 kannst du alle Aufträge in einer Liste öffnen oder du wählst Filter (icon:tune[set=material]), um die Suchergebnisse einzugrenzen.

[.instruction]
Aufträge in der Paketinhaltsliste suchen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*.
.. *_Möglichkeit 1_*: Klicke auf icon:search[set=material], um direkt eine Liste aller Aufträge zu sehen.
.. *_Möglichkeit 2_*: Gib eine Auftrags-ID in das Suchfeld ein und klicke auf den automatischen Vorschlag.
Klicke dann auf icon:search[set=material], um die Suche auszuführen.
.. *_Möglichkeit 3_*: Klicke auf icon:tune[set=material], um die Suchergebnisse mit Hilfe von Filtern einzugrenzen. Beachte die Erläuterungen zu den Filtern in <<#table-search-orders>>.
. Wenn du die Suche ausführst, siehst du die Ergebnisliste.

*[red]#Screenshot wird noch hinzugefügt#*
[[image-example-search-field]]
.Beispiel: Suchfeld mit ID 140
image::fulfillment:package-content-list-search-field-example.png[width=640, height=360]

[[table-search-orders]]
.Filteroptionen in der Paketinhaltsliste
[cols="1,3"]
|====
|Einstellung |Erläuterung

|*ID*
|Gib die Auftrags-ID ein.

|*Paketzustand*
|Wähle den Paketzustand aus der Dropdown-Liste.
|====

[#aktuellen-filter-speichern]
=== Aktuellen Filter speichern

Wenn du eine Suche ausführst, siehst du deine gewählten Filter oben als Chips dargestellt. Diese Filter kannst du speichern, um sie in Zukunft schneller und einfacher wieder verwenden zu können.

[.instruction]
Aktuellen Filter speichern:

. Führe eine Suche mit einem oder mehreren der in <<#table-search-orders>> aufgelisteten Filter aus.
. Klicke auf *Gespeicherte Filter* (icon:bookmarks[set=material]).
. Klicke auf icon:bookmark_border[set=material] *Aktuellen Filter speichern*.
. Gib einen Namen für den Filter ein.
. Entscheide, ob
** dieser <<#filter-als-standard, Filter als Standard>> festgelegt werden soll (icon:toggle-on[role="blue"])
** dieser Filter für alle Benutzer:innen erstellt werden soll (icon:toggle_on[set=material, role=skyBlue]).
. Klicke auf *SPEICHERN*. +
→ Der Filter erscheint nun unter *Gespeicherte Filter* (icon:bookmarks[set=material]).

[TIP]
Lege die Reihenfolge der Filter per Drag-and-drop mit einem Klick auf *Verschieben* (icon:drag_indicator[set=material]) fest. Klicke auf icon:delete[set=material], um einen Filter zu löschen.

[#gespeicherte-filter-anwenden]
=== Gespeicherte Filter anwenden

[.instruction]
Gespeicherte Filter anwenden:

. Klicke auf *Gespeicherte Filter* (icon:bookmarks[set=material]).
. Klicke auf einen bereits erstellten Filter. +
→ Die Suche wird ausgeführt und die verwendeten Filtereinstellungen werden oben als Chips dargestellt.

[#filter-als-standard]
=== Filter als Standard festlegen

Damit du einen häufig verwendeten Filter nicht jedes Mal beim Öffnen der Paketinhaltsliste 2.0 erst aus der Liste deiner <<#gespeicherte-filter-anwenden, gespeicherten Filter>> wählen musst, kannst du einen erstellten Filter als Standard festlegen. Jedes Mal, wenn du die Paketinhaltsliste 2.0 öffnest, wird dieser Filter dann also automatisch ausgeführt.

Du kannst einen Filter direkt beim Erstellen als Standard festlegen wie im Kapitel <<#aktuellen-filter-speichern, Aktuellen Filter speichern>> beschrieben oder du legst den Filter nachträglich aus der Übersicht heraus als Standard fest (siehe <<#image-filter-standard>>).

*[red]#Screenshot wird noch hinzugefügt#*
[[image-filter-standard]]
.Filter als Standard festlegen
image::fulfillment:package-content-list-default-filter.png[width=640, height=360]

Klicke in der Zeile des gespeicherten Filters auf icon:star_border[set=material] *Als Standard festlegen*.

Wenn du einen anderen Filter als Standard festlegen möchtest, deaktiviere den aktuell gewählten Standardfilter mit einem Klick auf icon:star[set=material] *Nicht als Standard verwenden*.

[#spalten-konfigurieren]
== Spalten konfigurieren

In der Übersicht der Paketinhaltsliste 2.0 kannst du dir die folgenden Spalten anzeigen lassen:

* ID
* Auftragstyp
* Anzahl der Artikel
* Auftragsstatus
* Erstellt am

Passe die Tabelle deinen Bedürfnissen und deinem Arbeitsablauf an. Du kannst entscheiden, welche Spalten angezeigt werden und außerdem die Reihenfolge bestimmen.

[.instruction]
Spalten konfigurieren:

. Klicke auf *Spalten konfigurieren* (icon:settings[set=material]). +
→ Das Fenster *Spalten konfigurieren* wird geöffnet.
. Wähle, welche Spalten angezeigt werden sollen.
. Verschiebe (icon:sort[set=material]) die Spalten so, dass sie in der Reihenfolge angezeigt werden, in der du sie benötigst.
. Klicke auf *Bestätigen*, um deine Auswahl zu speichern.

[#gruppenfunktion-nutzen]
== Gruppenfunktion nutzen

Sobald du einen Auftrag markiert hast (icon:check_box[set=material, role=skyBlue]), werden die weiteren Optionen (icon:more_vert[set=material]) sichtbar. Wähle mindestens 2 Aufträge aus der Liste, um die Gruppenfunktionen nutzen zu können. In <<#table-group-functions>> sind die Bedienelemente aus der Gruppenfunktion sowie deren Erläuterungen aufgelistet.

[[table-group-functions]]
.Gruppenfunktion in der Paketinhaltsliste nutzen
[cols="2,5"]
|====
|Bedienelement |Erläuterung

|*CSV-Datei exportieren*
|Exportiert die CSV-Datei von allen gewählten Aufträgen.

|*Alle Paletten schließen*
|Schließt alle Paletten. +
*_Beachte:_* Sobald eine Palette geschlossen ist, kann diese nicht weiter bearbeitet werden.

|*Alle Pakete schließen*
|Schließt alle Pakete. +
*_Beachte:_* Sobald ein Paket geschlossen ist, kann dieses nicht weiter bearbeitet werden.

|*Alle Paletten löschen*
|Löscht alle Paletten.

|*Alle Pakete löschen*
|Löscht alle Pakete.
|====

[#use-amazon-fba-service]
== Amazon FBA-Service nutzen

Du möchtest Zeit sparen und den Fulfillment-Prozess lieber jemand anderem überlassen? Kein Problem - der Amazon FBA-Service macht es möglich. Dank der Verknüpfung zwischen der Paketinhaltsliste und Amazon FBA kannst du mit wenigen Klicks einstellen, welche deiner Artikel über diesen Service versendet werden sollen. Entscheidest du dich für die Abwicklung über Amazon FBA, übernimmt Amazon ebenfalls den Kundenservice und kümmert sich um eventuelle Rücksendungen.
In einem Anlieferplan, den du auf Amazon erstellst, teilst du Amazon zunächst mit, welche Ware du in das Amazon FBA-Lager schicken möchtest. Gib dort an, welche Artikel du in welcher Menge und in welchen Paketen sendest. Sobald ein Auftrag existiert, kann Amazon die Ware dann in deinem Namen an die Endkund:innen verschicken. +
Weitere Informationen zu den Einstellungen für den Amazon FBA-Service findest du auf der Handbuchseite xref:maerkte:amazon-fba-nutzen.adoc#120[Amazon: FBA-Service nutzen].

////
Neuen Button im Menü Amazon FBA Inbound auf Inbound-Seite beschreiben + Links zur alten PCL anpassen und stattdessen auf neue PCL verlinken oder laufen beide parallel …dann alte Konfig kopieren und für neuen Button anpassen??. - Kathrin kontaktieren
Aktuelle Konfig anpassen (Titel des Buttons anpassen!)
////

[#work-with-pallets]
== Mit Paletten arbeiten

Paletten sind die größte Verpackungseinheit in einem Auftrag der Paketinhaltsliste und können Pakete und Artikel enthalten. Allerdings sind Paletten in der Paketinhaltsliste 2.0 nicht mehr notwendig, um ein Paket dem Fulfillment-Dienstleister zu übergeben. In der Abbildung siehst du die verschiedenen Ebenen eines Auftrags. In den folgenden Kapiteln ist beschrieben, wie du mit Paletten arbeitest.

*[red]#Abbildung wird noch hinzugefügt#*
image::auftraege:pcl-levels-de.png[]

[#create-pallet]
=== Palette erstellen

Um eine neue Palette für einen Auftrag zu erstellen, gehe vor wie folgt.

[.instruction]
Palette erstellen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*.
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Klicke oben auf icon:plus[role="darkGrey"] *Inhalt hinzufügen* und wähle die Option *Palette erstellen*.
→ Das Fenster *Neue Palette erstellen* wird geöffnet.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#table-create-new-pallet>>.
. Klicke auf icon:plus[role="darkGrey"] *Palette erstellen*. +
→ Die Palette wird erstellt.

[[table-create-new-pallet]]
.Neue Palette erstellen
[cols="1,3"]
|====
|Einstellung |Erläuterung

|*Höhe (cm)*
|Gib die Höhe der Palette ein.

|*Breite (cm)*
|Gib die Breite der Palette ein.

|*Länge (cm)*
|Gib die Länge der Palette ein.

|*Individuelles Palettengewicht (kg)*
|Gib das individuelle Gewicht der Palette ein.

|*Palette ist stapelbar*
|Aktiviere diese Option, wenn die Palette stapelbar ist.

|*Zusätzliche Informationen*
|Gib optional zusätzliche Informationen ein.

|*Barcode*
|Gib den Barcode der Palette ein.
|====


[#configure-pallet]
=== Palette konfigurieren

Um Paletten zu konfigurieren und vorhandene Einstellungen anzupassen, gehe vor wie folgt.

[.instruction]
Palette konfigurieren:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben der Palette, die du konfigurieren möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette konfigurieren* (icon:settings[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette konfigurieren* (icon:settings[set=material]). +
→ Das Fenster *Palette konfigurieren* wird geöffnet.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#table-create-new-pallet>>. +
. Klicke auf *Palette konfigurieren*. +
→ Die Palette wird deinen Eingaben entsprechend konfiguriert.

[#close-pallet]
=== Palette schließen

Sind alle Einstellungen für eine Palette abgeschlossen, kannst du die Palette schließen. Beachte allerdings, dass alle darin enthaltenen Pakete mit geschlossen werden und anschließend keine Änderungen mehr vorgenommen werden können. Um eine Palette zu schließen, gehe vor wie folgt.

[.instruction]
Palette schließen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben der Palette, die du schließen möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette schließen* (icon:lock[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette schließen* (icon:lock[set=material]). +
→ Das Fenster *Palette schließen* wird geöffnet.
. Wenn du sicher bist, dass du die Palette schließen möchtest, klicke auf *Palette schließen* (icon:lock[set=material]). +
→ Die Palette wird geschlossen.

[#delete-pallet]
=== Palette löschen

Wenn du eine Palette löschst, sind anschließend keine Änderungen mehr möglich. Um eine Palette zu löschen, gehe vor wie folgt.

[.instruction]
Palette löschen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben der Palette, die du löschen möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette löschen* (icon:delete[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Palette löschen* (icon:delete[set=material]). +
→ Das Fenster *Palette löschen* wird geöffnet.
. Wenn du sicher bist, dass du die Palette löschen möchtest, klicke auf *Palette löschen* (icon:delete[set=material]). +
→ Die Palette wird gelöscht.

[#work-with-packages]
== Mit Paketen arbeiten

Pakete sind die zweitgrößte Verpackungseinheit in einem Auftrag der Paketinhaltsliste und enthalten Artikel bzw. Varianten. Pakete können auf eine Palette gepackt werden. In den folgenden Kapiteln ist beschrieben, wie du mit Paketen arbeitest. Du kannst <<#create-package, neue Pakete erstellen>>, <<#add-item-to-package, Artikel einem Paket hinzufügen>>, <<#edit-items, Artikel bearbeiten>>, <<#add-package-to-pallet, Pakete auf eine Palette stellen>>, die Funktion <<#use-quick-packing, Schnelles Packen>> verwenden, <<#configure-package, Pakete konfigurieren>>, den <<#change-scan-type, Scan-Typ ändern>>, <<#close-package, Pakete schließen>> und <<#delete-package, Pakete löschen>>.

[#create-package]
=== Paket erstellen

Um ein neues Paket zu erstellen, gehe vor wie folgt.

[.instruction]
Paket erstellen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Klicke oben auf icon:plus[role="darkGrey"] *Inhalt hinzufügen* und wähle die Option *Paket erstellen*. +
→ Das Fenster *Neues Paket erstellen* wird geöffnet. +
*_Oder:_* Klicke links neben der Palette auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option icon:plus[role="darkGrey"] *Paket erstellen*.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#table-create-new-package>>.
. Klicke auf icon:plus[role="darkGrey"] *Paket erstellen*. +
→ Das Paket wird erstellt.


[[table-create-new-package]]
.Neues Paket erstellen
[cols="1,3"]
|====
|Einstellung |Erläuterung

|*Pakettyp*
|Wähle einen Pakettyp aus der Dropdown-Liste.

|*Typ der Verpackungseinheit*
|Wähle den Typ der Verpackungseinheit aus der Dropdown-Liste.

|*Paketgewicht (kg)*
|Gib das Paketgewicht ein.
|====

[#add-item-to-package]
=== Artikel verpacken

Um Artikel zu verpacken und einem Paket hinzuzufügen, gehe vor wie folgt.

[.instruction]
Artikel verpacken:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links im Bereich *Unverpackte Artikel* einen Haken neben den Artikeln, die du dem Paket hinzufügen möchtest.
. Klicke auf icon:plus[role="darkGrey"] *Artikel zum Paket hinzufügen*. +
→ Die Artikel werden in den Bereich *Verpackte Artikel* verschoben und somit dem Paket hinzugefügt. +
*_Tipp:_* Möchtest du dies wieder rückgängig machen, setze neben den Artikeln im Bereich *Verpackte Artikel* einen Haken und klicke auf *Artikel aus dem Paket entfernen* (icon:delete[set=material]). Klicke anschließend auf icon:delete[set=material] *Löschen*, um die Artikel wieder aus dem Paket zu entfernen.

[TIP]
.Artikel per Varianten-ID hinzufügen
======
Bereits vorhandene Artikel müssen in der Paketinhaltsliste 2.0 nicht mehr manuell erstellt werden. Um einen Artikel mit wenigen Klicks zu einem Paket hinzuzufügen, wähle die Varianten-ID als Scan-Typ. Sobald du die Varianten-ID im Eingabefeld eingibst, wird dir der Artikel vorgeschlagen und du kannst ihn hinzufügen.
======

[#edit-items]
=== Artikel bearbeiten

Wenn du dich in der Paketinhaltsliste befindest, kannst du auch auf Varianteneinstellungen zugreifen und diese bei Bedarf anpassen.

[.instruction]
Artikel bearbeiten:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Klicke auf ein Paket. +
→ Die im Paket enthaltenen Artikel werden angezeigt.
. Klicke in der Spalte *ID* auf die Varianten-ID, die du öffnen möchtest. +
→ Die Varianteneinstellungen im Menü *Artikel » Artikel bearbeiten* werden geöffnet.
. Nimm bei Bedarf Änderungen vor.
. Speichere (icon:save[set=plenty, role="green"]) die Änderungen.


[#add-package-to-pallet]
=== Paket auf Palette stellen

Hast du in einem Auftrag ein Paket, das sich bisher in keiner Palette befindet, kannst du dieses mit wenigen Klicks auf eine Palette stellen. In diesem Fall kannst du das Paket entweder auf eine bestehende Palette stellen oder dafür eine neue Palette erstellen. Es können auch mehrere Pakete gleichzeitig auf eine Palette gestellt werden.

[.instruction]
Paket auf Palette stellen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze neben dem Paket, das du auf eine Palette stellen möchtest, einen Haken (icon:check-square[role="blue"]). +
*_Tipp:_* Du kannst auch mehrere Pakete auf einmal wählen und diese gesammelt auf eine Palette stellen.
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket auf Palette stellen* (icon:table-rows[set=material]). +
→ Das Fenster *Paket auf Palette stellen* wird geöffnet.
. Wähle die ID der Palette aus der Dropdown-Liste oder wähle die Option *Neue Palette erstellen*. +
. Klicke auf *Paket auf Palette stellen* (icon:table-rows[set=material]).
→ Das Paket wird auf die gewählte Palette gestellt.


[#use-quick-packing]
=== Schnelles Packen verwenden

Die Funktion *Schnelles Packen* ermöglicht es dir, mehrere Pakete gleichzeitig zu erstellen und die unverpackten Artikel gleichmäßig auf die Pakete aufzuteilen. Dies ist besonders hilfreich, wenn du große Aufträge hast und viele Pakete mit demselben Inhalt gleichzeitig packen möchtest.

[.instruction]
Schnelles Packen verwenden:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze neben dem Paket oder der Palette, für die du das Schnelle Packen verwenden möchtest, einen Haken (icon:check-square[role="blue"]). +
*_Tipp:_* Du kannst auch mehrere Pakete, mehrere Paletten oder eine Kombination aus Paketen und Paletten für das Schnelle Packen wählen.
. Klicke auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Schnelles Packen* (icon:stock[set=plenty]). +
→ Das Fenster *Schnelles Packen* wird geöffnet.
. Klappe links die Artikeleinstellungen auf (icon:chevron-right[set=material]).
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#table-quick-packing>>. +
*_Hinweis:_* Gibt es eine Restmenge an Artikeln, die nicht verpackt werden kann, kannst du wählen, ob die übrigen Artikel in ein weiteres Paket gepackt werden sollen, oder ob du die Artikel unverpackt lassen möchtest.
. Klicke anschließend auf icon:plus[role="green"] *Hinzufügen*. +
→ Die Artikel werden den Einstellungen entsprechend gepackt.

[[table-quick-packing]]
.Einstellungen schnelles Packen
[cols="1,3"]
|====
|Einstellung |Erläuterung

|*Pakettyp*
|Wähle den Pakettyp aus der Dropdown-Liste.

|*Typ der Verpackungseinheit*
|Wähle die Verpackungseinheit aus der Dropdown-Liste.

|*Paketgewicht (kg)*
|Gib das Paketgewicht ein.

|*Artikel zu verpacken*
|Gib die Anzahl der Artikel ein, die verpackt werden sollen.

|*Artikel pro Paket*
|Gib die Anzahl der Artikel ein, die jeweils in 1 Paket gepackt werden sollen.

|*Anzahl der Pakete*
|Gib die Anzahl der Pakete ein, die gepackt werden sollen.

|*Gesamtgewicht der Pakete*
|Gib das Paketgewicht insgesamt ein.

|*Paketvolumen (cm³)*
|Gib das Paketvolumen ein.

|====

[#edit-scan-type]
=== Scan-Typ ändern

Zum Scannen von Artikeln gibt es verschiedene Möglichkeiten. Entscheide selbst, wie du deine Artikel scannen möchtest und verändere bei Bedarf den Scan-Typ.

[.instruction]
Scan-Typ ändern:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Klicke auf ein Paket. +
→ Die Artikel werden angezeigt.
. Klicke oben rechts auf *Scan-Typ ändern* (icon:settings[set=material]).
. Wähle eine der verfügbaren Optionen. +
→ Der Scan-Typ wird angepasst.

[#configure-package]
=== Paket konfigurieren

Um Pakete zu konfigurieren und vorhandene Einstellungen anzupassen, gehe vor wie folgt.

[.instruction]
Paket konfigurieren:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben dem Paket, das du konfigurieren möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket konfigurieren* (icon:settings[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket konfigurieren* (icon:settings[set=material]). +
→ Das Fenster *Paket konfigurieren* wird geöffnet.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#table-configure-package>>. +
. Klicke auf *Paket konfigurieren*. +
→ Das Paket wird deinen Eingaben entsprechend konfiguriert.

[[table-configure-package]]
.Einstellungen Paket konfigurieren
[cols="1,3"]
|====

|*Pakettyp*
|Wähle einen Pakettyp aus der Dropdown-Liste.

|*Typ der Verpackungseinheit*
|Wähle den Typ der Verpackungseinheit aus der Dropdown-Liste.

|*Paketgewicht (kg)*
|Gib das Paketgewicht ein.

|*Paketvolumen (cm³)*
|Gib das Paketvolumen ein.
|====

[#close-package]
=== Paket schließen

Sind alle Einstellungen für ein Paket und die darin enthaltenen Artikel abgeschlossen, kannst du das Paket schließen. Die Palette, in dem das Paket liegt, kann trotzdem weiterhin geöffnet bleiben. Beachte allerdings, dass anschließend keine Änderungen mehr an dem Paket vorgenommen werden können. Um ein Paket zu schließen, gehe vor wie folgt.

[.instruction]
Paket schließen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben dem Paket, das du schließen möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket schließen* (icon:lock[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket schließen* (icon:lock[set=material]). +
→ Das Fenster *Paket schließen* wird geöffnet.
. Wenn du sicher bist, dass du das Paket schließen möchtest, klicke auf *Paket schließen* (icon:lock[set=material]). +
→ Das Paket wird geschlossen.

[#delete-package]
=== Paket löschen

Wenn du ein Paket löschst, sind anschließend keine Änderungen mehr möglich. Um ein Paket zu löschen, gehe vor wie folgt.

[.instruction]
Paket löschen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Setze links neben dem Paket, das du löschen möchtest, einen Haken (icon:check-square[role="blue"]).
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket löschen* (icon:delete[set=material]). +
*_Oder:_* Klicke links in der Seitennavigation auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option *Paket löschen* (icon:delete[set=material]). +
→ Das Fenster *Paket löschen* wird geöffnet.
. Wenn du sicher bist, dass du das Paket löschen möchtest, klicke auf *Paket löschen* (icon:delete[set=material]). +
→ Das Paket wird gelöscht.

[#close-orders]
== Aufträge schließen

Wenn du dir sicher bist, dass du an einem Auftrag nichts mehr ändern möchtest, kannst du diesen schließen. Beachte, dass anschließend keine Änderungen mehr möglich sind.

[.instruction]
Aufträge schließen:

. Öffne das Menü *Aufträge » Paketinhaltsliste 2.0*. +
. Suche den Auftrag anhand der Filtereinstellungen. Beachte dazu die Erläuterungen in <<#search-orders, Aufträge suchen>>. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen. +
→ Der Auftrag wird links in der Navigation angezeigt.
. Klicke auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option (icon:lock[set=material]) *Auftrag schließen*. +
→ Das Fenster *Auftrag schließen* wird geöffnet.
. Wenn du dir sicher bist, dass du den Auftrag schließen möchtest, klicke auf (icon:lock[set=material]) *Auftrag schließen*. +
→ Der Auftrag wird geschlossen.
