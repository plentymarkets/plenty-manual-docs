...
. Klicke oben auf *Auftragsinhalte als CSV-Datei exportieren* (icon:receipt_long[set=material]). +
→ Die Auftragsinhalte werden als CSV-Datei exportiert.

[TIP]
.Inhalte mehrerer Aufträge als CSV-Datei exportieren
======
Möchtest du die Inhalte mehrerer Aufträge als CSV-Datei exportieren, setze in der Übersicht neben allen gewünschten Aufträgen ein Häkchen (icon:check_box[set=material, role=skyBlue]). Klicke anschließend oben auf *Auftragsinhalte als CSV-Datei exportieren* (icon:receipt_long[set=material]). Die Inhalte aller gewählten Aufträge werden dann gesammelt in einer CSV-Datei exportiert.
======
...
*_Oder:_* Klicke links in der Navigation neben dem Auftrag auf die weiteren Optionen (icon:more_vert[set=material]) und wähle die Option icon:plus[role="darkGrey"] *Transporteinheit hinzufügen* > icon:plus[role="darkGrey"] *Paket erstellen*.
...
. Wähle die Option *Palette konfigurieren* (icon:settings[set=material]). +
...
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]).
. Wähle die Option *Palette schließen* (icon:lock[set=material]). +
...
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]).
. Wähle die Option *Palette löschen* (icon:delete[set=material]). +
...
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Auftrags, um den Auftrag zu öffnen.
. Klicke oben auf icon:plus[role="darkGrey"] *Inhalt hinzufügen*.
...
======
Bereits vorhandene Artikel müssen in der Paketinhaltsliste 2.0 nicht mehr manuell erstellt werden. Um einen Artikel mit wenigen Klicks zu einem Paket hinzuzufügen, wähle die Option *Varianten-ID* als Scan-Typ. Sobald du die Varianten-ID im Eingabefeld eingibst, wird dir der Artikel vorgeschlagen und du kannst ihn hinzufügen.
======
...
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]).
. Wähle die Option *Paket auf Palette stellen* (icon:table_rows[set=material]). +
...
. Klicke auf *Paket auf Palette stellen* (icon:table_rows[set=material]). +
...
. Wähle die Option *Schnelles Packen* (icon:dynamic_feed[set=material]). +
...
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]).
. Wähle die Option *Paket konfigurieren* (icon:settings[set=material]). +
...
. Klicke anschließend oben auf die weiteren Optionen (icon:more_vert[set=material]).
. Wähle die Option *Paket schließen* (icon:lock[set=material]). +
