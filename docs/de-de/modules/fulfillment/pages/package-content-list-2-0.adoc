...
======
Möchtest du die Inhalte mehrerer Aufträge als CSV-Datei exportieren, setze in der Übersicht neben allen gewünschten Aufträgen einen Haken (icon:check-square[role="blue"]). Klicke anschließend oben auf *Auftragsinhalte als CSV-Datei exportieren* (icon:receipt_long[set=material]). Die Inhalte aller gewählten Aufträge werden dann gesammelt in einer CSV-Datei exportiert.
======
...
. Setze links im Bereich *Unverpackte Artikel* einen Haken (icon:check-square[role="blue"]) neben den Artikeln, die du dem Paket hinzufügen möchtest.
...
======
Möchtest du Artikel aus einem Paket wieder entfernen, setze neben den Artikeln im Bereich *Verpackte Artikel* einen Haken (icon:check-square[role="blue"]) und klicke auf *Artikel aus dem Paket entfernen* (icon:delete[set=material]). Klicke anschließend auf icon:delete[set=material] *Löschen*, um die Artikel aus dem Paket zu entfernen.
======
...
. Wähle die Option *Schnelles Packen* (icon:stock[set=plenty]). +
