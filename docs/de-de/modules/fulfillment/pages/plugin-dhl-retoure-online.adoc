...
Sobald du das xref:plugins:hinzugefuegte-plugins-installieren.adoc#plugin-installieren[Plugin in einem Plugin-Set installiert] hast, ist der Assistent im Menü *Einrichtung » Assistenten » Plugins* im relevanten Plugin-Set sichtbar und du kannst mit dem Einrichten beginnen.
