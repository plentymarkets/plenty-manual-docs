...
Neben der Möglichkeit, größere Mengen an Paketnummern komfortabel über das xref:fulfillment:dokumente-erzeugen.adoc#paketnummern-elasticsync[Import-Tool] in dein System zu importieren, kannst du dies auch xref:fulfillment:dokumente-erzeugen.adoc#paketnummern-manuell-importieren[manuell] oder mit Hilfe eines xref:fulfillment:dokumente-erzeugen.adoc#paketnummern-barcodescanner[Barcodescanners] erledigen.
...
Wenn du bereits eine Adressetikettenvorlage erstellt und eine PDF-Vorlage gespeichert hast, fahre mit Kapitel xref:fulfillment:dokumente-erzeugen.adoc#adressetikett-erzeugen[Adressetikett erzeugen] fort.
...
| *Artikelauflistung*
|Die Artikelauflistung bestimmt, wie die Artikeldaten auf dem Etikett angezeigt werden. Es gibt 2 Typen: *Standard* und *Individuell*. Eine Erläuterung zu diesen beiden Typen findest du im Kapitel xref:fulfillment:dokumente-erzeugen.adoc#artikelauflistung[Artikelauflistung].
...
| *Kundeneigenschaften*
|Die Koordinaten zur Positionierung der Kundeneigenschaften eingeben. Die xref:crm:kontakte-verwalten.adoc#950[Eigenschaft] aus der Dropdown-Liste wählen. Auf den Adressetiketten wird der Text angezeigt, der im Menü *CRM » Kontakte » [Kontakt öffnen] » Ansicht: Eigenschaften* gespeichert wurde.
