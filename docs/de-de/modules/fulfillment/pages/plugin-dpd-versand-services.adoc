...
Um die Schnittstelle für DPD Deutschland in deinem plentymarkets System einrichten und nutzen zu können, musst du dich bei link:https://www.dpd.com/de/de/[DPD^]{nbsp}icon:external-link[] registrieren und deinen Account für den Webservice freischalten lassen. Um die Zugangsdaten zu erhalten, die du für die Einstellungen in plentymarkets benötigst, wende dich bitte an deine DPD Kontaktperson.
...
* den xref:fulfillment:versand-vorbereiten.adoc#800[Versanddienstleister] *DPD Versand-Services* erstellen
** Versanddienstleister kannst du aktuell nur im Backend und _nicht_ über einen Assistenten erstellen.

* xref:fulfillment:versand-vorbereiten.adoc#1500[Portotabellen] für *DPD Versand-Services* erstellen +
 ...
 ** Das DPD Versand-Services-Plugin muss im link:https://marketplace.plentymarkets.com/plugins/integration/dpdshippingservices_6320[plentyMarketplace^]{nbsp}icon:external-link[] heruntergeladen worden sein, damit es dir hier angezeigt wird.
...
Aktiviere die Schaltfläche *ParcelShopFinder nutzen* (icon:toggle_on[set=material, role=skyBlue]), wenn du deinen Kund:innen den ParcelShopFinder in deinem plentyShop anbieten möchtest.
...
Wenn du international versendest, aktiviere die Schaltfläche (icon:toggle_on[set=material, role=skyBlue]). 3 weitere Felder erscheinen: Hier gibst du den Inhalt der Sendung ein, wählst eine der internationalen Handelsklauseln (Incoterms) aus der Dropdown-Liste und legst fest, welche Zollpapiere versendet werden.

Wenn du Gefahrgut versendest, aktiviere die Schaltfläche (icon:toggle_on[set=material, role=skyBlue]). 2 weitere Felder erscheinen: Hier gibst du eine Beschreibung des Gefahrguts ein und wählst die Art der Verpackung aus der Dropdown-Liste.
