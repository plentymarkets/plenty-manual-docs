...
| *Statuswechsel*
|Die Ereignisaktion wird ausgelöst, wenn ein Auftrag den xref:auftraege:order-statuses.adoc#[Auftragsstatus] wechselt. Wähle den passenden Statuswechsel aus der Dropdown-Liste aus.
...
|Die Ereignisaktion wird ausgelöst, wenn in einem Auftrag der xref:auftraege:order-statuses.adoc#[Reparaturstatus] geändert wird. Zur Auswahl für diese Ereignisaktion stehen die angelegten Reparaturstatus.
...
xref:auftraege:order-type-credit-note.adoc#correct-and-cancel-credit-note-document[Stornobeleg Gutschrift] +
xref:auftraege:proformarechnung-erzeugen.adoc#[Proformarechnung] +
xref:fulfillment:versand-center-2-0.adoc#retourenlabel-abrufen[Retourenlabel] +
xref:auftraege:gelangensbestaetigung-erzeugen.adoc#[Gelangensbestätigung] +
xref:auftraege:order-type-multi-order.adoc#generate-multi-order[Sammelrechnung] +
...
*_Wichtig:_* Diese Option wird nur angezeigt, wenn die für Lieferaufträge nötige xref:auftraege:grundeinstellungen.adoc#[Lagerzuordnung] korrekt vorgenommen wurde.
...
Die Aktion kann mehrmals hinzugefügt werden, sodass in einer Ereignisaktion Tags sowohl hinzugefügt als auch entfernt werden können, wenn nötig. +
...
| *Status des Elternauftrags ändern*
|Die Aktion ändert den Auftragsstatus eines Elternauftrags. Wähle den Auftragsstatus aus der Dropdown-Liste, den der Elternauftrag bei Teilung eines Auftrags (Funktion: *Auftrag teilen* in der Auftragsbearbeitung) oder nach *Lieferaufträge generieren* erhalten soll.
...
|Die Aktion erzeugt eine xref:auftraege:order-type-multi-order.adoc#generate-multi-order[Sammelrechnung] für einen Auftrag. +
...
|Die Aktion erzeugt eine xref:auftraege:auftragsbestaetigung.adoc#[Auftragsbestätigung] für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.
...
|Die Aktion erzeugt ein xref:auftraege:korrekturbeleg.adoc#[Korrekturbeleg] für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.

| *Sammelgutschrift erzeugen*
|Die Aktion erzeugt eine xref:auftraege:order-type-multi-order..adoc#generate-multi-credit-note[Sammelgutschrift] für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.
...
|Die Aktion erzeugt einen xref:auftraege:order-type-credit-note.adoc#correct-and-cancel-credit-note-document[Stornobeleg Gutschrift] für einen Auftrag. +
...
|Die Aktion erzeugt eine xref:auftraege:proformarechnung.adoc#[Proformarechnung] für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.
...
|Die Aktion erzeugt einen xref:auftraege:abhol-lieferschein.adoc#[Abhol-/Lieferschein] für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.
...
|Die Aktion legt Versandpakete für die ausgewählten Auftrag an, bevor sie im xref:fulfillment:versand-center-2-0.adoc#[Versand-Center] geöffnet werden.
...
|Die Aktion erzeugt ein DHL-Retourenlabel über xref:fulfillment:versand-vorbereiten.adoc#3200[DHL Online Retoure] oder xref:fulfillment:versand-vorbereiten.adoc#3100[DHL Retoure Beilegeretikett], das im xref:fulfillment:versand-center-2-0.adoc#[Versand-Center] bzw. im Webshop im Bereich *Mein Konto* verfügbar ist.
