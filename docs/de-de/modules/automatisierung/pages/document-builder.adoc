...
Tabelle mit Variablen hinzufügen - siehe EmailBuilder
Variablen werden noch neu gruppiert - in UI schauen, welche es gibt und hier in Tabelle auflisten

= Editor
kurze Einleitung...
Hinweis: der Editor wird nur sichtbar, wenn man Text markiert!
...
Nice to have (if it is there, add it): ask Kevin whether this is available - depends on the ShopBuilder
font colour and highlighting in colour
spacing (between lines; paragraphs)
