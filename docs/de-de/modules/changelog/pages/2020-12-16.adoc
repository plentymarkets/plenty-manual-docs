...
* Der Abgleich der Variante wird nun in einem eigenen Bereich names *Daten ergänzen* angezeigt. Da bei diesem Abgleichfeld die im System hinterlegten Informationen der Varianten geladen und dem Auftrag hinzugefügt werden. Weitere Informationen findest du auf der Handbuchseite <<daten/daten-importieren/sync-typen/elasticSync-auftraege#, Import-Typen » Aufträge>>.
