...
Sobald du beide Plugins im deinem System installiert hast, kann das Exportformat *{export-format}* erstellt werden. Weitere Informationen findest du auf der Handbuchseite xref:daten:elastischer-export.adoc#[Elastischer Export].
...
*_Hinweis:_* Für {price-search-engine} ist noch keine Auftragsherkunft in deinem plentymarkets System vorhanden. Du musst die Auftragsherkunft für {price-search-engine} deshalb <<#auftragsherkunft-erstellen, manuell im Menü *Einrichtung » Aufträge » Auftragsherkunft* erstellen.>>
