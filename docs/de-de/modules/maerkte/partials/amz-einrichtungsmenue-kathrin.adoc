...
Die gewählte Option greift nur für das Generieren von SKUs von Artikel, die zum ersten Mal zu Amazon exportiert werden.
...
| Welche Auftagspositionen sollen an Amazon übertragen werden? +

*_Hintergrund:_* Welche Option du wählst hängt davon ab, wie du FBA-Waren für Artikelpakete bei Amazon lagerst. Sind alle Paketbestandteile von Artikelpaketen gemeinsam verpackt bei Amazon eingelagert? Oder stellt Amazon die einzelnen Paketbestandteile selbst zu Artikelpaketen zusammen?
...
! Alle Paketbestandteile des Artikelpakets sind in einer einzigen Verpackung bei Amazon gelagert. Amazon pickt also nur eine Position.

! *Ohne Paketposition übertragen*
! Nur die Paketbestandteile werden an Amazon übertragen.
! Das Artikelpaket besteht aus einzelnen Artikeln. Bei Amazon sind diese Artikel alle einzeln eingelagert. Erst durch den Pick-Vorgang wird also aus den Einzelartikeln ein Artikelpaket. 
!===
