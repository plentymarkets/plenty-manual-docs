...
Mit den *Formateinstellungen*, *Exporteinstellungen* und *Download-Einstellungen* kannst du xref:daten:marktplatzexport.adoc#file-export[einmalig oder regelmäßig die Variantendaten des Katalogs in eine Datei exportieren]. Beachte jedoch, dass diese Datei keine Informationen darüber enthält, ob die darin enthaltenen Varianten erfolgreich zu {market} exportiert wurden.
====
