...
Beim Bestandsabgleich für zusätzliche SKUs werden auch FBA-Bestände importiert und in plentymarkets aktualisiert.
// TODO: WTF!?!?!?



    Eben nicht für den Ex-, sondern den Import (und hierbei Zuordnungsfehler zu verhindern).

Zusätzliche SKUS können für den Bestandsexport, sowie für den Preisexport zusätzlich zu der „echten“ SKU des Artikels/der Variante genutzt werden.
Hast du beispielsweise mehrere Angebote zu einem Artikel/einer Variante in deiner Seller Central aktiv, kann aus einem Artikel/einer Variante in plentymarkets heraus für die zusätzlichen SKUS auch der Bestand und Preis übermittelt werden.
Die zusätzlichen SKUS können außerdem für die Zuordnung des Artikels (Auftrag → Variante/Artikel im plentymarkets-System) genutzt werden.

:exclamation: Die Einschränkung hier ist, dass die zusätzlichen SKUS NICHT für den Artikelexport genutzt werden können (und von Amazon aus nicht „dürfen“), da es von Amazon aus generell nicht erwünscht ist, zu einem Artikel/einer Variante (eindeutige Identifizierung über die EAN/ASIN/etc.) mehrere Angebote zu erstellen.

Hier nochmal der Handbuchlink zur Konfiguration.
...
| Exportfilter: Wenn ein Artikel eine nicht freigegebene Verfügbarkeit hat, werden die Varianten des Artikels nicht exportiert.

| *Markierung 1*; +
*Markierung 2*
| Exportfilter: Diese Filter greifen nur für den täglichen Export.

| *Tags*
| Exportfilter: Nur Varianten mit den gewählten Tags werden exportiert. Es können ein oder mehrere Tags gewählt werden.

| *Export-Kategorien*
| Exportfilter: Kategorien aktivieren, um nur Varianten dieser Kategorien zu exportieren.

| *BMVD-Export-Kategorien*
| Exportfilter: BMVD steht für Bücher, Musik, Video und DVD. Kategorien aktivieren, um nur Varianten dieser Kategorien zu exportieren.

| *Nur im Webshop verfügbare Artikel exportieren*
| Exportfilter: Option aktivieren, um nur Varianten zu exportieren, die für diesen Mandanten freigegeben sind.
...
material:toggle_on[role="blue"] = Die folgenden Tags sind zugelassen: +
