...
. Klicke links auf das zuvor erstellte Konto in der Kontenübersicht. +
→ Das Tab *Einstellungen* für dieses Konto wird geöffnet.
. Klicke zuerst auf die Schaltfläche *Zugriff erlauben*. +
→ Hier erlaubst du plentymarkets Zugriff auf dein Amazon-Konto, damit Daten zwischen Amazon und plentymarkets ausgetauscht werden können. Du wirst zu Amazon weitergeleitet und musst die Zugangsdaten deines Amazon-Verkäuferkontos eingeben. Nachdem du dich bei Amazon eingeloggt hast, wird dir angezeigt, auf welche Daten aus Amazon plentymarkets zugreifen darf. Klicke dort auf *Confirm*, um die Autorisierung abzuschließen. +
