...
| Welche Auftragsinformationen sollen bei Artikelpaketen an Amazon übertragen werden? +

*_Hintergrund:_* Welche Einstellung du wählst hängt davon ab, wie du die Waren bei Amazon lagerst. Sind alle Paketbestandteile von Artikelpaketen gemeinsam verpackt bei Amazon eingelagert? Oder stellt Amazon die einzelnen Paketbestandteile selbst zu Artikelpaketen zusammen?
...
! Nur die Paketposition wird an Amazon übertragen. Wähle diese Option, wenn Artikelpakete zusammen verpackt als einzelne Artikel be Amazon gelagert sind.

! *Ohne Paketposition übertragen*
! Nur die Paketbestandteile werden an Amazon übertragen. Wähle diese Option, wenn Amazon Artikelpakete für dich aus Einzelartikeln zusammenstellt.
!===
