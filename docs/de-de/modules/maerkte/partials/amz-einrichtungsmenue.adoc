...
[#300]
==== Zugriff erlauben und Amazon-Konto einrichten

Nun erstellst du ein Konto in plentymarkets. Pro Amazon-Plattform erstellst du ein eigenes Konto. Du erlaubst plentymarkets Zugriff auf dein Amazon-Konto, damit Daten zwischen Amazon und plentymarkets ausgetauscht werden können. Danach nimmst du die weiteren Einstellungen vor.
...
→ Dir wird angezeigt, auf welche Amazon-Daten plentymarkets zugreifen darf.
...
| *Zugriff erlauben*
| Über diese Schaltfläche erlaubst du plentymarkets Zugriff auf dein Amazon-Konto, damit Daten zwischen Amazon und plentymarkets ausgetauscht werden können. Du wirst zu Amazon weitergeleitet und gibst dort die Zugangsdaten deines Amazon-Verkäuferkontos ein. Nachdem du dich bei Amazon eingeloggt hast, wird dir angezeigt, auf welche Daten aus Amazon plentymarkets zugreifen darf. Klicke dort auf *Confirm*, um die Autorisierung abzuschließen. +
→ In plentymarkets wird das Fenster *Autorisierung abschließen* geöffnet. Klicke auf Abschließen, um mit der Einrichtung deines Kontos fortzufahren.

*_Wichtig:_* Führe diese Schritte aus, bevor du die weiteren Kontoeinstellungen vornimmst.
