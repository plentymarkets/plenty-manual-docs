...
*_Hinweis:_* Wenn du die API-Umgebung wechselst, also zum Beispielvom Test-Modus in den Produktiv-Modus wechselst, musst du einen neuen <<#api-schluessel, API-Schlüssel>> erzeugen.
...
Die Eigenschaften für *Abzugsverfahren* und *Versandvorlaufzeit* sind vorausgewählt.

[cols="1a,3a"]
!===
! *Abzugsverfahren*
! Das sogenannte _Reverse-Charge-Verfahren_. Gültige Werte sind `true` oder `false`.

! *Versandvorlaufzeit*
! Die sogenannte _leadtime-to-ship_. Also die Zeit in Tagen, die Händler:innen von der Auftragsannahme bis zur Versandmeldung benötigt. Hier muss eine ganze Zahl zwischen 1 und 45 übermittelt werden.
...
! material:toggle_on[]
! Für Varianten ohne Bestandsbindung wird die Menge exportierst, die du im Feld *Menge für Artikel (Varianten) ohne Bestandsbindung* eingibst.
