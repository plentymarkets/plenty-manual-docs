...
*_Tipp:_* Um alle Varianten eines Artikels auf {market-availability} zu verkaufen, nimm die Einstellungen an der Hauptvariante vor. Die Verfügbarkeit wird dann für alle Varianten des Artikels aktiviert, wenn die xref:artikel:vererbung.adoc#[Vererbung] an den Varianten aktiviert ist.
