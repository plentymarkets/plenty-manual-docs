...
Beim Katalogexport werden Eigenschaften verwendet, um Datenfelder an den Marktplatz zu übergeben, für die es in plentymarkets keine eigene Einstellung gibt. Dazu erstellst du eine Eigenschaft. Diese Eigenschaft ordnest du anschließend im Katalog dem Datenfeld des Marktplatzes zu.
