...
[NOTE]
.Katalogexport vs. alter Variantenexport
====

Auf dieser Seite erfährst du, wie du deine Artikeldaten per Katalogexport zu Zalando exportierst. Der Katalogexport für Zalando befindet sich noch in der Beta-Phase. 

* Du richtest Zalando zum ersten Mal in deinem plentymarkets System ein? Dann empfehlen wir dir trotzdem, den Katalogexport zu wählen.
* Du möchtest trotzdem lieber den alten Export nutzen? Oder du verwendest den alten Export und möchtest etwas zur Einrichtung nachschlagen? Dann wechsle zur Handbuchseite xref:page$zalando.adoc[Zalando (alter Artikelexport)].
* Du verwendest noch den alten Export, möchtest aber wechseln? xref:partial$zalando-switch-to-catalogue.html[Dann mache dich mit den nötigen Schritten für einen Wechsel vertraut.]
====
...
| *Katalog nutzen*
| Wähle, ob du für den Export deiner Artikeldaten den alten XML-Export oder den neuen Katalogexport nutzen möchtest. 

* Du richtest Zalando neu in deinem plentymarkets System ein? Dann empfehlen wir dir, den Katalogexport zu nutzen.
* Du verkaufst schon auf Zalando und möchtest auf den Katalogexport wechseln? Dann aktiviere die Option, nachdem du den Katalog eingerichtet hast.
...
[#1100]
=== _Checkliste:_ Pflichtfelder

Zalando erwartet bestimmte Pflichtangaben. Nur Varianten werden also an den Marktplatz exportiert, für die diese Angaben gespeichert sind. Die in <<#CB94KIOU>> aufgeführten Angaben müssen an allen Varianten gespeichert sein, die an Zalando exportiert werden sollen.
