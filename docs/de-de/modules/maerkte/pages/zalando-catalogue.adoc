...
[INFORMATION]
.Doch lieber den alten Artikelexport?
====

Auf dieser Seite erfährst du, wie du deine Artikeldaten per Katalogexport zu Zalando exportierst. Dieser Export befindet sich noch in der Beta-Phase. Trotzdem empfehlen wir dir, den Katalogexport zu wählen, wenn du Zalando neu in deinem plentymarkets System einrichtest. 

Du möchtest trotzdem lieber den alten Export nutzen? Oder du verwendest den alten Export und möchtest etwas zur Einrichtung nachschlagen? +
Dann wechsle zur Handbuchseite xref:page$zalando.adoc[Zalando (alter Artikelexport)].
====
