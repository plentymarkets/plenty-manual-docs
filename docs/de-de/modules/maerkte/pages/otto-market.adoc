...
Bei Hermes können Retourenlabels nicht über eine Ereignisaktion erstellt werden. Bevor das Versandetikett für OTTO Market erstellt wird, muss deshalb das Retourenlabel für Hermes entweder manuell über das xref:fulfillment:versand-center.adoc#[Versand-Center] oder über eine xref:automatisierung:aktionen.adoc#preamble[Aktion] in einem Prozess erstellt werden. Weitere Informationen findest du im link:https://marketplace.plentymarkets.com/plugins/integration/hermesshippinginterface_5437[User Guide^] des Hermes-Plugins.
