...
| Bestandsabgleich
| alle 15 Minuten +
*_Hinweis:_* Fällt eine Variante auf Bestand 0, wird diese Variante zum Schutz des Artikel-Rankings auf OTTO Market nach 15 Tagen automatisch deaktiviert. +
Wenn neuer Bestand für eine solche deaktivierte Variante gebucht wird, wird die Variante mit dem nächtlichen Artikelexport wieder aktiviert.
...
'''
*Verkaufspreis erstellen* = Erstellt einen neuen Verkaufspreis und ordnet diesen Verkaufspreis dem Marktplatz-Datenfeld zu. Dieser automatisch erstellte Verkaufspreis kann im Menü xref:artikel:preise.adoc#400[Einrichtung » Artikel » Verkaufspreise] geprüft und bearbeitet werden.
...
[#katalog-vorschau]
=== Vorschau des Katalogs herunterladen

Nachdem du einen Katalog zum ersten Mal exportiert hast, kannst du eine Vorschau des Katalogs herunterladen. Die Vorschau enthält die ersten 5 Varianten, die in diesem Katalog zu OTTO Market exportiert werden.
...
→ Ein Fenster zum manuellen Download der Vorschaudatei wird geöffnet.
. Klicke auf *Herunterladen* (icon:download[set=plenty, role="darkGrey"]). +
→ Die Datei wird heruntergeladen. Sie enthält die ersten 5 Varianten des Exports.
