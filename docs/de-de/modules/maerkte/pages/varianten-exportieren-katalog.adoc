...
*_Hinweis:_* Wenn Amazon eine neue Version der Flatfile veröffentlicht, muss auch ein neuer Katalog erstellt werden. Die gute Nachricht ist: Du kannst den Katalog mit der alten Flatfile-Version einfach und schnell xref:daten:marktplatz-formate-exportieren.adoc#80[kopieren und anpassen].
