...
link:http://www.kauflux.de/[Kauflux.de^]{nbsp}icon:external-link[] ist ein offener Marktplatz mit unabhängigen Anbietern aus verschiedenen Bereichen. Kauflux.de vermarktet deine Produkte und Dienstleistungen und stellt für dich den direkten Kontakt zu Käufer:innen her.

[#100]
==  Bei Kauflux registrieren

Du musst dich zunächst bei link:http://www.kauflux.de/konto[Kauflux^]{nbsp}icon:external-link[] registrieren, um dein plentymarkets System für Kauflux einzurichten.
...
|Über diesen Link überträgt Kauflux die Aufträge zu plentymarkets. Dieser Link wird für den Auftragsimport von Kauflux benötigt und nicht für die Erstellung des Artikelkatalogs. Der Artikelkatalog wird über den elastischen Export im Menü *Daten » Elastischer Export* erstellt. Weitere Informationen findest du bei Kauflux unter link:http://www.kauflux.de/shop-anmelden/automatischer-auftragsdatenexport[Auftragsdatenexport^]{nbsp}icon:external-link[].
|====
