...
*Hinweis:_* Informationen zum Einrichten überzeugender, qualitativ hochwertiger und konsistenter Artikellistings findest Du unter *Decathlon Procukt-Vorgaben ansehen* mit Klick auf (material:more_vert[]) neben (material:save[])
...
*Hinweis:_* Wenn der *Auftragsimport* Schalter  *An* ist, sind Statuszuordnungen für *bereits versendete Aufträge, Gutschriften* und *Stornierte Aufträge* Pflicht. +
...
*Hinweis:_* Bei der Kontoerstellung wird das *Afutragsimport Startdatum*, automatisch, mit dem aktuellen Datum befüllt. Um alte Aufträge zu importieren, geben ein benutzerdefiniertes Startdatum an.
...
| If *FBD aktivieren* Schalter *An* ist, beachte bitte, dass der Warenbestandsabgleich und die Auftragsabwicklung von Decathlon verwaltet werden. Das zugewiesene Lager wird zur Synchronisierung Deines Warenbestands verwendet (Decathlon &rarr; plentysystems)

| *FBD Aktivieren* ausschalten
| Durch Deaktivieren dieser Option ändert sich das Verhalten aller Decathlon-bezogenen Importe und Exporte. Fahren fort, nur wenn Du dir sicher bist,dass Deine Vereinbarung mit Decathlon diesen Schritt ermöglicht.
|===
