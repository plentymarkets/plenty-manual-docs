...
| *Verbindung testen*
| Klicke hier, um zu testen, ob die Verbindung zu Decathlon hergestellt werden konnte. In diesem Schritt wird der API Key überprüft. +

Wenn der API Key geändert wird, muss die Verbindung erneut getestet werden.
...
*_Hinweis:_* Bei Decathlon können Aufträge nicht von Kund:innen storniert werden. Stattdessen wird bei Decathlon immer eine Erstattung angelegt, wenn ein Artikel zurückgegeben wurde. +
In plentymarkets werden Erstattungen von Decathlon in Stornierungen umgewandelt, je nachdem, welcher Status für die Einstellung *Vollständig erstatteten Auftrag automatisch stornieren bis Status* gewählt wurde.
