...
Wenn du einen Katalog für Decathlon erstellt hast, wird in deinem plentymarkets System die Eigenschaft *Kaufland.de-Kategorie* erstellt. Mit dieser Eigenschaft legst du fest, in welcher Kategorie deine Varianten auf Kaufland angeboten werden.
...
Im Menü *Einrichtung » Märkte » Decathlon » Datenaustausch* hast du die Möglichkeit, Daten manuell zu im- und exportieren. Dieses Menü ist vor allem für Testzwecke gedacht. Du kannst beispielsweise nur bestimmte Artikel- und Angebotsdaten exportieren oder einzelne Aufträge in plentymarkets importieren. Das kann nützlich sein, wenn du den Export testen willst oder wenn du fehlende Aufträge noch einmal importieren musst.
