...
*_Wichtig:_* Im Moment ist mit plentymarkets nur der Eigenversand durch Händler:innen möglich. Zalando nennt das Partner fulfillment (own logistics). Zalando Fulfillment Solutions (ZFS) wird noch nicht unterstützt.
...
*_Wichtig:_* Du musst den Assistenten zweimal durchlaufen:

. Schließe den Assistenten zum ersten Mal ab, um deine Kontodaten zu speichern.
. Melde dich im Forum, damit der plentymarkets Support dein Konto mit deinem zDirect-Konto verknüfen kann.
. Öffne den Assistenten erneut.
. Wähle im Schritt *Aufträge* die Länder, aus denen Aufträge importiert werden sollen.
. Aktiviere dein Zalando-Konto, um den Datenaustausch zu starten.
...
| Wenn du den Assistenten zum ersten Mal durchläufst, kannst du noch keine Länder wählen.
Zuerst muss dein Zalando-Konto freigeschaltet werden. link:https://forum.plentymarkets.com/t/sammelthema-aktivierung-neuer-zalando-konten/600409[Melde dich in diesem Forum-Thread, um die Freischaltung deines Kontos zu erbitten.]
...
Öffne danach den Assistenten noch einmal und wähle die Länder.

In welchen Ländern du über Zalando verkaufen kannst, hängt von deinem Vertrag mit Zalando ab. In plentymarkets können Aufträge aus den folgenden Ländern importiert werden:
