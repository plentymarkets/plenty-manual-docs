...
*_Wichtig:_* Im Moment ist mit plentymarkets nur der Eigenversand durch Händler:innen möglich. Zalando nennt das Partner Fulfillment (own logistics). Der Versand durch Zalando, also Zalando Fulfillment Solutions (ZFS), wird noch nicht unterstützt.
...
[#150]
== Übersicht: Schritte nach der Registrierung

Du hast einen Vertrag mit Zalando abgeschlossen. Super. Wie geht es jetzt weiter?

Hier siehst du eine Übersicht der Schritte, die dich erwarten. Jeder dieser Schritte ist auf dieser Handbuchseite beschrieben.

* Nimm die Grundeinstellungen im Menü *Einrichtung » Märkte » Zalando » Einstellungen* vor.
* Fordere im plentymarkets Forum das CSV-Formular zum Abgleichen von Artikeln/Silhouetten an.
...
* Durchlaufe den Assistenten *Kontoeinstellungen - Zalando* zum ersten Mal. +
*_Wichtig:_* Beim ersten Durchlauf kannst du nicht alle Einstellungen des Assistenten vornehmen.
* Bitte unseren Support, dein Konto mit deinem zDirect-Konto zu verknüpfen. +
*_Wichtig:_* Das geht erst, nachdem du den Assistenten zum ersten Mal durchlaufen hast.
* Durchlaufe den Assistenten *Kontoeinstellungen - Zalando* noch einmal, um die fehlenden Einstellungen vorzunehmen.
* Nimm alle weiteren Einstellungen vor, die auf dieser Seite beschrieben sind.
* Alles passt und es kann losgehen? Dann aktiviere dein Zalando-Konto im Assistenten, um den Datenaustausch mit Zalando zu starten.
...
. Trage in Zeile 6 der Datei das Kennwort ein, das du im Menü *Einrichtung » Märkte » Zalando » Einstellungen* als *Benutzer* gewählt hast.
...
* Durchlaufe den Assistenten *Kontoeinstellungen - Zalando* erneut:
  * Im Schritt *Aufträge* wählst du nun die Länder. +
  → Für jedes gewählte Land wird ein Schritt *Bestände* und ein Schritt *Preise* zum Assistenten hinzugefügt.
  * Durchlaufe die neuen Schritte. (<<#1400, Wie?>>)
...
*_Hinweis:_* Werden keine Gruppen angezeigt? Dann hat Zalando entweder noch keine Silhouetten bereitgestellt oder du hast Zalando noch keine FTP-Zugangsdaten zukommen lassen (siehe <<#150, Schritte nach der Registrierung>>).
. Speichere (icon:save[set=plenty]) die Einstellungen. +
...
Ativiere dein Zalando-Konto im Assistenten *Kontoeinstellungen - Zalando*, um den Datenaustausch mit Zalando zu starten.
