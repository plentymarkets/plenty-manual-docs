...
[#150]
== Übersicht der notwendige Schritte
...
Du hast den Assistenten zum ersten mal durchlaufen und abgeschlossen. Lass nun dein Zalando-Konto freischalten: link:https://forum.plentymarkets.com/t/sammelthema-aktivierung-neuer-zalando-konten/600409[Melde dich in diesem Forum-Thread. Bitte unseren Support, dein Konto freizuschalten.]
...
[#artikel-loeschen]
== Artikel bei Zalando löschen

Varianten werden bei Zalando gelöscht und können nicht mehr auf dem Marktplatz gekauft werden, wenn sie in plentymarkets keinen Bestand mehr haben. Dies passiert automatisch, denn Bestandsdaten werden alle 15 Minuten zu Zalando übertragen. Wenn die Variante in deinem plentymarkets System wieder Bestand hat, wird sie mit der nächsten Bestandsübertragung also auch wieder auf Zalando zum Verkauf angeboten.

Wenn du eine Variante, die in plentymarkets Bestand hat, nicht mehr auf Zalando verkaufen möchtest, musst du an der Variante die Verfügbarkeit für den Marktplatz entfernen.
...
. *Lösche* (icon:minus-circle[role="red"]) die Verfügbarkeiten *Zalando* und die Verfügbarkeit des Zalando-Marktplatzes, auf dem die Variante nicht mehr verkauft werden soll, zum Beispiel *Zalando DE*.
. *Speichere* (terra:save[]) die Einstellungen. +
→ Die Variante wird in den nächsten 15 Minuten bei Zalando gelöscht und kann dort nicht mehr gekauft werden.

[TIP]
.Zeitstempel für Marktplatzexporte aktualisieren
====
Wenn du sicherstellen möchtest, dass die Variante sofort bei Zalando gelöscht wird, kannst du zusätzlich mit der *Artikel-Gruppenfunktion* den *Zeitstempel für Marktplatzexporte aktualisieren*. +
Öffne dazu das Menü *Artikel » Artikel bearbeiten » Tab: Artikel-Gruppenfunktion*. Setze ein Häkchen bei *Zeitstempel für Marktplatzexporte aktualisieren* und klicke auf terra:execute[] *Ausführen*. Die Variante wurde gelöscht und kann nun nicht mehr auf Zalando gekauft werden.
====
