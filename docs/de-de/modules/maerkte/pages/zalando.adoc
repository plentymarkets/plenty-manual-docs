...
Im Normalfall werden pro Minute 1.000 Bestände aktualisiert. Wird die von Zalando vorgegebene API-Limitierung erreicht, werden die übrigen Bestände gesendet, sobald die API nach spätestens einer Minute wieder entsperrt ist.
...
Du möchtest am Partnerprogramm von Zalando teilnehmen? Dann link:https://www.zalando.de/partner-programm/[wende dich zunächst direkt an Zalando^]{nbsp}icon:external-link[].
...
* Melde dich in der Kategorie link:https://forum.plentymarkets.com/c/multi-channel/zalando[Multi-Channel: Zalando^]{nbsp}icon:external-link[] des plentymarkets Forums.
* Im Forum erhältst du von unserem Support-Team ein Formular im CSV-Format. Dieses Formular enthält die folgenden Informationen:
  ...
  *_Tipp:_* Auf diesen FTP-Server sendet Zalando deine Silhouetten, damit sie in dein System importiert werden können.
...
| *Lagerauswahl*
a| Diese Einstellung ist seit der Umstellung auf die zDirect-API (27.10.2021) ohne Funktion. Wähle stattdessen die Vertriebslager im Assistenten *Kontoeinstellungen - Zalando*.

2+|Erweiterte Optionen

| *Bestandspuffer*
| Diese Einstellung ist seit der Umstellung auf die zDirect-API (27.10.2021) ohne Funktion. Lege stattdessen einen Bestandspuffer im Assistenten *Kontoeinstellungen - Zalando* fest.

| *Menge für Artikel ohne Bestandsbindung*
| Diese Einstellung ist seit der Umstellung auf die zDirect-API (27.10.2021) ohne Funktion. Lege stattdessen die Menge im Assistenten *Kontoeinstellungen - Zalando* fest.

| *Grenzwert für Artikel-Upload*
| Diese Einstellung ist seit der Umstellung auf die zDirect-API (27.10.2021) ohne Funktion. Lege stattdessen den maximalen Bestand im Assistenten *Kontoeinstellungen - Zalando* fest.
...
Zalando erwartet bestimmte Pflichtangaben. Das bedeutet, dass nur Varianten an den Marktplatz exportiert werden können, für die diese Angaben gespeichert sind. Die in <<#zalando-varianten-pflicht>> aufgeführten Angaben müssen an allen Varianten gespeichert sein, die an Zalando exportiert werden sollen.
...
* <<fulfillment/versand-center#retourenetikett-hauptauftrag-abrufen, Ein Retourenetikett über Versandcenter erzeugen.>>
* <<automatisierung/prozesse/aktionen#380, Ein Retourenetikett per Prozess-Aktion erzeugen.>>
* link:https://developers.plentymarkets.com/rest-doc#/Returns/put_rest_orders__orderId__shipping_returns_assign_label__returnsId_[Das Retourenetikett per REST hinzufügen.^]{nbsp}icon:external-link[]
* link:https://developers.plentymarkets.com/rest-doc#/Order/put_rest_orders__orderId__shipping_packages__orderShippingPackageId_[Die Tracking-ID der Retoure per REST zu einem Paket am Auftrag hinzufügen.^]{nbsp}icon:external-link[]
--

Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Versandbestätigungen automatisch an Zalando zu senden, nachdem ein Warenausgang gebucht wurde.
...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Zalando automatisch über Stornierungen zu informieren. Beachte, dass als versendet markierte Aufträge nicht mehr storniert werden können.
...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Zalando automatisch über Retouren zu informieren. Beachte, dass der Auftrag bereits als versendet markiert sein muss.
...
--
Der Datenaustausch mit Zalando ist nicht aktiv. Um die zDirect-API-Schnittstelle von Zalando zu aktivieren, muss dein Zalando-Konto mit dem plentymarkets Entwicklungskonto verknüpft werden. Diese Verknüpfung passiert im Verlauf der Integration automatisch. Allerdings muss plentymarkets in einem zusätzlichen Schritt Zugangsdaten für die API generieren und diese Zugangsdaten in deinem System hinterlegen. Diese Zugangsdaten dürfen nicht von dir einsehbar sein. Deshalb kann plentymarkets dir die Zugangsdaten nicht bereitstellen. Melde dich in link:https://forum.plentymarkets.com/t/sammelthema-aktivierung-neuer-zalando-konten/600409[diesem Forum-Thread^]{nbsp}icon:external-link[], um die Freischaltung deines Kontos zu erbitten.
...
a| Das Export-Log *zalandoArticleExportLog.txt* enthält Log-Einträge für Varianten, die nicht zu Zalando exportiert werden konnten. Das Export-Log wird einmal pro Nacht automatisch generiert.
