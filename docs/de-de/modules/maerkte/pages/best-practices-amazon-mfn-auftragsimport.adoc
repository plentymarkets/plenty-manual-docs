...
Den Auth-Token kannst du unter diesem link:https://sellercentral.amazon.de/apps/manage[Link^] aktualisieren (das Ablaufdatum wird in der Zukunft gesetzt, somit ist die Berechtigung wieder aktiv).

*_Tipp:_* Du kannst den Auth-Token auch über den Assistenten *Amazon - Grundeinstellungen* im Menü *Einrichtung » Assistenten » Omni-Channel* aktualisieren. Klicke dazu im Schritt *Zugriff auf Amazon erlauben* auf *Zugriff erlauben*. Du wirst zu Amazon weitergeleitet und musst dich dort mit deinen Amazon-Zugangsdaten einloggen. Klicke dort auf *Confirm*, um einen neuen Authentifizierungstoken zu generieren.
