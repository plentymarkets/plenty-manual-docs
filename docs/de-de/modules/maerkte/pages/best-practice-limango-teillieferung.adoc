...
Im Folgenden wird beschrieben, wie du den Lieferauftrag und die Ereignisaktion für die Artikel erstellst, die bereits verschickt werden sollen. Erstelle anschließend einen Lieferauftrag und eine Ereignisaktion für die restlichen Artikel des Auftrags.
...
*_Hinweis:_* Der Status für die Ereignisaktion wurde beispielhaft gewählt. Wähle einen Status, der am besten zu deinem Arbeitsablauf passt. Dazu kannst du einen bereits vorhandenen Status wählen oder einen eigenen Status xref:auftraege:auftraege-verwalten.adoc#_neuen_status_erstellen[erstellen]. +
...
== Beispiel: Mehrere Lieferaufträge aus einem Auftrag erstellen
...
[cols="1a,1a,1a"]
|===
|Lieferauftrag |enthaltene Artikel |Ereignisaktion
