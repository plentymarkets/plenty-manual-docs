...
| Wählen, ob Rabatte von Amazon bei Retouren berücksichtigt werden sollen oder nicht. Wurde der FBA-Auftrag komplett oder teilweise mit einem Gutschein beglichen, wird der Gutschein bei der Retoure hinzugefügt und verrechnet. Dabei wird kein Mindestbestellwert für den Gutschein berücksichtigt.

Wenn diese Option aktiviert wird, dann werden die Artikelpositionen für die Amazon Delivery Campaigns in den automatisch angelegten FBA-Retouren angelegt.
