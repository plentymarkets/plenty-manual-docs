...
* xref:auftraege:buchhaltung.adoc#_neuen_standort_anlegen[So legst du einen neuen Standort an]
...
. Nimm die weiteren Einstellungen gemäß der Erläuterungen auf der Handbuchseite xref:warenwirtschaft:lager-einrichten.adoc#[Lager einrichten] vor.
. Speichere (icon:save[set=plenty]) die Einstellungen. +
...
. Nimm die Einstellungen im Bereich *Amazon* vor. Beachte dazu die Erläuterungen in <<artikelverfügbarkeit-amazon>> im Kapitel xref:maerkte:amazon-einrichten.adoc#700[Variantenverfügbarkeit einstellen].
. Speichere (icon:save[set=plenty]) die Einstellungen.
...
.SKU-Zuordnung
======
Für den manuellen Bestandsabgleich muss die SKU-Zuordnung in den xref:maerkte:amazon-einrichten.adoc#400[Grundeinstellungen] eingegeben werden.
======
...
. Speichere (icon:save[set=plenty]) die Einstellungen, um den Abgleich durchzuführen.
--
====
...
. Klicke für die gewünschte Sendung in der Spalte *Bestellung* auf *Umbuchung erstellen* (icon:stock[set=plenty]). +
...
[#170]
==== Paketinhaltsliste 2.0 öffnen

Eine xref:fulfillment:package-content-list-2-0.adoc#[Paketinhaltsliste] gibt Auskunft über den Inhalt von Paletten, Paketen und Artikeln. Die Paketinhaltsliste sorgt dafür, dass Amazon weiß, welche Artikel sich in den Paketen befinden, die Amazon für dich lagert und versendet.

Die Paketinhaltsliste 2.0 bereitet die Pakete anhand der Einstellungen an der Variante vor. Du kannst diese Einstellungen ändern. Öffne dazu das Menü *Artikel » Artikel bearbeiten » [Artikel öffnen] » Artikel-Tab: Einstellungen » Bereich: Versand* und ändere die Option in der Dropdown-Liste *Packstücke*.

[TIP]
.Menü *Aufträge » Paketinhaltsliste 2.0*
====
Du kannst die Paketinhaltsliste 2.0 auch über das Menü *Aufträge » Paketinhaltsliste 2.0* öffnen und abarbeiten. Die Schaltfläche *Anmelden* wird aber nur angezeigt, wenn du die Paketinhaltsliste 2.0 über das Menü *Waren » Amazon FBA Inbound* öffnest und du alle Pakete und Paletten geschlossen hast. Die Schaltfläche *Anmelden* benötigst du, wenn du die gepackten Pakete bei Amazon anmelden möchtest.
====
...
. Klicke auf den Auftrag. +
→ Die Optionen für die Auftragsbearbeitung werden angezeigt.
. Klicke auf *FBA-Inbound-Paketinhaltsliste 2.0*. +
→ Der Auftrag wird im Menü *Aufträge » Paketinhaltsliste 2.0* angezeigt.
. Packe den Auftrag anhand der xref:fulfillment:package-content-list-2-0.adoc#[Paketinhaltsliste 2.0].

[.collapseBox]
.Alte Paketinhaltsliste öffnen
--
...
→ Der Auftrag wird im Menü *Aufträge » Paketinhaltsliste 2.0* angezeigt.
. Klicke auf *Anmelden*. +
*_Hinweis:_* Die Schaltfläche *Anmelden* wird nur angezeigt, wenn du die Paketinhaltsliste 2.0 über das Menü *Waren » Amazon FBA Inbound* öffnest. +
...
→ Der Auftrag wird im Menü *Aufträge » Paketinhaltsliste 2.0* angezeigt.
. Klicke auf *Anmelden*. +
*_Hinweis:_* Die Schaltfläche *Anmelden* wird nur angezeigt, wenn du die Paketinhaltsliste 2.0 über das Menü *Waren » Amazon FBA Inbound* öffnest und du alle Pakete und Paletten geschlossen hast. +
→ Das Fenster *Auftrag anmelden* wird geöffnet.
...
[.collapseBox]
.FBA-Inbound-Auftrag über alte Paketinhaltsliste bei Amazon anmelden
--
...
[#460]
=== Fehlerbehebung: FBA-Bericht manuell generieren

Amazon generiert FBA-Berichte einmal alle vier Stunden. Danach werden die FBA-Auftragsdaten automatisch in plentymarkets importiert. Um Aufträge zu prüfen oder bei Problemen mit dem Import kannst du jedoch auch im plentymarkets Backend manuell einen Bericht deiner FBA-Aufträge generieren. Anders als der Bericht, den du in manuell im Seller Central erzeugen kannst, enthält der plentymarkets-FBA-Bericht auch die personenbezogenen Daten deiner Endkund:innen. Beim Generieren eines Berichts im plentymarkets Backend werden außerdem fehlende, gelöschte oder fehlerhafte FBA-Aufträge neu importiert.

IMPORTANT: Das Generieren von FBA-Berichten erzeugt eine hohe Datenlast. Wenn du zu viele Berichte generierst, wird deshalb die API deines Systems temporär deaktiviert. Dadurch können auch andere Prozesse deines Systems kurzzeitig in Mitleidenschaft gezogen werden. Generiere deshalb nur einen FBA-Bericht, wenn es Probleme mit dem automatischen Import gab.

[.instruction]
FBA-Bericht manuell generieren:

. Öffne das Menü *Einrichtung » Märkte » Amazon » Datenaustausch » Berichte*.
. Wechsle in das Tab *FBA Berichte*.
. Wähle ein *Amazon-Konto* aus der Dropdown-Liste.
. Wähle ein Startdatum. +
*_Tipp:_* Das Startdatum darf nicht mehr als ein Jahr zurückliegen und muss vor dem Enddatum liegen.
. Wähle ein Enddatum. +
*_Tipp:_* Das Enddatum darf nicht in der Zukunft liegen. Außerdem dürfen Start- und Enddatum nicht mehr als eine Woche auseinander liegen. +
*_Hinweis:_* Die FBA-Berichte enthalten Auftragsdaten von 0:00 Uhr des Startdatums bis 0:00 Uhr des Enddatums. Die Berichte enthalten also keine Auftragsdaten des gewählten Enddatums. Ausschlaggebend ist die Zeitzone des plentymarkets Systems.
. Klicke auf *Bericht generieren* (icon:execute[set=plenty]). +
→ Der FBA-Bericht wird generiert. +
→ Fehlende, gelöschte oder fehlerhafte FBA-Aufträge werden neu importiert.
