...
Wenn du eine Variante, die in plentymarkets Bestand hat, aber nicht mehr auf idealo verkaufen möchtest, musst du an der Variante die Verfügbarkeit für den Marktplatz entfernen.
...
. *Speichere* (terra:save[]) die Einstellungen. +
→ Die Variante wird in den nächsten 15 Minuten bei idealo gelöscht und kann dort nicht mehr gekauft werden.

[TIP]
.Zeitstempel für Marktplatzexporte aktualisieren
====
Wenn du sicherstellen möchtest, dass die Variante sofort bei idealo gelöscht wird, kannst du zusätzlich mit der *Artikel-Gruppenfunktion* den *Zeitstempel für Marktplatzexporte aktualisieren*. +
Öffne dazu das Menü *Artikel » Artikel bearbeiten » Tab: Artikel-Gruppenfunktion*. Setze ein Häkchen bei *Zeitstempel für Marktplatzexporte aktualisieren* und klicke auf terra:execute[] *Ausführen*. Die Variante wurde gelöscht und kann nun nicht mehr auf idealo gekauft werden.
====
