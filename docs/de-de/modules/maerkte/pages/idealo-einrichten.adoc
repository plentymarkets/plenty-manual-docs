...
Für Artikel mit der Versandart *Spedition* können die zusätzlichen Dienstleistungen *Zwei-Mann-Lieferung bis zum Aufstellort* (*twoManHandlingPrice*) und *Altgeräteabholung* (*DisposalPrice*) als Merkmale an idealo Direktkauf übergeben werden.
...
In plentymarkets kannst du mit dem Merkmal *Altgeräteabholung* (*DisposalPrice*) kennzeichnen, ob für einen Artikel die Altgeräteabholung möglich ist.

Gehe wie im Folgenden beschrieben vor, um das Merkmal *DisposalPrice* zu erstellen.
...
. Wähle bei *idealo Direktkauf-Merkmal* das Merkmal *DisposalPrice*.
. Speichere (icon:save[set=plenty]) die Einstellungen.
...
. Aktiviere das eben erstellte Merkmal *DisposalPrice*.
. Speichere (icon:save[set=plenty]) die Einstellungen. +
