...
<<maerkte/amazon/amazon-einrichten#, icon:long-arrow-left[] Amazon in plentymarkets einrichten>>
<<maerkte/amazon/variantendaten-exportieren#, Amazon-Variantenexport per Katalog einrichten icon:long-arrow-right[]>>

[.text-right]
_oder_

[.next-navigation]
<<maerkte/amazon/variantendaten-exportieren-alt#, Variantenexport über alte Technologie einrichten icon:long-arrow-right[]>>

'''

Du hast die Schnittstelle zu Amazon <<maerkte/amazon/amazon-einrichten#, eingerichtet>>? Super. Dann bereite nun deine Varianten für den Export zu Amazon vor.
...
====
TIP: Wenn du für den Export schon die neuen Kataloge nutzt, kannst du den Schritt *Verkaufspreis für Länderplattformen festlegen* überspringen. Der Assistent *Kataloge erstellen - Amazon* kann automatisch einen Verkaufspreis erstellen.
====
...
. Gib in das Feld *Name* den Namen *SaleStartDate* ein. +
*_Wichtig:_* Für die Eigenschaft darf nur dieser Name *SaleStartDate* benutzt werden. Der Aktionspreis wird sonst nicht an Amazon übermittelt.
. Gib eine Positionsnummer für die Eigenschaft ein.
. *_Optional:_* Wenn du international verkaufst, speichere den Namen *SaleStartDate* für weitere Sprachen.
...
. Erstelle nun die Eigenschaft *SaleEndDate* genauso. Als *Feld* wählst du *sales_end_date*.
...
. Gib im Bereich *Varianteneigenschaften* den Eigenschaftsnamen *SalesStartDate* ein.
...
*_Tipp:_* Um mehrere Artikel zu bearbeiten, nutze die Möglichkeiten für die <<artikel/import-export-anlage/anlage/massenbearbeitung#, Massenbearbeitung>>.
...
Die frei definierbaren Felder dienen der Anpassung der Daten, die du zu Amazon exportierst. Diese Felder müssen mit <<artikel/einstellungen/freitextfelder#, Freitextfeldern>> verknüpft werden. Die Freitextfelder ermöglichen dir eine individuelle Anpassung der Artikeldaten und somit eine genaue Beschreibung der Artikel.

[#1000]
=== Freitextfelder erstellen

Erstelle im ersten Schritt die <<artikel/einstellungen/freitextfelder#, Freitextfelder>>, die du mit den frei definierbaren Feldern bei Amazon verknüpfen möchtest. Gehe dazu wie im Folgenden beschrieben vor.
...
Auch <<artikel/einstellungen/attribute#, Attribute>> müssen verknüpft werden, bevor deine Artikel zu Amazon übertragen werden können. Die Attributverknüpfung ist für Amazon auch mehrsprachig möglich.
...
Mit <<artikel/einstellungen/eigenschaften#500, Eigenschaften>> hast du die Möglichkeit, Varianten detaillierter zu beschreiben. Mit einer Eigenschaft kannst du zum Beispiel technische Details einer Variante, wie Bluetooth oder WLAN, abbilden.
...
Im letzten Schritt aktivierst du das <<artikel/einstellungen/eigenschaften#100, Merkmal>> in den Artikeleinstellungen und trägst einen Wert ein.
...
Du hast nun die Schnittstelle zu Amazon in plentymarkets eingerichtet und deine Varianten vorbereitet. Entscheide nun, ob du deine Varianten per Katalog (BETA) oder über den alten Artikelexport zu Amazon exportieren möchtest.

* <<maerkte/amazon/variantendaten-exportieren#, Amazon-Variantenexport per Katalog einrichten>>
* <<maerkte/amazon/variantendaten-exportieren-alt#, Variantenexport über alte Technologie einrichten>>
