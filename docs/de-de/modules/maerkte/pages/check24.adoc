...
[discrete]
=== Produkt- und Angebotsexport bei Check24

Bei Check24 wird zwischen Angebotsdaten und Produktdaten unterschieden. Deswegen gibt es bei Check24 zwei verschiedene Exporte:

* den Angebotsexport
* den Produktexport.

Check24 ruft Artikeldaten in regelmäßigen Abständen per URL ab. Deswegen musst du eine URL im Backend von Check24 speichern. +
*_Hinweis:_* Wichtig zu erwähnen ist hierbei, dass es zwei verschiedene Katalog-Formate bzw. Exportformate für den elastischen Export und somit auch zwei verschiedene URLs gibt, eine URL für den Produktexport und eine URL für den Angebotsexport. Du kannst bei Check24 jedoch nur _eine_ URL speichern. Um sicherzustellen, dass alle Daten an Check24 übertragen werden, die für den Verkauf deiner Artikel benötigt werden, solltest du deshalb die URL des _Angebotsexports_ bei Check24 speichern.

* <<#url-check24-katalog, So kopierst du die URL für den Angebotsexport>>, wenn du Artikeldaten per Katalog an Check24 exportierst.
* <<#url-check24-elastischer-export, So kopierst du die URL für den Angebotsexport>>, wenn du Artikeldaten mit dem elastischen Export an Check24 exportierst.

Hier findest du weitere Informationen zum Produkt- und Angebotsexport bei Check24:
...
+
--
Angebotsdaten werden von Check24 per URL abgerufen. Diese URL muss direkt im Backend von Check24 gespeichert werden. Die Angebotsdaten werden dann automatisch in regelmäßigen Abständen von Check24 abgerufen. Der Angebotsexport enthält alle Daten, die Check24 benötigt, um deine Artikel dort zu verkaufen. +

* Wenn du Angebotsdaten mit dem Katalog an Check24 exportierst, musst du das Katalog-Format *Check24 Offer* verwenden.
* Wenn du Angebotsdaten mit dem elastischen Export an Check24 exportierst, musst du das Exportformat *Check24DE-Plugin* verwenden.

Je nachdem, ob du Kataloge oder den elastischen Export verwendest, um Artikeldaten zu exportieren, findest du die URL an unterschiedlichen Stellen in deinem plentymarkets System.

[[url-angebotsdaten-check24]]
.URL zum Abruf von Angebotsdaten
[cols="1,3a"]
|====
|Katalog |Elastischer Export

| * *Daten » Kataloge » [Katalog öffnen] » Einstellungen* +
Im Bereich *Optionen für öffentlichen Link* auf *URL kopieren* (icon:content_copy[set=material]) klicken. Die URL im Backend von Check24 speichern.
| * *Daten » Elastischer Export » [Exportformat öffnen]* +
Im Bereich *Einstellungen* die *URL* kopieren. Die URL im Backend von Check24 speichern.
|====
--
...
+
--
Produktdaten werden von Check24 per URL abgerufen. Dies ist jedoch nur in Ausnahmefällen und in Absprache mit Check24 möglich. Deshalb kannst du die URL zum Abruf von Produktdaten auch nicht bei Check24 speichern. +
Wenn du dennoch Produktdaten zu Check24 exportieren möchtest, wende dich direkt an Check24.

* Wenn du Produktdaten mit dem Katalog an Check24 exportieren möchtest, musst du das Katalog-Format *Check24* _oder_ *Check24 Fashion* verwenden.
* Wenn du Produktdaten mit dem elastischen Export an Check24 exportieren möchtest, musst du das Exportformat *Check24DE Fashion* verwenden.
...
Check24 ruft Angebotsdaten über eine URL ab. Damit Angebotsdaten an Check24 exportiert werden, musst du eine URL direkt bei Check24 speichern. Die Angebotsdaten werden dann automatisch in regelmäßigen Abständen von Check24 abgerufen. +
Kopiere die URL des Exportformats im Menü *Daten » Elastischer Export* und speichere sie anschließend im Backend von Check24. +
*_Hinweis:_* Du musst die URL des Exportformats *Check24DE-Plugin* bei Check24 speichern. Dieses Exportformat enthält die Angebotsdaten, die Check24 benötigt, damit deine Artikel auf Check24 verkauft werden können.
...
*_Hinweis:_* Du musst die URL des Katalog-Formats *Check24 Offer* bei Check24 speichern. Dieses Katalog-Format enthält die Angebotsdaten, die Check24 benötigt, damit deine Artikel auf Check24 verkauft werden können.
...
. Klicke unter *Download-Einstellungen* im Bereich *Optionen für öffentlichen Link* auf *URL kopieren* (icon:content_copy[set=material]).
