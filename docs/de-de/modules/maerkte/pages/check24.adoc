...
== Artikel bei Check24 löschen

Varianten werden bei Check24 gelöscht und können nicht mehr auf dem Marktplatz gekauft werden, wenn sie in plentymarkets keinen Bestand mehr haben. Dies passiert automatisch, wenn Artikeldaten an Check24 übertragen werden. Das Intervall, in dem Artikeldaten übertragen werden, legst du in den <<#catalogue-settings, Katalogeinstellungen>> fest. Wenn die Variante in deinem plentymarkets System wieder Bestand hat, wird sie mit der nächsten Artikeldatenübertragung also auch wieder auf Check24 zum Verkauf angeboten.
...
. *Speichere* (terra:save[]) die Einstellungen. +
→ Die Variante wird bei Check24 gelöscht und kann dort nicht mehr gekauft werden.

[#discrete]
=== Sichtbarkeit von Angeboten auf Check24

Angebote sind auf Check24 nur sichtbar, wenn es für ein Angebot mindestens ein aktives Angebot gibt. Wenn also ein Artikel nicht mehr im Angebotsexport enthalten ist oder mit dem Bestand *0* an Check24 übertragen wird, wird zu diesem Artikel auch kein Angebot mehr auf Check24 angezeigt.
