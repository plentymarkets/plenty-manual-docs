...
* *Katalog-Generator*: Wenn du produktiv mit den Katalogen arbeiten willst, d.h. wenn die erstellten Kataloge auch zu MyToys exportiert werden sollen, empfehlen wir, den Katalog-Generator zu verwenden.

* *Katalog-Generator BETA*: Der Katalog-Generator BETA kann momentan nur verwendet werden, um Kataloge zu erstellen. Diese Kataloge können aber noch nicht exportiert werden. +

*_Tipp:_* Nutze den BETA-Generator, um uns Feedback zu geben und zu prüfen, ob alle Felder im Katalog enthalten sind. Wenn das Feature vollständig entwickelt ist, können mit dem BETA-Generator erstellte Kataloge in Zukunft auch exportiert werden.
