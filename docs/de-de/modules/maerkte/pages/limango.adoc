...
*_Hinweis:_* Die Versandkosten werden in deinem Vertrag mit limango festgelegt und von limango gespeichert. Deshalb gibst du hier die ID des Versanddienstleisters und _nicht_ die ID eines Versandprofils.
...
Für Beschreibungen gelten die folgenden Regeln:
...
| Farbe
| Die Farbe wird für die eindeutige Produktbildung herangezogen. Es muss sich also um eindeutige Werte handeln, zum Beispiel um die Herstellerfarbe. Zum Bilden der Artikelnummer wird die Farbe an die SKU gehängt.
...
*_Beispiel Möbel:_* Einen Sessel gibt es in verschiedenen Farben, aber nicht in verschiedenen Größen. Der Größenwert `onesize` muss exportiert werden.
...
Jeder Marktplatz strukturiert Artikeldaten anders. In <<#catalogue-mappings>> findest du Hinweise dazu, was du beim Zuordnen von Datenfeldern für limango beachten solltest.
...
*Gewicht* +
etc.)
a| Maße dürfen nur für Produkte ohne Größenvarianten angegeben werden, also für Single-Size-Produkte wie zum Beispiel Möbel. Grund: Die Maße können im limango-Shop nur auf Produktebene, nicht aber auf Variantenebene (also Größenebene) ausgespielt werden.
...
| UVP
| Für die Angabe des UVP gelten die folgenden Regeln:

* Der UVP darf nicht niedriger sein als der Brutto-Verkaufspreis.
* Ist der UVP höher als der Verkaufspreis, wird im limango-Shop ein Streichpreis ausgespielt.
...
*Material 3* +
etc.)
a| Für alle Textilerzeugnisse mit einem Gewichtsanteil an Textilfasern von mindestens 80 % ist die Materialangabe in Prozent rechtlich verpflichtend.
...
Die Versandkosten werden in deinem Vertrag mit limango festgelegt und von limango gespeichert. Versandkosten können deshalb nur in Absprache mit dem limango Partnermanagement angepasst werden. In plentymarkets gibt es deshalb für die Versandkosten keine Einstellung.
