...
Wenn du Partner:in bei link:http://www.neckermann.at/[Neckermann Österreich^]{nbsp}icon:external-link[] werden möchtest, wende dich zunächst direkt an Neckermann.at.
...
*_Beispiel:_* Für einen Artikel mit einem Bestand von 50 und einem Bestandspuffer von 10 wird der Bestandswert 40 übertragen. Weitere Informationen findest du auf der Handbuchseite <<warenwirtschaft/warenbestaende-verwalten#, Warenbestände verwalten>>.
...
Als nächstes verknüpfst du deine <<artikel/einstellungen/eigenschaften#100, Merkmale>> mit den Neckermann-Komponenten.
...
Um Brand-IDs für Neckermann über das Import-Tool zu importieren, erstellst du einen Import vom Typ <<daten/daten-importieren/sync-typen/elasticSync-hersteller#, Hersteller>>. Nachdem du den Import erstellt hast, musst du noch die Importoptionen bestimmen, den Abgleich festlegen und die Zuordnung erstellen.
...
Gehe wie im Folgenden beschrieben vor, um eine <<auftraege/auftragsdokumente#170, PDF-Vorlage>> für Lieferscheine für Neckermann zu erstellen.

Hier lädst du die Vorlagendateien für link:https://cdn02.plentymarkets.com/pmsbpnokwu6a/frontend/Neckermann_document_templates.zip[Neckermann.at^]{nbsp}icon:external-link[] im DOCX- und ODT-Format herunter. Öffne die Datei in Word (DOCX) oder in LibreOffice (ODT), nimm wie gewünschten Anpassungen vor und exportiere die Datei dann als PDF-Datei. Diese PDF-Vorlage lädst du wie im Folgenden beschrieben in plentymarkets hoch.
