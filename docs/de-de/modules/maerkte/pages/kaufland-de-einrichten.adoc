...
[IMPORTANT]
.Auftragsimport für die Slowakei manuell anstoßen
====
Momentan kann es passieren, dass der Auftragsimport für Aufträge des Marktplatzes Kaufland SK (Slowakei) nicht funktioniert. Deshalb muss der Auftragsimport manuell angestoßen werden. Dazu müssen die Einstellungen für Kaufland neu gespeichert werden. Führe dazu die folgenden Schritte aus:

[.instruction]
Auftragsimport für SK manuell anstoßen:

. Öffne das Menü *Einrichtung » Märkte » Kaufland » Einstellungen » [Kaufland-Konto öffnen]*.
. Klicke oben links auf *Speichern* (terra:save[]). +
→ Die Konfiguration für den Marktplatz SK wird gespeichert.
