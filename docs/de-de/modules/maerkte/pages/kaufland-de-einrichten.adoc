...
* Wenn die *Sprache der plentymarkets Produktdaten* auf `de` eingestellt ist, prüft das System anhand der EAN, ob es bereits ein Produkt mit der Sprache Deutsch gibt. Wenn ja, wird dazu ein Angebot für den Marktplatz *SK* oder *CZ* erstellt und die Produktdaten werden in die jeweilige Sprache übersetzt. +
Wenn es keinen Artikel mit der Sprache Deutsch gibt, dann wird das Produkt erst auf Deutsch erstellt und anschließend in die jeweilige Sprache übersetzt.
