...
Nachdem du {market} in plentymarkets eingerichtet hast, stehen die unten beschriebenen automatisierten Abläufe bereit. +
Je nachdem, ob du Daten per Katalog oder mit dem alten Export an Kaufland überträgst, werden Daten in anderen Intervallen an Kaufland übertragen.

[#datenaustausch-katalog]
=== Datenaustausch bei Export über Kataloge

Wenn du Daten mit Katalogen an Kaufland überträgst, werden Daten in den in <<#tabelle-datenaustausch-katalog>> beschriebenen Intervallen mit Kaufland augetauscht:

[[tabelle-datenaustausch-katalog]]
.Häufigkeit des Datenaustauschs bei Datenexport über Katalog
[cols="1,3a"]
|===
|Daten |Intervall
...
[#datenaustausch-alter-export]
=== Datenaustausch bei altem Export

Wenn du Daten mit dem alten Export an Kaufland überträgst, werden Daten in den in <<#data-exchange-market>> beschriebenen Intervallen mit Kaufland augetauscht:

[[data-exchange-market]]
.Häufigkeit des Datenaustauschs bei altem Export
[cols="1,3a"]
|===
|Daten |Intervall
...
Beachte jedoch, dass Kaufland die Marktplätze in Tschechien und der Slowakei noch nicht eröffnet hat. Du kannst deine Artikel dort bereits listen, jedoch noch nicht verkaufen.
====
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#konto-einstellungen-kaufland>>.
. Speichere (terra:save[]) die Einstellungen.

[[konto-einstellungen-kaufland]]
...
*_Hinweis:_* Klicke auf material:open_in_new[], um neue Versanddienstleister in plentymarkets einzurichten. Du wirst zum Menü *Einrichtung » Aufträge » Versand » Optionen » Tab: Versanddienstleister* weitergeleitet.
...
Klicke zuerst auf *Kaufland-Versandgruppen aktualisieren* (material:refresh[]), damit alle aktuellen Kaufland-Versandgruppen in der Dropdown-Liste zur Verfügung stehen. +
...
| Aktivieren, wenn die Artikelverfügbarkeiten zu Kaufland übertragen werden sollen.

| *Artikelverfügbarkeiten zuordnen*
a| Klicke auf *Zuordnungstabelle öffnen* (material:swap_horiz[]), um die Artikelverfügbarkeiten in plentymarkets Lieferzeiten in Werktagen zuzuordnen. +
...
Wenn bei *Bearbeitungszeit* kein Wert eingegeben wird, wird der Wert, der bei *Min. Lieferzeit (in Werktagen)* eingegeben wurde, als Bearbeitungszeit übertragen.
...
*_Hinweis:_* Die Felder *Min. Lieferzeit (in Werktagen)* und *Max. Lieferzeit (in Werktagen)* sind veraltet und werden in Zukunft aus dem Menü entfernt werden. Du brauchst diese Felder also nicht mehr auszufüllen. Wichtig ist aber, dass du bei *Bearbeitungszeit (in Werktagen)* einen Wert eingibst.
|===
...
| *Aktiv*
| material:circle[role="blue"] = Das Konto ist aktiv und Daten werden mit Kaufland ausgetauscht. +
material:circle[] = Das Konto ist nicht aktiv. Daten werden nicht mit Kaufland ausgetauscht.
...
[cols="1,3a"]
!===
2+! material:settings[] *Artikeleinstellungen*

! material:refresh[] *Artikel aktualisieren*
! Erstellt eine Produktdatei mit allen Artikeln, die zu Kaufland exportiert werden sollen. Diese kann über *Artikelvorschau* (material:preview[]) angesehen werden.

! material:preview[] *Artikelvorschau*
! Vorschau der Produktdatei anzeigen. +
*_Hinweis:_* Wenn du die Schnittstelle zu Kaufland.de noch nicht eingerichtet hast und dir testweise eine Vorschau der Produktdatei ansehen möchtest, musst du die Schnittstelle kurzzeitig aktivieren, da die Vorschau der Produktdatei ansonsten leer ist. Um dir eine Vorschau der Produktdatei anzusehen, aktiviere die Schnittstelle und klicke dann auf *Artikelvorschau* (material:preview[]). Deaktiviere die Schnittstelle danach sofort wieder, indem du die Einstellung *Datenaustausch* (unter *Allgemein*) deaktivierst und speicherst. Dadurch vermeidest du, dass vorzeitig Artikel übertragen werden.

! material:playlist_play[] *Artikel zu Kaufland.de übertragen*
! Produktdatei zu Kaufland.de übertragen.

2+! material:settings[] *Angebotseinstellungen*

! material:refresh[] *Angebote aktualisieren*
! Erstellt eine Angebotsdatei. Diese kann über *Vorschau der Angebote* (material:preview[]) angesehen werden.

! material:preview[] *Vorschau der Angebote*
! Vorschau der Angebotsdatei anzeigen.

! material:playlist_play[] *Angebote zu Kaufland.de übertragen*
...
Wenn noch andere Händler:innen das gleiche Produkt anbieten, passt die Tiefpreisautomatik deine Preise automatisch an. Dein Preis inklusive Versandkosten ist dann immer einen Cent günstiger als das Angebot deiner Mitbewerber:innen. Der Mindestpreis ist also der niedrigste Preis, zu dem die Variante angeboten werden soll. +
*_Hinweis:_* Wähle für den Mindestpreis einen anderen Verkaufspreis als für den normalen Verkaufspreis, den du <<#400, für Kaufland festgelegt hast>>. Der Mindestpreis-Verkaufspreis muss für die Kaufland-Herkunft freigegeben sein. +
...
Gehe wie unten beschrieben vor, um einen Barcode für die Auftragsherkunft Kaufland festzulegen, der schon im System vorhanden ist. Dieser Barcode wird für deine Varianten an Kaufland übertragen.
...
Für jede Produktkategorie der einzelnen Länderplattformen, in der du deine Artikel anbieten möchtest, muss ein eigener Katalog erstellt werden. Du erstellst also zum Beispiel verschiedene Kataloge für die Kategorien *Fernseher* und *Kameras*. Du kannst die Produktkategorie wählen, nachdem du eine Länderplattform gewählt hast.
...
* Wenn die *Sprache der plentymarkets Produktdaten* auf `de` eingestellt ist, prüft das System anhand der EAN, ob es bereits ein Produkt mit der Sprache Deutsch gibt. Wenn ja, wird dazu ein Angebot für den Marktplatz *SK* oder *CZ* erstellt und die Angebotsdaten werden in die jeweilige Sprache übersetzt. +
Wenn es keinen Artikel mit der Sprache Deutsch gibt, dann wird das Produkt auf Deutsch erstellt und anschließend in die jeweilige Sprache übersetzt.

* Produktdaten werden auf den Länderplattformen Kaufland CZ und Kaufland SK auch aktualisiert, wenn die Produktsprache entweder auf `cz` oder `sk` eingestellt ist.
...
`sk` = Produktdaten werden auf Slowakisch übersetzt +
`cz` = Produktdaten werden auf Tschechisch übersetzt
...
[#retouren]
=== Retouren bei Kaufland

Im Gegensatz zu anderen Marktplätzen können Retouren für Kaufland noch nicht mit einer Ereignisaktion über plentymarkets gesendet werden. Retouren für Kaufland bearbeitest du direkt in deinem Kaufland-Händlerkonto.
...
.Referenztypen und Referenzwerte für Kaufland
[cols="1,3,3"]
|===
