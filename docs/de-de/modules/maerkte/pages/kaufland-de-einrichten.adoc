...
*_Tipp:_* Der Anzeigename wird nur für die Liste der Konfigurationen verwendet. Der Anzeigename dient also nur dazu, dir im Menü *Einrichtung » Märkte » Kaufland.de » Einstellungen » [Mandant öffnen]* einen besseren Überblick über die verschiedenen Konfigurationen zu geben.

image::maerkte:kaufland-anzeigename.png

| *Backend-Name* +
(*_Pflichtfeld_*)
| Gib einen einzigartigen Namen für diese Kombination aus Mandant (Shop)/plentyShop und Kaufland-Konto ein. Der Backend-Name wird an den folgenden Stellen im System verwendet:

* *_Artikelexport:_* Der Backend-Name wird in einer Spalte der CSV-Dateien exportiert.
* *_Auftragsimport:_* An Aufträgen wird der Backend-Name in der Auftragseigenschaft mit der ID 10 als Name des Verkäuferkontos gespeichert.

*_Wichtig:_* Der Backend-Name ist nach dem ersten Speichern der Konfiguration nicht mehr änderbar.
