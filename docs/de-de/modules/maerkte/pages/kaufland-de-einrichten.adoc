...
Artikeldaten werden über ein Marktplatz-Format zu Kaufland exportiert. Um deine Artikel zu {market} zu exportieren, erstellst du im Menü *Daten » Kataloge* einen Katalog. Verknüpfe dann im Marktplatz-Format die von Kaufland vorgegebenen Felder mit in plentymarkets gespeicherten Artikeldaten. Wenn du den Katalog aktiviert hast, werden die Katalogdaten in regelmäßigen Abständen automatisch zu {market} exportiert.
