...
[#250a]
=== _Alte Authentifizierungsmethode:_ Etsy-Konto freischalten
...
NOTE: Die Authentifizierung im Menü *Einrichtung » Märkte » Etsy » Authentifizierung* ist veraltet. In Zukunft wird die neue Authentifizierungsmethode OAuth2 die alte Authentifizierung ersetzen. Wir empfehlen dir deshalb, dein Etsy-Konto schon jetzt zusätzlich für <<#konto-freischalten, OAuth2 zu authentifizieren>>.
...
[#konto-freischalten]
=== _Neue Authentifizierungsmethode:_ Etsy-Konto freischalten
...
[IMPORTANT]
.OAuth2-Authentifizierung bald zwingend notwendig
====
Aufgrund einer Umstellung der Etsy-API wird die neue Authentifizierungsmethode OAuth2 in Zukunft die alte Authentifizierung ersetzen. Die Umstellung wird voraussichtlich am 16.01.2023 erfolgen. Wir empfehlen dir deshalb, jetzt schon dein Etsy-Konto für OAuth2 zu authentifizieren.
====
...
[IMPORTANT]
.Neue Ereignisaktionen für Etsy einrichten
====
Ab dem 16.01.2023 werden die alten Ereignisaktionen für Etsy aufgrund einer notwendigen Umstellung nicht mehr funktionieren. Die unten aufgeführte Ereignisaktion (*Zahlungsbestätigung an Etsy senden (veraltet)*) wird dann durch die Ereignisaktion *Zahlungsbestätigung an Etsy senden (neu)* ersetzt. Als Vorbereitung empfehlen wir dir deshalb, jetzt schon zusätzlich die Ereignisaktion *Zahlungsbestätigung an Etsy senden (neu)* einzurichten. Die alte Ereignisaktion wird bis zur Umstellung am 16.01.2023 noch wie gewohnt funktionieren.
====
...
| *Aktion*
| *Plugin > Zahlungsbestätigung an Etsy senden (veraltet)*
|
|===
...
.Neue Ereignisaktionen für Etsy einrichten
====
Ab dem 16.01.2023 werden die alten Ereignisaktionen für Etsy aufgrund einer notwendigen Umstellung nicht mehr funktionieren. Die unten aufgeführte Ereignisaktion (*Versandbestätigung an Etsy senden (veraltet)*) wird dann durch die Ereignisaktion *Versandbestätigung an Etsy senden (neu)* ersetzt. Als Vorbereitung empfehlen wir dir deshalb, jetzt schon zusätzlich die Ereignisaktion *Versandbestätigung an Etsy senden (neu)* einzurichten. Die alte Ereignisaktion wird bis zur Umstellung am 16.01.2023 noch wie gewohnt funktionieren.
====
...
| *Aktion*
| *Plugin > Versandbestätigung an Etsy senden (veraltet)*
|
|===
