...
=== _Optional:_ Versandprofil automatisch festlegen

[.instruction]
[_Optional_] Versandprofil automatisch festlegen:

. Öffne das plentymarkets Menü *Einrichtung » Aufträge » Ereignisse*.
. Klicke unten links auf *Ereignisaktion hinzufügen*. +
→ Das Fenster *Neue Ereignisaktion erstellen* wird geöffnet.
. Gib einen Namen ein, zum Beispiel *Wayfair Versandprofil zuordnen*.
. Wähle aus der Dropdown-Liste das Ereignis *Neuer Auftrag*.
. Klicke auf *Speichern*. +
→ Die Ereignisaktion wird erstellt.
. Aktiviere im Abschnitt *Einstellungen* das Kontrollkästchen *Aktiv*.
. Klicke im Bereich *Filter* auf *Filter hinzufügen*.
. Wähle *Auftrag » Herkunft*.
. Klicke auf *Hinzufügen*.
. Aktiviere in der Liste *Herkunft* alle *Wayfair*-Herkünfte.
. Klicke im Bereich *Aktionen* auf *Aktion hinzufügen*.
. Wähle *Auftrag » Versandprofil ändern*.
. Klicke auf *Hinzufügen*. +
→ Im Bereich *Aktionen* wird die Aktion *Versandprofil ändern* angezeigt.
. Klicke ganz links in der Zeile *Versandprofil ändern* auf den Pfeil nach unten, um die Details auszuklappen.
. Wähle in der Dropdown-Liste der Aktion das Versandprofil für *Wayfair*.
...
Wayfairs Systemen mit einem bestimmten Feld von Artikelvarianten in
...
. Verwende die linke Spalte, um ein plentymarkets Lager nach Namen auszuwählen.
* Wenn dem plentymarkets System kürzlich ein Warehouse hinzugefügt wurde, kann es bis zu fünf Minuten dauern, bis es auf dieser Seite verfügbar ist.
* Versuche nicht, dasselbe plentymarkets Warehouse für mehr als eine Zeile auf dieser Seite zu verwenden, da dies eine nicht unterstützte Konfiguration ist.
. Gib in der rechten Spalte eine numerische Lieferanten-ID ein.
. Klicke auf *Speichern*, sobald die neuen Zeilen abgeschlossen sind.
...
*Wenn diese Einstellung in deinem System angezeigt wird, empfiehlt Wayfair dringend,
