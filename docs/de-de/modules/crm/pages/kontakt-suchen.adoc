...
| *Tags*
|Wähle (icon:check_box[set=material, role=blue]) einen Tag, um nur Kontakte, denen dieses Tag zugeordnet ist, anzuzeigen. Du kannst nach einem oder mehreren Tags gleichzeitig filtern.
...
Klicke auf icon:more_vert[set=material] in der Zeile eines Kontakts in der Übersicht, um das Kontextmenü zu öffnen.
...
** Öffnet den Messenger. Mit einem Klick auf *Neue Nachricht* (icon:add[set=material]) erstellst du eine neue Nachricht für den Kontakt. +
