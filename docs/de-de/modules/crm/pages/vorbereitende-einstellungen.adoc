...
:keywords: Adress-Layout, Adresslayout, Adresslayout Dokumente, Kundenklasse, Kontaktklasse, Kundentyp, Typ, Klasse erstellen, Kontaktklasse erstellen, Kundenklasse erstellen, Typ erstellen, Eigenschaften für Kontakt erstellen, Kundeneigenschaften, Kunden Eigenschaften auf Dokumenten, Tags für Kontakte, Tags für Kunden, Kontakttags, Kunden-Tags, Rabattsystem, Rabatt auf Kundenklasse, Kundenklassenrabatt, Rabattstaffel auf Nettowarenwert, Rabatt auf Zahlungsart, Verkaufspreis als Rabatt, manueller Rabatt für Kunden, manueller Rabatt für Kontakte, Rabatt für Kunden gewähren
:description: Erfahre, welche vorbereitenden Einstellungen du vornehmen musst, um mit Kontaktdatensätzen in plentysystems zu arbeiten.
:page-pagination:
:author: team-crm
...
Im Menü *Einrichtung » CRM » Adress-Layout* kannst du individuell Adress-Layouts für deine Lieferländer für deine xref:auftraege:auftragsdokumente.adoc#[standortbezogenen Auftragsdokumente] wie xref:auftraege:lieferscheine-erzeugen.adoc#[Lieferscheine], xref:auftraege:rechnungen-erzeugen.adoc#[Rechnungen], xref:auftraege:gutschriften-erzeugen.adoc#[Gutschriften] und xref:auftraege:ruecksendeschein-erzeugen.adoc#[Rücksendescheine] einrichten. Die Adress-Layouts für die Lieferländer _Deutschland_, _Frankreich_ und das _Vereinigte Königreich_ sind bereits standardmäßig in deinem System vorhanden. Gehe wie im Folgenden beschrieben vor, um ein neues Adress-Layout für deine Auftragsdokumente einzurichten.
...
Damit deine xref:auftraege:auftragsdokumente.adoc#[standortbezogenen Auftragsdokumente] für die Lieferländer automatisch in den erstellten Adress-Layouts ausgegeben werden, musst du eine Einstellung in dem jeweiligen Auftragsdokument vornehmen. Dein System prüft dann automatisch das Lieferland aus dem Auftrag und gibt das entsprechende Auftragsdokument in den im Menü *Einrichtung » CRM » Adress-Layout* aufgelisteten Layout aus. Gehe dazu wie im Folgenden beschrieben vor.
...
:right-one: pass:quotes[*CRM > Adress-Layout > Lesen*]
...
:right-two: pass:quotes[*CRM > Kontakt > Aktualisieren*]
:right-three: pass:quotes[*CRM > Kontakt > Erstellen*]
:right-four: pass:quotes[*Einrichtung > Eigenschaft > Lesen*]
:right-five: pass:quotes[*Einrichtung > Tag > Lesen*]
...
Wenn Benutzer:innen ohne Adminrechte bestimmte xref:crm:kontakt-bearbeiten.adoc#erlaeuterungen-einzelne-bereiche[Bereiche im Kontaktdatensatz] und/oder Schaltflächen und Funktionen in diesen Bereichen nicht sehen können, prüfe die Rechte im Menü *Einrichtung » Kontoverwaltung » Rollen*.

[.collapseBox]
.Rechte für Bereiche, Schaltflächen und Funktionen im Kontaktdatensatz
--

<<#table-missing-areas-contact-data-record>> listet die Rechte auf, die in der xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#70[Rolle] aktiviert werden müssen, wenn eine Person ohne Adminrechte Bereiche im Kontaktdatensatz nicht sehen kann. Diese Rolle muss anschließend im Benutzer:innenkonto xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#110[zugewiesen] werden.
...
<<#table-missing-buttons-contact-data-record>> listet die Rechte auf, die in der xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#70[Rolle] aktiviert werden müssen, wenn eine Person ohne Adminrechte Schaltflächen und/oder Funktionen innerhalb bestimmter Bereiche im Kontaktdatensatz nicht sehen kann. Diese Rolle muss anschließend im Benutzer:innenkonto xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#110[zugewiesen] werden.
...
Alle Einstellungen, die sich auf die xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#116[Freigabe von Ansichten] beziehen, nimmst du im Menü *Einrichtung » Kontoverwaltung » Rollen » [Rolle wählen] » Bereich: Rollendefinierte Ansichten* vor.

* Ansichten zum Erstellen eines neuen Kontaktdatensatzes für Benutzer:innen freigeben: +
Öffne im Menü *Einrichtung » Kontoverwaltung » Rollen* die gewünschte Rolle und dort den Bereich *Rollendefinierte Ansichten*. Wähle (material:check_box[role=skyBlue]) die gewünschten Ansichten aus der Liste. Anschließend muss diese Rolle im Benutzer:innenkonto xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#110[zugewiesen] werden.
...
Du kannst beliebig viele Klassen im Menü *Einrichtung » CRM » Klassen* erstellen und individuell einrichten. Klassen erhalten automatisch eine ID, die in der Übersicht vor dem Namen der Klasse angezeigt wird. Die ID bestimmt jedoch _keine_ Reihenfolge, diese wird durch die *Position* festgelegt.
...
*_Wichtig:_* Diese Option legt nur die Anzeige der Preise fest, nicht die Berechnung der Preise für die Klasse. +
...
|Wähle die Rabattart. Je nach Rabattart werden mehr oder weniger Optionen eingeblendet. Wenn du keinen Rabatt vergeben möchtest, wähle die Option *Kein Rabatt*. Weitere Informationen sowie Erläuterungen zu den Rabattarten in plentysystems findest du im Kapitel <<#rabattsystem-nutzen, Rabattsystem nutzen>>.
...
|[#intable-customer-class-terms-of-payment]*Skontosatz, Skontofrist, Valuta, Zahlungsziel*
...
. Klicke auf *Neuen Typ hinzufügen* (material:add[]).
. Gib den deutschen und den englischen Namen ein.
. *Speichere* (icon:save[]) die Einstellungen.

[#job-titel-verwalten]
== Job-Titel verwalten

Im Menü *Einrichtung » CRM » Job-Titel* verwaltest du die Job-Titel, die du später deinen Kontakten im Bereich xref:crm:kontakt-bearbeiten.adoc#kontaktdetails[Kontaktdetails] zuweisen kannst. 25 Job-Titel sind bereits standardmäßig in deinem System vorhanden. Du kannst beliebig viele weitere Job-Titel in deutscher und englischer Sprache speichern. Außerdem hast du die Möglichkeit, bestehende Job-Titel in beiden Sprachen anzupassen.

[.collapseBox]
.Bereits vorhandene Job-Titel in deinem System 
--
[[table-available-job-positions]]
.Vorhandene Job-Titel
[cols="1,2,2"]
|====
|ID |Name (DE) | Name (EN)
...
. Öffne das Menü *Einrichtung » CRM » Job-Titel*.
. Klicke auf *Neuer Job-Titel* (material:add[]).
. Gib einen deutschen und ggf. einen englischen Namen ein.
. Klicke auf *Speichern* (material:save[]).
...
. Öffne das Menü *Einrichtung » CRM » Job-Titel*.
. Klicke auf material:search[], um die Liste aller Job-Titel zu sehen.
. Klicke auf einen Job-Titel in der Liste.
. Ändere den oder die Namen.
. Klicke auf *Speichern* (material:save[]).
...
Mit dem Import-Tool und dem Import-Typ *Eigenschaften* kannst du Eigenschaften in dein System importieren. Welche Felder du für den Abgleich und die Zuordnung verwenden kannst, ist auf der Handbuchseite des Import-Typs xref:daten:elasticSync-eigenschaften.adoc#[Eigenschaften] beschrieben.

Allgemeine Informationen zum Importieren von Daten in dein System findest du auf der Handbuchseite xref:daten:ElasticSync.adoc#[Import-Tool nutzen].
...
. Öffne das Menü *Einrichtung » CRM » Klassen*.
. Klicke auf die Klasse, um sie zu öffnen.
. Wähle im Bereich *Rabatt* aus der Dropdown-Liste *Rabattart* die Option *Klassenrabatt*. +
→ Eine weiteres Feld wird eingeblendet.
. Gib im Feld *Rabatt in %* den Prozentwert für den Rabattabzug ein.
. Wähle aus der Dropdown-Liste *Mengenrabatt*, ob der Mengenrabatt berücksichtigt werden soll.
. Wähle oben im Bereich *Allgemein*, ob die Preise im plentyShop *brutto* (inklusive Mehrwertsteuer) oder *netto* (exklusive Mehrwertsteuer) angezeigt werden sollen.
. Wähle oben rechts im Bereich *Zahlung*, welche der im plentyShop verfügbaren Zahlungsarten für die Klasse erlaubt sein sollen.
...
. Öffne das Menü *Einrichtung » CRM » Klassen*.
. Klicke auf die Klasse, um sie zu öffnen.
. Wähle im Bereich *Rabatt* aus der Dropdown-Liste *Rabattart* die Option *Rabattstaffel auf Nettowarenwert*. +
...
. Öffne das Menü *Einrichtung » CRM » Klassen*.
. Klicke auf die Klasse, um sie zu öffnen.
. Gib im Bereich *Zahlung* im Feld *Zahlungsziel in Tagen* die Anzahl an Tagen für das Zahlungsziel ein, das du dieser Klasse einräumen möchtest.
. Wähle im Bereich *Rabatt* aus der Dropdown-Liste *Rabattart* die Option *Rabatt auf Zahlungsart*. +
→ Zwei weitere Felder werden eingeblendet.
. Gib unter *Rabatt in %* den Prozentwert für den Rabattabzug ein.
. Wähle aus der Dropdown-Liste *Mengenrabatt*, ob der Mengenrabatt berücksichtigt werden soll.
. Wähle im Bereich *Zahlung*, welche der im Webshop verfügbaren Zahlungsarten für die Klasse erlaubt sein sollen.
. Gib einen Zahlungshinweis ein. +
→ Dieser Zahlungshinweis wird auf Einzelrechnungen angezeigt.
. Wähle im Bereich *Allgemein*, ob die Preise im Webshop *brutto* (inklusive Mehrwertsteuer) oder *netto* (exklusive Mehrwertsteuer) angezeigt werden sollen.
. Klicke auf *Speichern* (material:save[]).
...
. Aktiviere im Bereich *Klasse* die Klassen, für die der Verkaufspreis gelten soll.
...
Zusätzlich zur Vergabe von automatischen Rabatten hast du die Möglichkeit, einzelnen oder allen Artikelpositionen eines Auftrags manuell einen Rabatt zuzuweisen. Du kannst manuelle Rabatte in 2 verschiedenen Ansichten eingeben: in der Listenansicht der xref:auftraege:order-search.adoc#1300[Auftragssuche] und im Bearbeitungsfenster eines xref:auftraege:order-type-order.adoc#[Auftrags].
