...
. Klicke auf *Neues Layout* (icon:add[set=material]).
...
|Soll dieses Layout als Standard verwendet werden? Dann aktiviere die Umschaltfläche (icon:toggle_on[set=material, role=skyBlue]). In der Übersicht auf der linken Seite ist das Standard-Adress-Layout mit einem Sternchen gekennzeichnet.
...
Mit einem Klick auf icon:sync_alt[set=material] wechselst du zum Layout-Design. Die Bereiche *Länder*, *Layout* und *Felder* werden sichtbar.
...
Öffne im Benutzer:innenkonto im Menüpunkt *Ansichten* den Bereich *CRM > Kontakte > Neuer Kontakt > Eigene Ansichten*. Wähle (icon:check_box[set=material, role=skyBlue]) die gewünschten Ansichten aus der Liste.

* Ansichten zum Bearbeiten eines bestehenden Kontaktdatensatzes für Benutzer:innen freigeben: +
Öffne im Benutzer:innenkonto im Menüpunkt *Ansichten* den Bereich *CRM > Kontaktdetails > Eigene Ansichten*. Wähle (icon:check_box[set=material, role=skyBlue]) die gewünschten Ansichten aus der Liste.
...
. Klicke auf *Typ hinzufügen* (icon:add[set=material, role=green]).
. Gib einen Namen ein.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
