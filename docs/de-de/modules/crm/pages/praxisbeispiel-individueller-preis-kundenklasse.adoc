...
. Gib einen eindeutigen Namen für die Ereignisaktion ein, z.B. `Kundenklasse ändern nach x Anzahl von Aufträgen`.
. Wähle das Ereignis gemäß <<#table-event-procedure-customer-class-number-of-orders>>.
. *Speichere* (icon:save[role=green]) die Einstellungen.
. Nimm die Einstellungen gemäß <<#table-event-procedure-customer-class-number-of-ordersr>> vor.
