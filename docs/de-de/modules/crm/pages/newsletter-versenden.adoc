...
Lade dort ein Bild hoch (icon:file_upload[set=material]), kopiere den Link mit einem Klick auf icon:content_copy[set=material] in die Zwischenablage, wechsele zurück in das Newsletter-Menü und klicke im Editor im Bereich *HTML Newsletter* auf *Bild*. Das Fenster *Bild-Eigenschaften* öffnet sich. Füge dort im Tab *Bild-Info* den kopierten Link zum Bild in das Feld *URL* ein, gib einen alternativen Text ein und füge im Tab *Link* den kopierten Link erneut in das Feld *URL* ein, um das Bild mit dem Newsletter zu verknüpfen.
