...
Klicke auf material:pause_circle[], um die Stoppuhr anzuhalten bzw. auf material:play_circle[], damit die Zeit weiterläuft. Klicke auf icon:reset[set=plenty], um die Stoppuhr auf 0 zurückzusetzen.
...
Wenn du die hier eingegebenen Minuten abrechnen möchtest, musst du einen Tag vom Typ *Nachrichten (kostenpflichtig)* wählen und die Schaltfläche icon:pay_cash[set=plenty] material:toggle_off[] aktivieren.

| *Kostenpflichtig*
|Aktiviere die Schaltfläche icon:pay_cash[set=plenty] material:toggle_off[], um die im Feld *Minuten* eingegeben Minuten kostenpflichtig abzurechnen. +
*_Wichtig:_* Wenn du die Schaltfläche aktivierst hast (icon:pay_cash[set=plenty] material:toggle_on[role=skyBlue]), musst du zwingend ein <<#tags-messenger, Tag>> vom Typ *Nachrichten (kostenpflichtig)* mit der Nachricht verknüpfen. Dieses Tag muss zuvor im Menü *Einrichtung » Einstellungen » Tags* <<#tags-messenger, erstellt>> worden sein. Andernfalls kann die Nachricht _nicht_ erstellt werden.
