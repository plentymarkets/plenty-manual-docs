...
Dazu musst du eine Vorlage im Assistenten *E-Mail-Konten* im Schritt xref:crm:emailbuilder-vorbereitende-einstellungen.adoc#e-mail-automatischer-versand[Automatischer Versand] gespeichert haben. +
...
[TIP]
.Möchtest du bestimmte Bereiche standardmäßig ausklappen oder einklappen?
====
Du kannst pro MyView-Ansicht festlegen, welche Bereiche standardmäßig ausgeklappt und eingeklappt sein sollen. Öffne dazu einen Kontaktdatensatz und klicke oben rechts auf *Ansicht bearbeiten* (terra:design_inline_edit[]). Klicke dann in jedem einzelnen Bereich auf *Bearbeiten* (material:edit[]). Im Bereich *Einstellungen* legst du fest, ob der Bereich standardmäßig ausgeklappt oder eingeklappt sein soll:

image::crm:kontakte-portlet-eingeklappt-ausgeklappt.png[width=320, height=180]

Klicke anschließend auf *Schließen* (material:close[]) und nimm diese Einstellung für alle anderen Bereiche vor. Speichere dann die Ansicht mit einem Klick auf *Ansicht speichern* (material:save[]).
====
...
* <<#kontaktuebersicht-gastuebersicht, Kontaktübersicht / Gastübersicht>>
...
[#kontaktuebersicht-gastuebersicht]
=== Kontaktübersicht / Gastübersicht

In diesem Bereich siehst du eine Übersicht des Kontakts bzw. des Gastes. Die Daten in diesem Bereich sind festgelegt und können _nicht_ angepasst werden.

Wenn es sich um einen regulären Kontaktdatensatz handelt, sind die folgenden Informationen sichtbar:

* Firma
* Private E-Mail-Adresse
** Du kannst direkt eine E-Mail an den Kontakt senden (material:email[]) oder die E-Mail-Adresse in die Zwischenablage kopieren (material:content_copy[]).
* Typ
* Anzahl der Aufträge
* Rechnungsadresse

Wenn es sich um eine Gastbestellung handelt, sind die folgenden Informationen sichtbar:

* Firma
* E-Mail-Adresse
** Du kannst direkt eine E-Mail an die Person senden (material:email[]) oder die E-Mail-Adresse in die Zwischenablage kopieren (material:content_copy[]).
* Vollständiger Name
* Typ
* Anzahl der Aufträge
...
Weitere Informationen zum Debitorenkonto findest du auf der Handbuchseite xref:auftraege:buchhaltung.adoc#[Buchhaltung].
...
Klicke auf den folgenden Link und beachte die Erläuterungen auf der Seite *Kontakt erstellen*, wenn die Lieferadresse eine xref:crm:kontakt-erstellen.adoc#dhl-packstation-dhl-postfiliale[DHL Packstation oder DHL Postfiliale] ist.
...
*_Tipp:_* Klicke auf den folgenden Link und beachte die Erläuterungen auf der Seite *Kontakt erstellen*, wenn die Lieferadresse eine xref:crm:kontakt-erstellen.adoc#dhl-packstation-dhl-postfiliale[DHL Packstation oder DHL Postfiliale] ist.
...
| *Leitweg-ID*
|Ist der Empfänger der elektronischen Rechnung eine öffentliche Behörde der Bundesverwaltung? Dann muss die Leitweg-ID immer als Pflichtangabe übermittelt werden. Weitere Informationen zur Leitweg-ID findest du auf der link:https://www.e-rechnung-bund.de/faq/leitweg-id/#:~:text=Die%20Leitweg%2DID%20ist%20eine,Rechnungen%20an%20diesen%20zu%20adressieren[Seite des Bundesministeriums des Innern und für Heimat^].
...
Weiter unten im Bereich *Zahlungsübersicht* in der Tabelle sind Aufträge vom Typ *Auftrag*, *Retoure*, *Gutschrift*, *Gewährleistung*, *Reparatur* und *Vorbestellung* zu diesem Kontakt aufgeführt. Die Spalten *Zahlungsstatus*, *Ausstehend* und *Zahlungsziel* beziehen sich in der Tabelle jeweils auf den einzelnen Auftragstyp. +
...
|Das Zahlungsziel des Auftrags. Damit es hier angezeigt werden kann, muss es in den xref:auftraege:grundeinstellungen.adoc#[Zahlungsbedingungen] am Auftrag hinterlegt sein und eine Rechnung muss erstellt worden sein.
...
Verfügbare Auftragstypen: *Auftrag*, *Retoure*, *Gutschrift*, *Gewährleistung*, *Reparatur* und *Vorbestellung*.
...
Der Bereich *Kontaktnotizen* dient ausschließlich für die interne Kommunikation. Alle Notizen, die du hier erstellst, sind also nur intern sichtbar und können _nicht_ von deinen Kontakten eingesehen werden.

Im Messenger steht dir die Gruppe *Kundennotizen* zur Verfügung, in der du alle an den Kontaktdatensätzen gespeicherten Notizen einsehen kannst. Weitere Informationen zu den Gruppen im Messenger findest du auf dieser xref:crm:messenger.adoc#gruppen-verwalten[Handbuchseite].

// TODO: Messenger-Gruppe umbenennen, wenn es in der UI geändert wurde.
...
| material:email[]
a| Versendet die gewählte Vorlage an alle aktivierten (material:check_box[role=skyBlue]) Kontakte. +
*_Wichtig_*: Du musst die E-Mail-Vorlage zuvor im xref:crm:emailbuilder-testphase.adoc#[neuen EmailBuilder] im Menü *CRM » EmailBuilder* erstellt haben.
...
|Ändert die aktuelle Klasse aller aktivierten (material:check_box[role=skyBlue]) Kontakte in die gewählte Klasse. Die Liste zeigt alle Klassen, die du xref:crm:vorbereitende-einstellungen.adoc#kundenklasse-erstellen[erstellt] hast. +
Diese Funktion befindet sich hinter dem Kontextmenü material:more_vert[]. +
*_Tipp_*: Eine detaillierte Anleitung zum Ändern der Klasse über die Gruppenfunktion findest du in diesem <<#gruppenfunktion-klasse-aendern, Kapitel>>.

| *Sprache ändern*
| material:translate[]
|Ändert die aktuelle Sprache aller aktivierten (material:check_box[role=skyBlue]) Kontakte in die gewählte Sprache.  +
Diese Funktion befindet sich hinter dem Kontextmenü material:more_vert[]. +
*_Tipp_*: Eine detaillierte Anleitung zum Ändern der Sprache über die Gruppenfunktion findest du in diesem <<#gruppenfunktion-sprache-aendern, Kapitel>>.
...
Gehe wie im Folgenden beschrieben vor, um eine xref:crm:emailbuilder-e-mail-versand-automatisieren.adoc#e-mail-service-kontaktuebersicht-gruppenfunktion[E-Mail-Vorlage an mehrere Kontakte über die Gruppenfunktion zu senden].
...
. Klicke in der Symbolleiste oben auf *E-Mail-Vorlage senden* (material:email[]). +
→ Das Fenster *E-Mail-Service* wird geöffnet.
. Im Feld *Empfänger* siehst du alle in der Übersicht gewählten (material:check_box[role=skyBlue]) Kontakte, die die E-Mail-Vorlagen erhalten sollen.
. Nutze ggf. die Filterfunktion. Beachte dazu die Erläuterungen im Kapitel xref:crm:emailbuilder-e-mail-versand-automatisieren.adoc#filteroptionen-e-mail-service[Filteroptionen im E-Mail-Service].
. Wähle die E-Mail-Vorlage.
...
[#gruppenfunktion-klasse-aendern]
=== Klasse bei mehreren Kontakten ändern

Gehe wie im Folgenden beschrieben vor, um die Klasse bei mehreren Kontakten zu ändern.
...
. Suche die Kontakte anhand der Filtereinstellungen. Beachte dazu die Erläuterungen zu den Filtern auf der Seite xref:crm:kontakt-suchen.adoc#[Kontakt suchen]. +
→ Die Kontakte, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Wähle (material:check_box[role=skyBlue]) die Kontakte, für die du die Klasse ändern möchtest.
. Klicke in der Symbolleiste auf material:more_vert[].
. Klicke auf material:manage_accounts[] *Klasse ändern*. +
→ Das Fenster *Klasse ändern* wird geöffnet.
. Wähle die Klasse aus der Liste.
. Klicke auf material:manage_accounts[] *Klasse ändern*. +
→ Die Klasse wird bei den gewählten Kontakten geändert.

[#gruppenfunktion-sprache-aendern]
=== Sprache bei mehreren Kontakten ändern

Gehe wie im Folgenden beschrieben vor, um die Sprache bei mehreren Kontakten zu ändern.
...
. Suche die Kontakte anhand der Filtereinstellungen. Beachte dazu die Erläuterungen zu den Filtern auf der Seite xref:crm:kontakt-suchen.adoc#[Kontakt suchen]. +
→ Die Kontakte, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Wähle (material:check_box[role=skyBlue]) die Kontakte, für die du die Sprache ändern möchtest.
. Klicke in der Symbolleiste auf material:more_vert[].
. Klicke auf material:translate[] *Sprache ändern*. +
→ Das Fenster *Sprache ändern* wird geöffnet.
. Wähle die Sprache aus der Liste.
. Klicke auf material:translate[] *Sprache ändern*. +
→ Die Sprache wird bei den gewählten Kontakten geändert.
