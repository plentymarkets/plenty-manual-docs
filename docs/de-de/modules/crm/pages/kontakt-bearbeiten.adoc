...
| *Aktiv*
|Standardmäßig ist der Kontakt aktiv (icon:toggle_on[set=material, role=skyBlue]). Das bedeutet, der Kontakt ist für den eingestellten Mandanten _nicht_ gesperrt und kann sich in deinem plentyShop einloggen. +
Wenn du auf die Schaltfläche klickst (icon:toggle_off[set=material, role=darkGrey]), ist der Kontakt nicht aktiv. Das bedeutet, der Kontakt ist für den eingestellten Mandanten _gesperrt_ und kann sich _nicht_ in deinem plentyShop einloggen.
