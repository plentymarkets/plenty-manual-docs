...
Um DSGVO-konform zu arbeiten, wird bei Gastbestellungen, die über deinen plentyShop LTS in dein System kommen, _kein_ Kontaktdatensatz mit einer Kontakt-ID erstellt. Gastbestellungen die über einen Marktplatz in dein System kommen, erhalten jedoch eine ID. +
Bestellungen, die als Gast über deinen plentyShop aufgegeben wurden, können daher _nicht_ über das Menü *CRM » Kontakte* gefunden werden. Die Daten des Gastes können nur über die Adressdaten im Menü *CRM » Schnellsuche* gefunden werden.

Du erkennst eine Gastbestellung an dem Haken (icon:done[set=material]) in der Spalte *Gast* in der Übersicht.
