...
| *Bearbeiten*
| Öffnet die gewählten (material:check_box[role=skyBlue]) Kontaktdatensätze zur weiteren Bearbeitung. +
*_Tipp_*: Eine detaillierte <<#schritt-fuer-schritt, Schritt-für-Schritt-Anleitung>> zum Bearbeiten mehrerer Kontaktdatensätze über die Gruppenfunktion findest du unterhalb dieser Tabelle.

// TODO: Erklärung für "Bearbeiten" ergänzen.

| *Löschen*
| Löscht die gewählten (material:check_box[role=skyBlue]) Kontaktdatensätze. +
...
Mehrere Kontaktdatensätze bearbeiten::
+
--
Gehe wie im Folgenden beschrieben vor, um mehrere Kontaktdatensätze zum Bearbeiten zu öffnen.
...
. Suche die Kontakte anhand der Filtereinstellungen. Beachte dazu die Erläuterungen zu den Filtern auf der Seite xref:crm:kontakt-suchen.adoc#[Kontakt suchen]. +
→ Die Kontakte, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Wähle (material:check_box[role=skyBlue]) die Kontakte, die du bearbeiten möchtest. 
. Klicke in der Symbolleiste auf *Bearbeiten* (material:edit[]). +
→ 

// TODO: Konfig ergänzen / Werden dann alle gewählten Kontaktdatensätze geöffnet?
...
. Klicke in der Symbolleiste auf *Löschen* (material:delete[]). +
→ 

// TODO: Konfig ergänzen // Gibt es eine Sicherheitsabfrage beim Löschen mehrere Kontakte?
