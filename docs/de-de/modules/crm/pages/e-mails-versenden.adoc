...
icon:map-signs[] *_Oder:_* Erstelle einen neuen Ordner (icon:create_new_folder[set=material]).
. Klicke auf *Datei hochladen* (icon:file_upload[set=material]) oder ziehe die Datei per Drag-and-drop in den Ordner. +
→ Das Bild wird hochgeladen.
. Klicke in der Zeile des Bildes auf *In Zwischenablage kopieren* (icon:content_copy[set=material]), um die URL des Bildes zu kopieren.
