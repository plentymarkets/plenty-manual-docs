...
→ Klicke auf *Neue Nachricht* (material:add[]). Die Stoppuhr oben rechts startet automatisch, sobald die Nachricht geöffnet wird. Klicke neben dem Feld *Minuten* auf *Kostenpflichtig* (material:toggle_off[] icon:pay_cash[set=plenty]), um die Zeit abzurechnen. +
...
→ Klicke auf *Event hinzufügen* (material:add[]), um diese Zeit zu speichern. +
...
[TIP]
Wenn du im Menü *CRM » Schnellsuche* einen Datensatz öffnest, kannst du mit einem Klick auf *Start* (material:play_circle[]) den Timer starten. Mit einem Klick auf *Pause* (material:pause_circle[]) hältst du den Timer an. Klicke auf *Zurücksetzen* (icon:reset[set=plenty]), um die Zeit auf 0 zu setzen.
...
Mit einem Klick auf material:add[] *Neue Nachricht* rechts neben *MESSENGER* öffnet sich weiter unten das Bearbeitungsfenster zum Erstellen einer neuen Nachricht. +
*_Hinweis:_* Wenn es sich bei dem Datensatz um eine Gastbestellung handelt, sind diese Felder ausgegraut.
...
. Klicke in der Kopfzeile auf *Gast in Kontakt umwandeln* (material:swap_horiz[]). +
