...
--
<<#image-emailbuilder-css-positive-example-without-class-name>> zeigt ein Positivbeispiel für ein CSS-Styling. Hier hat die Klasse keinen Namen.

[[image-emailbuilder-css-positive-example-without-class-name]]
.CSS-Styling: Beispiel ohne Klassenname
image::crm:emailbuilder-css-positiv-beispiel-ohne-klassenname.png[width=320, height=180]

<<#image-emailbuilder-css-negative-example>> zeigt im Gegensatz zum ersten Bild ein Negativbeispiel. Hier fehlt die Klasse. Deshalb würde das CSS-Styling auch _nicht_ für die Vorlage greifen.

[[image-emailbuilder-css-negative-example]]
.CSS-Styling: Negativbeispiel
...
| `Status-ID`
|ID des xref:auftraege:auftraege-verwalten.adoc#1200[Auftragsstatus] wie im Menü *Einrichtung » Aufträge » Auftragsstatus* gespeichert.
| `order.statusId`
| $OrderStatusID
