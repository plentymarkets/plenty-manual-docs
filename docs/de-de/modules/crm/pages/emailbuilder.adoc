...
Du kannst im EmailBuilder beliebig viele <<#ordner-erstellen, Ordner>> und <<#unterordner-erstellen, Unterordner>> erstellen. Nachdem der Ordner erstellt ist, kannst du mit einem Klick auf icon:more_vert[set=material] neue Unterordner, neue Vorlagen allgemein und neue Vorlagen für Header und Footer erstellen und die Ordner umbenennen oder löschen.
...
. Klicke unten links auf icon:add[set=material] *Neuer Hauptordner*. +
...
→ Ein Fenster wird geöffnet.
. Klicke auf icon:add[set=material] *Neuer Unterordner*. +
→ Das Fenster *Neuen Ordner erstellen* wird geöffnet.
...
→ Ein Fenster wird geöffnet.
. Klicke auf icon:add[set=material] *Neuer Header*.
...
→ Ein Fenster wird geöffnet.
. Klicke auf icon:add[set=material] *Neuer Footer*. +
...
====
Wenn du später mehrere Vorlagen zur selben Zeit im Editor bearbeitest, kannst du über die Navigation auf der linken Seiten zwischen den einzelnen Vorlagen hin- und herwechseln. Mit einem Klick auf icon:account_tree[set=material] *Konfiguration* gelangst du wieder in den Ordnerbaum. Dort ist dann automatisch die Vorlage geöffnet, die du zuletzt geöffnet hattest. Schließe geöffnete Vorlagen mit einem Klick auf icon:close[set=material].
====
...
→ Ein Fenster wird geöffnet.
. Klicke auf icon:add[set=material] *Neue Vorlage*.
...
|In dieser Dropdown-Liste stehen dir alle Anhänge zur Verfügung, die du vorher im Menü *CMS » Dokumente* hochgeladen hast. Dies sind zum Beispiel Installationsanleitungen oder Produktdatenblätter. Wähle (icon:check_box[set=material, role=skyBlue]) bis zu 3 statische Anhänge aus der Dropdown-Liste.

|[#intable-template-settings-dynamic-attachments]*Wähle dynamische Anhänge*
|In dieser Dropdown-Liste stehen dir alle xref:auftraege:auftragsdokumente.adoc#100[standortbezogenen Dokumente] zur Verfügung, die du im Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Dokumente* eingerichtet hast. Wähle (icon:check_box[set=material, role=skyBlue]) bis zu 3 dynamische Anhänge aus der Dropdown-Liste.
...
*_Tipp:_* Speichere die Einstellungen. Anschließend ist die Schaltfläche *Vorschau* (icon:preview[set=material]) sichtbar, mit der du deine Eingaben prüfen und ggf. weitere Anpassungen vornehmen kannst.
...
Klicke auf *Einstellungen schließen* (icon:close[set=material]), um die Einstellungen zu schließen. Klicke auf *Widget löschen* (icon:delete[set=material]), um das Widget zu entfernen.
...
Mit einem Klick auf icon:settings[set=material] öffnen sich die Einstellungen. Hier kannst du das Bild hochladen, die Breite und die Höhe des Bildes in px oder % angeben, einen Alternativtext eingeben, das Styling im CSS-Format anpassen und den Bild-Link angeben. +
...
. Klicke auf icon:edit[set=material] bzw. auf icon:settings[set=material] und nimm die Einstellungen vor. Beachte die Erläuterungen in <<#table-available-widgets-emailbuilder>>.
. *Speichere* (icon:save[set=material]) die Einstellungen.
...
Wenn du im Code-Widget beispielsweise mit link:https://twig.symfony.com/doc/3.x/tags/if.html[if-Abfragen^] arbeitest, benötigst du in der if-Abfrage selbst keine doppelt geschweiften Klammern zusätzlich, sondern nur im Fließtext. Allgemeine Informationen zu Twig findest du in der link:https://twig.symfony.com/doc/3.x/[Twig Documentation^].

Im Kapitel <<#code-beispiele-code-widget, Code-Beispiele für das Code-Widget>> findest du einige if-Abfragen, die du einfach in dein Code-Widget kopieren kannst.
...
** <<#variablen-kontakt, Kontaktvariablen allgemein>>
...
Setze in deinen E-Mail-Vorlagen Fallunterscheidungen nach den Prinzipien der if-Abfragen ein. Im Folgenden findest du einige if-Abfragen, die du einfach in dein <<#intable-widget-code, Code-Widget>> kopieren kannst.
...
Das Code-Beispiel findest du link:https://raw.githubusercontent.com/plentymarkets/manual/master/de/crm/assets/emailbuilder-code-beispiel-versandprofil.txt[unter diesem Link^].
...
Das Code-Beispiel findest du link:https://raw.githubusercontent.com/plentymarkets/manual/master/de/crm/assets/emailbuilder-code-beispiel-bankdaten.txt[unter diesem Link^].
...
Das Code-Beispiel findest du link:https://raw.githubusercontent.com/plentymarkets/manual/master/de/crm/assets/emailbuilder-code-beispiel-auftragspositionen.txt[unter diesem Link^].
