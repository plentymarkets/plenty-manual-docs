...
Erfahre in den folgenden Unterkapiteln, welche Variablen dir zur Verfügung stehen, die du in einigen der Widgets verwenden kannst, und wie die dazugehörigen twig expressions lauten. Variablen werden dann in der E-Mail an deine Kund:innen durch den korrekten Inhalt ersetzt. Die sogenannten twig expressions benötigst du für den Betreff deiner E-Mail und für das Code-Widget, da du in diesen beiden Fällen keine Variablen verwenden kannst.
...
Möchtest du wissen, welche Variablen für die einzelnen Kategorien und Unterkategorien zur Verfügung stehen und wie die twig expressions lauten, die du im Code-Widget und im Betreff verwenden kannst? Dann klappe einfach die folgenden Bereiche auf (icon:expand_more[set=material]).
