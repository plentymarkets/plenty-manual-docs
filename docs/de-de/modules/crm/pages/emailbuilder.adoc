...
|Klicke auf material:more_vert[] und dann auf material:preview[] *Vorschau* und wähle *Vorschau HTML* oder *Vorschau reiner Text*. Die Vorschau wird in einem separaten Fenster geöffnet.
...
|In dieser Dropdown-Liste stehen dir alle xref:auftraege:auftragsdokumente.adoc#100[standortbezogenen Dokumente] zur Verfügung, die du im Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Dokumente* eingerichtet hast. Wähle (material:check_box[role=skyBlue]) bis zu 4 dynamische Anhänge aus der Dropdown-Liste.
...
*_Tipp:_* Mit einem Klick auf *HOCHLADEN* material:file_upload[] kannst du die Anhänge auch direkt von deinem Computer aus hochladen, ohne sie vorher im Menü *CMS » Dokumente* hochladen zu müssen. +
...
In <<#table-emailbuilder-examples-css-styling>> findest du einige gängige Beispiele, um das CSS-Styling deiner Vorlage anzupassen. Du nimmst die Anpassung entweder in den sprachabhängigen Einstellungen (material:translate[]) der Vorlage im Feld *Benutzerdefiniertes Styling* vor oder mit einem Klick auf material:edit[] in der Vorlage über das Symbol material:css[].
...
[.collapseBox]
.Verfügbare Spaltennamen und einzelne Positionen
--

*Verfügbare Spaltennamen*:
...
. Nimm die Änderungen im Bereich *Einstellungen* vor. Beachte dazu die Erläuterungen in <<#table-emailbuilder-create-template>>.
...
Du kannst dir Vorlagen im Expertenmodus auch im "dark mode" anzeigen lassen. Klicke dazu in der Expertenmodusvorlage auf material:light_mode[] (*Dark mode*).
...
Wenn du wieder in den "light mode" zurück wechseln möchtest, klicke erneut auf das Symbol material:light_mode[] (*Light mode*).
...
*_Hinweis:_* Weil wir uns im Expertenmodus befinden, wird hier die entsprechende twig expression eingefügt und _nicht_ der Variablenname.
...
Im Bereich *Einstellungen* der Vorlage wählst du einen Eigner, den Mandanten und den Header und Footer, falls noch nicht geschehen. Dort kannst du auch bis zu 4 dynamische Anhänge wählen und die Felder *Antwort an*, *CC* und *BCC* befüllen.
...
|Kontoinhaber:in aus den xref:willkommen:schnelleinstieg-basiseinstellungen.adoc#100[Stammdaten] deines Systems. Wird im *Basics*-Assistenten im Schritt *Bankdaten* gespeichert. Du findest den Assistenten im Menü *Einrichtung » Assistenten » Grundeinstellungen*. +
Alternativ findest du den:die Kontoinhaber:in im Menü *Einrichtung » Einstellungen » Bank*.
| `globals.bank.owner`
|

| `BIC (Stammdaten)`
|BIC aus den xref:willkommen:schnelleinstieg-basiseinstellungen.adoc#100[Stammdaten] deines Systems. Wird im *Basics*-Assistenten im Schritt *Bankdaten* gespeichert. Du findest den Assistenten im Menü *Einrichtung » Assistenten » Grundeinstellungen*. +
Alternativ findest du die gespeicherte BIC im Menü *Einrichtung » Einstellungen » Bank*.
| `globals.bank.bic`
|

| `Bankname (Stammdaten)`
|Bankname aus den xref:willkommen:schnelleinstieg-basiseinstellungen.adoc#100[Stammdaten] deines Systems. Wird im *Basics*-Assistenten im Schritt *Bankdaten* gespeichert. Du findest den Assistenten im Menü *Einrichtung » Assistenten » Grundeinstellungen*. +
...
| `IBAN (Stammdaten)`
|IBAN aus den xref:willkommen:schnelleinstieg-basiseinstellungen.adoc#100[Stammdaten] deines Systems. Wird im *Basics*-Assistenten im Schritt *Bankdaten* gespeichert. Du findest den Assistenten im Menü *Einrichtung » Assistenten » Grundeinstellungen*. +
...
Die Einstellung für das Datumsformat, die du im Bereich *Sprachabhängige Einstellungen* vorgenommen hast, greifen _nicht_ für datumsbezogene twig expressions, die du im Code-Widget verwendest. Hier musst du die Formatierung also selbst vornehmen.

*_Beispiel:_* Wenn du für die englische Datumsschreibweise in den sprachabhängigen Einstellungen einen Bindestrich gewählt hast, im Code-Widget aber eine der folgenden twig expressions für das Auftragsdatum einfügst, wird in der E-Mail-Vorlage die im Code-Widget verwendete Schreibweise für das Datum ausgegeben. Die Einstellung aus dem Bereich *Sprachabhängige Einstellungen* wird hier also überschrieben.
...
Die Einstellung für das Zahlenformat, die du im Bereich *Sprachabhängige Einstellungen* vorgenommen hast, greifen _nicht_ für Zahlenwerte in twig expressions, die du im Code-Widget verwendest. Hier musst du die Formatierung also selbst vornehmen.

*_Beispiel:_* Wenn du für die englische Zahlenschreibweise in den sprachabhängigen Einstellungen einen Punkt als Dezimaltrennzeichen gewählt hast, im Code-Widget aber eine der folgenden twig expressions für den Gesamtrechnungsbetrag einfügst, wird in der E-Mail-Vorlage die im Code-Widget verwendete Schreibweise für den Betrag ausgegeben. Die Einstellung aus dem Bereich *Sprachabhängige Einstellungen* wird hier also überschrieben.
