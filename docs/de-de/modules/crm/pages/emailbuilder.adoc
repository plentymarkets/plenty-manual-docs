...
|In dieser Dropdown-Liste stehen dir alle Anhänge zur Verfügung, die du vorher im Menü *CMS » Dokumente* hochgeladen hast. Dies sind zum Beispiel Installationsanleitungen oder Produktdatenblätter. Wähle (icon:check_box[set=material, role=blue]) bis zu 3 statische Anhänge aus der Dropdown-Liste.

|[#intable-template-settings-dynamic-attachments]*Wähle dynamische Anhänge*
|In dieser Dropdown-Liste stehen dir alle xref:auftraege:auftragsdokumente.adoc#100[standortbezogenen Dokumente] zur Verfügung, die du im Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Dokumente* eingerichtet hast. Wähle (icon:check_box[set=material, role=blue]) bis zu 3 dynamische Anhänge aus der Dropdown-Liste.
