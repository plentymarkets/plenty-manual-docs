...
[IMPORTANT]
.Neue EmailBuilder-Seiten mit Unterseiten
====
Beachte den xref:crm:emailbuilder-uebersicht.adoc#[neuen EmailBuilder-Handbuchbereich] mit den unterschiedlichen Unterseiten, um die Menge an Informationen rund um den EmailBuilder übersichtlicher aufrufen zu können. +
Diese Seite ist demnach veraltet und wird zukünftig nicht mehr aktualisiert. Änderungen werden nur noch in die neuen Seiten eingearbeitet.
====

////
Hinweis: Damit alle twig expressions, die "| raw" am Ende enthalten, korrekt im Handbuch angezeigt werden, muss ein Backslash (\) vor dem Pipe-Zeichen eingefügt werden --> "\ | raw"
