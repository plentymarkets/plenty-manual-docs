...
. Klicke auf *Hinzufügen* (material:add[]), um eine Sprache hinzuzufügen. +
→ Ein Fenster öffnet sich.
...
====
Wenn du später mehrere Vorlagen zur selben Zeit im Editor bearbeitest, kannst du über die Navigation auf der linken Seiten zwischen den einzelnen Vorlagen hin- und herwechseln. Die Vorlage, die du gerade geöffnet hast, ist blau hinterlegt. +
Wenn du im Editor einer Vorlage bist und in eine andere Vorlage wechseln möchtest, ohne dass du die Änderungen vorher gespeichert hast, wird ein Hinweis angezeigt.
====
...
*_Hinweis:_* Wenn du die Vorlage nicht innerhalb eines Ordners erstellen möchtest, klicke direkt unten links auf material:add[] *Hinzufügen* material:play_arrow[] und wähle dann material:description[] *Vorlage*. Die Vorlage wird dann auf der ersten Ebene erstellt, ohne dass sie zu einem Ordner gehört.
...
2+^| Bereich *Einstellungen* auf der rechten Seite +
Wir empfehlen zunächst die Einstellungen im Bereich *Einstellungen* vorzunehmen, bevor der eigentliche Inhalt der Vorlage im mittleren Bereich bearbeitet wird. +
Der Vorteil bei dieser Reihenfolge ist, dass du auch die Einstellungen und nicht nur den Inhalt der Vorlage <<#vorlage-kopieren, kopieren>> kannst.
...
*_Breite der Vorlage:_* Gib `max-width:` gefolgt von der gewünschten Breite in Pixel in dieses Feld ein, z.B. `max-width:1200px`.
...
|Klicke auf material:more_vert[] und dann auf material:copy_content[] *Kopieren*, um den Inhalt und die Einstellungen der Vorlage für eine andere Sprache zu kopieren.
...
In einer Standardvorlage sind neben dem Inhalt der E-Mail-Vorlage bereits der Name der Vorlage sowie der Betreff ausgefüllt. Das bedeutet, du musst nur noch Header und Footer wählen und die Felder *Antwort an* und ggf. *CC* und *BCC* befüllen.
...
// TODO: wie verhält es sich mit der Suche? Wird die gefundene Vorlage (z.B. mit der ID 31) dann gehighlighted angezeigt? oder öffnet sich der Ordner, in dem die gefundene Vorlage enthalten ist, automatisch?
...
→ Ein Fenster öffnet sich.
. Klicke auf material:copy_content[] *Kopieren*. +
→ Ein Fenster öffnet sich.
