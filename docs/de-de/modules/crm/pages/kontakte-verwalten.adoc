...
====
Das Menü *CRM » Kontakte (veraltet)* wird nicht mehr weiterentwickelt. Du kannst das Menü weiterhin sehen und nutzen. Erweiterungen und Neuerungen werden aber nicht mehr vorgenommen. Wir empfehlen dir, das neue Menü *CRM » Kontakte* zu nutzen. Weitere Informationen findest du auf der Handbuchseite xref:crm:kontakte.adoc#[Kontakte] und in diesem link:https://forum.plentymarkets.com/t/eol-alte-kontakte-ui-wird-abgeschaltet-eol-old-contact-ui-will-be-deactivated/679028[Forumsbeitrag^].
====
...
Nun hast du die Möglichkeit, die einzelnen Komponenten zu bearbeiten (icon:pencil[]) oder zu löschen (terra:trash[]). Wenn du die Komponente bearbeitest, siehst du auf der rechten Seite im Bereich *Elemente*, welche Elemente in dieser Komponente noch nicht verwendet wurden und dir somit noch zur Verfügung stehen. Bereits verwendete Elemente sind ausgegraut und können nicht ein weiteres Mal verwendet werden.
...
[[bild-bearbeitungsmodus-spalten]]
...
Mit einem Klick auf *Ansicht speichern* (icon:save[]) speicherst du die Ansicht unter dem aktuellen Namen. Auf diese Weise kannst du beliebig viele Ansichten nach deinen Wünschen speichern. Wenn du das nächste Mal manuell einen Kontaktdatensatz erstellen möchtest, kannst du die von dir bevorzugte Ansicht über die Dropdown-Liste oben rechts öffnen und mit dem Erstellen eines neuen Kontakts beginnen. Ebenfalls über diese Dropdown-Liste erstellst du mit einem Klick auf terra:add[role=green] *Neue Ansicht erstellen* eine neue Ansicht.
...
Du hast auch die Möglichkeit, alle gespeicherten Daten eines Kontakts inklusive Nachrichten, Aufträgen, Nachrichten und Tickets über die Schnellsuche anzuzeigen. Wie du dazu vorgehst, ist auf der Handbuchseite xref:crm:schnellsuche.adoc#[Schnellsuche] im Kapitel xref:crm:schnellsuche.adoc#datensatz-suchen[Datensatz suchen] beschrieben.
...
→ Weitere Informationen zum Messenger findest du auf der Handbuchseite xref:crm:messenger-testphase.adoc#[Messenger].
...
|Klicke auf *Rechnungsadresse* (terra:credit_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:copy[] *Neue Rechnungsadresse ableiten* wird eine neue Rechnungsadresse auf Basis der aktuell gewählten Adresse erstellt. Um eine neue Adresse mit einer neuen ID zu erstellen, müssen Änderungen an der neuen Rechnungsadresse vorgenommen werden.

|[#intable-adresse-bearbeiten-rechnungsadresse-typ-hinzufuegen]*Rechnungsadresstyp hinzufügen*
|Klicke auf *Rechnungsadresse* (terra:credit_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:add[] *Rechnungsadresstyp hinzufügen* wird der Typ *Rechnungsadresse* zu der Adresse hinzugefügt. Die ID ändert sich nicht. +
...
|Klicke auf *Rechnungsadresse* (terra:credit_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:delete[] *Rechnungsadresstyp löschen* wird der Typ *Rechnungsadresse* in der Adresse gelöscht. +
...
|Klicke auf *Rechnungsadresse* (terra:credit_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:flag_star[] *Als primäre Rechnungsadresse definieren* wird die aktuell gewählte Rechnungsadresse als primäre Rechnungsadresse gesetzt. Die ID ändert sich nicht. +
Wenn der Typ *Rechnungsadresse* nicht vorhanden ist, ist diese Option deaktiviert. +
*_Hinweis:_* Wenn die Rechnungsadresse bereits als primär definiert wurde, kann man diese Einstellung mit einem Klick auf terra:flag_star[] *Primäre Rechnungsadresse zurücksetzen* wieder rückgängig machen.
...
|Klicke auf *Lieferadresse* (terra:delivery_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:copy[] *Neue Lieferadresse ableiten* wird eine neue Lieferadresse auf Basis der aktuell gewählten Adresse erstellt. Um eine neue Adresse mit einer neuen ID zu erstellen, müssen Änderungen an der neuen Lieferadresse vorgenommen werden.

|[#intable-adresse-bearbeiten-lieferadresse-typ-hinzufuegen]*Lieferadresstyp hinzufügen*
|Klicke auf *Lieferadresse* (terra:delivery_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:add[] *Lieferadresstyp hinzufügen* wird der Typ *Lieferadresse* zu der Adresse hinzugefügt. Die ID ändert sich nicht. +
...
|Klicke auf *Lieferadresse* (terra:delivery_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:delete[] *Lieferadresstyp löschen* wird der Typ *Lieferadresse* in der Adresse gelöscht. +
...
|Klicke auf *Lieferadresse* (terra:delivery_note[role=blue]), um diese Option zu wählen. +
Mit einem Klick auf terra:flag_star[] *Als primäre Lieferadresse definieren* wird die aktuell gewählte Lieferadresse als primäre Lieferadresse gesetzt. Die ID ändert sich nicht. +
*_Hinweis:_* Wenn der Typ *Lieferadresse* nicht vorhanden ist, ist diese Option deaktiviert. +
Wenn die Lieferadresse als primär definiert wurde, kann man diese Einstellung mit einem Klick auf terra:flag_star[] *Primäre Lieferadresse zurücksetzen* wieder rückgängig machen.
...
Je nachdem, welchen Statistiktyp du gewählt hast, findest du weitere Informationen in den folgenden Kapiteln der Handbuchseite xref:daten:statistiken.adoc#[Statistiken verwalten]:
