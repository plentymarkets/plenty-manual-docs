...
:keywords: Paketverfolgung, Tracking-URL, Tracking-Link, Paket verfolgen E-Mail
...
* Ereignisaktion verknüpfen, die die Vorlage versendet, wenn der Auftrag beim Versanddienstleister angemeldet wurde und eine Paketnummer am Auftrag verfügbar ist.
...
. Klicke oben links auf material:add[] (*Hinzufügen*), um die Sprache für die Vorlage hinzuzufügen. +
→ Das Fenster *Betreff eingeben* öffnet sich.
. Gib einen Betreff für die Vorlage ein, z.B. `Dein Paket kommt bald`.
. Klicke auf *SPEICHERN*.
. Klicke in der Zeile des Betreffs auf material:edit[] (*Bearbeiten*). +
...
[discrete]
=== Beispiel: Vorlagen mit Variablen
