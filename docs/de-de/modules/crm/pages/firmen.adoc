...
Klicke in der Firmenübersicht oben rechts auf *Spalten konfigurieren* (material:settings[]) und aktiviere (material:check_box[role=skyBlue]) die Spalten, die du in der Übersicht sehen möchtest. Du kannst aktivierte Spalten auch wieder deaktivieren (material:check_box_outline_blank[]). Klicke auf material:drag_indicator[] und halte das Symbol gedrückt, um die Reihenfolge der Spalten per Drag-and-drop zu verschieben. Die folgenden Spalten sind verfügbar:
...
Klicke im Firmendatensatz im Bereich *Verknüpfte Kontakte* oben rechts auf *Spalten konfigurieren* (material:settings[]) und aktiviere (material:check_box[role=skyBlue]) die Spalten, die du in der Übersicht sehen möchtest. Du kannst aktivierte Spalten auch wieder deaktivieren (material:check_box_outline_blank[]). Klicke auf material:drag_indicator[] und halte das Symbol gedrückt, um die Reihenfolge der Spalten per Drag-and-drop zu verschieben. Die folgenden Spalten sind verfügbar:
