...
. Klicke auf icon:add[set=material, role=darkGrey]. +
...
Klicke auf icon:square[set=material] und wähle die Farbe aus.
...
Wähle alle zutreffenden Bereiche (icon:check_box[set=material, role=skyBlue]).
...
Wähle alle zutreffenden Webshops (icon:check_box[set=material, role=skyBlue]).
...
. Klicke auf icon:add[set=material, role=darkGrey] und wähle eine Sprachversion aus der Dropdown-Liste.
