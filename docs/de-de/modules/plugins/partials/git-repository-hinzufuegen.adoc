...
. Gib in die entsprechenden Felder den Kontonamen deines Git-Kontos, den zuvor erzeugten Zugriffstoken sowie die URL des Repositorys in die dafür vorgesehenen Felder ein. +
...
--
+
. Gib in das Feld **Name des zu installierenden Branch (Muss gültige plugin.json enthalten)** den Namen eines Branches, auf dem es eine Datei namens `plugin.json`. Diese Datei muss so formatiert sein, wie in den link:https://developers.plentymarkets.com/dev-doc/plugin-information#plugin-json[Anforderungen^] beschrieben. +
