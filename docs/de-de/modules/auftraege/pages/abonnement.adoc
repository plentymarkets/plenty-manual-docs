...
. Navigiere zu icon:shopping_cart[set=material] *Aufträge* > *Neues Abonnement [Beta]*. +
...
|Wenn du einen xref:auftraege:abonnement.adoc#intable-subscription-default-status[Standardstatus] eingestellt hast, ist dieser hier vorausgewählt. Darüber hinaus stehen dir hier alle xref:auftraege:auftraege-verwalten.adoc#1220[Status] zur Verfügung, die du in deinem plentymarkets System angelegt hast. Wähle den passenden Status manuell aus der Dropdown-Liste aus.
...
. Füge die Artikel zum Warenkorb (icon:add_shopping_cart[set=material]) hinzu. +
...
Eine manuelle Anpassung des Preises kannst du im Warenkorb vornehmen. Weitere Informationen dazu findest du im Kapitel xref:auftraege:abonnement.adoc#225[Warenkorb: Preisauswahl].
====
...
Wählst du einen anderen Verkaufspreis als den ersten ermittelten oder änderst Mengen, muss der Verkaufspreis aktualisiert werden. Klicke dafür auf *Verkaufspreis aktualisieren* (icon:refresh[set=material]). Du kannst jeden Artikel einzeln aktualisieren oder mit der Schaltfläche über der Warenkorbtabelle alle Auftragspositionen auf einmal aktualisieren. Nach der Aktualisierung ist die Speichern-Schaltfläche (icon:save[set=material]) wieder verfügbar.

Folgendes gilt für die Verkaufspreise:

* Gibst du manuell einen Preis ein, bleibt dieser in der Dropdown-Liste als Auswahl für diesen Auftrag erhalten, auch wenn du zwischendurch einen anderen Preis auswählst.
* Konnte kein gültiger Verkaufspreis ermittelt werden, kannst du einen manuellen Preis im Warenkorb eingeben. Die Anlage des Abonnements ist auch ohne gültige Preise möglich, sollte dies gewünscht sein.
...
Durch Klicken auf *Hinzufügen* (icon:add[set=material]) in der *Artikelliste* eines Abonnements wirst du direkt zur Artikelsuche weitergeleitet und kannst wie unter xref:auftraege:abonnement.adoc#400[Artikelliste erstellen] beschrieben Artikel hinzufügen. Auch die unten aufgeführten Bearbeitungsmöglichkeiten stehen dir hier offen.

Oder gehe über *Bearbeiten* (icon:edit[set=material]) in der *Artikelliste* eines Abonnements und gelange direkt zum bestehenden *Warenkorb* für das ausgewählte Abonnement. Folgende Bearbeitungsmöglichkeiten stehen dir dort zur Verfügung:
...
|Ändere die Menge eines Artikels im Warenkorb des Abonnements durch Klicken in die Zeile des Artikels im Warenkorb. Beachte, dass eine Änderung an Mengen eine Aktualisierung der Preise erfordert, bevor der Warenkorb gespeichert werden kann. Klicke dafür auf *Verkaufspreis aktualisieren* (icon:refresh[set=material]).

|[#intable-subscription-change-prices]*Preise anpassen*
|Passe den Preis eines Artikels im Warenkorb des Abonnements an. Beachte, dass die Preisänderung nur für das ausgewählte Abonnement gilt. Zudem erfordert eine Änderung an Preisen eine Aktualisierung, bevor der Warenkorb gespeichert werden kann. Klicke dafür auf *Verkaufspreis aktualisieren* (icon:refresh[set=material]).
