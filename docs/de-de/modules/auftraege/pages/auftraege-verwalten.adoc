...
. Klicke in der Zeile des Kontakts ganz links auf icon:more_vert[set=material]. +
→ Eine Auswahl der verfügbaren Optionen wird geöffnet.
. Navigiere zu icon:shopping_cart[set=material] *Aufträge* > *Neuer Auftrag*. +
...
. Klicke in der Zeile des Kontakts ganz links auf icon:more_vert[set=material]. +
→ Eine Auswahl der verfügbaren Optionen wird geöffnet.
. Navigiere zu icon:shopping_cart[set=material] *Aufträge* > *Neuer Auftrag [Beta]* oder *Neues Angebot [Beta]*. +
...
Zudem können Auftragspositionen im Warenkorb auch gelöscht werden. Klicke dafür auf *Löschen* (icon:delete[set=material]). Eine weitere Bearbeitungsmöglichkeit der Auftragspositionen im Warenkorb stellen die Eigenschaften dar. Beachte dafür das folgende Kapitel xref:auftraege:auftraege-verwalten.adoc#287[Eigenschaften von Auftragspositionen].
...
Um den Hauptauftrag öffnen zu dürfen, müssen variable Benutzer:innen über die xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#90[Berechtigung] für alle Lager der Lieferaufträge eines Hauptauftrags verfügen. Prüfe dazu die Einstellungen im Menü *Einrichtung » Einstellungen » Benutzer » Konten » Tab: Berechtigungen*.
...
====
Aufträge, die über xref:pos:pos-kassenbenutzer.adoc#7[POS (Kasse)] in das System gelangen, sind aus Gründen der Rechtssicherheit zur Bearbeitung gesperrt. Nur wenige Einstellungen lassen sich bearbeiten.
====
...
- Wenn die Benutzer:in nicht xref:business-entscheidungen:benutzerkonten-zugaenge.adoc#90[berechtigt] ist, Aufträge zu löschen. +
...
| *Bankdaten*
|Auf das *kleine Plus* klicken, um die xref:payment:bankdaten-verwalten.adoc#63[Bankdaten des Kunden] einzublenden und bei Bedarf zu bearbeiten.
...
. Klicke auf *Anwenden* (icon:execute[set=plenty]) über den Gruppenfunktionen. +
