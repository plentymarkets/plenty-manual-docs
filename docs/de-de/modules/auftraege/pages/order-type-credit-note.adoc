...
. Öffne das Menü *Aufträge » Aufträge (Testphase)*.
. Führe die Suche (material:search[]) aus, um Aufträge anzuzeigen.
. Öffne den Gutschriftauftrag, in dem du das Gutschriftdokument stornieren möchtest.
...
. Klicke anschließend erneut auf *Dokument erstellen* (material:add_note[]).
...
| *Haben/Soll*
|Währe aus der Dropdown-Liste, ob es sich um eine Haben-/oder Soll-Buchung handelt.
