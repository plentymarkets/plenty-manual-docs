...
→ Mit der Funktion *Mehrwertsteuer-Standardsatz anwenden* (material:auto_fix_high[]) kannst du das system veranlassen, die Standard-Mehrwertsteuersätze des jeweiligen Landes anzuwenden. +
...
*Hinweis*: Nutze (material:auto_fix_high[]) , damit die Umsatzsteuer-Sätze automatisch vom System eingetragen werden. +
*_Wichtig_*: +
...
. Klicke auf *Neuen Eintrag hinzufügen* (material:add[role="darkGrey"]).
...
. Öffne das Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Buchhaltung*.
. Klicke auf (terra:icon-execute[]) *Einstellungen übertragen nach* .
