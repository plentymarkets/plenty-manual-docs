...
Das Untermenü *Einstellungen* enthält die ID, den Namen und das Land des Standortes. Das Untermenü *Buchhaltung* enthält mehrere Tabs und die Möglichkeiten werden auf dieser Seite ab dem Kapitel xref:auftraege:auftragsdokumente.adoc#[Auftragsdokumente].
...
* xref:auftraege:buchhaltung.adoc#550[folgenden Kapitel] beschrieben anlegen oder du nutzt den Assistenten für EU-Umsatzsteuersätze. +
...
Grundlegend dafür ist, dass du zunächst die Zolltarifnummern im System pflegst. Die xref:artikel:buchhaltung.adoc#550[Umsatzsteuersätze] für die entsprechenden Länder eingerichtet sein. Umsatzsteuersätze lassen sich auch nachtragen, wir empfehlen aber unbedingt die rechtzeitige Erstellung im System.
...
Für den Import von Zolltarifnummern steht dir der Import-Typ xref:daten:ElasticSync.adoc#[Import-Tool nutzen].
