...
:description: Erfahre mehr über buchhaltungsrelevante Einstellungen in plentysystems. Hierzu zählen unter anderem Umsatzsteuersätze, Rundungseinstellungen, Buchungsschlüssel, Erlöskonten, Debitorenkonten, aber auch Zolltarifnummern.
...
[#100]
== Grundlagen der Preisberechnung in plentysystems
...
| Preise innerhalb von Aufträgen werden mit nur 2 Nachkommastellen gespeichert. Dadurch wird indirekt auch erreicht, dass der Einzelpreis gerundet wird. Die weitere Berechnung der Summen und des Rechnungsbetrages erfolgt dann basierend auf den gerundeten Einzelpreisen. +
...
. Wähle das *Land* des neuen Standorts. Zur Wahl stehen nur Länder, die in plentysystems bereits als Lieferländer aktiviert sind.
...
Wenn du Kleinunternehmer:in bist und erreichen willst, dass auf Rechnungen keine Steuersätze angewendet werden, genügt das Aktivieren einer Option in den allgemeinen Einstellungen. Für eine korrekte Funktionsweise des Systems musst du jedoch trotzdem Umsatzsteuersätze speichern. Diese werden allerdings nicht angewendet.

[TIP]
.Kleinunternehmereinstellung nur sichtbar bei Deutschland als Systemland
====
Die Einstellung *Kleinunternehmer* ist nur sichtbar, wenn für dein System *Deutschland* als Systemland gespeichert wurde. Es handelt sich hierbei um eine Einstellung, auf die du keinen Zugriff hast und die anhand deiner Informationen beim Bestellen eines plentysystems gewählt wird.
====
...
. Klicke auf *Neuen Eintrag hinzufügen* (icon:add[role="darkGrey"]).
. Wähle ein *Land*.
...
→ Mit der Funktion *Mehrwertsteuer-Standardsatz anwenden* (icon:magic-wand[role="darkGrey"]) kannst du das system veranlassen, die Standard-Mehrwertsteuersätze des jeweiligen Landes anzuwenden. +
...
*Hinweis*: Nutze (icon:magic-wand[role="darkGrey"]) , damit die Umsatzsteuer-Sätze automatisch vom System eingetragen werden. +
*_Wichtig_*: +
...
[#820]
=== Buchungskonten für Kassenvorfälle speichern (ab plentysystems App Version 1.7.3)

Aufträge, die über plentyPOS generiert werden, werden den in plentysystems gespeicherten Erlöskonten zugewiesen. Da Einlagen, Entnahmen und Differenzen aus Kassenstürzen keine Aufträge sind, kannst du für diese Kassenvorfälle eigene Buchungskonten speichern. Wie du Buchungskonten für Kassenvorfälle speicherst, erfährst du auf der Handbuchseite xref:pos:pos-einrichten.adoc#[POS einrichten].

[#850]
== Einstellungen auf andere Standorte übertragen

Mit der Funktion *Einstellungen übertragen nach* kopierst du Einstellungen des aktuellen Standortes auf einen Standort. Dabei kannst du die Einstellungen entsprechend der Tabs der Buchhaltung einzeln oder zusammen übertragen.
...
. Öffne das Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Buchhaltung*.
. Klicke auf (icon:settings_b_roll[role="darkGrey"]) *Einstellungen übertragen nach* .
...
Der Name eines Steuersatzes muss exakt dem Namen entsprechen, wie er in Xero definiert ist. Im Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Buchhaltung » Bereich: Länderspezifische Umsatzsteuersätze* gibst du hinter dem Steuersatz den Namen ein.
...
*Ein Datensatz pro Artikelposition (Standard)* = Eine Buchungszeile pro Artikelposition. Die Auswahl *Ein Datensatz pro Artikelposition (Standard)* ist z.B. dann zu verwenden, wenn für Artikel eigene <<#650, Erlöskonten>> in plentysystems hinterlegt wurden.
