...
//link für artikelstatus ggf. noch anpassen, wenn kapitel umgezogen ist
...
[#create-return]
=== Retoure erfassen
// auf neue UI anpassen - so korrekt?
...
|Leert den Warenkorb komplett zu leeren. Möchtest du nur bestimmte Artikel entfernen, klicke rechts neben dem jeweiligen Artikel auf *Artikel aus Warenkorb entfernen* (material:delete[]).
...
. Klicke anschließend auf *Ausführen*, um den Rücksendeschein zu generieren.
