= Auftragsdokumente (Vorgänger-System zum DocumentBuilder)
...
Dieses Kapitel beschreibt das Vorgänger-System zum xref:auftraege:document-builder.adoc[DocumentBuilder]. Bestehende Kund:innen, die Zugriff auf das Vorgänger-System haben, können es weiterhin für die Einrichtung und Verwaltung von Dokumentenvorlagen nutzen.

*_Hinweis:_* Das Vorgänger-System wird nicht mehr weiterentwickelt. Wir empfehlen deshalb, den xref:auftraege:document-builder.adoc[DocumentBuilder] zu nutzen.

Auf dieser Handbuchseite erfährst du, wie du im Vorgänger-System <<#100, standortbezogene Dokumente>> und <<#200, Fulfillment-Dokumente>> einrichtest und anpasst, und wie du Einstellungen von einer bestehenden Vorlage übernimmst. Außerdem lernst du, wie du dein Geschäftspapier als PDF-Vorlage speichern und mit einer Dokumentenvorlage verknüpfen kannst. Zudem wird das Anlegen von Nummernkreisen erläutert.
...
* xref:auftraege:rechnungen-erzeugen.adoc#600[Stornobelege Rechnung]
...
Ob das System mit 2 oder 4 xref:auftraege:buchhaltung.adoc#150[Nachkommastellen] Preise berechnet, stellst du im Menü *Einrichtung » Mandant » [Mandant wählen] » Standorte » [Standort wählen] » Buchhaltung* ein.
...
|Stelle ein, ob der Zahlungshinweis der Klasse ausgegeben werden soll. Weitere Informationen zum Zahlungshinweis der Klasse findest du auf der Handbuchseite xref:crm:vorbereitende-einstellungen.adoc#[Vorbereitende Einstellungen vornehmen] im Kapitel xref:crm:vorbereitende-einstellungen.adoc#kundenklasse-erstellen[Klasse erstellen].
