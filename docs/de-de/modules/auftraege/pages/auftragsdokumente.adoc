...
Dieser Bereich wird nur angezeigt, wenn im System xref:crm:kontakte-verwalten.adoc#950[Eigenschaften] für die Sprache der Dokumentenvorlage hinterlegt sind. Diese Eigenschaften kannst du dann hier über die Textfelder benennen und über die Koordinatenfunktion im Dokument positionieren.
...
_Wichtig_: Die Zahlungsbedingungen werden nur auf Dokumenten übernommen, wenn es sich um einen Auftrag mit der xref:payment:vorkasse.adoc#[Zahlungsart Vorkasse] handelt. Möchtest du Valuta und Skonto nutzen, muss ein Zahlungsziel angeben sein. Wenn für Zahlungsziel nichts oder der Wert 0 eingetragen ist, sind die Zahlungsbedingungen nicht gültig und werden auch nicht auf Dokumenten übernommen. +
...
|Wie *Steuerrechtlicher Hinweis 1*. +
Dieser Hinweis wird bei einer B2B-Lieferung mit xref:auftraege:buchhaltung.adoc#510[Reverse-Charge-Verfahren] ausgegeben. Die Formulierung kann hier beispielsweise lauten *Reverse-Charge-Verfahren gem. Artikel 194 der MwStSystRL.*
...
|Einstellen, ob der Zahlungshinweis der Kundenklasse ausgegeben werden soll. Weitere Informationen zum Zahlungshinweis der Kundenklasse findest du auf der Handbuchseite xref:crm:kontakte-verwalten.adoc#15[Kontakte].
