...
. Klicke auf Warenkorb (icon:shopping_cart[set=material]) *Aufträge erstellen*, um auf die Auswahl zu gelangen. Es öffnet sich ein Bearbeitungsfenster. Wähle *Reparatur*, um eine Reparatur anzulegen.
. Wähle *Für bestimmte Positionen*.  +
→ Ein Bearbeitungsfenster wird geöffnet.
. Nimm die Einstellungen anhand <<table-xxx>> vor.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Die Reparatur wird angelegt und in einem Bearbeitungsfenster ?? geöffnet.
...
In einem Auftrag findest du das Symbol *Neuer Auftrag* (icon:plus-square[role="green"]) im Portlet *?*. Gehe folgendermaßen vor, um einen neuen Auftrag über das Portlet *?* eines bestehenden Auftrags anzulegen. ...
// TODO: Text auf neue UI anpassen - erklären lassen
...
Unbezahlte Aufträge können in einem Sammelauftrag zusammengefasst werden. Du kannst im Menü *Aufträge » Aufträge [Testphase]* oder im Portlet *?* eines Kontakts mit dem Suchfilter *Auftragsstatus*, z.B. anhand eines Status im Bereich *[3] Warten auf Zahlung*, nach solchen unbezahlten Aufträgen suchen. Somit hast du zwei Möglichkeiten, Sammelaufträge zu erzeugen, die nachfolgend beschrieben werden.
