...
. Wiederhole bei Bedarf den Prozess, um weitere Lieferaufträge anzulegen.
...
////
nach Rücksprache mit Patrick Beispiele weiterhin nutzbar
...
Der Warenkorb zeigt folgende Informationen:
Bild
Artikelname
Code ??
Preisauswahl mit Dropdown
Preis
Menge
*Auftragspositionseigenschaften* über Icon Stift zu bearbeiten.
Klicke auf den Stift, um die Eigenschaften zu bearbeiten: Name, Wert, Aufpreis.
Eigenschaft währlen (Dropdown)
Über Icon Plus kannst du *Eigenschaft hinzufügen*. Durch Eigenschaften speichern kannst du die Änderungen sichern.
Über Icon roter Kreis minus kannst du *Artikel löschen*.
Aufpreis, Rabatt, Summe pro Artikel

Aufpreis gesamt
Zwischensumme
gelb hinterlegt Icon *Verkaufpreis aktualsieren*.

Durch Klick auf x im schwarzen Kreis kannst du den Warenkorb schließen.
...
Klicke auf icon:add_shopping_cart[set=material], um einen Artikel in den Warenkorb zu legen; 
// TODO: Klicke links auf Icon 3 Striche plus *Füge alle offenen Mengen in den Warenkorb*
...
| *Bemerkung*
|Eine *Bemerkung* kann in das Textfeld eingefügt werden. (default) + 

| *Artikelstatus*
|Der *Artikelstatus* wird angezeigt. (default) Es gibt keine Vorauswahl.

// TODO: Rechts icon:edit[set=material] und icon:delete[set=material]
...
// TODO: einheitlich für alle: rechts icon:settings[set=material] *Spalten konfigurieren* und Mülleimer (tooltip fehlt). Durch Klick auf Mülleimer geht der Artikel zurück in die Auswahl.
...
Über icon:playlist_add[set=material] +Füge alle offenen Mengen in den Warenkorb* kannst du...
