...
Da jeder Gutschein einer Kampagne angehört, muss zuerst eine Kampagne erstellt werden. Kampagnen erstellst du im Menü *Aufträge » Gutscheine*. Bereits erstellte Kampagnen enthalten neben dem Tab *Einstellungen* 2 weitere Tabs: <<auftraege/gutscheine#gutschein-codes-generieren, Codes>> und <<auftraege/gutscheine#gutscheinvorlage-einrichten, Vorlagen>>.

[[bild-kampagne-erstellen]]
.Kampagne im Menü *Aufträge » Gutscheine* erstellen
image::auftraege/assets/Gutscheine-Kampagne-erstellen.png[width=640, height=360]
...
| *Mandanten (Shops)*
|<<webshop/mandanten-verwalten#, Mandanten (Shops)>> wählen, für die der Gutschein einlösbar sein soll. Mindestens ein Mandant muss gewählt werden.
...
| *Variable*
|Template-Variable für <<crm/e-mails-versenden#, E-Mail-Vorlagen>> wählen. Die Option *Keine Variable* ist voreingestellt. +
...
| *Gutscheinart*
|Zu den Gutscheinarten *Aktionsgutschein* und *Mehrzweckgutschein* die Erläuterungen in <<#tabelle-einstellungen-gutscheinkampagne>> sowie das <<auftraege/gutscheine#beispiel-berechnung-gutscheine, Beispiel>> unter der Tabelle beachten.
...
Verwende das Import-Tool im Menü *Daten » Import*, um <<daten/daten-importieren/sync-typen/elasticSync-gutschein-codes#, Gutschein-Codes>> zu importieren. Auf diese Weise ist es möglich, größere Mengen an Gutschein-Codes in einem Arbeitsgang zu importieren, statt sie einzeln manuell einzugeben. Erstelle dazu einen Import des Typs *Gutschein-Code*.

Um Kampagnen zu importieren, verwende den Import-Typ <<daten/daten-importieren/sync-typen/elasticSync-kampagnen#, Kampagnen>>.
...
So kannst du all deinen Kund:innen einen automatisch generierten Gutschein-Code zusenden. Dazu muss der Kampagne zunächst eine <<crm/e-mails-versenden#4200, Template-Variable>> zugewiesen werden, z.B. `$CouponCode1`.
...
Bevor du Gutscheine in deinem Webshop zum Verkauf anbieten kannst, muss die <<auftraege/gutscheine#kampagne-erstellen, Gutscheinkampagne>> mit der Gutscheinart *Mehrzweckgutschein* erstellt sein. Weitere Informationen findest du in <<#tabelle-einstellungen-gutscheinkampagne>>.
...
Zudem ist es notwendig, dass du die <<webshop/shop-builder#97, Bestellbestätigungsseite im ShopBuilder>> angelegt und verknüpft hast.
...
====
Damit Gutschein-Codes per Ereignisaktion versendet werden können, benötigst du eine <<auftraege/gutscheine#kampagne-erstellen, Gutscheinkampagne>>. Diese musst du am Artikel mittels Freitextfeld hinterlegt haben.
====
