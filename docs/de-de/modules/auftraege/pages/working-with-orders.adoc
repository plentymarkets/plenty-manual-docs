...
. Wähle (material:done[]) alle Aufträge, für die du gesammelt eine Aktion ausführen möchtest.
...
Im Bearbeitungsmodus (terra:design_inline_edit[role="darkGrey"]) der Auftragsübersicht kannst du
*Standard-Gruppenfunktionen* festlegen, die beim Wählen von Aufträgen direkt sichtbar werden. Alle übrigen Gruppenfunktionen werden automatisch in das Kontextmenü (material:more_vert[]) verlagert.
...
|Zeigt die Paketnummern einer Retoure kommasepariert an. Dieses Feld ist nur für den Auftragstyp *Retoure* verfügbar. Mit Klick auf terra:order_return[] wirst du zum Versand-Center weitergeleitet.
...
Bei einer Adresse kannst du entweder einzelne Felder der aktuellen Adresse anpassen oder sogar eine komplett neue Adresse zuweisen, die die aktuelle Adresse ersetzt. Gehe wie im Folgenden beschrieben vor, um eine Adresse zu ändern.
...
→ Ein Fenster mit den bereits verfügbaren Adressen wird geöffnet.
