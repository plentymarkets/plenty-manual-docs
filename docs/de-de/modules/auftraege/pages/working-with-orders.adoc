...
.Gutschein-Dokumente
======
Bei Dokumenten vom Typ *Gutschein* handelt es sich nicht um Gutscheine, die im Auftrag eingelöst wurden, sondern um Gutscheine, die mit diesem Auftrag verkauft wurden. Über das Kontextmenü (material:more_vert[]) kann ein Gutschein mit Klick auf *Neu erzeugen* (material:restore_page[]) neu erzeugt werden.
======
