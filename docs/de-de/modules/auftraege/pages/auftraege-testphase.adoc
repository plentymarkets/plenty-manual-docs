...
Überschrift
Hier wird erklärt, wie du Filter und Operatoren auswählen kannst.
...
. Gehe auf *Standardansicht » Symbol Ansicht bearbeiten » Neue Ansicht erstellen* - Name - speichern, um eine neue Ansicht zu erstellen.
→ Die neue Ansicht Test wurde erfolgreich erstellt.
. Öffne das Portlet *Auftragsübersicht* - rechts oben Ansicht - Ansicht Test
. Klicke auf Filter-Einstellungen
→ Es öffnet sich die zweispaltige Filterauswahl
. Öffne die gewünschte Gruppe der *Verfügbaren Filter*.
→ Die Auswahl öffnet sich.
. Klicke auf das +-Zeichen (= Filter hinzufügen), um die gewünschten Filter von *Verfügbare Filter* in *Angezeigte Filter* zu verschieben.
