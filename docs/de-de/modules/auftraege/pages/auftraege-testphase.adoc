...
Widget Artikel zurückbuchen
// TODO: in Entwicklung, erklären lassen
 Neuer Eintrag in der Seitennavigation = Book back items 
(Sales Order & Warranty: Only visible if a outgoing item is set, 
...
reverse (validation reverse cant be higher than "quantity in order" - "Open"
warehouse (single select)
storage location (see purchase order transactions)
quantity in order (display field)
batch (display field)
bbd (display field)
Item Id (display field)
variantion id (display field)
Variationname (display field)