...
Über Icon Stift kannst du...
Über Zahnrad: icon:settings[set=material] kannst du die Spalten der Tabelle konfigurieren und auf deinen Bedarf anpassen.
...
// TODO: alte UI - wie jetzt beschreiben?
...
Im Menü *Aufträge » Aufträge [Testphase]* werden deine Aufträge in der oben beschriebenen xref:auftraege:auftraege-verwalten.adoc#1400[Übersicht] angezeigt. Wie oben erklärt, stehen dir bereits in der Übersicht einige Bearbeitungsmöglichkeiten zur Verfügung, um einen schnellen Zugriff auf oft genutzte Funktionen zu gewährleisten. Zudem siehst du so auf einen Blick die grundlegenden Informationen, die du zu einem Auftrag benötigst. +
...
je nachdem, wie viele Kacheln du wählst, wird der Bereich dargestellt

. Klappe die Elemente auf.
Die dort zu sehenden Elemente = Bereiche können per Drag & Drop hinzugefügt werden. Im Bereich kannst du sie löschen, um sie wieder zurück in die Auswahl der Elemente zu schieben.
Artikelmenge
...
[TIP]
Über icon:settings[set=material] kannst du für jeden Bereich die Spalten konfigurieren. Du kannst die Reihenfolge über Drag&drop mit einem Klick auf *Verschieben* (icon:drag_indicator[set=material]) verändern. Durch Haken setzen oder Haken herausnehmen kannst du die Anzahl der Spalten verändern.
...
// TODO: Einleitung; Icons - wie gestalten?
Über den Bereichen eines geöffneten Auftrags findest du die Optionen:
* Speichern: icon:save[set=material] 
* Sperren: icon:lock[set=material]
* Aufträge erstellen: icon:shopping_cart[set=material] 
* Auftrag teilen: icon:call_split[set=material]
...
[#portlet-overview]
=== Bereich: Übersicht
Im Bereich *Übersicht* siehst du alle Informationen zur Menge der Artikel und Kosten.
...
Über icon:more_vert[set=material] rechts öffnet sich jeweils das Kontextmenü und ein Bearbeitungsfenster.
...
Als Standard: Name, Straße, PLZ und Ort, Land, E-Mail-Adresse sowie die Idee.

| *Lieferung* 
|Lieferadresse mit Land und ID (VAT fehlt). +
Als Standard: Name, Straße, PLZ und Ort, Land, E-Mail-Adresse sowie die Idee.

// TODO: Info aus Kontextmenü - wie richtig darstellen?
Adressangaben
* Anrede
* Firma (Name 1)
* Vorname
...
Durch das Symbol *Öffne das Versancenter* wird weitergeleitet zum Versandcenter 2.0.
...
//// 
// TODO: beschreiben oder auf Versandcenter 2.0 verweisen?
...
Im Bereich *Zahlungsbedingungen* siehst du die Werte *Zahlungsziel (Tage)*, *Valuta (Tage)*, *Skonto (Tage)* und *Skonto (%)* werden die Zahlungsbedingungen dargestellt.
...
Im Bereich *Zugeordnete Zahlungen* siehst du...
...
. Wähle die Retoure
. Wähle im Bereich Allgemein das Datum für die Rückbuchung. 
...
Es ist genauso aufgebaut wie die anderen Auftragstypen.
