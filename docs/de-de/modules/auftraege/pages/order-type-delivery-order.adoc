...
====
Im Menü unter *Einrichtung » Einstellungen » Aufträge » Auftragstypen » Auftrag [Beta]* kannst du mit der Einstellung *Endpunkt nach Abschluss eines Auftrags* definieren, welche Ansicht nach Abschluss eines Auftrags angezeigt wird.
====
...
. Wähle die Option *Für bestimmte Artikel*. Beachte dazu die Erläuterungen in <<table-create-{ordertype}>>.
...
//menü artikel bearbeiten verlinken
...
. Klicke oben auf *Aufträge erstellen* (material:shopping_cart[]) > *Lieferauftrag*.
. Wähle die Option *Automatisch aufteilen*. Beachte dazu die Erläuterungen in <<table-create-{ordertype}>>. +
→ Die Lieferaufträge werden anhand der vorgenommenen <<#basic-settings-delivery-order, Grundeinstellungen>> automatisch gesplittet und angelegt.
