...
[.TIP]
.Weitere Ebene für Auftragspositionen
====
Für die Auftragspositionen kann eine weitere Ebene mit den folgenden Informationen zu den Eigenschaften ausgeklappt werden:

* *Name*: Zeigt den Namen der Eigenschaft an.
* *Wert*: Zeigt den Wert der Eigenschaft an.
* *Aufpreis*: Zeigt den Aufpreis der Eigenschaft an. +
*_Hinweis:_* Möchtest du die Eigenschaften bearbeiten, gehe links in der Seitannavigation in den Bereich *Artikel bearbeiten*. Öffne anschließend über das Kontextmenü die Option *Eigenschaften bearbeiten* (material:edit[]).
====
