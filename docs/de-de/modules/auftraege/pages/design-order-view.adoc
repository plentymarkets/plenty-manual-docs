...
Die Detailansicht eines Auftrags unterscheidet sich je nach Auftragstyp. Für jeden Typ gibt es somit eine individuelle Übersicht mit jeweils relevanten Informationen.
...
image::toolbar-new-order-ui.png[width=640, height=360]
...
| material:call_split[]
|Teilt einen Auftrag. +
Diese Schaltfläche ist im Kontextmenü (material:more_vert[]) der Toolbar verfügbar. Weitere Informationen findest du im Kapitel xref:auftraege:working-with-orders.adoc#splitting-orders[Aufträge teilen].
...
Diese Schaltfläche ist im Kontextmenü (material:more_vert[]) der Toolbar verfügbar.

| *Lagerorte zuweisen/lösen*
| material:warehouse[]
a|Weist Lagerorte zu oder löst zugewiesene Lagerorte. +
...
| *Auftrag löschen*
| material:delete[]
a|Klicke rechts oben auf material:delete[] *Auftrag löschen*, um einen Auftrag zu löschen. Bestätige anschließend die Sicherheitsabfrage. +
Diese Schaltfläche ist im Kontextmenü (material:more_vert[]) der Toolbar verfügbar. +
...
Die Tabellenspalten in der Auftragsübersicht sind für beide Ebenen konfigurierbar. Beim ersten Öffnen des Menüs *Aufträge » Aufträge (Testphase)* wird dir die Standardansicht der Tabelle angezeigt. Mit Klick auf *Spalten konfigurieren* (material:settings[]) rechts öffnen sich die Optionen *Tabelle Aufträge* und *Tabelle Auftragspositionen*. Im Fenster *Spalten konfigurieren* kannst du jeweils wählen, welche Spalten du sehen möchtest und in welcher Reihenfolge diese angezeigt werden sollen.
...
//prüfen, ob Auftragspositionen bearbeitet werden können
*_Hinweis:_* Die Auftragspositionen sind nicht für alle Auftragstypen gleich.

//welche Spalten sind (keine) Standardspalten?
[[table-order-items]]
.Auftragspositionen
[cols="1,3"]
|===
...
*_Hinweis:_* Dieses Datenfeld gibt es für die Auftragstypen *Retoure* und *Reparatur*.
...
Diese Spalte ist eine Standardspalte. +
*_Hinweis:_* Dieses Datenfeld gibt es für die Typen *Retoure* und *Reparatur*.
...
image::unsaved-changes-de.png[width=640, height=360]
