...
- *Für Ereignisaktionen*: Jede Dokumentaktion muss in *einem einzelnen Ereignis* erstellt werden und darf nicht in einem gruppiert werden. Vorhandene Ereignisaktionen sollten dann voneinander getrennt werden. +
Wenn z.B. eine gemeinsame Ereignisaktion für die Erstellung eines Dokuments und den Versand dieses Dokuments per E-Mail besteht, solltest du diese Ereignisaktion trennen. Dann wird die E-Mail nur versendet, wenn das Dokument verfügbar ist. +
