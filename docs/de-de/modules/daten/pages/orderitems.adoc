...
Mit dem Datenformat **OrderItems** exportierst und bearbeitest du die Artikelpositionen eines Auftrags oder fügst neue Positionen zu einem Auftrag hinzu. Du erstellst und exportierst das Datenformat im Menü **Daten » Dynamischer Export**. Wie du das Datenformat exportierst, ist auf der Handbuchseite <<daten/export-import/daten-exportieren#, Daten exportieren>> beschrieben.
