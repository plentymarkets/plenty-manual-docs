...
Mit dem Datenformat **OrderComplete** exportierst du Aufträge. Dieses Datenformat steht ausschließlich für den Export zur Verfügung. Du erstellst und exportierst das Datenformat im Menü **Daten » Dynamischer Export**. Wie du das Datenformat exportierst, ist auf der Handbuchseite xref:daten:daten-exportieren.adoc#[Daten exportieren] beschrieben.
