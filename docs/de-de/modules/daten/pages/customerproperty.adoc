...
Mit dem Datenformat *CustomerProperty* exportierst du die im Menü xref:crm:kontakte-verwalten.adoc#4000[Einrichtung » CRM » Eigenschaften] gespeicherten Kundeneigenschaften. Du erstellst und exportierst das Datenformat im Menü *Daten » Dynamischer Export*. Wie du das Datenformat exportierst, ist auf der Handbuchseite xref:daten:daten-exportieren.adoc#[Daten exportieren] beschrieben.
...
Die in Tabelle 2 aufgelisteten Datenfelder stehen zum xref:daten:daten-importieren.adoc#25[Datenabgleich] zur Verfügung. Bei Pflichtabgleichfeldern (*P*) muss für die *Importaktion* die Option *Abgleich* gewählt werden. Alternative Abgleichfelder sind mit einem *A* gekennzeichnet.
