...
Mit dem Datenformat *CustomerPropertyLink* exportierst du die im Tab *Eigenschaften* deiner Kundendaten verfügbaren xref:crm:kontakte-verwalten.adoc#4000[Kundeneigenschaften] und die für die Kunden eingegebenen Werte. Mit Hilfe dieses Datenformats verknüpfst du zeitsparend Kundeneigenschaften mit bestimmten Kunden. Du erstellst und exportierst das Datenformat im Menü *Daten » Dynamischer Export*. Wie du das Datenformat exportierst, ist auf der Handbuchseite xref:daten:daten-exportieren.adoc#[Daten exportieren] beschrieben.
