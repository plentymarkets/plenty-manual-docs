= Katalogstatus prüfen
:keywords:
:description:
:page-index:
:author: team-plenty-channel

Im Menü *Daten » Katalogexportstatus* kannst du den Status der Katalogexporte nachvollziehen.
...
* Dev-Cache =  +
*_Tipp:_* Exportvorgänge des Typs *Dev-Cache* werden nicht in der Übersicht angezeigt. Du kannst aber nach diesem Typ filtern, um Dev-Cache-Exporte anzuzeigen.
...
| *Fortschritt*
| Zeigt den Fortschritt des Exportvorgangs an. Wenn der Export noch läuft, wird angezeigt, wie viele Jobs bereits verarbeitet wurden.
...
.Was ist ein Job?
--
Bei Exporten des Typs *Asynchron* werden die zu exportierenden Daten vor dem Export in Pakete zerlegt. Jedes Paket enthält 500 Zeilen, das letzte Paket ggf. weniger. Jedes dieser Pakete entspricht einem Job. Jeder dieser Jobs wird in die Warteschlange eingereiht.
...
| *Erfolgreiche Jobs*
| Wie viele Jobs wurden erfolgreich abgeschlossen?
...
* *Benutzer:in* = Der Exportvorgang wurde von einer Benutzer:in gestartet. Wenn du mit der Maus auf den Namen der Person zeigst, wird auch die Benutzer:innen-ID angezeigt.
* *Cron* = Der Exportvorgang wurde automatisch durch einen Cron gestartet.
* *Debug* = Der Exportvorgang wurde 

| *Startzeit*
| Wann wurde der Exportvorgang gestartet?

| *Letzte Statusänderung*
| Wann wurde der Status des Exportvorgangs zuletzt geändert?
...
* icon:tune[set=material] = Blendet die Filter ein.
...
*_Tipp:_* Dieser Filter ist nur verfügbar, wenn du den Filter *Initiator* auf *Benutzer:in* gestellt hast.
...
| *Letzte Statusänderung von*
| Reduziert die Liste auf Exporte, deren Status sich seit diesem Datum geändert hat. 

| *Letzte Statusänderung bis*
| Reduziert die Liste auf Exporte, deren Status sich bis zu diesem Datum geändert hat.
|===
--
...
| *Fehler*
| Eine Liste der Fehler. Diese Informationen werden nicht automatisch aktualisiert. Klicke auf icon:refresh[set=material], um die Informationen manuell zu aktualisieren.

| *Fortschritt-Logs* (icon:playlist_play[set=material])
...
| Diese Einstellung hat aktuell noch keine Funktion. Stay tuned.
...
*_Wichtig:_* Aufgrund der Datenmenge und -komplexität können die Daten nicht als Tabelle angezeigt werden.
