...
Ein Import dient dazu, die Daten einer per FTP oder HTTPS zugänglichen Datei in plentymarkets zuzuordnen und zu importieren.
...
[NOTE]
.Importintervall erst am Ende einstellen
====
...
. Klicke auf *Import hinzufügen* (icon:plus[set=material]). +
...
| *Typ*
a| Welche Arten von Daten möchtest du importieren?
...
| Festlegen, ob der globale Cache für die Datei genutzt werden soll oder nicht. icon:check_box_outline_blank[set=material] = Ein und dieselbe Datei kann jederzeit erneut importiert werden. +
