...
| **ItemImageNameDelete**
| **1** = Bildnamen löschen +
Löscht Artikelbildnamen unwiederbringlich, wenn der Wert 1 eingegeben wird. Weitere Informationen zu Filtern, siehe Handbuchseite xref:daten:alte-tools-daten-exportieren.adoc#20[Daten exportieren].
|
...
Die in <<tabelle-datenformat-itemimagename-abgleich>> aufgelisteten Datenfelder stehen zum xref:daten:alte-tools-daten-exportieren.adoc#20[Datenabgleich] zur Verfügung. Bei Pflichtabgleichfeldern (P) muss für die **Importaktion** die Option **Abgleich** gewählt werden.
