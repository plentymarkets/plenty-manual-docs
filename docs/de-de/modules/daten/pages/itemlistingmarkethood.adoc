...
Das Datenformat *ItemListingMarketHood* wird verwendet, um neue Hood-Listings zu erstellen oder bestehende Hood-Listings zu bearbeiten. Du kannst z.B. einzelne Parameter ändern oder löschen. Du erstellst und exportierst das Datenformat im Menü *Daten » Dynamischer Export*. Wie du das Datenformat exportierst, ist auf der Handbuchseite xref:daten:daten-exportieren.adoc#[Daten exportieren] beschrieben.
