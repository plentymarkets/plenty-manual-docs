...
Mit dem Datenformat **Order** exportierst und bearbeitest du Aufträge. Du erstellst und exportierst das Datenformat im Menü **Daten » Dynamischer Export**. Wie du das Datenformat exportierst, ist auf der Handbuchseite xref:daten:daten-exportieren.adoc#[Daten exportieren] beschrieben.
...
Die in Tabelle 2 aufgelisteten Datenfelder stehen zum xref:daten:daten-importieren.adoc#25[Datenabgleich] zur Verfügung. Eins dieser Pflichtabgleichfelder (P) genügt allerdings für den Abgleich. Wähle daher mindestens eines der Datenfelder.
