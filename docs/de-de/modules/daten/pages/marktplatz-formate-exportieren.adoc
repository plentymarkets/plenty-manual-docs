...
Mit den Marktplatz-Formaten des Katalogs exportierst du Varianten- und Bestandsdaten an viele Marktplätze und Preisportale.
plentymarkets arbeitet daran, nach und nach alle Marktplätze auf den Katalogexport umzuziehen.

Auf dieser Handbuchseite erfährst du, wie du mit Katalogen Variantendaten an Marktplätze exportierst. Dabei wird nur die allgemeine Vorgehensweise beschrieben.

Wie du einen Katalog für die einzelnen Marktplätze erstellst und aufbaust erfährst du auf den Handbuchseiten der einzelnen xref:maerkte:maerkte.adoc#[Marktplätze].
...
Jeder Marktplatz strukturiert Produktdaten anders. Oft werden für verschiedene Produktkategorien (Tierfutter, Haushalt, Lebensmittel) sogar unterschiedliche Produktdaten an denselben Marktplatz exportiert. Klar: Für Kinderschuhe brauchst du andere Produktdaten als für Katzenfutter. Für Kinderschuhe zum Bespiel die Schuhgröße und das Material. Für Katzenfutter zum Beispiel Grundpreisangaben, die Zutatenliste, für welche Katzen das Futter geeignet ist und so weiter.
...
[#formula]
=== Zahlenwerte beim Export neu berechnen (Formel)

[discrete]
===== Du speicherst eine Formel, die beim Export auf Zahlenwerte angewendet wird
