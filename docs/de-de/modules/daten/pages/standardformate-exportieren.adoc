...
* Um zum Beispiel deine Warenbestände zu exportieren, nutzt du das Standardformat xref:daten:katalog-warenbestaende.adoc#[Warenbestände].
* Zum Export von Artikeldaten steht das Standardformat xref:daten:katalog-artikel.adoc#[Artikel] zur Verfügung.

NOTE: Bisher kannst du noch nicht alle Arten von Daten über Kataloge exportieren. Bis Standardformate für alle Datenarten verfügbar sind, kannst du weiterhin den xref:daten-exportieren:FormatDesigner.adoc#[Formatdesigner] nutzen.
...
* *Navigation* +
  Standardmäßig wird die Ansicht *Datenfelder* angezeigt, wenn du einen Katalog öffnest. Über die Navigation wechselst du zu anderen Ansichten des Katalogs. Rufe die Katalogeinstellungen auf oder setze Filter, um nur bestimmte Daten zu exportieren. Außerdem kannst du zurück zur Katalogübersicht wechseln oder einen neuen Katalog erstellen.
...
* *Auswahl der plentymarkets Datenfelder* +
  Im Bereich links neben der Navigation findest du eine Liste aller plentymarkets Datenfelder, die exportiert werden können. Klicke auf das Plus rechts neben dem Datenfeld, um ein Datenfeld zum Export hinzuzufügen.

  ** Zusätzlich zu den Datenfeldern kannst du auch xref:daten:kataloge-verwalten.adoc#164[eigene Werte] zum Export hinzufügen. Für jeden Datensatz wird dann derselbe Wert exportiert.
  ** Nutze die Option xref:daten:kataloge-verwalten.adoc#mathematische-berechnungen[Formel], um bestimmte Daten während des Exports zu aktualisieren. So kannst du zum Beispiel alle exportierten Artikelpreise schon beim Export bearbeiten.

* *Gewählte Datenfelder* +
  Links werden alle Datenfelder angezeigt, die du zum Export hinzugefügt hast.

  ** Du kannst die Datenfelder per xref:daten:kataloge-verwalten.adoc#165[Drag-and-drop] verschieben, um die Export-Reihenfolge anzupassen.
  ** Du kannst den xref:daten:kataloge-verwalten.adoc#162[Export key] anpassen. Die "Export Keys" sind die Namen der Spaltenüberschriften, die in deiner Exportdatei erscheinen werden. Jeder "Export Key" muss eindeutig sein.
  ** Du kannst xref:daten:kataloge-verwalten.adoc#160[Ausweich-Datenfelder] hinzufügen. Wenn das Datenfeld leer oder ungültig ist, wird das erste Ausweich-Datenfeld geprüft und stattdessen dieser Wert exportiert.
 ...
 *_Hinweis_*: Zurzeit sind die Formate xref:daten:katalog-warenbestaende.adoc#[Warenbestände] verfügbar.

|*Format* +
(Pflichtfeld)
| Wähle ein Format aus der Dropdown-Liste. +
*_Hinweis:_* Zurzeit gibt es pro Format-Typ nur ein Format. Wähle deshalb einfach die Option aus der Dropdown-Liste, die angezeigt wird.
|====
...
Nachdem du den Katalog erstellt hast, nimmst du die Formateinstellungen vor.
...
* Ergebnis 2: `bestellung_bestellung_2022_01_02.csv`
...
!===
! *Nie*
! Es wird keine Datei exportiert.
...
* Über den Admin-Link kannst du nur auf die exportierte Datei zugegriffen, wenn du als Admin im plentymarkets System eingeloggt bist.
...
→ Die URL für den Download der Datei wird angezeigt, sobald der Export abgeschlossen ist. Im Menü wird immer die URL der zuletzt exportierten Datei angezeigt.
...
icon:toggle-off[role="darkGrey"] = Es wird kein Hash verwendet. +
icon:toggle-on[role="darkGrey"] = Bei jeder Änderung am Katalog wird eine neue URL mit Hash-Schlüssel erstellt. Das Hash steht also für die für den Katalogexport relevanten Einstellungen. Wenn diese Einstellungen geändert werden, ändert sich das Hash und alle zukünftigen Links enthalten das neue Hash. Ältere Links sind ab dem letzten Export mit diesem Hash für 15 Tage gültig. Aktiviere die Option nicht, wenn über die URL immer die aktuellste Exportdatei verfügbar sein soll.

2+| *Optionen für Admin-Link*

| *Admin-Link*
| Für jede Datei wird automatisch ein Admin-Link erstellt. Über diesen Admin-Link ist die exportierte Datei nur erreichbar, wenn du als Admin im plentymarkets System eingeloggt bist.
...
| *Optionen aktivieren*
|
icon:toggle-off[role="darkGrey"] = Es wird kein öffentlicher Link erstellt. Die Optionen *Hash verwenden* und *Token verwenden* sind nicht wählbar. +
icon:toggle-on[role="darkGrey"] = Es wird ein öffentlicher Link erstellt. Nach dem ersten Export ist die Datei über diesen Link für alle verfügbar, die den Link kennen.
...
| *Token verwenden*
|
icon:toggle-off[role="darkGrey"] = Es wird kein Token generiert. +
icon:toggle-on[role="darkGrey"] = Die Download-URL wird mit einem Token versehen. Die Datei ist dann nur erreichbar, wenn der aktuelle Token bekannt ist.
...
Lege nun fest, welche plentymarkets Daten mit dem Katalog exportiert werden sollen.
...
*_Was ist ein Export Key?_* Der "Export Key" ist der Name der Spaltenüberschrift, der in deiner Exportdatei erscheinen wird. Jeder "Export Key" muss eindeutig sein. Export Keys dürfen keine Leerzeichen enthalten. +
...
[#delete-fields]
=== Datenfelder aus Export löschen
...
* Je nach Standardformat sind unterschiedliche Filter verfügbar. Diese Filter sind auf den Handbuchseiten der einzelnen Standardformate erklärt.
...
Du hast den Katalog erstellt und alle Datenfelder hinzugefügt, die deine Exportdatei enthalten soll? Dann aktiviere jetzt den Export.
