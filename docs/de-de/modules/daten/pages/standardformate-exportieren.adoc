...
* Ein anderes Datenfeld exportieren, wenn das erste Datenfeld leer ist (*Ausweich-Datenfelder hinzufügen*)
* Für jeden Datensatz denselben Wert exportieren (icon:pencil[role="black"]) *Eigener Wert*)
* Mehrere Datenfelder miteinander kombinieren
* Die Reihenfolge der Datenfelder ändern
* Zahlenwerte beim Export neu berechnen (icon:calculate[set=material] *Formel*)

Diese optionalen Exportmöglichkeiten werden unterhalb der Anleitung genauer erklärt.
...
. Prüfe, ob der Export Key deine Anforderungen erfüllt. +
  ...
  *_Wichtig:_* Wenn mehrere Datenfelder denselben Export Key haben, wird nur eines dieser Datenfelder exportiert.
