...
*_Empfehlung:_* Verwende den Port `21`.
...
→ Die Servereinstellungen werden geprüft:
...
Protokoll-Präfixe sind zum Beispiel `ftp://` und `ftps://`.
. Gib einen *Port* ein oder verwende die Standardeinstellung. +
*_Empfehlung:_* Verwende den Port `22`.
. Gib den *Benutzernamen* für den Server ein.
. Gib das *Passwort* für den Server ein.
. Gib den *Pfad* zu dem Speicherort ein, an dem der Export gespeichert werden soll. +
*_Hinweis:_* Gib den vollständigen Pfad ein.
. Klicke auf *Verbindung testen*. +
→ Die Servereinstellungen werden geprüft:
  * Kann eine Verbindung zu dem Server hergestellt werden?
  * Kann eine Exportdatei an dem Speicherort gespeichert werden? +
...
! *Google Sheets*
! Der Export wird in Google Sheets gespeichert.

[.collapseBox]
.Bereitstellung einrichten
--
. Wähle die Option *Google Sheets*. +
→ Die Einstellung *Tabellen-URL* wird eingeblendet.
. Gib die URL der Google Sheets-Datei ein.
. Klicke auf *Connect with Google*. +
...
ifdef::marketplace-export[*_Hinweis:_* Getestet wird nur, ob mit den aktuellen Exporteinstellungen ein Dateiexport möglich ist. Die Option prüft _nicht_, ob Verbindung zu einem Marktplatz hergestellt werden kann.]

| Tabellen-URL +
(bei Google Sheets)
|

. Gib die URL der Google Sheets-Datei ein, in der der Export gespeichert werden soll.
. Klicke auf *Connect with Google*. +
→ Die Verbindung wird hergestellt.
