...
Der Export Key ist der Name der Spaltenüberschrift, der in deiner Exportdatei erscheinen wird.
  ...
  ** Wenn der Export Key einen Punkt enthält, werden die Daten als Baumstruktur exportiert. In den meisten Anwendungsfällen werden die Daten dann nicht wie erwartet exportiert. Wir empfehlen dir deshalb, keine Punkte zu verwenden.
