...
[#catalogue-menu]
=== Katalogmenü verstehen

image::daten:menüaufbau-standardformat.png[]
_Menüaufbau eines Katalogs mit Standardformat_
...
include::partial$default-catalogue-navigation.adoc[]
...
*_Hinweis:_* Nicht für alle Templates sind Filter verfügbar.
...
include::partial$default-catalogue-toolbar.adoc[]
...
* icon:toggle_off[set=material, role="darkGrey"] = Blende optionale Marktplatz-Datenfelder ein oder aus.
...
include::partial$default-catalogue-data-fields.adoc[]
...
include::partial$default-catalogue-added-fields.adoc[]
 ...
 *_Hinweis_*: Zurzeit sind die Formate xref:daten:katalog-hersteller.adoc#[Hersteller], *Newsletter-Empfänger*, xref:daten:katalog-auftraege.adoc#[Auftrag], xref:daten:katalog-bestellungen.adoc#[Bestellung], xref:daten:katalog-artikel.adoc#[Artikel (Neu)], xref:daten:katalog-warenbewegungen.adoc#[Warenbewegungen] und xref:daten:katalog-warenbestaende.adoc#[Warenbestände] verfügbar.
...
* <<#optional-own-value, Für jeden Datensatz denselben Wert exportieren (icon:pencil[role="black"]) *Eigener Wert*)>>
* <<#optional-combine, Mehrere Datenfelder miteinander kombinieren>>
* <<#optional-recalculate, Zahlenwerte beim Export neu berechnen (icon:calculate[set=material] *Formel*)>>
...
→ Die Katalog-Übersicht wird geöffnet.
. Klicke auf den Katalog. +
→ Die Ansicht *Datenfelder* (icon:order_return_create_edit[set=plenty]) des Katalogs wird geöffnet. +
→ Die verfügbaren plentymarkets Datenfelder werden in Gruppen angezeigt. Außerdem werden die Optionen *icon:pencil[role="black"] Eigener Wert* und *icon:calculate[set=material] Formel* angezeigt. +
...
. Prüfe, ob der Export Key deine Anforderungen erfüllt. +
  ** *_Was ist ein Export Key?_* +
  Der "Export Key" ist der Name der Spaltenüberschrift, der in deiner Exportdatei erscheinen wird. Jeder "Export Key" darf in diesem Katalog nur einmal vorkommen. Export Keys dürfen außerdem keine Leerzeichen enthalten. +
...
[.collapseBox]
._Optional:_ Zahlenwerte beim Export neu berechnen (*Formel*)
--
...
* Um eine ganze Zeile zu verschieben:
  ** Klicke auf das Symbol (icon:drag_indicator[set=material]) am linken Rand und ziehe die Zeile mit gedrückter Maustaste an die neue Position.
--
...
. Prüfe, ob die Daten so exportiert werden, wie du es erwartet hast.
...
. Klicke im Notification Center auf icon:link[set=material] *Herunterladen*. +
