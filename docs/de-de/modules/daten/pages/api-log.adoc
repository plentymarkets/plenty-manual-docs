...
Wenn die Prüfung fehlgeschlagen ist, enthält die Response einen Fehler. Für die Kund:in werden bei einem Fehler die Einstellungen für ein negatives Bonitätsergebnis, xref:payment:atriga.adoc#[rote Bonität], verwendet.
|====
...
In diesem Kapitel findest du alle Einträge, die im Zusammenhang mit der Nutzung des Abrufes der xref:payment:paypal.adoc#80[PayPal-Abrechnungsberichte] im Log aufgeführt werden können.
...
Die Log-Einträge zu PayPalSettlement betreffen die in den Abrechnungsberichten enthaltenen Transaktionen. Die Transaktions-ID von PayPal wird in plentymarkets im Verwendungszweck einer xref:payment:beta-zahlungen-verwalten.adoc#[Zahlung] angezeigt.
