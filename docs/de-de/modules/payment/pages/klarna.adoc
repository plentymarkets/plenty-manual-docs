...
Mit dem Klarna-Plugin bindest du die *Klarna-Zahlungsarten* in deinen Webshop ein und stellst sie so deinen Kund:innen zur Auswahl. Lade dafür das Klarna-Plugin im link:https://marketplace.plentymarkets.com/klarna_6731[plentyMarketplace^]{nbsp}icon:external-link[] herunter. Die Einrichtung der Zahlungsarten erfolgt erfolgt über einen Assistenten. Zudem musst du einige grundlegende Einstellungen in deinem plentymarkets System ausführen, damit die Zahlungsarten auch in deinem System aktiviert sind. +
