...
|Wähle *0.7* (nicht so genaue Übereinstimmung zur Zuordnung nötig) oder *1.0* (genaue Übereinstimmung nötig). Ist dieser Wert höher eingestellt, kann das Problem auftreten, dass Zahlungen nicht zugeordnet werden können. Daher ist in der *Standard-Einstellung* ein Wert von *0,75* eingestellt.
...
| [#intable-exclude-orders-referrers-automatic-assignment]*Aufträge mit folgenden Herkünften von der automatischen Zuordnung ausschließen*
...
Zahlungen gehen entweder automatisch in dein System ein oder werden manuell eingebucht. Nach dem Eingang in plentymarkets werden die Zahlungen Aufträgen über eine unscharfe Suche automatisch zugeordnet. Wenn die Zuordnung fehlschlägt, können Zahlungen Aufträgen manuell zugeordnet werden. Verwalte und bearbeite Zahlungen im Menü *Aufträge » Zahlungsverkehr*. Unzugeordnete Zahlungen erkennst du am Kreditkartensymbol (icon:credit_card[set=material, role=darkGrey]). Du kannst Zahlungen Aufträgen direkt in der Übersicht oder in der Detailansicht der <<#30,Zahlung zuordnen>>. +
...
* *_Möglichkeit 1:_* Klicke auf icon:search[set=material, role=darkGrey] (*Suchen*), um eine Liste aller Zahlungen zu sehen. 
...
. Klicke auf icon:search[set=material, role=darkGrey] *Suchen*. +
...
Klicke in der Zeile des gespeicherten Filters auf icon:star_border[set=material, role=darkGrey] *Als Standard festlegen*. Wenn du einen anderen Filter als Standard festlegen möchtest, deaktiviere den aktuell gewählten Standardfilter mit einem Klick auf icon:star[set=material, role=darkGrey] *Nicht als Standard verwenden*.

[#payments-myview]
== MyView nutzen

Die Bedienoberfläche der Detailansichten von Zahlungen sowie der Ansicht zum Teilen einer Zahlung werden dir als MyView zur Verfügung gestellt. Das bedeutet, dass Benutzer:innen sich jeweils eine eigene Ansicht mit den zur Verfügung stehenden Elementen erstellen können. Dadurch kann jede:r selbst bestimmen, welche Informationen an welcher Stelle benötigt werden. Durch diese Individualisierung wird das Arbeiten nicht nur komfortabler, sondern auch beschleunigt. In diesem Kapitel wird erklärt, wie man mit MyView umgeht und sich eine eigene Ansicht anlegt. Die Bearbeitung von Zahlungen, z.B. das <<#30, Zuordnen>>, das <<#40, Lösen>> oder auch das <<#50, Teilen>> von Zahlungen, wird in den nachfolgenden Kapiteln erklärt.
...
Zur Ansicht zum Teilen von Zahlungen kommst du, indem du in der Übersichtstabelle in der Zeile der Zahlung auf *Zahlung teilen* (icon:call_split[set=material, role=darkGrey]) klickst. +
...
. Klicke auf icon:add[set=material, role=darkGrey] *Neue Ansicht erstellen ...*.
. Gib einen Namen ein.
...
.. Klicke auf icon:ellipsis-v[role=blue] und dann auf icon:add[set=material, role=darkGrey] *Zeile hinzufügen*.
.. Klicke auf icon:add[set=material, role=darkGrey] *Spalte hinzufügen*.
...
. Falls erforderlich:
.. Klicke erneut auf *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]) und passe die Ansicht weiter an.
...
. Suche (icon:search[set=material, role=darkGrey]) und öffne das betreffende Benutzerkonto.
. Klicke auf *Ansichten*.
. Erweitere die Listeneinträge (icon:chevron-right[role=darkGrey]) und wähle die Ansichten (icon:check_box[set=material, role=skyBlue]), auf die der Benutzer Zugriff haben soll.
...
. Klicke auf *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]).
. Klicke auf icon:open_in_new[set=material, role=darkGrey] *Rechteverwaltung*.
...
. Suche (icon:search[set=material, role=darkGrey]) und öffne die betreffende Benutzerrolle.
. Klicke auf *Ansichten*.
. Erweitere die Listeneinträge (icon:chevron-right[set=material, role=darkGrey]) und wähle die Ansichten (icon:check_box[set=material, role=skyBlue]), auf die die Benutzerrolle Zugriff haben soll.
. Speichere (icon:save[set=material, role=darkGrey]) die Einstellungen.
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[set=material]) die gewünschte Zahlung wie im Kapitel <<#20, Zahlungen suchen>> beschrieben.
...
Für den Fall, dass in der Zuordnungstabelle keine Übereinstimmungen angezeigt werden, hast du weitere Möglichkeiten. Du kannst über das Kontextmenü (icon:more_vert[set=material, role=darkGrey]) folgende Optionen auswählen:
...
. Klicke auf *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]). +
→ Das Fenster *Spalten konfigurieren* öffnet sich.
. Wähle aus, welche Spalten angezeigt werden sollen.
. Verschiebe (icon:drag_indicator[set=material, role=darkGrey]) die Spalten so, dass sie in der Reihenfolge angezeigt werden, in der du sie brauchst.
. Klicke auf *Bestätigen*, um deine Auswahl zu speichern. +
...
Manuell gebuchte Zahlungseingänge erkennst du an dem Symbol icon:extension[set=material, role=darkGrey] *Manuelle Buchung*.
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[set=material]) die gewünschte Zahlung wie im Kapitel <<#20, Zahlungen suchen>> beschrieben.
. Klicke in der Zeile der Zahlung, für die du die Zuordnung lösen möchtest, auf *Zahlung lösen* (icon:money_off[set=material, role=darkGrey]). +
...
. Klicke auf *Zahlung lösen* (icon:money_off[set=material, role=darkGrey]). +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[set=material]) die gewünschte Zahlung wie im Kapitel <<#20, Zahlungen suchen>> beschrieben.
. Klicke in der Zeile der gewünschten Zahlung auf *Zahlung aufteilen* (icon:call_split[set=material, role=darkGrey]). +
...
. Klicke auf *Speichern* (icon:save[set=material, role=darkGrey]). +
...
Mit der Funktion *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]) kannst du selbst bestimmen, welche Spalten dir in welcher Reihenfolge angezeigt werden. +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Klicke in der Zeile einer Zahlung auf *Zahlung löschen* (icon:delete[set=material, role=darkGrey]). +
...
→ Du wirst weitergeleitet zur Detailansicht dieser Zahlung.
. Klicke über den Zahlungsdetails auf *Zahlung löschen* (icon:delete[set=material, role=darkGrey]). +
→ Die Abfrage *Zahlung löschen* wird geöffnet.
. Klicke in der Abfrage auf *Ja*. +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Wähle die Zahlungseingänge aus (icon:check_box[set=material, role=skyBlue]), die du zuordnen oder löschen möchtest.
. Klicke über der Übersichtstabelle entweder auf *Zahlungen löschen* (icon:delete[set=material, role=darkGrey]) oder auf *Zahlungen automatisch zuordnen* (icon:credit_card[set=material, role=darkGrey]). +
→ Du musst das Löschen oder das Zuordnen in einer Abfrage bestätigen oder ablehnen.
. Nachdem du die Aktion ausgeführt hast, wird dir angezeigt, wie viele der ausgewählten Zahlungen automatisch zugeordnet oder gelöscht wurden.

[TIP]
.Zahlungen, die einem Auftrag zugeordnet sind, können nicht gelöscht werden
====
Es kann vorkommen, dass nicht die gesamte Auswahl bearbeitet werden konnte. Zum Beispiel könnte eine oder mehrere der Zahlungen, die du löschen möchtest, einem Auftrag zugeordnet sein, dann sind Zahlungen _nicht_ löschbar. Wenn du aufgrund der Anzeige genauere Informationen haben möchtest, kannst du diese im Log einsehen. Gehe dafür ins Menü *Daten » Log*.
====
...
Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]) noch das Element *Absender der Zahlung* hinzufügen oder andere Elemente entfernen. Verändere die Anordnung der Elemente im Portlet, indem du sie per Drag & Drop verschiebst. +
...
Optional kannst du über Ansicht bearbeiten (icon:design_inline_edit[set=plenty, role=darkGrey]) Elemente entfernen oder die folgende Infoboxen noch als zusätzliche Elemente zu diesem Portlet hinzufügen:
...
Verändere die Anordnung der Elemente im Portlet, indem du sie per Drag & Drop verschiebst. Entscheide in den *Einstellungen* des Portlets zudem, wie viele Kacheln (Infoboxen) dir in einer Zeile angezeigt werden sollen.
...
Zudem kannst du eine zugeordnete Zahlung in der Zuordnungstabelle lösen. Klicke dafür auf *Zahlung lösen* (icon:money_off[set=material, role=darkGrey]). Beachte allerdings, dass das <<#40, Lösen von Zahlungen>> nicht unbedacht geschehen sollte.
...
Mit der Funktion *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]) kannst du selbst bestimmen, welche Spalten dir in welcher Reihenfolge angezeigt werden. +
Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]) außerdem in den *Einstellungen* des Portlets *Zuordnung* festlegen, ob es beim Öffnen der Detailansicht einer Zahlung standardmäßig aus- oder eingeklappt sein soll. Nutze dafür die Option *Portlet eingeklappt*. +
Möchtest du den Namen des Portlets ändern, dann gib im Feld *Titel* den gewünschten Namen ein.
...
Das Portlet *Historie* bietet dir eine Übersicht über alle Aktionen, die in Zusammenhang mit dieser Zahlung ausgeführt wurden. Angezeigt wird auch, wer oder was (z.B. durch ein Plugin) die Aktion zu welchem Zeitpunkt ausgeführt hat, sodass eine Nachvollziehbarkeit immer gewährleistet ist.
...
Beachte, dass die Einträge in der Spalte *Wert* immer auf Englisch sind, weil diese Daten direkt aus der Datenbank entnommen werden. Über *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]) kannst du selbst bestimmen, welche Spalten dir in welcher Reihenfolge angezeigt werden.

Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]) außerdem in den *Einstellungen* des Portlets *Historie* festlegen, ob es beim Öffnen der Detailansicht einer Zahlung standardmäßig aus- oder eingeklappt sein soll. Nutze dafür die Option *Portlet eingeklappt*. +
Möchtest du den Namen des Portlets ändern, dann gib im Feld *Titel* den gewünschten Namen ein.
...
Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty, role=darkGrey]) in den *Einstellungen* des Portlets *Eigenschaften* festlegen, ob es beim Öffnen der Detailansicht einer Zahlung standardmäßig aus- oder eingeklappt sein soll. Nutze dafür die Option *Portlet eingeklappt*. +
Möchtest du den Namen des Portlets ändern, dann gib im Feld *Titel* den gewünschten Namen ein.

[#100]
== Zahlungseingänge auf dem Dashboard anzeigen

Auf dem xref:willkommen:system-personalisieren.adoc#110[Dashboard] deines plentymarkets Backends kannst du das Element *Zahlungen* hinzufügen. In diesem Element findest du Informationen zu unzugeordneten Zahlungen sowie alle Zahlungseingänge vom vorherigen und aktuellen Tag. Wenn du dieses Element nicht auf deiner Startseite verwenden möchtest, gelangst du über *Aufträge » Zahlungsverkehr* und durch die Verwendung der Filter in der Suche zu den gleichen Ergebnissen.
...
Die unzugeordneten Zahlungen solltest du mindestens einmal wöchentlich bearbeiten und bereinigen, damit dir der Überblick im Tagesgeschäft nicht verloren geht. Wie du nicht zugeordnete Zahlungen und Buchungen den Aufträgen zuweist, erfährst du im Kapitel <<#30, Zahlungen zuordnen>>.
