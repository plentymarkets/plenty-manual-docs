...
|Wähle *0.7* (nicht so genaue Übereinstimmung zur Zuordnung erforderlich) oder *1.0* (genaue Übereinstimmung erforderlich). Ist dieser Wert höher eingestellt, kann das Problem auftreten, dass Zahlungen nicht zugeordnet werden können. Daher ist in der *Standard-Einstellung* ein Wert von *0,75* eingestellt.
...
| *von* und *bis*
|Wähle in Verbindung mit *Datumstyp* einen Zeitraum, um nach Zahlungen zu suchen, die in diesem Zeitraum importiert wurden, eingegangen sind oder zugeordnet wurden, je nach Auswahl.
...
Für den Fall, dass in der Zuordnungstabelle keine Übereinstimmungen angezeigt werden, hast du weitere Möglichkeiten. Du kannst über das Kontextmenü (icon:more_vert[set=material, role=darkGrey]) folgende Optionen wählen:
