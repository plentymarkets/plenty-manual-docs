...
In der Ansicht zum Teilen einer Zahlung geben dir die folgende Infoboxen einen schnellen Überblick über die wichtigsten Informationen:
...
Mit der Funktion *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]) kannst du selbst bestimmen, welche Spalten in welcher Reihenfolge angezeigt werden. +
...
Verändere die Anordnung der Elemente im Portlet, indem du sie per Drag & Drop verschiebst. Entscheide in den *Einstellungen* des Portlets außerdem, wie viele Kacheln (Infoboxen) in einer Zeile angezeigt werden sollen.
...
Beachte, dass die Einträge in der Spalte *Wert* immer auf Englisch sind, weil diese Daten direkt aus der Datenbank entnommen werden. Über *Spalten konfigurieren* (icon:settings[set=material, role=darkGrey]) kannst du selbst bestimmen, welche Spalten in welcher Reihenfolge angezeigt werden.
