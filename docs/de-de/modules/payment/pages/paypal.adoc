...
====
Da es einige Unterschiede zwischen der Plugin-Version 5.X und 6.0 gibt, sind auf dieser Seite einige Kapitel mit Tabs versehen. Wähle jeweils das entsprechende Tab, um die für dich relevanten Informationen zu bekommen.
====
...
+
--
Wenn du auf Version 5.0 aufwärts des PayPal-Plugins bist, sind hier keine weiteren Schritte erforderlich. Wenn du auf Version 6.0 updaten möchtest, wechsele in das Tab zu Version 6.0.
--
Plugoin-Version 6.0::
+
--
...
====
In Version 6.0 des PayPal-Plugins gibt es einige Neuerungen. Beachte daher einige Punkte, wenn du auf Version 6.0 des PayPal-Plugins aktualisieren möchtest: +
...
–
Die Schritte zur Aktualisierung des PayPal-Plugins auf Version 6.0 und die Aktivierung von PayPal Checkout sind in diesem Click-through-Guide zusammengestellt. Screenshots helfen dir durch die einzelnen Schritte. Du kannst den Guide auch als PDF herunterladen.

PayPal Checkout – Update auf Version 6.0 - HIER PFD DES GUIDES HINTERLEGEN
...
|Mit *My account: Additional payment information* verknüpfen, um Kund:innen im Mein-Konto-Bereich die Möglichkeit zu geben, nach nicht erfolgreicher Zahlung eine erneute Zahlung durchzuführen. Für PayPal wird die Schaltfläche *Jetzt zahlen*. +
Mit *Order confirmation: Additional payment information* verknüpfen, um Kund:innen in der Bestellbestätigung die Möglichkeit zu geben, nach nicht erfolgreicher Zahlung eine erneute Zahlung durchzuführen. Für PayPal wird die Schaltfläche *Jetzt zahlen*.
...
In den Ereignisaktionen stehen dir Aktionen für PayPal zur Verfügung. Richte diese ein, so dass sie in deinen Arbeitsablauf passen.
...
* Zur Nutzung von PayPal PLUS muss die xref:payment:paypal.adoc#80[Zahlungsart in Kundenklassen erlaubt sein und darf in Versandprofilen nicht gesperrt] sein.
...
Bei Problemen mit der Kontoverknüpfung ist es empfehlenswert einen anderen <<willkommen/schnelleinstieg/systemvoraussetzungen#100, Browser>> zu verwenden, um diese Fehlerquelle auszuschließen. Des Weiteren sollte grundsätzlich überprüft werden, ob es Probleme mit dem PayPal-Konto gibt. Möglicherweise wurde zum Beispiel die E-Mail noch nicht bestätigt. Weitere mögliche Fehlerquellen und deren Lösung werden im Folgenden beschrieben.

[#140]
==== Was mache ich, wenn bei der Kontoverknüpfung nur die Schaltfläche *Zur PayPal-Einrichtung* angezeigt wird?

Überprüfe zuerst, ob die link:https://marketplace.plentymarkets.com/paypal_4690[aktuelle Version]{nbsp}icon:external-link[] des PayPal-Plugins in deinem plentymarkets System installiert und bereitgestellt ist. Wie du beim Aktualisieren vorgehst, erfährst du im Handbuchkapitel <<plugins/installierte-plugins-aktualisieren#, Installierte Plugins aktualisieren>>.
...
[%interactive]
* *Lösung 1:* Wechsle den Browser und versuche es erneut. plentymarkets unterstützt die Browser Google Chrome und Mozilla Firefox.
* *Lösung 2:* Es existiert noch eine API-Berechtigung im PayPal-Konto. Entferne die Berechtigung für plentymarkets. Die API-Berechtigung ist link:https://www.paypal.com/cgi-bin/webscr?cmd=_profile-api-list-auths[hier]{nbsp}icon:external-link[] zu finden.
...
Seit die alten Schnittstellen für alle Plugin-Zahlungsarten nicht mehr unterstützt werden, gilt für Ereignisaktionen generell, dass die Filter aus Filtergruppe *Plugins* sowie die Aktionen aus der Aktionsgruppe *Plugins* für Plugin-Zahlungsarten genutzt werden sollen. Allerdings greift ab Version 5.3.1 des PayPal-Plugins die Aktion *Rückzahlung der PayPal-Zahlung* auch für Zahlungen über die alte PayPal-Schnittstelle. +
Um andere Änderungen für das PayPal-Plugin einzusehen und Aktualisierungen nicht zu verpassen, kannst du den link:https://marketplace.plentymarkets.com/paypal_4690[Changelog des PayPal-Plugins]{nbsp}icon:external-link[] nutzen. Generell empfehlen wir die Plugin-Zahlungsarten, die über den link:https://marketplace.plentymarkets.com/plugins/payment/payment-integrationen[plentyMarketplace^]{nbsp}icon:external-link[] zur Verfügung gestellt werden, zu nutzen.
