...
* Orders with the OTTO Market status ANNOUNCED are imported to plentymarkets without address details and with the status *[1] Incomplete data*. These are orders that can still be cancelled by the customer, or orders with the payment method cash in advance that have not been paid. As soon as the order changes to the status PROCESSABLE at OTTO Market, the address is added to the order and the order is moved to status *[4] In preparaton for shipping*. Orders with the payment method cash in advance are moved to status *[3] Waiting for payment*. +
