= Preparing variations for Amazon
:keywords: preparing variations for amazon, variations amazon, amazon variations
:description: Learn which settings you need to carry out in order to prepare your variations for the export to the market Amazon.
:page-pagination:
...
include::partial$amz-variation-availability.adoc[]

[#750]
== Defining SKU manually

include::partial$amz-sku-manually.adoc[]

[#760]
== Defining additional SKUs manually

include::partial$amz-additional-sku-manually.adoc[]
...
Proceed as described below to define a sales price for Amazon channels. This price is displayed on the channel.

[.instruction]
Defining the sales price for channels:
...
Proceed as described below to define an RRP for Amazon channels. This price is transferred to Amazon as *list price*.

[.instruction]
Defining the RRP for channels:
...
In order to transfer special offers to Amazon, create an additional sales price with the price type *Special offer* and link it to the variation. In addition, create the properties *sale_from_date* and *sale_end_date* and link these properties to the variation. Proceed as described below to define special offers for Amazon items.

[.instruction]
Defining a special offer for Amazon channels:
...
Now create the properties *sale_from_date* and *sale_end_date* and map these properties to Amazon categories. Proceed as decribed below to create and map the property *sale_from_date*. Create the property *sale_end_date* in the same way but enter the name *sale_end_date* and link the Amazon field *sale_end_date* instead.
...
. In the field *Name*, enter the name *sale_from_date*. +
*_Important:_* Only use this name *sale_from_date* for the property. Otherwise, the special offer price will not be exported to Amazon.
. Enter a position number for the property.
. *_Optional:_* If you sell internationally, save the name *sale_from_date* for additional languages.
...
. Now create the property *sale_end_date* in the same way. In the drop-down list *Field*, select *sale_end_date*.

You have already created a special offer price and the properties *sale_from_date* and *sale_end_date*. Now specify the period in which the offer price is valid. You also save a price.
...
. In the *Variation properties*, enter the property name *sale_from_date*.
...
. In the *Variation properties*, enter the property name *sale_end_date*.
. Click on *Add*. +
→ The property is added.
. Click into the column *Value*. +
→ The calendar opens.
...
*_Tip:_* To process several items at once, use xref:item:mass-processing.adoc#[mass processing].
...
The freely definable fields are used to customise the data that you export to Amazon. Those fields have to be linked to xref:item:free-text-fields.adoc#[free text fields]. The free text fields allow you to individually customise the item data and therefore save an exact description of the item.

[#1000]
=== Creating free text fields

First, create the xref:item:free-text-fields.adoc#[free text fields] that you want to link to freely definable fields on Amazon. To do so, proceed as described below.
...
include::partial$amz-link-categories.adoc[]

[#1350]
== Linking attributes and attribute values

xref:item:attributes.adoc#[Attributes] also have to be linked before your items can be transferred to Amazon. It is also possible to link attributes for several languages.
...
Use xref:item:properties.adoc#500[properties] to define individual features of a variation. For example, you can use properties to highlight a variation’s technical features, such as Bluetooth or WLAN.
...
In the last step, activate the xref:item:managing-items.adoc#80[characteristic] in the item settings and enter a value.
...
You have set up the interface to Amazon and have prepared your variations. Now, set up the item export to Amazon.
