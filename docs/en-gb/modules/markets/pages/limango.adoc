...
[#filter-export]
==== NEW: Only exporting specific items (_optional_)

include::partial$catalogue-filters.adoc[]

[#preview]
=== NEW: Previewing item export data (_optional_)

When you have finished mapping all data fields, you can test the first five data rows of the export using the preview function.

include::data:page$export-standard-formats.adoc[tag=catalogue-preview]
