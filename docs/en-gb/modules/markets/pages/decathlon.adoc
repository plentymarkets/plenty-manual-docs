...
To deactivate an offer on Decathlon, for the respective variation, a stock of 0 is exported to Decathlon. A stock of 0 is exported if the variation has an active SKU for Decathlon and at least one of the following cases applies to the variation:

- The variation is inactive.
- The main order referrer 173.00 Decathlon is deactivated.
- All subordinate Decathlon order referrers (e.g. 173.01, 173.02…) are deactivated.

This means that if you want to deactivate an offer for a variation on Decathlon, you need to do either of the following:

- deactivate the variation,
- or deactivate the order referrer 173.00 Decathlon,
- or deactivate all subordinate Decathlon order referrers.
...
You are free to use HTML to structure content that is intended for use with Decathlon.
...
[#upload-return-label]
=== Automatically uploading return labels to Decathlon

Set up an xref:automation:event-procedures.adoc#[event procedure] to automatically upload return labels to Decathlon.

:table-event-procedure: <<#return-label-decathlon>>

include::_includes:partial$event-procedure.adoc[]

*_Note:_* The selected statuses and filters in <<#return-label-decathlon>> only serve as an example. Select filters and statuses for the event procedure which best suit your workflow.

[[return-label-decathlon]]
.Event procedure for uploading invoices to Decathlon
[cols="1a,3a,3a"]
|===
|Setting |Option |Selection
...
* *Documents: Return slip generated* +
...
| *Procedure*
| *Plugin*
| *Upload return label to Decathlon*
|===
