...
// The person who lists the product first is owner. Only owners can update the item data. eMAG manually checks the updates. So don't update too often. eMAG checks all new offers manually.
...
* Whoever lists item data for a product first on {market} becomes the owner of this *product*. Only owners can update the item data of the product. The owner's price is displayed to the right of the item description.
...
*_Tip:_* eMAG checks _all_ new products manually. As such, do not update your product data unless necessary.
...
* {market} checks all offer data manually. As we said, patience is a virtue.
* Your offer is published on the product page below the item description as an additional offer.
* Every time you update the offer data, someone at {market} manually checks the data again. +
*_Tip:_* eMAG checks all new offers manually. As such, do not update your offer data unless necessary.
...
// Why does the general referrer exist? When is this needed?
...
// One catalogue or several?

[#L8371H8B]
=== Creating a catalogue

Item data is exported to eMAG via catalogues using the marketplace format *eMAG*. To export your items to eMAG, create a catalogue in the *Data » Catalogue* menu. Link the eMAG data fields with item data saved in plentymarkets in the marketplace format afterwards. The catalogue data is automatically exported to eMAG once a day if you have activated the *Item export* in the *Setup » Markets » eMAG » Settings* menu.
...
// Who creates the order documents?
// Is a location necessary?
// Shipping via hub in Rumania?
// In which language do we need to send the item data?
// Assistant: once or per country?
// Items vs. Offers
