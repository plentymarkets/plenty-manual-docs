...
[IMPORTANT]
.Trigger order import for Kaufland SK orders manually
====
The order import for orders generated on the market Kaufland SK (Slovakia) does not work properly at present. Therefore, you have to trigger the order import manually. To do so, save the settings for Kaufland again. To save the settings, proceed as follows:

[.instruction]
Triggering the order import for SK orders manually:

. Go to *Setup » Markets » Kaufland » Settings » [Open Kaufland account]*.
. In the top left corner, click on *Save* (terra:save[]). +
→ The configuration for the market SK is saved.

*_Note:_* Note that the market Kaufland CZ (Czech Republic) has not been launched yet. This means that no orders for this market will be generated at present.
====
...
However, note that Kaufland has not launched the market in the Czech Republic yet. You can already list your items there, but not yet sell them.
====
...
*_Note:_* Click on material:open_in_new[] to set up new shipping service providers in plentymarkets. Go to *Setup » Orders » Shipping » Settings » Tab: Shipping service providers*.
...
Link warehouses with the option *Match warehouses* in the menu *Setup » Markets » Kaufland » Settings » [Open account]*.
====
...
*_Note:_* For legal reasons, the payment method on Kaufland invoices is displayed as *Payment via real.de*.
