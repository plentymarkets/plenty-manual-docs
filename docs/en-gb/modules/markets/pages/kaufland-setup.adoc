...
[cols="1,3a"]
!===
2+! *Item settings* (material:settings[])

! *Update items* (material:refresh[])
...
! *Send items to Kaufland.de* (material:playlist_play[])
! Send the item file to Kaufland.de.

2+! *Offer settings* (material:settings[])

! *Update offers* (material:refresh[])
! This option creates an offer file. This file can be viewed via *Offer preview* (material:preview[]).

! *Offer preview* (material:preview[])
! Display the offer file preview.

! *Send offers to Kaufland.de* (material:playlist_play[])
