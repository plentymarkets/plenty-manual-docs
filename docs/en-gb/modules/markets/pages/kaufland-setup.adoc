...
link:https://www.Kaufland.de/[Kaufland^] is one of the largest fixed price markets in Germany. You can sell items from various product categories on Kaufland. Kaufland also processes payments for you. In other words, you are neither charged with payment costs nor do you have to worry about debt collection. +
...
[#100]
== Registering with Kaufland

In order to use the Kaufland interface, you have to register your Kaufland user account as a seller account. To do so, go to the link:https://www.Kaufland.de/seller/new/[Kaufland website^] and save your VAT number in the *Mein Konto* (My account) area.
...
If you export data to Kaufland using catalogues, then data are exchanged with Kaufland in the intervals described in <<#table-data-exchange-catalogue>>:

[[table-data-exchange-catalogue]]
...
. Go to *Setup » Markets » Kaufland » Settings*.
. Click on *New account* (material:add[]).
...
*_Note:_* This name is only shown in your plentymarkets system and not exported to Kaufland. As such, the shop name’s purpose is to help you better keep track of the list of configurations in the *Setup » Markets » Kaufland » Settings* menu.
...
After the shipment has been confirmed, this option allows Kaufland to use the package number in order to send the customer an email with the correct tracking link. +

*_Important:_* You have to assign each of your service providers to one of the service providers from the drop-down list.

*_Tip:_* Click on material:open_in_new[] to set up new shipping service providers in plentymarkets. Go to *Setup » Orders » Shipping » Settings » Tab: Shipping service providers*.

| *Status for orders with failed shipping confirmations*
| Select a status. If no shipping confirmation could be sent to Kaufland, the order is moved to this status. If necessary, create a xref:orders:managing-orders.adoc#create-status[new status]. +
...
First, click on *Update Kaufland shipping groups* (material:refresh[]) so that the latest Kaufland shipping groups are available in the drop-down list. +
...
a| Click on *Open matching table* (material:swap_horiz[]) to match the item availabilities in plentymarkets with delivery times in working days. +
...
The *Setup » Markets » Kaufland » Settings* menu provides an overview of your Kaufland accounts. <<table-account-overview>> lists the columns and available actions in the account overview.

*_Tip:_* To open a Kaufland account and to carry out settings, simply click on the line of the account in the overview.
...
| *Active*
| material:circle[role="blue"] = The account is active and data is exchanged with Kaufland. +
material:circle[] = The account is not active. Data is not exchanged with Kaufland.
...
[cols="1,3a"]
!===
2+! material:settings[] *Item settings*

! material:refresh[] *Update items*
! Creates a product file with all items that should be transferred to Kaufland. This file can be viewed via *Item preview* (material:preview[]).

! material:preview[] *Item preview*
! Display the item file preview. +
*_Note:_* If you have not fully set up the interface to Kaufland but want to take a look at the item file preview, then you have to activate the interface temporarily. Otherwise, the item file preview will be empty. In order to view the product file preview when the interface is not active, activate the interface and click on *Item preview* (material:preview[]). Deactivate the interface immediately afterwards by deactivating the setting *Data exchange* (in the *General* settings) and saving the changes. This way, you can prevent items from being transferred too soon.

! material:playlist_play[] *Send items to Kaufland.de*
! Send the item file to Kaufland.de.

2+! material:settings[] *Offer settings*

! material:refresh[] *Update offers*
! This option creates an offer file. This file can be viewed via *Offer preview* (material:preview[]).

! material:preview[] *Offer preview*
! Display the offer file preview.

! material:playlist_play[] *Send offers to Kaufland.de*
...
// item settings and offer settings not relevant for catalogue

[#225]
=== Deemed Supplier Model

On 01/07/2021, the new EU Digital Package came into force. This package results in changes to the regulations regarding VAT in EU countries. Due to these legal changes, Kaufland becomes a deemed supplier and is thus partially liable for VAT. Kaufland has reacted to this by introducing the *Deemed Supplier* model. The model is based on the location of the company, the location of warehouses and the value of a delivery. As such, the model is relevant for the following types of deliveries:
...
* During the import of Kaufland orders to your plentymarkets system, an automatic check determines if these orders are covered by the Deemed Supplier model. Information about these orders are provided to you by Kaufland via the seller portal, CSV reports and via the API.
...
*_Important_*: Kaufland is obliged to withhold and pay VAT for Deemed Supplier orders. As such, Kaufland issues invoices for these orders. As a result, you cannot create invoices for these orders.

Also note the required warehouse settings for warehouses outside of the EU. If items are delivered from outside the EU, the corresponding warehouses have to be linked to Kaufland warehouses. You do this via the setting *Match warehouses* in the *Setup » Markets » Kaufland » Settings* menu.

[TIP]
.Pay attention to warehouse mapping for Kaufland
====
When updating offers, pay attention to mapped warehouses for Kaufland. +
It is not possible to assign more than one Kaufland warehouse to an offer on Kaufland. If the warehouses of a variation in plentymarkets are assigned to different warehouses on Kaufland, only the stock of one of those plentymarkets warehouses will be exported to Kaufland. Ideally, you should assign the warehouses of one variation in plentymarkets to the same warehouse on Kaufland. Then the sum of all plentymarkets warehouses will be shown on Kaufland. +
Assign warehouses with the option *Match warehouses* in the menu *Setup » Markets » Kaufland » Settings » [Open account]*.
====
...
[IMPORTANT]
.Do not group Kaufland orders
====
During the order import, Kaufland orders are synchronised by the external order number. However, in grouped orders, the external order number no longer exists. As such, grouping orders might result in orders being created anew when they are retrieved again.
====
...
// not relevant for catalogue

Variations must have a barcode that is activated for {market}.

Proceed as described below to define a barcode for the Kaufland order referrer that is already saved in your system. This barcode is transferred to Kaufland for your variations.

[.instruction]
Defining a barcode for Kaufland:
...
. Click on *Apply*. +
→ The ID and category path of the Kaufland category are displayed.
. If you already know the ID of the Kaufland category, enter it directly into the *Market category field* to link it to your plentymarkets category.
. Save (terra:save[]) the settings.

[IMPORTANT]
.Subcategories do not have to be linked
====
In some cases, your subcategories may not match the Kaufland subcategories. In such cases, link your items with Kaufland's corresponding main category. Kaufland automatically assigns your items to a subcategory. You should not transfer items to Kaufland that have not been assigned to a Kaufland category.
====
...
[#550]
===== Linking plentymarkets characteristics with Kaufland attributes

It is possible to overwrite attributes with plentymarkets characteristics or to add Kaufland attributes. +
*_Important:_* In order for the plentymarkets characteristics to be correctly assigned during the Kaufland export, the internal name of the plentymarkets characteristic must be the same as the column header in the Kaufland Product.csv file. +
The Kaufland attributes EAN, title, description, manufacturer and picture are mandatory entries for the item export. These attributes are transferred by plentymarkets by default. These attributes can only be overwritten by an attribute. Other attributes, such as "material_composition", "clothing_size" or "weight" can be added if a corresponding characteristic is created in plentymarkets.

[WARNING]
.Mandatory entries for the item export
====
The Kaufland attributes *ean*, *title*, *description*, *manufacturer* and *picture* are mandatory entries for the item export.
====
...
. Enter the name of the Kaufland attribute into the *Name (internal)* field. +
...
You have to create a separate catalogue for each Kaufland product category and for each country platform that you want to offer your items on. For example, you create different catalogues for the product categories *TVs* and *Cameras*. You can select the product category after you selected the country platform.
...
[#size-unit-kaufland]
===== Specifying the size unit as own value for Length, Width, Height, and Weight

In many Kaufland catalogues, the data fields *Length*, *Width*, *Height*, and *Weight* are mandatory and have to be mapped. We recommend the following mappings for these data fields:
...
* *Weight*: *Variation » Gross weight in g*, _specify size unit_
...
. *Save* the settings.
. Repeat the steps above for the data fields *Width*, *Height*, and *Weight*.
...
. Copy the text for Kaufland invoices into the input field.
. Save (terra:save[]) the settings. +
...
* _If your bank details are saved in the menu *Setup » Client » Standard » Locations » Germany (standard) » Documents » Invoice » Tab: PDF template » Option: Payment instruction*:_
  ** No additional settings are necessary to make sure that your bank details are not shown on Kaufland invoices.
...
* _If you show your bank details directly on the xref:orders:order-documents.adoc#1700[PDF template] for invoice documents:_
  ** Create an additional PDF template for Kaufland that does not contain any bank details.
  ** *_Optional:_* Save the Kaufland logo on this PDF template (see logo below).
  ** Go to *Setup » Client » Standard » Locations » Germany (standard) » Documents » Invoice » Tab: PDF template*.
  ** Click on *Add template*.
  ** In the window *Upload new PDF template*, select the payment method *Kaufland* and the language *German*.
...
* Send shipping confirmation (<<#shipping-confirmation, How?>>)
...
Set up an xref:automation:event-procedures.adoc#[event procedure] to automatically inform Kaufland of cancellations. Whenever you are dealing with cancellations, you need to send Kaufland a reason for the cancellation. Open the *Setup » Orders » Status* menu and create additional order statuses as needed. The following cancellation reasons can be transferred to Kaufland:
...
.Event procedure for automatically sending cancellations to Kaufland
[cols="1,3,3"]
|===
|Setting |Option |Selection
...
|Select a reason. +
*_Note:_* You can cancel individual order items for Kaufland. However, you cannot adjust the cancellation amount. As such, you can only refund the full sales price of an order item. You cannot grant discounts or reduce the price of cancelled order items.
|===
...
[#returns]
=== Returns for Kaufland

Unlike other marketplaces, you cannot send returns to Kaufland with an event procedure via plentymarkets yet. Process returns for Kaufland directly in your Kaufland seller account.
...
.Reference types and reference values for Kaufland
[cols="1,3,3"]
|===
...
| *externalOrderId*
|Filters by the Kaufland order number of the order.
|

| *externalOrderItemId*
|Filters by the ID of the Kaufland order item.
|
...
The event procedure <<#shipping-confirmation, Send shipping confirmation to Kaufland.de>> automatically informs {market} that the items of an order have been shipped. If the event procedure cannot be completed, the order is moved to the status that you selected for the setting *Order status for orders with failed shipping confirmations*.
