...
| Up to now, shipping confirmations were sent to Kaufland with an hourly cron. However, this cron is deactivated very soon. As such, set up the event procedure <<#shipping-confirmation, Send shipping confirmation to Kaufland.de>> now and set the option to *event procedure*.

[cols="1,3"]
!===
! *hourly cron*
! The hourly cron is deactivated very soon. As such, set up the event procedure <<#shipping-confirmation, Send shipping confirmation to Kaufland.de>> now. Then set the option *Send shipping confirmations via* to *event procedure*.
