...
[#old-export]
=== _Alternative 1:_ Export format KaufluxDE-Plugin _(old method)_
...
. In the *Automatic despatch* step, select the previously created email template for the *New order from Kauflux* option.
. Save (terra:save[]) the settings.
