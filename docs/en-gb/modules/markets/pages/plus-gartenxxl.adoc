...
:referrer-option: pass:quotes[options *Netto* with the ID *154* and *plus.de* with the ID *131*]
...
.Using properties
====
Use xref:item:properties.adoc#500[properties] to export values for marketplace data fields that have no corresponding field in plentymarkets.
====
...
You need specific delivery notes and return slips for Netto Marken-Discount. This chapter describes how you set up xref:orders:order-documents.adoc#1700[PDF templates] for delivery notes and return slips. This information is structured as follows:
