...
:keywords: eBay, eBay listing, eBay Plus, ePID, eBay token, eBay shop categories, Second Chance Offer, SCO, eBay platform
:description: Multi-Channel in plentymarkets: Set up the interface to the market eBay in your plentymarkets system.
:position: 10
:url: markets/ebay/ebay-setup
:id: DWYPTTK
...
You need your eBay access data consisting of the user name and password. You will receive this data from eBay. First, you need to become an link:https://reg.ebay.co.uk/reg/PartialReg?src=oldbizreg[eBay member^]{nbsp}icon:external-link[] and create an eBay account.

[#200]
== Basic eBay settings in plentymarkets
...
====
You need a valid <<business-decisions/system-administration/ssl-certificate#1200, SSL certificate>> for the eBay OAuth2 authentication.
====

Many calls already use the new eBay APIs. Those new APIs are based on the authentication method OAuth2 instead of the Auth-n-Auth method which was used before. The new authentication method OAuth2 is needed for more and more essential processes. Therefore, you should already authenticate your eBay accounts for OAuth2 when setting up eBay in plentymarkets.

[IMPORTANT]
.OAuth2 will become mandatory
====
Due to an adjustment on the part of eBay, processes relating to eBay business policies will soon not function any longer without OAuth2. Therefore you have to authenticate your eBay accounts for OAuth2 if you have not already done so.
====
...
:right-one: pass:quotes[*Rights > Markets > Credentials* and all subordinate rights]
:right-two: pass:quotes[*Rights > Markets > eBay > Marketplace > Display*]
...
. Expand (icon:plus-square-o[]) the *eBay shop categories* area to display an overview of your account’s shop categories. +
...
====
After changing the eBay account data, update your shop categories and return policy. Load the *shop categories* directly in this menu. Go to *Setup » Markets » eBay » Business policies » Tab: eBay return policy* to save the *return policy*.
====
...
* *Yes* = Further item details from the eBay catalogue are automatically added. For further details, refer to the link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/listing-item-product-details-catalogue?id=4653[eBay help^]{nbsp}icon:external-link[]. +
...
* *Yes* = Further item images from the eBay catalogue are automatically added. For further details, refer to the link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/listing-item-product-details-catalogue?id=4653[eBay help^]{nbsp}icon:external-link[]. +
...
* *Yes* = Listings are not ended if the items are unavailable. The offer is displayed but cannot be purchased. The number of items sold is not set to zero when the item is available again. Rather, the sales history remains as it is. In addition to this option, the <<Activating the automatic stock update, automatic stock update>> in the *Initial settings* tab must be activated. Furthermore, this option takes priority over the <<Setting up stock, End offers automatically>> option, i.e. offers are not ended but displayed and cannot be purchased. +
...
Before linking eBay shipping profiles to the plentymarkets shipping profiles, you have to enter terms of payment and shipping as well as general conditions in the *Setup » Markets » eBay » Business policies* menu.
...
→ You can select an already existing status or you can <<orders/managing-orders#1220, create>> a new status that best suits your workflow.
. Save (icon:save[set=plenty]) the settings.
...
+
--
The <<automation/procedure-manager#>> automatically retrieves the latest positive feedback at the same time every day and posts positive feedback for your customers.
...
+
--
With an <<automation/event-procedures#, event procedure>>, feedback is automatically given when a specific event occurs.
...
You can create multiple prices for eBay. For example, you can create separate prices for each eBay account or each eBay platform. Read the <<markets/ebay/best-practices-ebay-faq#400, eBay FAQ>> to learn how the sales prices are prioritised if you have created more than one sales price.
...
. Edit the template text or create a new <<crm/sending-emails#1200, email template>>. +
...
. From the drop-down list *New order from eBay*, select the email template. Note <<#table-events-automatic-email-despatch>>.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send eBay messages.
...
With link:https://www.ebay.co.uk/help/selling/selling/selling-vehicles-parts-accessories?id=4647[parts compatibility listings^]{nbsp}icon:external-link[], you link the items that you want to sell to the correct vehicles. That way you increase your sales prospects because items of parts compatibility listings have a higher ranking in the search results. Create, edit and delete parts compatibility listings in the *Setup » Markets » eBay » Parts compatibility listing* menu.

TIP: You can also import and update parts compatibility listings with the import tool in the *Data » Import* menu. Use the <</data/importing-data/sync-types/elasticsync-parts-compatibility-listing#, parts compatibility listing>> import type. For further information, refer to the <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-parts-compatibility-listing#, Creating parts compatibility listings>> best practice.
...
For further information about troubleshooting, refer to the <<markets/ebay/ebay-setup#11750, FAQ: Second Chance Offer>> chapter.
...
. Go to *Setup » Markets » eBay » Business policies » Tab: eBay terms of payment » New*.
...
Use this menu to define domestic and international shipping conditions. In addition to the region of delivery, select specific <<fulfilment/preparing-the-shipment#1000, shipping profiles>> or service providers for international shipping. Go to *Setup » Orders » Shipping » Settings » Tab: Countries of delivery* and check your language settings for the countries that you want to ship your eBay items to. For further information, refer to eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme[Use the Global Shipping Programme^]{nbsp}icon:external-link[] page.

[.instruction]
Setting up domestic shipping:

. Go to *Setup » Markets » eBay » Business policies » Tab: eBay shipping conditions » Tab: New*.
...
* *Express shipping* = Items are shipped the same day they were ordered if the order was placed before a certain time. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/posting-items/estimated-delivery-dates?id=4086[Estimated delivery dates for sellers^]{nbsp}icon:external-link[] page.

| *Shipping discount for combined payment*
| Select whether the shipping discount for combined payment should be applied. link:https://signin.ebay.com/ws/eBayISAPI.dll?SignIn&UsingSSL=1&pUserId=&co_partnerId=2&siteid=0&ru=https%3A%2F%2Fcgi5.ebay.com%2Fws%2FeBayISAPI.dll%3FSellerShippingPreferences%26guest%3D1&pageType=3727[Shipping discount for combined payment^]{nbsp}icon:external-link[] must be set up on eBay.

| *Rules governing shipping at a special rate*
| Select whether the rule governing shipping at a special rate should be applied. link:https://www.ebay.com/help/selling/shipping-items/shipping-rates/shipping-discounts?id=4168[Rules governing shipping at a special rate^]{nbsp}icon:external-link[] must be set up on eBay.

| *Rate tables (additional regions)*
| Select whether the rate tables should be applied. Rate tables must be set up on eBay. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/posting-items/setting-postage-options?id=4089[Setting up your postage options^]{nbsp}icon:external-link[] page.
...
====
For further information, refer to the <<Selling internationally, Selling internationally>> chapter and eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme[Using the Global Shipping Programme^]{nbsp}icon:external-link[] page.
====

[#2100]
==== Defining the return policy

You can set up your return policy either on eBay or in the *Setup » Markets » eBay » Business policies » Tab: eBay return policy* menu. The return policy saved on eBay is overwritten when entering the return policy in plentymarkets. An overview of existing profiles for the return policy is displayed. Edit or delete existing profiles here or create new profiles.

[.instruction]
Creating a new profile:

. Go to *Setup » Markets » eBay » Business policies » Tab: eBay return policy » Tab: New*.
. Carry out the settings as described in <<#ebay-return-policy>>.
. Save (icon:save[set=plenty]) the settings.

[[ebay-return-policy]]
.Settings for *eBay return policy*
[cols="1,3a"]
|====
| Setting | Explanation
...
You can upload items with multiple variations as a single eBay offer. In order to create variations, you have to create attributes and link them with your items. Variations are <<item/introduction/structure#400, combinations of different attributes>>. Surcharges are applied to every combination, for example size, colour and material. Variations linked with a listing are also transferred to eBay when starting, restarting or updating the listing. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/creating-listings-variations?id=4150[Creating listings with variations^]{nbsp}icon:external-link[] page.
...
In order to be able to use your own properties for the eBay market, the properties have to be linked with eBay in the *Setup » Item » Characteristics* menu.
...
Use the template function *GetCrossSellingData* to add <<item/managing-items#970, cross-selling items>> that are linked to an item to a <<Managing layout templates, layout template>> and <<Linking a layout template, link>> this layout template to the desired listings. Add cross-selling items to layout templates in the *Setup » Listings » Layout* menu.
...
image::markets/ebay/assets/example-cross-selling.png[]
...
A product which is sold on eBay as a single product and in a set can have multiple ePIDs.

[TIP]
.Automatic ePID matching
====
New listings are automatically matched with an ePID when they are started. If there is more than one ePID available for a listing, then the first ePID is automatically matched. Further ePIDs are saved in a log message. +
For existing listings, you have to match ePIDs manually.
====

Link variations with ePIDs in the *Setup » Markets » eBay » ePID matching* menu.
...
- The selection *Transmit ePID* must be selected for the *Transfer as item number* option within the *Listings » Edit listings » [Open listing ID] » Edit listing* menu.
- An EAN must be saved for the variation within *Item » Edit item » [Open item] » Tab: Variation ID* in the *Barcode* area.
- - In the *Setup » Markets » eBay » Settings » Tab: Initial settings* menu, the *Add item details from the eBay catalogue* option has to be set to *Yes*.
...
- At the moment, only products from specific eBay categories can be matched with ePIDs. Further information about the affected categories is available link:https://developer.ebay.com/api-docs/sell/static/inventory/pbse-phase1-categories.html[here^]{nbsp}icon:external-link[].

[IMPORTANT]
.OAuth2 authentication required
====
In order to link ePIDs, you have to authenticate your eBay account for OAuth2 in the *Setup » Markets » eBay » Accounts » OAuth2* menu.
====

[.instruction]
Matching ePIDs:

. Go to *Setup » Markets » eBay » ePID matching*.
...
. Cick on *Match ePID* in the field of the ePID that you want to match.

[discrete]
===== Filtering variations without ePIDs

Go to *Setup » Markets » eBay » ePID matching* to filter variations without ePID matches. For example, you can search for MLIDs or item IDs, or you can display only variations which are sorted into a specific directory. For further information about filter options for variations without ePID matching, refer to <<#filter-epid>>.

[[filter-epid]]
.Filter options for variations without ePID match
[cols="1,3a"]
|====
| Filter | Explanation
...
Participate in the loyalty program and benefit from the advantages of eBay Plus in your plentymarkets system. In order to use eBay Plus, certain link:https://verkaeuferportal.ebay.de/ebay-plus-program-sellers-english#24899anchor1[requirements^] for sellers and listings must be met. For further information about the loyalty programme, refer to the link:https://verkaeuferportal.ebay.de/ebay-plus-program-sellers-english[eBay Plus program for sellers^]{nbsp}icon:external-link[] page.
...
One of the requirements of eBay Plus is link:https://verkaeuferportal.ebay.de/ebay-plus-program-sellers-english#24899anchor5[free and fast shipping^]{nbsp}icon:external-link[]. Not all shipping methods are appropriate for eBay Plus. For further information about appropriate shipping methods, refer to eBay. Select at least one permitted shipping method for eBay. Otherwise, an error message will be displayed when starting an eBay Plus listing.

[#7010]
== Setting up Click & Collect at Argos

Set up Click & Collect at Argos in plentymarkets to benefit from it. With Click & Collect at Argos, customers can collect the items they ordered on eBay UK in one of the many Argos stores in the UK. Refer to link:https://sellercentre.ebay.co.uk/business/click-collect#what[eBay^]{nbsp}icon:external-link[] for further information about Click & Collect Argos. Click & Collect at Argos is available for the UK platform only.

[#7020]
=== Activating Click & Collect for listings

Activate Click & Collect by activating the option *Buyers can collect the item at Argos with Click & Collect* in the *Setup » Markets » eBay » Business policies » Tab: eBay shipping conditions* menu. Next, you have to link the shipping conditions with the shipping profile from eBay that is saved for the listing.

[.instruction]
Activating Click & Collect for listings:

. Go to *Setup » Markets » eBay » Business policies » Tab: eBay shipping conditions*.
...
. Now select the shipping condition that you just edited for the *eBay shipping conditions* setting within the *General information* area.
. Save (icon:save[set=plenty]) the settings.
...
. Click on *Group function*. +
→ The *Group function* window opens.
. Select the shipping profile that is linked with Click & Collect for the *Shipping information* option in the *Update eBay options* area.
. Click on *Execute* to update the listings.
...
You need to set up an automatic procedure in the <<automation/procedure-manager#, procedure manager>> in order to automatically send dunning information. You can enter the conditions under which the procedures are carried out and define a time for them.
...
| *Send email template*
| Automatically send the selected email template, forexample *[6] eBay dispute process*.
...
For further information about automatic procedures, refer to the <<automation/procedure-manager#, Procedure manager>> page of the manual.
...
eBay returns and return statuses of the last 14 days are automatically imported from eBay to plentymarkets and updated every hour. When importing, new returns are created with the *Standard status for returns* in the *Orders » Edit orders* menu. The reason for return chosen by the customer on eBay is assigned to a <<orders/managing-orders#410, reason for return>> in plentymarkets. Imported eBay returns are edited the same way as other <<orders/managing-orders#400, returns>> in plentymarkets.
...
Set up <<automation/event-procedures#, event procedures>> to automatically exchange information with eBay. With event procedures, you initiate refunds from PayPal payments, mark returns as received and inform eBay that you have paid the refund. Event procedures are triggered when you change a return’s status in the *Orders » Edit orders » [Open return]* menu.

[IMPORTANT]
.Editing returns in the order type "return" or "credit note"
====
Before you set up event procedures for refunds, decide whether refunds should be edited in the order type *Return* or whether credit notes should be created. If you create credit notes, then select the filter *Order type > Credit notes* for event procedures with refunds. Because edited eBay statuses are imported into both the credit note and into the return, the event procedure would otherwise be triggered twice, i.e. for the return and for the credit note.
====

[#7800]
==== Accepting returns with eBay label

Set up an <<automation/event-procedures#, event procedure>> to automatically inform eBay about accepting a return and processing it with a return label from eBay.

:table-event-procedure: <<#event-procedure-accepting-returns>>

include::_includes:page$EA.adoc[]

*_Note:_* The status in the event procedure below was selected as an example. Select the status that best suits your workflow. To do so, select an already existing status or <<orders/managing-orders#1220, create>> a new status.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically inform eBay about accepting a return and processing it with your own label, not with a return label from eBay.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically inform eBay about rejecting a return.

[IMPORTANT]
.Pay attention to eBay’s terms of return
====
Only reject those returns that violate the terms specified in the menu *Setup » Markets » eBay » Business policies » Tab: eBay return policy*.
====
...
Set up an <<automation/event-procedures#, event procedure>> to offer your customer a partial refund of the purchase price.
...
Set up an <<automation/event-procedures#, event procedure>> to upload your own label to eBay.

[TIP]
.Creating a return label
====
You can upload a return label with an event procedure at eBay as described below or you can <<fulfilment/shipping-centre#retrieve-return-label, create>> and retrieve a return label in plentymarkets.
====
...
Set up an <<automation/event-procedures#, event procedure>> to inform eBay about having received the item that was returned by the customer.
...
Set up an <<automation/event-procedures#, event procedure>> to initiate a refund to the customer.
...
Set up an <<automation/event-procedures#, event procedure>> to inform eBay about having initiated a refund for the customer. This event procedure only works for orders that were not paid for with PayPal.
...
Purchase cancellation statuses are used to process purchase cancellations in the eBay back end. You can either use existing order statuses or <<orders/managing-orders#1200, create new order statuses>>. To import and manage eBay purchase cancellation requests in plentymarkets, you need the following order statuses:
...
. Save (icon:save[set=plenty]) the settings. +
→ eBay will be automatically informed about your decision if you have set up the event procedure <<#9800, Communicating a purchase cancellation acceptance>>.

Use the group function to change multiple orders at once. If you want to accept the request for unpaid orders, then search for all unpaid orders with the appropriate status and change the orders’ status with the <<orders/managing-orders#1700, group function>>.

It might be possible to set up an <<automation/event-procedures#, event procedure>> based on your specific workflows that accepts requests automatically using certain filters.
--
...
. Save (icon:save[set=plenty]) the settings. +
→ eBay will be automatically informed about your decision if you have set up the event procedure <<#9900, Communicating a purchase cancellation rejection>>.
--
...
Set up two <<automation/event-procedures#, event procedures>> to always automatically reject purchase cancellation requests for paid orders.
...
You can either use existing order statuses or <<orders/managing-orders#1200, create new order statuses>>.
...
Set up an <<automation/event-procedures#, event procedure>> for each reason to automatically start a purchase cancellation on eBay.
...
====
Go to *Setup » Markets » eBay » Business policies » Tab: eBay shipping conditions* to select the *shipping discounts for combined payments* that were imported from eBay.
====
...
| Select which <<Creating directories, directory>> the listings should be imported into. +
...
<<Activating the automatic stock update, Activate automatic stock update>> and <<Setting up stock, End offers automatically>>.
...
If you select this option, the following settings must be saved for the item: The item must be *active*, it must be *available* for eBay, and it must have a *sales price*. Further information can be found in the best practice <<markets/ebay/best-practices-ebay-activate-listings#, Activating listings>>.
...
//The *eBay Turbo Lister* is a listing tool that is especially intended for sellers who upload thousands of items in bulk. The *eBay turbo lister* is a desktop-based offline tool. (Quelle: link:http://pages.ebay.de/turbo_lister/tips.html[eBay^]{nbsp}icon:external-link[]).

//You are required to have a CSV file that contains your Turbo Lister data. plentymarkets only supports the *Turbo Lister format* CSV. If you do not have such a CSV file yet, then first export your items from Turbo Lister into a CSV file.
...
Select both the *shipping region* and country-specific <<fulfilment/preparing-the-shipment#1000, shipping profiles>> or *shipping providers* for local domestic shipping as the <<Defining shipping conditions, shipping conditions>>. Here, you also set up shipping providers for international shipping when shipping items to other countries from the respective country.
...
Use the import tool in the *Data » Import* menu to exchange data between plentymarkets and eBay. Listing data can be imported into plentymarkets with the <<data/importing-data/sync-types/elasticsync-market-listing#, Market listing>> import type. Use the <<data/importing-data/sync-types/elasticsync-parts-compatibility-listing#, parts compatibility listing>> import type to import or update parts compatibility listings.

[#11700]
== FAQ

include::../_textblocks/ebay-faq/ebay-faq.adoc[]
