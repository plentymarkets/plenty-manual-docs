...
| Item export
| daily at 7:00 h, 11:00 h, 15:00 h, and 19:00 h
...
*_Tip:_* The plenty ID of your clients is saved in the *Setup » plentyShop » (Select client) » Client Settings » Option: Plenty ID* menu.
...
[#limango-delete-items]
== Deleting items on limango

If you do not want to offer a variation on limango any longer, then you can either delete the market availability for limango or deactivate the variation. The variation will then be excluded from the nightly item export to limango and it will be deleted on limango.

*_Note:_* Variations will also be excluded from the item export and be deleted on limango if they cannot be validated. To find out why a variation could not be exported, check the <<#validation-file, validation file>> in the *Data » Log* menu.
...
[.instruction]
Deleting the market availability:

. Go to *Item » Edit item » [Open item] » [Open variation] » Variation tab: Availability*.
. *Delete* (icon:minus-circle[role="red"]) the availability *Limango*.
. *Save* (terra:save[]) the settings.

_Alternative 2:_

[.instruction]
Deactivating the variation:

. Go to *Item » Edit item » [Open item] » [Open variation] » Variation tab: Settings*.
. In the *Availability* area, remove the checkmark for the option *Active*. +
*_Note:_* Note that the variation can possibly not be sold on other markets either if it is deactivated.
. *Save* (terra:save[]) the settings.
