...
image::assets/rewe-promotion-price-example.png[]
--
====
...
For further information about the plentymarkets data fields that can be linked, refer to the <<data/exporting-data/managing-catalogues#_mapping_data_sources, Mapping data fields>> chapter.
...
Set up <<automation/event-procedures#, event procedures>> to automatically update the order status on REWE whenever specific events occur. You can set up the following event procedures:
