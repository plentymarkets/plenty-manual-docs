...
| *Status*
| Select the xref:orders:order-statuses.adoc#[status] that is to be assigned to orders that are imported from Amazon.
...
| Select the xref:orders:order-statuses.adoc#[status] for orders that were successfully transferred to FBA.
...
| *Status*
|Select the xref:orders:order-statuses.adoc#[status] for orders that were imported from Amazon.
...
In the menu *Stock » Amazon FBA Inbound* you can print a xref:orders:generating-pick-list.adoc#[pick list] for the FBA Inbound order.
...
A xref:fulfilment:package-content-list-test-phase.adoc[packet content list] lists the content of pallets, packages and items. The package content list informs Amazon about what items are in the packages that it stores and ships for you.
...
. Pick and pack the order based on the xref:fulfilment:package-content-list-test-phase.adoc[package content list].
 ...
 ** The contact details of customers are imported. +
 *_Exception:_* Reports from North America do not contain customer addresses. As a result, orders from these reports are created as incomplete orders with the status [1]. Missing addresses are then imported every 5 minutes.
