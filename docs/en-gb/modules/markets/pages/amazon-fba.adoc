...
* xref:orders:accounting.adoc#_creating_a_new_location[How to set up a new location]
...
======
If you want to manually synchronise data, then you also have to specify how SKU numbers should be assigned to the stock. This is specified in the xref:markets:amazon-setup.adoc#400[initial settings].
======
...
. Save (icon:save[set=plenty]) the settings to carry out the synchronisation.
--
====
'''
...
. In the column *Purchase order*, click on *Create redistribution* (icon:stock[set=plenty]). +
...
The statuses *Open*, *Done* and *All* are available. +
...
red: The shipment has the status *RECEIVING* or *CLOSED*. Not all shipped items have been received.
...
. Select the desired columns (icon:check_box[set=material, role=skyBlue]).
. Click on *Confirm*.
...
. Click on *Configure columns* (icon:settings[set=material]).
. Move your mouse cursor over the entry with the symbol icon:drag_indicator[set=material]. +
→ Your mouse cursor changes its shape (icon:open_with[set=material]).
...
. Click on icon:arrow_downward[set=material] or icon:arrow_upward[set=material] to change the sorting order.
|====
...
[#460]
=== Troubleshooting: Manually generating FBA reports

Amazon generates FBA reports once every 4 hours. FBA order details are then imported into plentymarkets automatically. However, to check orders or if there are problems with the import, you can manually generate a report of your FBA orders in the plentymarkets back end. Unlike the report that you can manually generate in Seller Central, this plentymarkets FBA report also contains personally identifiable information of your end customers. When you generate a report in the plentymarkets back end, any missing, deleted or corrected FBA orders are also imported into plentymarkets.

IMPORTANT: Generating FBA reports requires a lot of processing power. As such, the API of your system is temporarily deactivated if you generate too many reports. This can also negatively affect other processes in your system. To avoid this, only generate an FBA report if problems occurred with the automatic import.

[.instruction]
Manually generating FBA reports:

. Go to *Setup » Markets » Amazon » Data exchange » Reports*.
. Click on the *FBA reports* tab.
. Select an *Amazon account* from the drop-down list.
. Select a starting date. +
*_Tip:_* The start date must not be more than one year ago and must be before the end date.
. Select an ending date. +
*_Tip:_* The end date must not be in future. Also, start date and end date must not be more than one week in apart. +
*_Note:_* The FBA reports contain orders from 00:00 AM on the start date to 00:00 AM on the end date. As such, the reports do not contain any orders of the selected end date. The time zone of the plentymarkets system is applied.
. Click on *Generate report* (icon:execute[set=plenty]). +
→ The FBA report is generated. +
→ Missing, deleted or incorrect FBA orders are imported into plentymarkets.
