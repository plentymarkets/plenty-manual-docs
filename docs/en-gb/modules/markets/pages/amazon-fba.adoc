...
.FBA order workflow
--
include::amazon-fulfilment.adoc[tag=amazon-fba-workflow]
--
...
* <<orders/accounting#_creating_a_new_location, How to set up a new location>>
* <<orders/accounting#550, How to enter VAT rates>>
...
| *Status*
| Select the <<orders/managing-orders#1200, status>> that is to be assigned to orders that are imported from Amazon.
...
In the menu *Stock » Amazon FBA Inbound* you can print a <<orders/order-documents/pick-list#, pick list>> for the FBA Inbound order.
...
A <<orders/package-content-list#, packet content list>> lists the content of pallets, packages and items. The package content list informs Amazon about what items are in the packages that it stores and ships for you.
...
. Click on the order. +
→ The options for editing the order are shown.
. Click on *Package content list*. +
→ The order is shown in the *Orders » Packet content list*.
. Pick and pack the order based on the <<orders/package-content-list#, package content list>>.
