...
.What is a listing?
--
Use listings to sell your items on the markets eBay, Hood and ricardo. In the listing, you carry out settings that apply to all listings regardless of the market. For example, these are settings for stock dependence and listing contents. Here, you can also define the listing type.

The following listing types are available:

* *Fixed price*: Listing offered at a fixed price and not up for auction.
* *Auction*: Auktionen are possible on the marketplaces eBay, Hood and ricardo. *Auction* type listings start at a predefined price, then customers bid on the item.
* *Store item*: Listing type for the Hood marketplace. The item is offered in the Hood store.

Each listing may be associated with multiple market listings.
--

[.collapseBox]
.What is a market listing?
--
An offer on the marketplaces eBay, Hood and ricardo. A market listing is always associated with exactly one listing. Use the market listing for example for carrying out settings specific to a single marketplace.
--

[.collapseBox]
.What is a listing template?
--
A template for listings. You can create a template using either the group function in the *Listings » Edit listings » Tab: Group function* menu or REST. A listing template contains settings for listings, market listings and actions for listings, as well as specific options for eBay, Hood and ricardo. You can use the same template for any number of listings.

Listing templates can be applied to a listing when you create a new listing. Apply listing templates either in the menu *Open item » New Listing* in the window *New market listing* with the option *Listing template* or in the tab *Multi-Channel* of an item with the option *Listing template*. You can also select existing listing templates. Do this in the listing group function of the menu *Listings » Edit listings » Group function* in the drop-down list *No template* or in the item group function of the menu *Item » Edit item » Item group function* using the option *Listing template*.
--
...
Unknown order items are imported with the item ID *-2*. Unknown means: No related item for the order item is found in plentymarkets. Possible reasons are:
...
| *_Note:_* This setting only applies to the market eBay. +
...
a| *_Note:_* This setting only applies to the market eBay. +
...
*_Tip:_* You also have the possibility to manually import open orders.
|===
...
Make sure that the images used are available via URL. To do so, upload the images on the server for example using xref:data:ftp-access.adoc#[FTP].
|===
...
image::markets:EN-Settings-Listings-Layout-Templates-01.png[]

__Image 1: Linking listings to different layout templates__

* Example code for the tab link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/markets/ebay/assets/ebay-Main.html[Mainframe^]

* Example code for the tab link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/markets/ebay/assets/ebay-html.html[HTML structure^]

* Example code for the tab link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/markets/ebay/assets/ebay-C.html[CSS^]
...
TIP: Link more than one listing in a listing template.
//end::link-layout[]
...
ifdef::ebay[]
* using the data format xref:data:itemlistingmarketebay.adoc#[ItemListingMarketEbay] to automatically create large quantities of items
endif::ebay[]
...
ifdef::ebay[]
The settings that have to be set in order to create a listing are listed on this xref:markets:best-practices-ebay-activate-listings.adoc#[best practice] page.
endif::ebay[]
...
For eBay, the listing types *Auction* and *Fixed price* are available.

| *Number per lot*
| Define the quantity of items offered in a single listing. The listing price applies to the entire quantity.

| *Stock dependence*
a| Select the xref:stock-management:managing-stocks.adoc#[stock dependence] of the listing. Select one of the following options: +
...
For further information about the settings of a market listing, refer to the <<{listings-edit}, Editing listings>> chapter.
...
====
You can define a listing template in the *Listing default settings* area of the *Multi-Channel* tab of an item. A listing is created for an *item* using the group function or batch processing if no *listing template* was defined in the item’s settings.
====
...
| *Special sale*
| Special sales set up for the listing are displayed here. Create new link:https://www.ebay.co.uk/help/selling/selling-tools/selling-tools-overview?id=4073[special sales^] directly on eBay. Special sales will be available in plentymarkets immediately after they have been created and are automatically synchronised once an hour.
|===
//end::edit-listings[]
...
. Carry out the desired settings. Pay attention to the explanations given in <<#editing-market-listing>>, <<#base-tab-marketlisting>>, <<#description-layout-market-listing>>, and <<#tab-marketplace>>.
. Save (terra:save[]) the settings.

image::markets:toolbar-market-listing.png[]
...
| *Listing scheduler*
| Select a listing scheduler for the market listing. Select a <<#{listings-sales-planner}, listing scheduler>> in the *Activate listing scheduler* window.
...
| *Check*
| Check whether the market listing is ready to be uploaded to the market.
...
| Should the unit price be displayed? If so, where? For listings of the *Fixed price* type or *Buy it now* listings with *Weight* or *Volume* options, the link:https://www.ebay.co.uk/help/selling/selling/pricing-items?id=4133[unit price^] must be entered. Values for *units* which are saved for an item are automatically transferred.

| *Order status*
| Which status is to be assigned to the sold listing during import?

| *Warehouse*
| Select a warehouse or *use the basic setting* that was selected for the *Warehouse selection* option.
...
Uploading listings with variations is only possible on certain platforms, for example Germany, USA, United Kingdom and Italy. +
Additionally, the function is restricted to certain categories (for example clothes). For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/creating-listings-variations?id=4150[Creating listings with variations^] page.
...
//tag::edit-market-settings-ebay[]

In the *Market* tab you can change the settings for the market {market}.
...
Best offers are not available in all categories. Best offers are unavailable for offers with variations. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/selling-buy-now/adding-best-offer-listing?id=4144[Adding Best Offer to your listing^] page.
...
For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/listing-tips/promoted-listings?id=4164[Promote your listings^] page.
...
| *Properties*
| Edit properties. All properties for the selected category are displayed in the *eBay properties* window. Click on *Add* to create your own property. Enter a name for the property into the left-hand text field. Enter a value for the property of no more than 128 characters into the right-hand text field. The properties saved here take priority over an item’s properties when transferring to eBay. You will receive the error message *Code: 046 In this category, item properties are deactivated by eBay* if there are faulty entries or if no item properties are available. For further information about characteristics, refer to the xref:markets:ebay-setup.adoc#6000[Uploading offers with variations] chapter and eBay’s link:https://www.ebay.co.uk/help/listings/creating-managing-listings/creating-managing-listings?id=6000[Create a listing^] page.
|===
//end::edit-market-settings-ebay[]

[#tab-market-hood]
==== Changing the market settings

//tag::edit-market-settings-hood[]
In the *Market* tab you can change the settings for the market {market}.
...
| *Payment conditions*
| Select one or more payment conditions which you want to offer for the listing.

| *Additional options*
| *_Possible options:_* *Subtitle*, +
*Bold for item lists*, +
*Background colour for item lists*, +
*Gallery premium in item lists*, +
*Best offer in category and search*, +
*Best offer on homepage*, +
*Best offer on homepage plus image*, +
*Do not show adverts*, +
*XXL photo*,

| *Display on store homepage*
| *_Possible options:_* *Do not display on store homepage*, +
*Display on store homepage*, +
*Display on store homepage with picture*
|===
//end::edit-market-settings-hood[]

[#tab-market-ricardo]
==== Changing the market settings

//tag::edit-market-settings-ricardo[]
In the *Market* tab you can change the settings for the market {market}.
...
| *Payment conditions*
| Select the payment conditions for the listing.

| *Payment methods*
| Select one or more payment methods which you want to allow for this listing.

| *Delivery costs*
| Enter the delivery costs in the format `0,00`.

| *Individual shipping costs for each item*
| Place a check mark if shipping costs have to be paid separately for each item.

| *Delivery conditions*
| Select the delivery conditions for the listing.

| *Package size*
| Select the package size.

| *Availability*
| Select the availability of the listing.

| *Additional options*
| *_Possible options:_* *Promo-package small* +
*Promo-package middle*, +
*Promo-package big*, +
*Homepage*

| *Increment*
| Enter the icrement in the format `0,00`. +
The increment defines in which steps customers have to bid on the listing in auctions.

| *Number of reactivations*
| Select how many times the listing should be restarted after it has expired.

| *Reactivate until sold out*
| Place a check mark if the listing should be restarted until it is sold out.

| *Use secondary language*
| Place a check mark if a secondary language should be used.

| *Link to template*
| Select a template that you want to use for this listing.
|===
//end::edit-market-settings-ricardo[]
...
| *Stock dependence*
|Select the xref:stock-management:managing-stocks.adoc#[stock dependence] of the listing.
...
2+^| eBay +
*_Note:_* These settings only apply to the market eBay.
...
For further information about the settings of a listing, refer to the <<#{listings-edit}, Editing listings>> chapter.
...
. Click on *Group function*. +
→ The *Group function* window opens.
. Carry out the settings. Pay attention to the explanations given in <<#group-function-batch-processing>> in the <<#{listings-edit}, Editing listings>> chapter.
...
. Carry out the settings according to <<procedures-group-function>>.
