...
ifdef::voelkner[]
| Data fields for discount prices: +
*Discount start time* +
*Discount end time*
| You can export promotion prices that are valid for a specific period. Graduated prices can also be discounted. To set up the discount, create special prices with the same values for the setting *Minimum quantity* as for the graduated prices for {market}.

*_Preparations:_*

* Create a property of the type *Date* for the start date. For example, name this property `Voelkner discount start time`.
* Create a property of the type *Date* for the end date. For example, name this property `Voelkner discount end time`.
* In this step of the assistant, map the following data fields:
  ** Marketplace data field *Discount start time* = plentymarkets property *Voelkner discount start time* of the type *Date*
  ** Marketplace data field *Discount end time* = plentymarkets property *Voelkner discount end time* of the type *Date*

*_Create a special price for a sales price:_*
...
*_Create special price for graduated prices:_*
...
* For the special prices, enter the same values for the setting *Minimum quantity* as for the sales prices used for the graduation.

[.instruction]
Defining discount prices for a variation:

. Link the sales price of the type *Special offer* to the variation.
. Enter a price for the special sales price.
. *_Optional:_* To reduce graduated prices, also link the other *special prices* for graduated prices with the variation and enter a price.
. Link the variation with the property *Voelkner discount start date*.
. Select a date for the property. The discount price is started on this date.
. Link the variation with the property *Voelkner discount end date*.
. Select a date for the property. The discount price is ended on this date.
...
You can find this setting in the menu xref:item:managing-items.adoc#200, Item » Edit item » [Open variation] » Tab: Settings » Area: Availability » Drop-down list: Limitation]. This setting is valid for all sales channels, not just for {market}.
