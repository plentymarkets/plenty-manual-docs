...
| For each shipping profile in plentymarkets, enter the corresponding ID of the shipping service provider. You can find the ID in the Mirakl menu *{mirakl-menu-shipping}*.
...
[cols="1a,7a"]
!===
! icon:toggle_off[set=material]
! The net stock is exported for all variations.

! icon:toggle_on[set=material]
! The quantity you enter in the field *Quantity for items not limited to stock* is exported for variations without stock limitation.

  . Activate the option.
  . In the field *Quantity for items not limited to stock*, enter a number. +
  → This number is exported as the stock for variations without stock limitation. +
  *_Important:_* If you do not enter a number or the number "0", the net stock is exported for all variations.
!===

*_What are variations without stock limitation?_*

Variations without stock limitation are variations for which one of the following options is selected for the setting *Limitation*:
