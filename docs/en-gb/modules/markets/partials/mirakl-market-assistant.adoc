...
You can find this setting in the menu xref:item:managing-items.adoc.html#intable-limitation[Item » Edit item » [Open variation\] » Tab: Settings » Area: Availability » Drop-down list: Limitation]. This setting is valid for all sales channels, not just for {market}.
