...
The properties for *Leadtime to ship* and *Reverse charge* selected by default.

[cols="1a,3a"]
!===
! *Reverse charge*
! The _reverse charge procedure_. Valid values are `true` or `false`.

! *Leadtime to ship*
! The _lead time to ship_. This is the time in days that a seller needs from accepting the order to registering the shipping. A whole number between 1 and 45 has to be transferred.
