...
| *_Zalando Fulfillment Solutions is not yet available. Therefore, do not activate the option._*
...
[.instruction]
_Option 1:_ Only specify a default warehouse:
...
[.instruction]
_Option 2:_ Specify a default warehouse and additional warehouses for each Zalando channel:
