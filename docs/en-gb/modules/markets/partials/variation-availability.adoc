...
ifdef::limango[During the first export, variations with the stock 0 will also be exported to limango. Therefore you should only activate the variation availabilities for those variations that you actually want to export.]
...
→ The *Settings* tab of the variation is shown. +
*_Tip:_* To sell all variations of an item on {market-availability}, carry out the settings for the item’s main variation. This activates the availability of all variations of the item if the xref:item:inheritance.adoc#[inheritance function] is active for the variations.
