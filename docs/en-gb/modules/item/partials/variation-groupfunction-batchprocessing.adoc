...
*_Background info_*: plentymarkets can automatically remove variations from the plentyShop if they are no longer in stock. This automatic process is controlled by the settings in xref:item:managing-items.adoc#360[Tab: Settings » Area: Availability].
...
| **Generate GTIN**
|Activate and select the GTIN type. This will xref:item:managing-items.adoc#600[generate GTINs of this type from the number range] for all of the selected variations.
