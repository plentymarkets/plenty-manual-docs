...
* *Item* (icon:document[set=plenty, role="darkGrey"]) = Product groups that help you structure your merchandise clearly. xref:item:categories.adoc#300[New categories] are given the type *Item* by default. +
...
* *Blog* (icon:document[set=plenty, role="yellow"]) = Do you write a xref:online-store:setting-up-ceres.adoc#970[blog for your plentyShop]? You can use blog categories to sort your entries into specific topics like “News” or “About us”.
endif::manual[]
...
|`item`
|Item.
Product groups that help you structure your merchandise clearly. xref:item:categories.adoc#300[New categories] are given the type *Item* by default. +
If you’ve xref:item:categories.adoc#1300[already linked a category to items] and you switch the type from *Item* to *Content*, then the item links will not automatically be deleted. However, the category will no longer be displayed in the item data record.
