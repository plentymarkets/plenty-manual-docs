...
. <<item/settings/properties#1200, Group the properties>> if needed.
...
//. <<item/frontend-item-search#, Create webshop filters>> if needed. For example, so that your customers can filter for Bluetooth compliant products in plentyShop.
...
. Create properties like Wi-Fi or Bluetooth and group them if needed.
...
//. Create webshop filters if needed. For example, so that your customers can filter for Bluetooth compliant products in plentyShop.
//You can do this with the <<data/importing-data/sync-types/elasticSync-facets#, import type Facet>>.
//See a <<data/importing-data/elasticsync-best-practices/facets#, practical example>> as well.
--
////
