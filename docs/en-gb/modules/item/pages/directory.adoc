...
* *Tie to item price* = Select this option (icon:check-square[role="blue"]) if you want to tie the listing's price to the price of the item in plentymarkets.
** Listing type fixed price: When the price changes in plentymarkets, it is not automatically changed on the market. You have to update the price manually or carry out a stock synchronisation. The price on the market can also change after the stock is synchronised if the exchange rate changed.
