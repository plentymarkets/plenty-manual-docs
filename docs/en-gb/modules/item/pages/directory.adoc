...
You’ll need to save settings in this tab if you’re preparing items to be sold on marketplaces like xref:markets:neckermann-at-setup.adoc#[Neckermann.at].
...
Keep in mind that the xref:item:inheritance.adoc#[inherit] its data.
...
Choose settings in this area to have the xref:item:combining-products.adoc#2000[calculated for item bundles].
...
====
Note that there is also the xref:item:managing-items.adoc#200[Tab: Settings » Area: Availability], which you’ll use to specify _when_ the variation should be visible or purchasable.
====
