...
+
--
Would you prefer to _import_ cross-selling links, rather than create them manually?
...
*_Example_*:
Imagine you don’t just want to link _one guitar_ to a pack of replacement strings, a song book and a guitar strap.
Rather, you have _50 additional guitars_ that should also have these cross-selling links.
...
|Select this option if you want to _link the other way around_.
...
Use _ShopBuilder_ and so-called _item lists_ to display cross-selling items in the plentyShop.
...
*_Note_*:
Click on Design (icon:author[set=plenty]) to configure the various colour schemes and fonts in advance.
