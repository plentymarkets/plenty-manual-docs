...
*_What about item bundles?_*: It’s also possible to search for item bundles or individual bundle components. Use the xref:item:search.adoc#intable-bundle[Bundle] filter to do so.
...
* *Yes* = The search results will only include items that are part of a listing or an auction, i.e. items with a xref:item:search.adoc#800[yellow listing symbol] (icon:listings_show[set=plenty, role="yellow"]).
* *No* = The search results will only include items that are _not_ part of a listing or an auction, i.e. items with a xref:item:search.adoc#800[grey listing symbol] (icon:listings_show[set=plenty]).
...
|icon:eye[role="blue"]
|Click on *Visible in online store* (icon:eye[role="blue"]) to see xref:item:online-store.adoc#[what the item looks like in your plentyShop]. If the symbol is crossed out, then the item is *Invisible in the online store* (icon:item_webshop_invisible[set=plenty, role="blue"]).
...
|Click on *New listing* (icon:listing_create[set=plenty, role="green"]) to sell this item in a xref:markets:ebay-setup.adoc#3400[listing or auction on eBay], xref:markets:hood.adoc#2300[Hood] or xref:markets:ricardo-ch.adoc#1900[ricardo].

|icon:order_note_create[set=plenty, role="green"]
|Click on *Add note* (icon:order_note_create[set=plenty, role="green"]) to write a xref:item:managing-items.adoc#2090[note for the variation].
|====
...
Items always include one or more variations. When you open an item, the first variation, i.e. the xref:item:structure.adoc#600[main variation], will automatically open as well. But how do you access an item’s additional sub-variations? Proceed as described below to search for an item’s variations.
