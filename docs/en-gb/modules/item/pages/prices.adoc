...
*_Background info_*: While configuring your plentyShop, you’ll decide which xref:online-store:setting-up-ceres.adoc#201[currencies] your store should be available in.
...
In most cases, you’ll create a _standard sales price_.
But in some cases, you might want to create an “RRP”, “set price” or “special offer” instead.
...
* If you only want to save prices _for a few items_, then it makes sense to xref:item:managing-items.adoc#240[manually link sales prices and enter monetary amounts].
* If you want to save prices _for lots of items_, then we would recommend xref:item:import.adoc#800[importing the sales prices and monetary amounts].
...
//This list only includes the properties that you’ve xref:item:properties.adoc#intable-pricecalc[Price calculation property].
...
include::page$directory.adoc[tag=apply-price-calculation]

--
Import::
+
--
If you want to save the price calculation _for lots of items_, then we would recommend linking the price calculation xref:data:ElasticSync.adoc#[via import].
...
| *Client (store)*
|This option _only_ relates to xref:item:manufacturers.adoc#300[manufacturer commissions]. Select a client (store) to be linked with the price calculation.

| *Referrer*
|This option _only_ relates to xref:item:manufacturers.adoc#300[manufacturer commissions]. Select a referrer to be linked with the price calculation.
|====
