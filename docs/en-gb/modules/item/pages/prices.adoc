...
Or you can sell products cheaper to B2B customers than to B2C customers. In plentymarkets, such conditions are called sales prices.
...
* Third, you can have plentymarkets xref:item:prices.adoc#1000[automatically calculate your monetary prices].
...
New plentymarkets systems come equipped with two default sales prices.
...
This name appears in a variety of different places within your plentymarkets back end. For example, you’ll choose the sales price by name when you xref:item:prices.adoc#900[link it to individual items].
...
*_Automatic conversion_*: Activate the option *Live conversion* (icon:check-square[role="blue"]) to have plentymarkets automatically convert your prices from the xref:payment:currencies.adoc#30[standard currency], e.g. EUR, into the other currencies, e.g. PLN, CHF and GBP. +
...
*_Background info_*: With plentymarkets, you can manage several online stores, i.e. xref:online-store:setting-up-clients.adoc#[several clients], with only one software. Consequently, it is possible to manage several different areas of business with one plentymarkets system.
...
*_Note_*: xref:crm:preparatory-settings.adoc#create-customer-class[Create the necessary customer classes] in advance, if they don’t already exist in your plentymarkets system.
...
Therefore, xref:markets:ebay-setup.adoc#[link your eBay or Amazon account to plentymarkets] in advance.

| *Amazon accounts*
|Tell plentymarkets which Amazon account the sales price should apply to.
Therefore, xref:markets:amazon-setup.adoc#[link your Amazon account to plentymarkets] in advance.
...
Therefore, xref:markets:ebay-setup.adoc#[link your eBay account to plentymarkets] in advance.
...
*_Background info_*: While configuring your plentyShop, you’ll decide which xref:online-store:setting-up-ceres.adoc#220[languages] and xref:online-store:setting-up-ceres.adoc#201[currencies] your store should be available in.
For example, let’s assume that customers can choose the currencies EUR, PLN, CHF and GBP from the header.
In this case, it would make sense to have plentymarkets automatically convert your prices from the xref:payment:currencies.adoc#30[standard currency], e.g. EUR, into the other currencies, e.g. PLN, CHF and GBP.
...
*_Background info_*: This setting is only visible if you use plentymarkets to manage multiple online stores, i.e. xref:online-store:setting-up-clients.adoc#[multiple clients].
...
* Rather than manually entering the monetary amounts, you might want plentymarkets to xref:item:prices.adoc#1000[automatically calculate them for you].
...
A new link:https://www.bmwk.de/Redaktion/DE/Downloads/P-R/novelle-der-preisangabenverordnung-pangv.pdf?__blob=publicationFile&v=4[regulation on specifying prices^] will go into effect on May 28th, 2022.
...
The lowest price can manually be entered into plentymarkets, imported or automatically calculated.

IMPORTANT: This value is displayed in the webshop starting from link:https://marketplace.plentymarkets.com/ceres_4697[plentyShop LTS version 5.0.51^].

[.instruction]
Manual entry:

. Open the menu *Item » Item UI » [Open variation] » Element: Sales prices*.
. In a line, click on icon:ellipsis-v[role="darkGrey"] and then on icon:sell[set=material, role="darkGrey"] *Lowest price »* icon:pencil[role="darkGrey"] *Activate manual entry*. +
...
. Open the menu *Item » Item UI » [Open variation] » Element: Sales prices*.
. In a line, click on icon:ellipsis-v[role="darkGrey"] and then on icon:sell[set=material, role="darkGrey"] *Lowest price »* icon:calculate_acquisition_pricepng[set=plenty, role="darkGrey"] *Activate automatic calculation*. +
→ The lowest price of the last 30 days is automatically entered into the column.
This value is calculated with the help of the xref:item:changes.adoc#[change history].
...
IMPORTANT: In order for the lowest price to automatically be calculated, the change history must have been activated _and_ the price must have been changed at least once since activating the change history.
This means that you might have to change the item price by a few cents in order for the automatic function to take effect.

TIP: The lowest price will automatically be calculated approximately 90 seconds after you have activated and saved the setting for the first time. Afterwards, it will automatically be calculated every night.
...
* xref:data:elasticSync-item.adoc#2330[Further information about the mapping fields].
...
Select the desired calculations (icon:check-square[role="blue"]) and click on icon:copy[set=plenty, role="darkGrey"].
...
. Click on icon:execute[set=plenty, role="darkGrey"] *Execute* to apply the settings to the variations.
...
. Click on icon:execute[set=plenty, role="darkGrey"] *Apply* to apply the settings to the variations.
