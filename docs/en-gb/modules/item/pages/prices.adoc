...
Thanks to plentysystems, you can be very flexible with your prices.

* First, you can xref:item:prices.adoc#100[define the conditions] under which your products are sold at a specific price.
For example, you can sell products for a lower price in your plentyShop than on Amazon.
Or you can sell products cheaper to B2B customers than to B2C customers. In plentysystems, such conditions are called sales prices.
* Second, you can save xref:item:prices.adoc#900[prices for each individual variation].
...
* Third, you can have plentysystems xref:item:prices.adoc#1000[automatically calculate your monetary prices].
...
Sales prices define the conditions under which a variation is sold at a specific price.
...
New systems come equipped with two default sales prices.
...
Sales prices are created and managed centrally in plentysystems.
You can either create new sales prices with the help of an assistant or you can manually create new sales prices in the setup menu.
...
+
--
Create any number of sales prices to map the entire range of your prices in plentysystems.
...
. Finally, xref:item:prices.adoc#900[link the sales prices] to individual variations.
...
Example: If you create two different sales prices that are both valid for B2B customers, Amazon UK and the country UK, then plentysystems will not know which sales price to use.
plentysystems will randomly choose one of the sales prices since both apply equally.
...
| *ID*
|As soon as you save the sales price, it automatically receives a consecutive and unique ID.
...
If you xref:item:prices.adoc#900[link multiple sales prices to a variation], then you’ll see these sales prices sorted by their position number in the item data record.

| *Internal name EN*
|Enter a name for the sales price.
This name appears in a variety of different places within your plentysystems back end. For example, you’ll choose the sales price by name when you xref:item:prices.adoc#900[link it to individual items].

| *External name EN*
|Enter an external name for the sales price.

*_Note_*: Currently, this name is not displayed in the plentyShop.
That means: There is not currently an option within the ShopBuilder widget, which allows you to display the external name in the plentyShop.
If you want to display the external name in the plentyShop, then you will need to customise the webshop's code.

| *Minimum quantity*
|Should the sales price only apply if a customer buys multiple copies of the item?
...
*_Tip_*: Add even more quantity-based sales prices to create a sliding scale of prices.

*_Additional setting_*:
Quantity-based discounts also need to be permitted for the customer class.
Go to *Setup » CRM » Classes » [Open customer class]*.
Make sure that the setting *Volume discount* was set to *include*.

| *Currency*
|Which currencies should the sales price apply to?
Select them from the list (icon:check-square[role="blue"]).

*_Background info_*: While configuring your plentyShop, you’ll decide which xref:online-store:setting-up-ceres.adoc#220[languages] and xref:online-store:setting-up-ceres.adoc#201[currencies] your store should be available in.
...
*_Automatic conversion_*: Activate the option *Live conversion* (icon:check-square[role="blue"]) to have plentysystems automatically convert your prices from the xref:payment:currencies.adoc#30[standard currency], e.g. EUR, into the other currencies, e.g. PLN, CHF and GBP. +
If you don’t activate this option, then your prices will all have the same number, but different currencies, e.g. 19 EUR and 19 PLN. Therefore, it usually makes sense to have plentysystems automatically convert the prices.

Make sure you’re happy with the xref:payment:currencies.adoc#20[conversion factors] saved in the menu *Setup » Orders » Payment » Currencies*.
...
*RRP* = A recommended retail price, which only serves to show the price difference between the RRP and a standard sales price.
Products cannot be sold for the recommended retail price.
In order for the RRP to be displayed in the plentyShop, the variation must be linked to the RRP and the standard price.
Furthermore, the RRP must be higher than the standard price.
...
*Set price* = xref:item:combining-products.adoc#3000[Sets] are a way to combine products and sell them as a unit. If you decide to combine products and sell them as a set, then you’ll also need to give the individual set components a set price.

| *Display*
|Should this sales price automatically be linked to new item data records?
If so, then activate the option *Always display for new items* (icon:check-square[role="blue"]).

If this setting is deactivated, then you will still be able to xref:item:prices.adoc#900[manually add the sales price to items].
...
*_Background info_*: With plentysystems, you can manage several online stores, i.e. xref:online-store:setting-up-clients.adoc#[several clients], with only one software. Consequently, it is possible to manage several different areas of business with one system.
That’s a great advantage. But it also means that you’ll always need to tell plentysystems which online store the sales price applies to.
...
*_Note_*: xref:crm:preparatory-settings.adoc#create-customer-class[Create the necessary customer classes] in advance, if they don’t already exist in your system.
...
*_Note_*: If you select eBay or Amazon, then the corresponding account settings will appear further below. Here you’ll tell plentysystems which eBay or Amazon account the sales price should apply to.
Therefore, xref:markets:ebay-setup.adoc#[link your eBay or Amazon account to plentysystems] in advance.

| *Amazon accounts*
|Tell plentysystems which Amazon account the sales price should apply to.
Therefore, xref:markets:amazon-setup.adoc#[link your Amazon account to plentysystems] in advance.

This setting is greyed out until you select an Amazon platform from the list above.

| *eBay accounts*
|Tell plentysystems which eBay account the sales price should apply to.
Therefore, xref:markets:ebay-setup.adoc#[link your eBay account to plentysystems] in advance.
...
. Click on *New configuration* (icon:plus[role="darkGrey"]).
...
| *Type*
|Which type of sales price do you want to configure?

In most cases, you’ll create a standard sales price.
But in some cases, you might want to create an “RRP”, “set price” or “special offer” instead.
If so, then activate this option (icon:toggle-on[role="blue"]).
...
*_Background info_*: While configuring your plentyShop, you’ll decide which xref:online-store:setting-up-ceres.adoc#220[languages] and xref:online-store:setting-up-ceres.adoc#201[currencies] your store should be available in.
For example, let’s assume that customers can choose the currencies EUR, PLN, CHF and GBP from the header.
In this case, it would make sense to have plentysystems automatically convert your prices from the xref:payment:currencies.adoc#30[standard currency], e.g. EUR, into the other currencies, e.g. PLN, CHF and GBP.
...
*_Note_*: If you choose eBay or Amazon, then an additional configuration step will appear. During this step, you’ll tell plentysystems which eBay or Amazon account the sales price should apply to.
Therefore, xref:markets:ebay-setup.adoc#[link your eBay or Amazon account to plentysystems] in advance.
...
*_Background info_*: This setting is only visible if you use plentysystems to manage multiple online stores, i.e. xref:online-store:setting-up-clients.adoc#[multiple clients].
Since it’s possible to implement several different areas of business with just one system, you always need to tell plentysystems which online store the sales price applies to.
|======
...
. Go to *Setup » Item » Sales prices*.
. xref:item:prices.adoc#600[Search for] and open the sales price.
...
As soon as you xref:item:prices.adoc#200[create a new sales price], it is automatically added to the overview in the menu *Setup » Item » Sales prices*.
...
| *Currency*
|Select a currency to search for sales prices for this currency.

| *Language*
|When you select a language, the sales price's internal and external names will be displayed in this language in the table on the right.
...
After xref:item:prices.adoc#200[creating a new sales price], you can translate the name of the sales price into the required languages.
...
Sales prices are linked to xref:item:structure.adoc#[individual variations], rather than to the item as a whole.
For example, if you sell a t-shirt in S, M, L and XL, then you can decide whether the XL variation should be a bit more expensive than the rest.

There are a few different ways to link sales prices to variations and enter the actual monetary prices:

* If you only want to save prices for a few items, then it makes sense to xref:item:managing-items.adoc#240[manually link sales prices and enter monetary amounts].
* If you want to save prices for lots of items, then we would recommend xref:item:import.adoc#800[importing the sales prices and monetary amounts].
Alternatively, you could also import this data with an xref:welcome:assistants.adoc#[assistant].
* Rather than manually entering the monetary amounts, you might want plentysystems to xref:item:prices.adoc#1000[automatically calculate them for you].
...
Yes.
This is done with the help of a so-called "price calculation".
xref:item:prices.adoc#1000[Further information].
...
[.collapseBox]
.*An item was sold for the wrong price. How can I check who changed the price and when?*
--

//tag::faq-change-history[]

This can be done with the xref:item:changes.adoc#[change history], assuming that it was already activated in your system for a fee.

. Open the menu *Data » Change history*.
. Set the search filters as follows:
...
[.collapseBox]
.*I am required to display the lowest price of the last 30 days. How can I do that?*
--
...
If these conditions are met, then the RRP will be visible in the plentyShop. It will be displayed in strikethrough text and positioned next to the regular sales price.
...
Let plentysystems calculate your prices for you.
...
With plentysystems, you decide how your prices should be calculated.
...
* xref:item:prices.adoc#1100[Create the necessary price calculations]
* xref:item:prices.adoc#2100[Apply the calculation to your products]
* xref:item:prices.adoc#2000[Check the error log and fix any problems]
...
| *Name*
|Enter a distinct name for the calculation.
The name is only used internally and is not visible to customers.
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Net purchase price]
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Moving average purchase price (net)]
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Net transportation costs]

| *VAT +
(standard client)*
|Corresponds to the percentage of VAT under: +
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: VAT]
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Customs]
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Operating costs]
...
xref:item:managing-items.adoc#280[Tab: Settings » Area: Costs » Option: Net storage costs]

| *Extra shipping charge 1*; +
*Extra shipping charge 2*
a|Corresponds to the extra shipping charge under: +
xref:item:managing-items.adoc#290[Tab: Settings » Area: Shipping » Option: Extra shipping charge 1 and 2]
...
The manual also includes several xref:fulfilment:preparing-the-shipment.adoc#1100[example calculations for extra shipping charges].
...
This list only includes the sales prices that you’ve xref:item:prices.adoc#200[already created in your system].
...
This list only includes the properties that you’ve xref:item:properties.adoc#500[already created in your system].
//This list only includes the properties that you’ve xref:item:properties.adoc#500[already created in your system] and that have a check mark next to the option xref:item:properties.adoc#intable-pricecalc[Price calculation property].

*_Have you also saved a value for the property?_* +
Make sure that you’ve xref:item:properties.adoc#1400[linked] the property to the corresponding variations and that you’ve saved a whole number or a decimal number as the value.
...
Then you could xref:item:prices.adoc#1500[add other calculations] to further modify the formula.
...
Each price calculation is made up of one or more calculation building blocks.
Within each building block, you’ll:

* xref:item:prices.adoc#1300[create a formula]
* xref:item:prices.adoc#1400[select target prices and rounding behaviour]

image::item:further-calculation.gif[]
...
They turn into xref:item:prices.adoc#1350[variables], which you can select in the calculator.
====
...
|Opens the screen for creating a xref:item:prices.adoc#1100[new price calculation].
...
Select the desired calculations (icon:check-square[role="blue"]) and click on terra:copy[role="darkGrey"].
...
*_Note_*:
Links to variations will not be copied.
...
*_Note_*: A price calculation can only be deleted if it is not linked to a variation.
|====
...
| *Active*
|xref:item:prices.adoc#1200[Active price calculations] are marked with a blue dot (icon:circle[role="blue"]).
...
| *Name*
|The xref:item:prices.adoc#1200[name] that was entered while creating the price calculation.
In the overview, the price calculations are sorted alphabetically by their names.

| *Calculated sales prices*
|The xref:item:prices.adoc#1400[target prices] that were chosen while creating the price calculation.
...
xref:item:prices.adoc#2000[Open the error log] to see what’s causing the problem.
|====
...
+
--
If you only want to save the price calculation for a few items, then it makes sense to manually link the price calculation.

include::page$import-export-create-directory.adoc[tag=apply-price-calculation]

--
Import::
+
--
If you want to save the price calculation for lots of items, then we would recommend linking the price calculation xref:data:ElasticSync.adoc#[via import].
Use the xref:data:elasticSync-item.adoc#[import type Item] and the mapping field *Item variations / Price calculation UUID* to do so.
...
+
--
If you want to save the price calculation for lots of items, then we would recommend linking the price calculation via group function or batch processing.
...
. On the right, activate the option *Price calculation* (icon:check-square[role="blue"]) and select an existing calculation from the drop-down list.
...
* In the item data record, you can decide whether you want gross prices or net prices to be displayed.
However, this only affects how the prices are displayed.
* Only gross prices are saved internally in the database.
So if you save a net price, then it will be converted internally into a gross price and saved as such in the database.
====
...
xref:item:prices.adoc#1350[The variables] that can be added to a formula are constantly being monitored for changes.
...
xref:item:prices.adoc#2000[Open the error log] to see if any values are missing for your products.
...
plentysystems prevents prices from accidentally being saved with the value 0.00.
Both before and after rounding, the system checks whether the calculated price is 0.
If this is the case, then the price will not be saved and the xref:item:prices.adoc#2000[error log] will include information about the problem.
...
| *Client (store)*
|This option only relates to xref:item:manufacturers.adoc#300[manufacturer commissions]. Select a client (store) to be linked with the price calculation.

| *Referrer*
|This option only relates to xref:item:manufacturers.adoc#300[manufacturer commissions]. Select a referrer to be linked with the price calculation.
|====
