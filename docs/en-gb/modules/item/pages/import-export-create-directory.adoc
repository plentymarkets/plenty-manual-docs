...
. *Save* (icon:save[set=plenty, role="green"]) the changes.
...
. Drag & drop images into the appropriate area or click on icon:upload[set=plenty, role=purple] *Select files* and upload the images.
...
. Drag & drop files into the appropriate area or click on icon:upload[set=plenty, role=purple] *Select files* and upload the files.
...
In the tab *Statistics*, you can analyse xref:business-decisions:statistics.adoc#90[statistical data about the item].
...
Keep in mind that the xref:item:structure.adoc#600[main variation] can pass data onto its sub-variations, i.e. they can xref:item:inheritance.adoc#[inherit] its data.
...
| *Min. order quantity*
|
include::partial$min-order-quantity.adoc[]
...
To do so, proceed as described above. Rather than manually entering the barcode into the *Code* field, click on *Generate GTIN* (icon:execute[set=plenty, role="darkGrey"]).
====
...
| icon:calculate_acquisition_pricepng[set=plenty, role="darkGrey"] *Acquisition price*
...
. Click on *Deactivate inheritance* (icon:item_variation_inheritance[set=plenty, role="darkGrey"]).
...
. Drag & drop images into the appropriate area or click on icon:upload[set=plenty, role=purple] *Upload files* and upload the images. +
...
* icon:order_note[set=plenty, role="blue"] = The variation has a note.
Click on this symbol to open a pop-up window where you can directly view and manage your notes.
* icon:order_note_create[set=plenty, role="green"] = The variation does not have a note yet.
