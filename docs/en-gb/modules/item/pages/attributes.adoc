...
. Enter an internal name.
Pay attention to upper and lower-case letters, since plentymarkets understands e.g. "Yellow" and "yellow" as two different attribute values.
. Click on terra:save[role="green"] *Save*. +
...
| *Name (internal)*
|Unique name of the attribute value. The internal name is only used in the plentymarkets back end and is not visible in the plentyShop.
