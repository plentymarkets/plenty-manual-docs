...
* <<item/managing-items#94, Link your variations to the appropriate images>>.
* <<item/managing-items#intable-availability, Make sure that you’ve made these images available for the marketplace>>.
* <<item/managing-items#intable-market-attribute-link, Link the variation images to the market attributes>>.

These steps are described in further detail <<item/settings/attributes#900, in another chapter>>.
...
image::item/settings/assets/attribute-selection-plentyshop.png[]
...
Upload an image here (icon:upload[set=plenty, role="purple"]) if you want to visually display the attribute value.
...
These steps are described in further detail <<item/settings/attributes#700, in another chapter>>.
...
image::item/settings/assets/attributevalues-names-translation.png[]

//end::steps-attribute-value-name-translations[]

| *Name (internal)*
|Unique name of the attribute value. The internal name is only used in the plentymarkets back end and is not visible in the plentyShop. The internal name can only be assigned once per attribute. The internal name must not contain the following characters, otherwise errors may occur while exchanging data: +
...
plentymarkets has an <<data/importing-data/ElasticSync#, import tool>>, which you can use to import attributes.
The idea is simple.
Rather than manually saving attribute data in plentymarkets, you’ll enter the same information into a CSV file and then import this file into your system.

. Create attributes (like colour and size) and attribute values (like blue, red, green and S, M, L).
You can do this with the import type <<data/importing-data/sync-types/elasticSync-attributes#, Attribute>>.

. Use the attributes to generate variations.
You can do this with the import type <<data/importing-data/sync-types/elasticSync-item#2380, Item>>.
See a <<item/import-export-create/import#, practical example including video>> to learn how variations are generated.

[TIP]
.Attributes can also be exported
====
* Use the format type <<data/exporting-data/FormatDesigner/format-types/attributes#, Attribute>> to export information about the attributes and attribute values.
* Use the format type <<data/exporting-data/FormatDesigner/format-types/item#900, Item / VariationAttributeValues>> to see which attribute values are linked with a specific variation.
====
...
image::item/settings/assets/attribute-variants-images-plentyshop.png[]
...
image::item/settings/assets/attribute-selection-via-image.png[]
...
* Have you already <<item/managing-items#94, linked your variations to the corresponding images>>?
* Have you <<item/managing-items#intable-availability, made these images available for the marketplace>>?
* Have you <<item/settings/attributes#intable-allow-image-link, allowed image links in the basic settings>>?
...
* Characteristics and properties define products _without_ simultaneously creating saleable variations:
...
<<item/settings/attributes#1100, Further information and step-by-step instructions>>.
...
<<data/backup#_importing_a_backup, Further information and step-by-step instructions>>.
