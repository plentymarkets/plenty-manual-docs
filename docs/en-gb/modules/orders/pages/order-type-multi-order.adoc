...
You can combine unpaid orders in a multi-order. You can search for such unpaid orders either in the *Orders » Orders (Test phase)* menu or in the *Orders* tab of a contact data record. To do so, use the search filter *Order status* and for example the status *[3] Waiting for payment*. Therefore, you have two options for creating multi-orders. Both are described below.
With both options, unpaid orders are combined in a multi-order in the menu *Orders » Multi-orders*. Multi-invoices are then created in the *Documents* area of a multi-order.
...
For these cases, go to *CRM » Contacts* to create a multi-order in the contact data record. Open the contact and click on *New multi-order* in the menu on the left. A multi-order including all unpaid orders of the contact is then automatically created.

[TIP]
.Checking orders
======
In order to make sure that appropriate orders for creating a multi-order are available, use the *Orders* tab in the contact data record to look for unpaid orders.
======
...
. Open the multi-order for which you want to generate a multi-invoice.
...
. Go to *Orders » Orders (Test phase)*. +
. Carry out the search (material:search[]) to display orders.
. Open the multi credit note for which you want to generate the document Multi credit note.
. Go to the *Receipts* tab.
. On the right, click on *Create document* (material:note_add[]).
