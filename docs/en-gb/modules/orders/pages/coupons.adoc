...
| *Use*
| Select *single orders* or xref:orders:subscription-new.adoc#[subscriptions] or both options.
...
Each coupon that is sold is displayed as an individual variation on the invoice. To display the sum of redeemed vouchers on invoices, a name has to be entered in the *Coupon amount* field in the layout of the xref:orders:invoice-new.adoc#[invoice].
