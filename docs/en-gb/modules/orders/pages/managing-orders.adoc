...
Depending on the xref:orders:managing-orders.adoc#intable-shopping-cart-type[shopping cart type] you selected in the default settings, a sideward overlay opens or the view jumps directly to the shopping cart table. Irrespective of the shopping cart type, you can edit the *Quantity* as well as the *Sales price*. As soon as field entries are adapted, updates have to be carried out. You can do these either by clicking on *Refresh sales price* (icon:refresh[set=material]) directly next to the item. Or you use the button *Refresh sales prices* below the shopping cart. Only after updating the prices can you move on to the next step of the order creation.
...
If the values were already saved in the xref:crm:managing-contacts.adoc#300[customer data], they will be taken from there and automatically be displayed here. The customer data have priority. If nothing is saved in the customer data, the customer classes are checked. If values are defined in a xref:crm:managing-contacts.adoc#15[customer class], these are then transferred to the payment terms of the order. If no values are defined in either the customer data or the customer class, the payment terms fields of the order are left empty. +
...
The values entered on the corresponding order are transferred to the order documents. Note also the xref:orders:order-documents.adoc#intable-display-payment-terms[document settings] for this. The payment terms can be displayed on the order documents xref:orders:generating-invoices.adoc#[invoice], <<orders/order-documents/generating-pro-forma-invoice#, pro forma invoice><, xref:orders:generating-order-confirmations.adoc#[order confirmations] as well as xref:orders:generate-offer.adoc#[offer]. +
...
Check the xref:crm:sending-emails.adoc#100[access data] if the email was not sent correctly. Go to <<data/api-log#, Data » API log>> and use the *Email* process to see the corresponding error messages.
...
. Click on *Anwenden* (icon:execute[set=plenty]) above the group functions. +
...
| *Newsletter*
| By using this group function you can add a newsletter to an order. This newsletter is then sent to the customers of these orders. Make sure that you've set up a <</crm/sending-newsletters#, newsletter>> beforehand.
