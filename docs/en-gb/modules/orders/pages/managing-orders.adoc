...
. Click in the line of the contact on the left side on icon:more_vert[set=material]. +
. Click on icon:shopping_cart[set=material] *Orders* > *New order*. +
...
. Search for the contact by means of the filter settings. Note the explanations about the filters on the xref:crm:search-contact.adoc#[Searching for a contact page]. +
→ The contacts that match the search criteria are displayed.
. Click on icon:more_vert[set=material] in the line of the contact. +
→ The available options open.
. Select icon:shopping_cart[set=material] *Orders* > *New order [Beta]* or *New offer [Beta]*. +
