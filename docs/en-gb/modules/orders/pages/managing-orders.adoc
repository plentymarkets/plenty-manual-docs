...
Orders depict your sales and the business transactions connected to them. Those different business transactions are represented by order types. Progress during order processing is made visible by <<orders/managing-orders#1200, order statuses>>. The following descriptions will show you how to use order types and order status and thus process orders efficiently. All orders are registered in one central menu and in the same format. The <<orders/managing-orders#1400, order overview>> provides you with access to all important information and enables you to edit orders.

[#100]
== General information on creating orders in plentymarkets

In plentymarkets, an order is automatically created when customers place orders in your plentyShop or on one of your implemented markets. Separate orders with different order types are created for business transactions such as <<orders/managing-orders#300, delivery orders>>, <<orders/managing-orders#400, returns>>, <<orders/managing-orders#500, credit notes>>, <<orders/managing-orders#600, warranties>> and <<orders/managing-orders#700, repairs>>. These orders are always linked to the main orders. The <<orders/managing-orders#1400, order overview>> allows you to open and edit orders regardless of the <<orders/order-referrer#, referrer>>. For many order types, such as <<orders/managing-orders#300, delivery orders>>, it makes sense to set up <<automation/event-procedures#, event procedures>> that allow you to automate part of your order processing. You will find some examples for using event procedures during order processing in the following chapters. +
Orders that were not generated in the online shop or on a market can be created <<orders/managing-orders#200, manually>> in your system. This can be the required for orders via telephone, email or cash sales.

Before processing the first orders in your plentymarkets system, adjust the <<orders/basic-settings#, basic settings>> in the *Setup » Orders » Settings* menu. Save those settings once even if you have not changed the default values.
...
* in the <<orders/managing-orders#240, contact overview>>
* in the <<orders/managing-orders#210, customer account>>
* from an existing <<orders/managing-orders#220, order>>
* from a <<orders/managing-orders#230, cash sale>>
...
.Emailing
====
When you manually create orders, emails will not be sent automatically to the customer. The reason that no emails will be sent is that your system does not know when you are done with all settings for a manual order. To send emails for a manual order you need to open the <<orders/managing-orders#1550, Email>> tab and send the emails individually.
====
...
|If you select an <<orders/managing-orders#295, order template>> as the default order template, it will always be selected for new orders and offers.The values of the selected order template are then automatically applied to the new order or offer. Only already created order templates can be selected. +
...
|Activate this option for it to be preselected in the item search. It means that only <<item/online-store/checklist-items-visibility#100, activated items>> are displayed in the search results.
...
|By activating this option you permit the adaptation of VAT rates in the last step of order creation, before completing the order. Changes are then allowed in the <<orders/managing-orders#290, Overview>> step. The VAT rates displayed there are dependent on the delivery address of the order. +
...
. Search the contact by means of the filter settings as described in the <<crm/managing-contacts#200, Searching for a contact>> chapter. +
→ The contacts that match the search criteria are displayed.
. Click on *Actions* (icon:bars[]) in the row of the contact. +
→ The available options open.
. Select *New order [Beta]* or *New offer [Beta]*. +
→ You are redirected to the order details of the new order or offer. +

Use the drop-down list above the order details to choose between order and offer, even later. Moreover, you can optionally select an <<orders/managing-orders#295, order template>> which is then applied to this order. As a default, no order template is selected.
...
Because the new order or the new offer is created from the contact overview, some information are transferred from the contact data to the order details. Which information is transferred depends on the data saved in the contact. Otherwise the <<orders/managing-orders#250, default settings>> are preselected. Take <<table-order-details-new-order>> into account for this. All fields in the order details are mandatory fields.
...
Clicking on the the preview (icon:visibility[set=material]) redirects you to the contact data. The already carried out settings in the order details will remain.
...
|*Payment method*
|If a payment method is defined for the contact, for example in a <<crm/managing-contacts#15, customer class>>, this is preselected. Otherwise you can enter a payment method and select it. Available are all payment methods integrated in your plentymarkets system. +
If you use an <<orders/managing-orders#295, order template>>, the payment method selected in the template is taken over as soon as you select the template for an order. After applying the template, the payment method can be changed manually if required.
...
A discount is already filled in if the contact is assigned to a customer class in which a value has been entered for the option *Default discount for manual order*. You can find this option in the <<crm/managing-contacts#intable-customer-class-discount-type, Discount type>> area in the *Setup » CRM » Classes* menu. The prefilled value in the order details is editable.

|*Currency*
|The <<payment/currencies#30, system currency>> is preselected. You can change the currency for the order in the drop-down list.

|*Warehouse*
|Which options are available depends on the settings you set for <<orders/basic-settings#intable-warehouse-assignment, assignment of warehouses>>. +
...
|*Client*
|Select the client for this order from the drop-down list. Preselected is either the client set in the contact data or as a next option the <<orders/managing-orders#intable-default-client, default client>> if it is set.

|*Owner*
|Select the owner for this order from the drop-down list. If you set a <<orders/managing-orders#intable-default-owner, default owner>>, it is preselected. Otherwise, owners with the user rights to create orders can be selected from the drop-down list.

|*Referrer*
|Select the referrer for this order from the drop-down list. Preselected is either the referrer set in the contact data or as a next option the <<orders/managing-orders#intable-default-referrer, default referrer>> if it is set. Otherwise the activated <<orders/order-referrer#, referrers>> can be selected from the drop-down list.
...
. Select the option *(icon:add[set=material]) Add new delivery address* from the dropdown-list *Delivery address*. +
...
To complete an order, order items have to be added. Execute a search (icon:search[set=material]) to find and then add the required items. You can choose between different filters (icon:filter_alt[set=material]) for the item search:
...
Additionally, you can activate the options *Only display active items*, *Only display items in stock*, *Only display sales warehouses* and *Only display items with valid sales prices*. If you set *a) a warehouse can be assigned for each order* in the basic settings, the option *Only display sales warehouses* is not available for the search because you already determined the warehouse before. Moreover, the <<orders/managing-orders#250, default settings>> are applied if you set them. +
...
Carry out a search (icon:search[set=material]) to find the required items and to add them to the order.
...
. Move (icon:sort[set=material]) the columns, so that they are displayed in the order you need them.
...
|*Status*
|This column shows whether the variation is set as active or inactive in the online shop. Information about activating or deactivating a variation can be found on the manual page <<item/online-store/checklist-items-visibility#100, Have you activated the variation?>>. +
This is a default column.
...
If you change the quantity, the sales price changes as well and needs to be updated. To do so, click on *Refresh sales price* (icon:refresh[set=plenty]). After the update the shopping cart button (icon:add_shopping_cart[set=material]) is available again and the items can be added to the shopping cart. +
This is a default column.

|*Sales price*
|The sales price you selected from the <<orders/managing-orders#intable-item-search-price-selection, Price selection>> for this item is displayed. +
This is a default column.

|[#intable-item-search-price-selection]*Price selection*
| All valid sales prices that are determined for this order are available to you in a drop-down list. You can select one of the displayed sales prices, this is then taken as the base of the order calculation. If the price includes a rebate, this is shown behind the price name and amount. The columns *Customer class rebate* and *Category rebate* show the percentage of rebate applied. +
You can manually set a sales price in the <<orders/managing-orders#284, shopping cart>>. +
If you select a different sales price than the first determined one, the sales price needs to be updated. To do so, click on *Refresh sales price* (icon:refresh[set=material]). After the update the shopping cart button (icon:add_shopping_cart[set=material]) is available again and the items can be added to the shopping cart. This also applies to quantity changes. +
...
|*Availability*
|This columns displays the availability of a variation. Which symbol is displayed depends on your settings in the *Setup » Items » Availability* menu. For further information about these settings reger to the chapter <<item/settings/availabilities#, Availabilities>> of the manual. +
This is a default column.
...
|*Cart icon*
|The item is added to the shopping cart by clicking on this cart icon (icon:add_shopping_cart[set=material]). +
This is a default column.
...
. Search (icon:search[set=material]) for the required items.
...
→ The sales price has to be updated when quantities and prices are changed. Click on *Refersh sales price* (icon:refresh[set=material]) to do so.
. Add the items to the order by clicking on the shopping cart (icon:add_shopping_cart[set=material]) at the end of the row. +
→ When an item was successfully added, the shopping cart icon (icon:add_shopping_cart[set=material]) briefly changes size and colour.
...
The number of items added is displayed on the shopping cart icon (icon:shopping_cart[set=material]) which is located above the table. If you want to check or delete the added items, open this shopping cart (icon:shopping_cart[set=material]).

Depending on the <<orders/managing-orders#intable-shopping-cart-type, shopping cart type>> you selected in the default settings, a sideward overlay opens or the view jumps directly to the shopping cart table. Irrespective of the shopping cart type, you can edit the *Quantity* as well as the *Sales price*. As soon as field entries are adapted, updates (icon:refresh[set=plenty]) have to be carried out. You can do these either by clicking on *Refresh sales price* (icon:refresh[set=plenty]) directly next to the item. Or you use the button *Refresh sales prices* below the shopping cart. Only after updating the prices can you move on to the next step of the order creation.
...
Moreover, you can also delete items in the shopping cart. To do so, click on *Delete* (icon:delete[set=material]). A further editing option of the order items are the properties. To do so, take note of the following chapter <<orders/managing-orders#287, Order item properties>>.
...
. Click on *Configure columns* (icon:setting[set=material]). +
→ The window *Configure columns* opens.
. Select which columns you want to be displayed.
. Move (icon:sort[set=material]) the columns so they are displayed in the order you need them.
. Click on *Confirm* to save your settings.
...
If you want to create and edit properties or are not yet familiar with the structure of properties, refer to the manual chapter <<item/settings/properties#500, Properties>> first.

In order to link a property with an order item, the property has to be marked as *Order property* in the *Options* area of the property settings. Also set possible surcharges and VAT rates for the property in the <<item/settings/properties#property-options, Options>> area. The values added here will be included on the order calculations.

If a property is already linked with an order item, it will be displayed on the corresponding order item. How the order item properties are displayed depends on your <<orders/managing-orders#intable-shopping-cart-type, shopping cart type>> selection.
...
To edit the properties or to add more already existing properties, click on icon:edit[set=material] underneath *Order item properties*. The same editing window as from the shopping cart as table opens.
...
+
--
The properties are displayed in a second row of the table underneath the corresponding order item. There, you can find information about the *Name*, *Value*, *Surcharge* and *VAT %* of the property. You can edit the *VAT %*, i.e. the saved VAT rate of the property, directly in the table if the property allows for that. Moreover, you can delete (icon:delete[set=material]) order item properties in the table. Deleted properties are not included in this order and the order calculations. +
To edit the properties or to add more already existing properties, click on *Edit properties* (icon:edit[set=material]). The same editing window as from the shopping cart as table opens.
...
Moreover, some further editing options are available to you here. By clicking on *Delete* (icon:delete[set=material]), you remove the property from this order item. This property is then not included in the order and the order calculations. +
...
If you want to have the properties displayed on documents, you have to set this in the configurations of the property itself. Make sure that in the <<item/settings/properties#property-visibilities, Visibilities>> area of each property the option
...
Below the order details, a table with the added order items is displayed. In addition to the information given in the previous adding items view, more prices and the VAT rate are shown. The VAT rates are editable if you allowed this in the <<orders/managing-orders#250, default settings>>.  +
...
To complete the order, click on *Complete order*. The order is now created and you are redirected to the menu you chose in the <<orders/managing-orders#intable-endpoint-after-completion, default settings>>. The order is now also available in the order overview in the *Orders » Edit orders* menu.

[#295]
=== Creating and applying order templates

You can create order templates in the *Setup » Orders » Order template* menu. You can then select these when you manually create a new order or you can apply an order template with an <<automation/event-procedures#intable-order-template, event procedure>>. If you select an order template for an order, the order template data is applied to the order. After using an order template, you can edit an order as usual as well as change and adapt order information.
...
====
To use delivery orders, you have to select the option *b) a warehouse can be assigned for each stock unit* for the <<orders/basic-settings#intable-warehouse-assignment, Assignment of warehouses>> in the menu *Setup » Orders » Settings*.
====
...
To open main orders, variable users have to be <<business-decisions/user-accounts-access#100, authorised>> to view all warehouses linked to the delivery orders of the main order. Check the settings in the menu *Setup» Settings » User »Accounts » Tab: Rights* for this.
...
Before returns can be registered in your system, you have to set up <<orders/managing-orders#410, reasons for returns>> and <<orders/managing-orders#420, item statuses>> for returned items.

The default status for returns is status 9. Every registered return is automatically assigned this status. However, you can change the default status according to your requirements in the menu *Setup » Orders » Settings* by selecting another <<orders/basic-settings#intable-default-status-return, default status for returns>>.
...
| *Status*
|Select a status for the return. The <<orders/basic-settings#intable-default-status-return, default status for returns>> defined in the menu *Setup » Orders » Settings* is displayed here by default.
...
Before creating return slips, the document template <<orders/order-documents/generating-return-slips#, Return slip>> has to be set up in the menu *Setup » Client » [Select client] » Locations » [Select location] » Documents » Return slip*.
...
Before creating credit note orders, check the <<orders/basic-settings#intable-default-status-credit-note, default status for credit notes>> in the *Setup » Orders » Settings* menu. Change the default status when necessary and then save your settings.
...
====
Before generating credit notes, you have to set up the <<orders/order-documents/generating-credit-notes#, document template for credit notes>>. This is done in the *Setup » Client » [Select client] » Location » [Select location] » Documents » Credit note* menu.
====
...
Because the information in a reversal document is identical with the information included in an invoice, further information on the required settings can be found on the <<orders/order-documents/generating-invoices#, invoice>> manual page.
...
If the conditions for a warranty apply, you can decide whether to refund the customer’s costs or to exchange the damaged item with a new one. Create a *warranty* to replace the damaged item. For refunding the customer, create a refund in the *Overview* tab of the warranty. Determine in the basic settings *Setup » Orders » Settings* menu whether the <<orders/basic-settings#intable-shipping-costs-in-warranty, shipping costs should be covered in the warranty>>.
...
Since repairs constitute a separate order type, you can use the *Overview* tab to proceed with a credit note, a warranty or a return order. The necessary steps for each of these order types such as creating the necessary documents, booking outgoing or incoming items or booking payments are identical to the order types <<orders/managing-orders#500, credit note>>, <<orders/managing-orders#600, warranty>> and <<orders/managing-orders#400, return>>.
...
. Click on icon:execute[set=plenty]. +
...
. Create an <<crm/sending-emails#1200, Email template>>.
...
|Is automatically assigned in the following cases: +
...
|Is automatically assigned to a regular order if a partial payment or a down payment for an order is booked and the items are in stock.
...
| *[8]* Cancelled
|We recommend to use this status to indicate that a regular order was cancelled. Status 8 is the default status for cancelled orders. However, if required, you can adjust the default statuses in the *Setup » Orders » Settings* menu. Define a <<orders/basic-settings#intable-default-status-cancelled-orders, default status for cancelled orders>>. If you assign a different status to cancelled orders, you can re-assign status 8 as required.

| *[8.1]* Cancelled by customer
|We recommend to use this status to indicate that an order was cancelled by a customer.

| *[9]* Return
|Is automatically preselected when a return is created. You can also change this <<orders/basic-settings#intable-default-status-return, default status for returns>> in the *Setup » Orders » Settings* menu.
...
| *[11]* Credit note
|This status is automatically assigned to a credit note order, when a credit note is generated. You can change the <<orders/basic-settings#intable-default-status-credit-note, default status for credit notes>> if required. In order to do this, go to the menu *Setup » Orders » Settings*.

| *[11.1]* Credit note disbursed
|We recommend to use this status to indicate that a credit note was disbursed and is completed.

| *[12]* Repair
|Is automatically preselected when a repair is created. You can change the <<orders/basic-settings#intable-default-status-repairs, default status for repairs>>, if required. In order to do this, go to the menu *Setup » Orders » Settings*.

| *[13]* Multi-order
|Is automatically assigned to a multi-order when it is created. You can change the <<orders/basic-settings#intable-default-status-multi-order, default status for multi-orders>>, if required. In order to do this, go to the menu *Setup » Orders » Settings*.

| *[14]* Multi credit note
|Is automatically assigned to a multi credit note when it is created. You can change the <orders/basic-settings#intable-default-status-multi-credit-note, default status for multi credit notes>>, if required. In order to do this, go to the menu *Setup » Orders » Settings*.
|====

[#1220]
=== Creating and editing order statuses

In the *Setup » Orders » Status* menu, you can add statuses to the ones described in <<table-default-statuses>> . Already existing statuses can be edited or renamed. Maintain statuses in different languages in order to inform your customers about the progress via email or in the customer account of the online shop. Apart from statuses linked to automated processes within the system, all statuses can be deleted. Statuses linked to automated processes within the system can be renamed. However, you should make sure not to misuse those statuses linked to automated processes within the system. +
...
====
Entering a name is not mandatory. However, if you do not maintain names you will only see a list of apparently empty statuses in the filters and an empty status field in the order overview. Therefore, enter at least the status number also as name in the language you use the most often, German or English. Translations to other languages are only used for display in the customer account of your online shop and for customer information via email.
====
...
|This is an individual icon of the assigned provider. However, an icon is only displayed if you set up flags while creating the <<fulfilment/preparing-the-shipment#1000, shipping profile>>.
...
| *Dunning level*
|<<automation/procedure-manager#intable-dunning-level-procedure-manager, Dunning levels>> can be allocated with the procedure manager.
...
|The tracking URL is used to track a shipment. The <<fulfilment/preparing-the-shipment#840, tracking URL>> has to be entered and saved when setting up the shipment providers.
...
====
In order for variable users to be able to open orders, go to the menu *Setup » Orders » Settings* and save <<orders/basic-settings#intable-default-status-return, default statuses>> for *returns* and credit notes. No status is saved here as the default setting. Variable users also need to be given the <<business-decisions/user-accounts-access#105, right>> to edit orders. Go to *Setup » Settings » User » Accounts* in order to carry out the required settings.
====
...
Go to <<online-store/setting-up-clients#, Setup » Client » [Select client] » Order process » Settings>> to activate or deactivate the *Purchaser's IP address* option.
...
- The user is not <<business-decisions/user-accounts-access#105, authorised>> to delete orders. +
- After outgoing items are booked. +
- A delivery order was created. +
Generally, orders should not be deleted. Errors can occur when orders are deleted because the orders are linked to items, shipping settings and other functions such as cancellations and returns. However, it is possible to delete newly created orders.

| [#intable-splitting-orders]*Split up*
|Splits the order. The behaviour during splitting depends on your <<orders/basic-settings#intable-behaviour-after-splitting, basic setting>> for this. +
...
If you want a different <<orders/basic-settings#intable-splitting-behaviour-shipping-costs, behaviour of shipping costs>> than the standard described above, you can select a different setting in the basic settings for orders.
...
| *Bank details*
|Click on the *small plus* icon to display and edit the <<payment/managing-bank-details#63, customer’s bank details>>.
...
_Tip_: You can also use <<automation/event-procedures#, event procedures>> to assign storage locations to items.
...
The note *Missing owner right order* is displayed if the owner does not have the needed <<business-decisions/user-accounts-access#40, owner rights>>. Go to the menu *Setup » Settings » User » Account* and select that the module Order is available to this user as an owner. +
In case the owner of the order does not have a plentymarkets user account, the note *Unkown user* is displayed. If needed, you can create a <<business-decisions/user-accounts-access#10, user account>> for this user.
...
If the values were already saved in the <<crm/managing-contacts#300, customer data>>, they will be taken from there and automatically be displayed here. The customer data have priority. If nothing is saved in the customer data, the customer classes are checked. If values are defined in a <<crm/managing-contacts#15, customer class>>, these are then transferred to the payment terms of the order. If no values are defined in either the customer data or the customer class, the payment terms fields of the order are left empty. +
...
The values entered on the corresponding order are transferred to the order documents. Note also the <<orders/order-documents#intable-display-payment-terms, document settings>> for this. The payment terms can be displayed on the order documents <<orders/order-documents/generating-invoices#, invoice>>, <<orders/order-documents/generating-pro-forma-invoice#, pro forma invoice><, <<orders/order-documents/generating-order-confirmations#, order confirmations>> as well as <<orders/order-documents/generate-offer#, offer>>. +
*_Important_*: The payment terms will only be displayed on these documents if the corresponding order was paid with the payment method <<payment/payment-plugins/invoice#, Invoice>> or the payment method <<payment/payment-plugins/cash-in-advance#, Cash in advance>>.
...
If a package number was saved, then the *preview* (icon:eye[role="blue"]) can be used for tracking the package. However, this can only be done if a corresponding tracking URL was saved in the *Setup » Orders » Shipping » Shipping options » Tab: Shipping service provider* menu for this <<order-process/fulfillment/preparing-the-shipment#, shipping provider>>. Otherwise, the preview icon will not be visible.
|====

[#1550]
=== Tab: Email

The *Email* tab contains all of the <<crm/sending-emails#, email templates>> that were configured in the menu *Setup » Client » [Select client] » Email » Templates* and that were set up as a type of template that is visible for orders. You can manually send these email templates to customers. This is useful if you unexpectedly need to send an email to a customer, i.e. an email that is not sent with an automatic procedure. Proceed as described below to send an email template.
...
Check the <<crm/sending-emails#100, access data>> if the email was not sent correctly. Go to <<data/api-log#, Data » API log>> and use the *Email* process to see the corresponding error messages.
...
Find more detailed information about how to <<payment/beta-managing-payments#, manage payments>> in the manual.
...
The documents are created with the help of <<orders/order-documents#, document templates>>. Check the document templates to make sure that the settings are right before you create receipts. Check the templates also if you receive an error message.
...
You can increase the quantities of variations in reorders as long as the reorder is still open. This is possible for values where percentages were saved during the <<item/settings/attributes#200, distribution>>. Proceed as follows to increase the quantities of variations:
...
. Click on *Create delivery order* (icon:execute[set=plenty]) to create the delivery order.
...
Note that an invoice can only be reversed if no dunning letter exists for the corresponding order. In this case, the dunning letter has to be reversed first. To do this, use the <<orders/order-documents/generating-dunning-letters#400, reversal dunning letter>>.
...
Since the information in a reversal document is identical with the information included in an invoice, further information on the required settings can be found on the <<orders/order-documents/generating-invoices#, invoice>> manual page.
...
By using the group function *Group orders*, you can group certain orders. This means that the selected orders are combined into a new order. The behaviour for grouping depends on your <<orders/basic-settings#intable-behaviour-after-grouping, basic setting>> for this. The standard behaviour is that the original orders are deleted. Shipping costs are recalculated for the new order. Orders can be grouped when: +
- They are not locked, +
- outgoing items have not been booked yet, +
- no payments have been assigned to the order yet, +
...
. Click on *Apply* (icon:execute[set=plenty]) to carry out the procedure.

[TIP]
.Default settings for editing orders by variable users
====
In order for variable users to be able to open orders, go to the menu *Setup » Orders » Settings* and save <<orders/basic-settings#200, default statuses>> for *returns* and credit notes. There is no default status saved for *returns* and *credit notes*.
Variable users also need to be given the <<business-decisions/user-accounts-access#105, right to edit orders>>. Go to *Setup » Settings » User » Accounts* in order to carry out the required settings.
====
...
→ Make sure that you set up an <<crm/sending-emails#1200, email template>> first. That way you can now select it here for the group function.
. Click on *Apply* (icon:execute[set=plenty]) to carry out the procedure. +
...
If you could place a check mark next to these options, you should be able to edit the order items. In case you are still encountering problems, you can find more detailed information about sales prices in the manual under <<item/settings/prices#, Sales prices and price calculation>> or you can ask our supporters in the link:https://forum.plentymarkets.com/[Forum]{nbsp}icon:external-link[].
