...
Moreover, you can enter one or more coupon codes in the *Coupon codes* field to redeem these with this order. Enter a coupon code and press _enter_ or click outside the field. It is checked whether the coupon can be redeemed. Redeemable coupons are displayed in green with the coupon’s value. A tooltip shows which kind of coupon it is. Coupons that cannot be redeemed are displayed in red with the info *Not redeemable*. You can also enter several coupon codes. +
The values of the coupons are set off against the order value. Coupons are not added as an order position in the shopping cart. Coupons cannot be redeemed for offers.  This means that the input field *Coupon codes* is only available for the order creation, but not for creating offers.
