...
.Sending emails
====
When you manually create orders, no emails will automatically be sent to the customer. The reason that no emails will be sent is that your system does not know when you are done with all settings for a manual order. Send xref:crm:emailbuilder-overview.adoc#[emails] manually by using the *Email service* function in an order.
====
