...
But what is the difference between the two views? While the list view displays one order per line with the selected columns, the compact view also directly shows the selected order details in the overview in a space-saving way. Note however that orders can only be opened in the list view for further editing. +
