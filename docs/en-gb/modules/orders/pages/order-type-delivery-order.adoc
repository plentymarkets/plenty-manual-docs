...
Delivery orders can be created <<#manually-create-delivery-orders, manually>>, <<#automatically-create-delivery-orders, automatically>> or via an <<#create-delivery-orders-via-event-procedure, event procedure>>.
...
| *Shipping profile*
|The delivery orders are created according to the different *Shipping profiles* of the items included in the order.

| *Warehouse and shipping profile*
|The delivery orders are created according to the different *Warehouses and shipping profiles* of the items included in the order.

| *Warehouse stock and net stock*
|The delivery orders are created according to the different *Warehouse stock and net stock* of the items included in the order.
|===

[TIP]
.Defining an end point after completing an order
======
Go to *Setup » Orders » Order types » Sales order [Beta]* to determine the view that is shown after an order has been completed. Use the setting *Endpoint after completing an order* for this.
======
...
. Start the search (material:search[]) to display orders.
...
. Select the option *For specific items* to select items.
...
. Go to *Orders » Orders (Test phase)*.
. Start the search (material:search[]) to display orders.
. Open the order or which delivery orders should be created.
. At the top, click on *Create orders* (material:shopping_cart[]) > *Delivery order*.
. Select the option *Split automatically*. +
→ Based on the <<#basic-settings-delivery-order, basic settings>> that you carried out, the delivery orders are automatically splitted and created.
