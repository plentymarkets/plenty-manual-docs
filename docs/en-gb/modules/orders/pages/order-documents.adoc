...
* xref:orders:adjustment-form.adoc#[Adjustment forms]
...
* xref:orders:generating-invoices.adoc#600[Reversal invoices]
* xref:orders:order-type-credit-note.adoc#correct-and-cancel-credit-note-document[Reversal credit notes]
* xref:generating-pro-forma-invoice.adoc#[Pro format invoices]
...
Whether the system calculates prices with xref:orders:accounting.adoc#[2 or 4 decimal places] is set up in the *Setup » Client » [Select client] » Location » [Select location] » Accounting* menu.
...
|See *Tax note 1*. +
This note is displayed for B2B orders with the xref:orders:accounting.adoc#[Reverse charge procedure]. In Germany, an example of this tax note is: *Reverse charge procedure in accordance with article 194 of the VAT Directive .*
