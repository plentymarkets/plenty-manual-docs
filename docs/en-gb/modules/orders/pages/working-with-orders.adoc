...
Orders that were not generated in the online shop or on a market can be created xref:orders:managing-orders.adoc#[manually] in your system. This can be the required for orders via telephone, email or cash sales.

Before processing the first orders in your plentymarkets system, adjust the xref:orders:basic-settings.adoc#[preparatory settings] in the *Setup » Orders » Settings* menu. Save those settings once even if you have not changed the default values.
...
|Changes the flag for all selected orders. Select a flag from the drop-down list.

| *Merge documents*
|material:picture_as_pdf[]
|Merges the PDF documents of a selected document type of all selected orders into one document.
...
| *Cancel outgoing items*
|material:output[]
|Cancels the outgoing items. A notification asks you to confirm this action. You can then re-book the outgoing items whenever you wish.
...
| *Send email*
|material:mail[]
|Sends an email for the selected order. First, select a *Recipient* and a *Template* for the email. As soon as the email is sent, a success message is displayed. Afterwards, you can see the sent emails in the detail view of the email service (material:mail[]) and in the *Messenger* area. *_Note:_* In order to see the email in the *Messenger* area, you have to complete the *Messenger configuration* assistant and activate the function first.

| *Merge documents*
|material:picture_as_pdf[]
|Merges the PDF documents of a selected document type of the selected order into one document.
...
All rights that are necessary for each area, button and function in an order are described on the page xref:orders:basic-settings.adoc#[Carrying out the preparatory settings] in the chapter xref:orders:basic-settings.adoc#[Assigning rights].
====
...
|Shows the order's early payment discount.
...
| *Ebay account*
|Shows the order's eBay account.
...
| *Contact ID*
|Shows the contact ID of the order.
...
*_Note:_* You can add a maximum of 5 properties.
...
| *Currency*
|Shows the currency. +
If applicable, the exchange rate of the selected currency is also displayed.
...
|Shows the "Preferred storage location ID" property. +
This is mainly used for picking.
...
. Go to *Orders » Orders*.
. Carry out the search (material:search[]) to display orders.
. Open the order. Note that orders can only be opened in the list view.
...
| *Expand*
|Allows you to expand the field for the order item to display the item’s properties. +
...
| *Total weight*
|Shows the total weight of the order item +
This is a default column.
...
| *Referrer*
|Shows the referrer of the order item.

| *Profit margin (net)*
|Shows the net profit margin.

| *Profit magin (net) total*
...
| *Early payment discount (%)*
|Shows the early payment discount expressed as a percentage.

| *Value date (days)*
|Shows the value date in days.

| *Payment due (days)*
|Shows the payment due in days.

| *Early payment discount (days)*
|Shows the early payment discount in days.
|===
...
| *SEPA direct debit mandate granted at*
|Select the date the SEPA direct debit mandate was granted on.
...
| *Payment method ID (MOP ID)*
|Shows the ID of the payment method.

| *Payment method icon*
|Shows the icon of the payment method.

| *Payment method name*
|Shows the name of the payment method.

| *Action*
|Shows the action menu (material:more_vert[]) where you can detach the payment, if you wish.
...
| *Transaction code*
|Shows the code of the transaction.
...
A reference ID links payments, e.g. a payment and a refund.
...
The function for splitting orders can be found in the toolbar of an opened order in the *Orders » Orders* menu. The behaviour when splitting orders depends on your xref:orders:basic-settings.adoc#[basic settings] for this. +
...
If you want a different xref:orders:basic-settings.adoc#[behaviour of shipping costs] than the standard one described above, you can select a different setting in the basic settings for orders.
