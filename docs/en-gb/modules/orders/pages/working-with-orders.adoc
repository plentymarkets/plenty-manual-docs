...
. Select (material:done[]) all orders for which you want to carry out a certain action at once.
...
In the editing mode (terra:design_inline_edit[role="darkGrey"]) of the order overview you can determine
*Default group functions* that become directly visible when selecting orders. All remaining group functions will automatically be added to the context menu (material:more_vert[]).
...
In addresses, you can either change single fields of the current address or you can even assign a new address that already exists. Proceed as described below to change an address.
...
→ A window with the already existing addresses opens.
