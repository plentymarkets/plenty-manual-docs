...
| *Create document*
|material:note_add[]
|Creates a document for all selected orders. Select a document type from the drop-down list and enter a creation date for the document or select a date from the calendar (material:today[]).
...
In the *Messenger* area, you can see all messages that belong to the order. You can xref:crm:messenger-testphase.adoc#intable-whisper-mode[whisper] messages so that they are only internally visible and _not_ for your customers. You can xref:crm:messenger-testphase.adoc#create-message[send] messages and you can xref:crm:messenger-testphase.adoc#capture-minutes[capture minutes for the message] and save these minutes as free minutes or paid minutes. 
