...
* Set up xref:orders:accounting.adoc#550[following chapter] or you use the VAT rate assistant. +
...
Thus, as a first step, you have to maintain the taric codes in your system. Taric codes are entered in the xref:item:accounting.adoc#550[VAT rates] for the required countries. These VAT rates could also be entered later, but we recommend setting these in the system in time.
...
The xref:data:ElasticSync.adoc#[Using the import tool].
...
Orders generated in plentyPOS are assigned to the xref:orders:integrating-plentymarkets-pos.adoc#940[Integrating POS] page of the manual.
