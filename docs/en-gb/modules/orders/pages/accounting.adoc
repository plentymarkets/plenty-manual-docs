...
. Click on *Add new entry* (icon:add[]).
. Select a *Country*.
...
→ With the *Apply default VAT* (icon:magic-wand[]) function, you can instruct the system to apply the standard VAT rates of the respective country. +
...
*Note*: Use (icon:magic-wand[]) for the VAT rates to be automatically entered by the system. +
*_Important_*: +
...
. Go to *Setup » Client » [Select client] » Locations » [Select location] » Accounting*.
. Click on (icon:settings_b_roll[]) *Transfer settings to*.
