...
:description: Learn how to determine the basic settings for your orders and find out how to transfer properties of an order to another order via the property transfer.
...
Standard values are selected by default in new systems. For example, status [9] for returns and status [11] for credit notes. These settings can be changed by selecting a different status.  Alternatively, go to *Setup » Orders » Order status* and create a new xref:orders:managing-orders.adoc#1200[order status]. Statuses that are created there are displayed in the *Basic settings* menu and can be selected as standard statuses.
...
Moreover, define what users are allowed to see in the system. To do so, determine xref:business-decisions:user-accounts-access.adoc#100[visibilities], for example for warehouses and order statuses.
