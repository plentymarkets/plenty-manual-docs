...
====
Standard templates are configured to serve as a fallback if no other template of this document type exists, so they are activated by default and cannot be edited or deleted.
====
...
Two *image file formats* are available for the barcode widget: *PNG*, which is the default format, and *SVG*, which you can select from the drop-down list. 

*_Note:_* Switching image file format in an existing template can cause the size of the barcode to change in the template. Always check the barcode size in the template preview after making such a switch.
...
[NOTE]
.Deactivating/activating default templates
====
The *default* templates can only be deactivated/activated collectively using the toggle in the DocumentBuilder overview. + 
If you deactivate all default templates, they can no longer serve as fallbacks. 
