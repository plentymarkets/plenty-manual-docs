...
image::auftraege:menu-structure-orders-en.png[]
...
* *Toolbar*: Here you can xref:orders:working-with-orders.adoc#order-search[search for orders], xref:orders:working-with-orders.adoc#configure-columns[configure columns in the overview], set the desired amount of results per page and click through the pages with the orders. +
If you select several orders (material:check_box[role=skyBlue]), you can also use the xref:auftraege:working-with-orders.adoc#order-group-functions[group functions].
