...
plentysystems’ DocumentBuilder enables you to create document templates, tailor them to your needs, and manage them centrally. To access DocumentBuilder, open the *Settings » Documents » DocumentBuilder* menu. DocumentBuilder also contains preconfigured xref:orders:documentbuilder-using-default-templates.adoc[default templates] for certain document types.
...
* *Toolbar*: You can do the following in the toolbar: +
...
* *Main window*: This is where the area currently selected in the navigation is displayed.
...
|Indicates that the template’s header/footer is taken from another template and displays the name of the referrer template.
|===
...
*_Note:_* You can export templates, adjust them in external systems, then reimport them.
