...
*_Note:_* If the document type is not enabled in this way, the template will not be used, even if it is active in DocumentBuilder. 
====
...
[NOTE]
.The option to delete a template* is _not available_ for default templates or active templates
====
