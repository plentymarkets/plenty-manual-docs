...
* xref:business-decisions:gdpr.adoc#500[view documents that are relevant to GDPR]
...
| *Stock*
|Create an export format for your xref:data:stock.adoc#[stock data] in the *Data » Elastic export* menu. We recommend to select at least the following export fields:
...
| *Stock movements*
|Create an export format for your xref:data:stock-movement.adoc#[stock movement] in the *Data » Elastic export* menu. We recommend to select at least the following export fields:
...
Additionally, in the *Cleared postings* area, you can filter also for the clearing period via the fields *From* and *To* and for the order ID. Moreover, you can find the function *Download report* here. Use this function to download either a PDF or a CSV file. Activate (material:check_box[role=skyBlue]) the option *Also download message* if you want to include both the message's content and subject in the download file. Note that the export is limited to 100 rows. +
...
If you would prefer to only give the agency access to specific functions or menus, xref:business-decisions:user-accounts-access.adoc#[create a new user account], e.g. a back end account, and customise its access rights.
====
