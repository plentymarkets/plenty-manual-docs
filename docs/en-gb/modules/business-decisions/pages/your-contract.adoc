...
Hover over the account icon (icon:user-circle[]) and click on *My account* to see information about your plentymarkets system. You can make changes to the information saved here.
...
Hover over the account icon (icon:user-circle[]) and click on *My account » Contracts* to see and edit information about your contract with plentymarkets.
For example, you can:

* <<#30, switch to a different edition>>
* <<#40, view the services that you have booked>>
* <<#50, view transactions in the clearing account>>
* xref:business-decisions:gdpr.adoc#500[view GDPR relevant documents]
...
. Hover over the account icon (icon:user-circle[]) and click on *My account*. +
...
| *Documents*
|Use the document archive to export relevant documents. To do so, go to *Data » Document archive » Tab: Export*. Select the appropriate settings and export all relevant documents. When selecting the documents and the time period, take legal tax requirements and retention periods into consideration.

| *Stock*
|Create an export format for your xref:data:formatdesigner-stock.adoc#[stock data] in the *Data » Elastic export* menu. We recommend that you select at least the following export fields:
...
| *Stock movements*
|Create an export format for your xref:data:formatdesigner-stock-movement.adoc#[stock movement] in the *Data » Elastic export* menu. We recommend that you select at least the following export fields:
...
Additionally, you can filter for the clearing period using the fields *From* and *To* and for the order ID in the *Cleared postings* area. Moreover, you can find the function *Download report* here. Use this function to download either a PDF or a CSV file. Activate (material:check_box[role=skyBlue]) the option *Also download message* if you want to include both the message's content and subject in the download file. Note that the export is limited to 100 rows. +
In the *Reference* area, click on the message ID to open the message in an overlay.
|====
...
In plentymarkets, the term xref:online-store:online-store.adoc#[client (store)] refers to an additional online store, which is managed with the same plentymarkets system as the default store.
...
====
You need to have a xref:business-decisions:domains.adoc#[domain] before you can order a client (store). You have to create this domain for the main system beforehand.
====
...
OHover over the account icon (icon:user-circle[]) and click on *My account » Agencies* to grant certified service providers access to your system.
...
If you would prefer to only give the agency access to specific functions or menus, xref:business-decisions:user-accounts-access.adoc#[create a new user account], such as a back end account, and customise its access rights.
====
...
====
Always follow the steps described above to safely and permanently withdraw agency access to your system. It is *not* sufficient to simply xref:business-decisions:user-accounts-access.adoc#85[delete the user account] created by the agency in your plentymarkets back end.
====
...
image::business-decisions:support-hotline.png[]

[TIP]
.Another way to find your support PIN
====
Alternatively, Hover over the account icon (icon:user-circle[]) and click on *My account » Support-Hotline*.
