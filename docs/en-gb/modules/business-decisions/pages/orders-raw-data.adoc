...
:description: plentyBI: Use the orders data format creator to export orders and save this data in the cloud.
...
|INTEGER NOT NULL
|order plenty ID
|ID of the plentysystems system or the client via which the order was created
...
|order status
|Status of the order in decimal format. Depending on the statuses set up in the system, numbers with up to 4 decimal places are exported. You can find the IDs of the standard statuses on the xref:data:internal-IDs.adoc#120[Internal IDs in plentysystems] page of the manual.
...
You can find further possible values for order referrers on the xref:data:internal-IDs.adoc#90[Internal IDs in plentymarkets] page of the manual.
