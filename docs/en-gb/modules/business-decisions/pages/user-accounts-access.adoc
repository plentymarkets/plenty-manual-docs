...
. Carry out the settings. Note <<#table-seo-settings>>.
...
. Take the changes according to <<#tabelle-user-data>> 
. *Save* (icon:save[]) the settings.
...
If the user account should be an owner and in which areas, is determined by specific permissions in the advanced mode of the role view. For example: if a role needs to be an owner for incoming items, you need to assign the permissions*Stocks*, or more specific *Stocks » Incoming items*. Under <<#table-owner>> you will find which permissions activate which owner rights. If the user is an administrator, they are owner in every area.
...
. Select a role.
. Switch to the advanced mode (material:toggle_on[role=skyBlue]).
. Activate the areas in which the role should be available as an owner.
. *Save* (icon:save[]) the settings.

[[table-owner]]
.Necessary permissions for different owner rights
[cols="1,3"]
|====
|Owner |Necessary permission
...
. Carry out the additional settings. Note <<tabelle-e-mail-adresssen>>.
. Click on *Create mailbox*. +
