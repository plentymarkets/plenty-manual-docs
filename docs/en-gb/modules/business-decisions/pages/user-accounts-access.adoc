...
====
CUL stands for concurrent user licencing. The licence determines how many user accounts can have access to the plentysystems back end, the REST API and the plentymarkets app. As such, we limit simultaneous use and not the number of user accounts you can create.
====
...
. Carry out the additional settings. Note <<table-user-data>>. +
. *Save* (material:save[]) the settings.
...
|*|*
 Enable Forum Account*|Click if a forum account should be created for the user. The user must have entered a valid Email address 
...
. Open the menu *Setup » Account management » Account*.
. Click on *Search* (material:search[]).
...
This email address will receive an automated invitation email (expires after 14 days) to connect the existing user account with a plentysystems account. If the existing user account is connected with plentysystems account, the detail view of the user account changes. You will find the following fields in account settings of your plentysystems account: *User*, *Name*, *Timezone*, *Language*, *Password* and *Repeat Password*.
...
. Open the menu  *Setup » Account management » Roles*.
. Select a role.
. Open *plentymarkets App / plentyPOS* in the simple mode.
