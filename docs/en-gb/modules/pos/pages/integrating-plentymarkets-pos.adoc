...
The mobile POS solution plentyPOS allows you to integrate your brick and mortar store seamlessly both with your online presence and plentysystems.
...
* <<#960, Changing POS orders in the plentysystems back end>>
...
plentyPOS can be operated in Euro (EUR), Swiss Franks (CHF) and Pound Sterling (GBP). However, you can only operate the POS in the xref:payment:currencies.adoc#30[system currency]. The system currency is set in the *Setup » Orders » Payment » Currencies* menu.

[IMPORTANT]
====
.Book a POS
To use plentyPOS, you need to link:https://www.plentymarkets.com/product/pricing/[book one or more POS for your plentysystems edition^]. Visit our link:https://www.plentymarkets.com/product/modules/point-of-sale/[plentyPOS product page^] to get further information on prices and terms and conditions.
====

After you have booked POS, it takes two or three minutes for the activation process to finish in the background. Then, log out of your plentysystems system and log in again. This step is also necessary if you have already set up points of sale and book one or several new points of sale later.

Afterwards, you carry out the settings for the points of sale in the plentysystems back end. First, you assign a POS to a client by creating a POS in the *Setup » Client » [Select client] » POS* menu and carrying out further settings. Proceed as described below.
...
First, create a POS for the client (store) in the plentysystems back end.
...
You are now in the *Setup » POS » [Open client]* menu.

Make sure that you have opened the newly created POS by clicking on its name in the menu tree on the left. Then, carry out the desired settings in the *Basic settings* tab of the POS. Pay attention to 
the information provided in <<table-POS-basic-settings>>.
...
2+^|[#pos-item-information]*Item information*

| *Attributes*
|Activate (icon:check-square[role="blue"]) this option to display the item xref:item:attributes.adoc#[attributes] in the POS view. +
*_Note:_* This option is activated by default.

| *Recommended retail price*
|Activate (icon:check-square[role="blue"]) this option to display the recommended retail price of the item in the POS view.

| *Age rating*
|Aktivate (icon:check-square[role="blue"]) this option to display the xref:item:import-export-create-directory.adoc#intable-age-rating[age rating] saved for the item in the POS view.

| *Net stock*
|Activate (icon:check-square[role="blue"]) this option to display the xref:glossary:glossary.adoc#net-stock[net stock] of the item in the POS view.

| *External variation ID*
|Activate (icon:check-square[role="blue"]) this option to display the xref:item:import-export-create-directory.adoc#intable-external-variation-id[external variation ID] in the POS view.

| *Variation number*
|Activate (icon:check-square[role="blue"]) this option to display the xref:item:import-export-create-directory.adoc#intable-variation-number[variation number] in the POS view.

| *Purchase price*
|Activate (icon:check-square[role="blue"]) this option to display the xref:item:import-export-create-directory.adoc#intable-pp-net[purchase price] of the item in the POS view.
...
====
POS users can create their own favourites in plentyPOS. The POS user's favourites take priority over those favourites saved in the plentysystems back end.
====
...
| *Header*
|If the option is selected, the field's content is displayed as a header on all receipts issued by the POS. By default, the header contains the information saved in the *Setup » Settings » Master data* menu. Make changes as needed.
...
* Activate rights for communication with the plentysystems back end via REST API
...
To give new POS users access to plentyPOS, create xref:business-decisions:user-accounts-access.adoc#[user accounts] for those users in the plentysystems back end. To do so, proceed as described below.
...
Now assign rights for the communication between plentyPOS and the plentysystems back end.

:rights-purpose: use the basic functionalities of plentyPOS as POS users
:right-one: pass:quotes[*POS > Read*]
:right-two: pass:quotes[*CRM > Campaign > Code > Read*]
:right-three: pass:quotes[*CRM > Campaign > Code > Create*]
:right-four: pass:quotes[*CRM > Campaign > Code > Delete*]
:right-five: pass:quotes[*CRM > Address > Read*]
:right-six: pass:quotes[*Orders > Read*]
:right-seven: pass:quotes[*Orders > Create*]
:right-eight: pass:quotes[*Stock > Warehouses > Warehouse location > Read*]
...
. Click on *Rights*.
. Activate (icon:check-square[role="blue"]) the option *plentymarkets App / plentyPOS » POS functions » See*.
. *Save* (icon:save[role="darkGrey"]) the settings.
...
By assigning roles in the plentysystems back end, you determine the POS functions that POS users are allowed to use. You also define which additional functions and key figures are visible in the plentymarkets app.
...
| *Upload orders*
|Allows POS users to upload POS orders to the plentysystems back end.
...
→ A list of available order referrers is displayed. In this list, each POS system linked to your plentysystems system is shown as a separate order referrer.
...
If an incorrect password is entered several times in succession, the login is blocked for a certain amount of time. As a user with admin rights, you can unlock the user manually in the plentysystems back end.
...
*_Note:_* Pay attention to the information provided in the *Description* tab of the plugin on plentyMarketplace.
...
====
You can return to the assistant and finish it again at a later point in time if you wish to change the payment method names again. The name is displayed in the plentysystems back end, on plentyPOS receipts, in the X and Z report and in all exports generated via plentyPOS.
====
...
====
For the payment method *11 » Cash sale/cash card*, also go to *Setup » Client » [Select client] » POS » [Open POS] » Tab: Payment method* and select *Unconnected device* in the *POS terminal* area.
====

[#1070]
=== Linking payment terminals

Some card terminals can be linked to plentyPOS by provisioning plugins in the plentysystems back end. plentyPOS currently supports the following card terminals:
...
Orders generated in plentyPOS are assigned to the xref:orders:accounting.adoc#800[revenue accounts] saved in plentysystems. Because deposits, withdrawals and till count discrepancies are not orders, you can save booking accounts for these POS events. This helps you to manage which booking accounts to associate with different POS deposits and withdrawals. In addition, you can save booking accounts for discrepancies resulting from till counts.
...
For POS orders to be processed correctly in plentysystems, you need to allow the shipping method *Self pickup* for those items you want to bill through plentyPOS. The following preparations are necessary:
...
| *Internal name EN*
|Enter an internal name. The internal name is only used in the plentysystems back end. It is not visible to customers.

| *External name EN*
|Enter an external name. The external name is displayed in the online store. It is visible to customers.

| *Minimum quantity*
|Sales prices with different minimum quantities can be used to create a quantity based graduation of prices in plentysystems.
...
Totals are always shown with 2 decimal places on plentyPOS receipts. However, the settings in the plentysystems back end determine if order prices are rounded to 2 decimal places or if 4 decimal places are used for calculations. The rounding behaviour for orders generated in plentyPOS is determined by the following 3 options:
...
For POS orders to be saved correctly in plentysystems, every order must be associated with a contact. Proceed as described to create a *default customer* for POS orders. Only settings necessary to configure the default customer for POS orders are described.  For general information about creating customers, refer to the xref:crm:create-contact.adoc#[Creating a contact] page of the manual.
...
| *Company*
|Enter a company name, for example plentysystems POS.
...
2+^| *Setup » POS » [Select client] » [Open POS] » Tab: Receipt*

| *Query price if price is 0*
|Activate the option.

2+^| *Setup » Account management » Roles » [Open role] » Rights » plentymarkets App / plentyPOS*
...
In plentysystems, multipurpose coupons are treated as a product that customers can buy in the form of _gift cards_. If someone has a multipurpose coupon, they can redeem it in exchange for products or services.
...
Even though multipurpose coupons and promotional coupons are used for different purposes, they are both configured the same way in plentysystems. Therefore, both topics are explained here in the same chapter.

The following preparations in the plentysystems back end are necessary to sell multipurpose coupons and promotional coupons via plentyPOS:
...
To redeem multipurpose coupons in plentyPOS, POS users enter the coupon code. A coupon code must be present in the plentysystems back end at the time it is redeemed.

From the point of view of POS users, the configuration options in plentysystems result in three possible scenarios when selling coupons:

* The coupon code is generated automatically at the POS.
* The POS user is issued with a list of coupon codes for each coupon value that have been saved in plentysystems. The POS user enters a code. The value of the coupon code is retrieved.
* The POS user is issued with a list of coupon codes that have not been saved in plentysystems. The POS user enters a code and a value.
...
To save coupon codes in the plentysystems back end before selling them:
...
* POS users enter a coupon code that is not yet saved in plentysystems and the associated coupon value when selling a coupon.
...
Instead of generating coupon codes in plentysystems, you also can save your own coupon codes in plentysystems before selling them. Valid characters for coupon codes are lower and upper case letters, numbers, underscores (_) and hyphens (-).
...
* POS users enter a coupon code saved in plentysystems. The value of the coupon code becomes the price of the coupon.
...
2+^| *Item » Edit item » Open item » Open variation » Tab: Settings » Coupon item*
...
* In case you have not done so yet. create an xref:pos:integrating-plentymarkets-pos.adoc#830[appropriate campaign] in plentysystems.
* Set up a xref:pos:integrating-plentymarkets-pos.adoc#2600[coupon item with a flexible value] for this coupon type in plentysystems.
...
:right-one: pass:quotes[*CRM > Campaign > Code > Read*]
...
Note the following information on handling returns in plentyPOS:
...
The following preparations in the plentysystems back end are necessary to create coupons for refunds.
...
* <<#2300, The POS user enters a code that is not saved in plentysystems.>>
...
====
To correctly synchronise POS orders and z reports with the plentysystems back end, the date and time of the device must be set correctly. We recommend that you let the device set the date and time automatically. This way, you can avoid discrepancies in z reports and other receipts.
====
...
====
You can only unlink a POS from a device in the settings of the device itself. Contact the plentysystems support to unlink the device in case of device failure or loss.
====

[#250]
== Emailing receipts to existing customers

plentysystems allows you to set up an event procedure that automatically emails POS receipts to the email address saved for a contact data record. First create an email template with the PDF attachment *Receipt*. Then, you create an event procedure. Note that for POS orders created in <<#1100, offline mode>>, the event procedure only takes effect once the POS order is uploaded to the plentysystems back end. Before activating an event procedure for automatic emails, check the current legal restrictions regarding emailing customers.
...
* Contact data records are managed in the plentysystems back end. xref:crm:create-contact.adoc#create-contact[Add new contacts] or use the xref:crm:search-contact.adoc#[search] in the *CRM » Contacts* menu to view the contact IDs for the contacts that you want to provide with loyalty cards.
...
To use additional printers, you need a printer that can be accessed via the network that is connected to one or more printers on which you want to print the POS receipts. On this computer, you install the plugin xref:automation:installing-plentybase.adoc#[plentyBase]. plentyBase establishes a connection between the plentymarkets app and the local computer. This allows the POS to access any printers that the local computer can also access. Go to the plentysystems back end menu *Setup » Settings » Printers* to configure these printers.

[#1010]
=== Installing and setting up plentyBase

plentyBase is a plugin developed by plentysystems. Unlike many other plentysystems plugins, plentyBase is not activated in the plentysystems back end. Instead, you install and run plentyBase on a local computer. plentyBase establishes an interface that allows you to use the printers that are connected to the computer in the plentysystems back end and/or the plentymarkets app.
...
* The printers must be configured in the plentysystems back end menu *Setup » Settings » Printers*.

[#1020]
=== Saving printers for POS receipts

After you have set up plentyBase, you can select the printers that were configured in the plentysystems back end from the drop-down lists of the *Printers* area in the *plentymarkets App » Settings » POS* menu.
...
*_Recommendation:_* Configure plentyBase to run automatically when the computer is started up.
* Has the printer been xref:automation:printer.adoc#[saved and set up] in the plentysystems back end?
...
* Was the printer configuration removed from the plentysystems back end? +
...
* [ ] The plugin link:https://marketplace.plentymarkets.com/en/elasticexport_4763[Elastic Export^] has to be installed and deployed in your plentysystems system
* [ ] The plugin link:https://marketplace.plentymarkets.com/en/elasticexport_4763[FormatDesigner^] has to be installed and deployed in your plentysystems system

After you have completely set up the plugin *plentyBase ItemCachePOS*, you can sell items via plentyPOS without an internet connection. The plugin works as a temporary storage in which the transactions are saved during times when there is no internet connection. As soon as the internet connection is reestablished, the completed transactions are automatically synchronised with plentysystems in the background.

[WARNING]
.Offline orders are deleted when you uninstall the app
====
If you uninstall the app on your device before all orders were uploaded to plentysystems, the orders saved on your device are deleted permanently and cannot be recovered by you or the plentysystems support team. As such, check that no offline orders exist on the device before you uninstall the app.
====
...
[#960]
== Changing POS orders in the plentysystems back end

An order is created for POS orders in the plentysystems back end. After the order has been created, legally relevant order details can no longer be changed. Only order details that change in the course of processing and fulfilling orders or that document the order history can be changed:
