...
The mobile POS solution plentyPOS allows you to integrate your brick and mortar store seamlessly into plentysystems.
...
* where you can xref:app:installation.adoc#500[download the plentymarkets app] free of charge.
...
After you have booked POS, it takes two or three minutes for the activation process to finish in the background. Then, log out of the plentysystems back end and log in again. This step is also necessary if you have already set up points of sale and book one or several new points of sale later.

Afterwards, you carry out the settings for the points of sale in the plentysystems back end. First, you assign a POS to a client by creating a POS in the *System » System settings » Client » [Select client] » POS* menu and carrying out further settings. Proceed as described below.
...
First, create a POS for the desired client (store) in the plentysystems back end.
...
Make sure that you have opened the newly created POS by clicking on its name in the menu tree on the left. Then, carry out the desired settings in the *Basic settings* tab of the POS and save these settings. Pay attention to the explanations given in <<table-POS-basic-settings>>.
...
| *Age rating*
|Activate (icon:check-square[role="blue"]) this option to display the xref:item:import-export-create-directory.adoc#intable-age-rating[age rating] saved for the item in the POS view.
...
| *Select customer*
||Allows POS users to look up customer data from the POS shopping cart and to select an existing plentysystems customer for the order.
...
Some card terminals can be linked to plentyPOS by downloading provisioning plugins free of charge. plentyPOS currently supports the following card terminals:
...
. Go to *System » Client » [Select client] » Locations » [Select location] » Accounting*.
. Scroll down until you see the areas *POS events deposit* and *POS events withdrawal*.
. Click on *Add* (icon:plus[role="darkGrey"]) in one of the areas, depending on the purpose of the booking account.
→ An empty entry is created.
...
*_Tip:_* Click and hold (material:drag_indicator[role=darkGrey]) to rearrange the entries. This way, you can adjust the order in which the events are displayed in the app.
. *Save* (icon:save[role="darkGrey"]) the settings. +
...
Totals are always shown with 2 decimal places on plentyPOS receipts. The settings in the plentysystems back end determine if order prices are rounded to 2 decimal places or if 4 decimal places are used for calculations. The rounding behaviour for orders generated in plentyPOS is determined by the following 3 options:
...
For POS orders to be saved in plentysystems correctly, every order must be associated with a contact. Proceed as described to create a contact of the type *default customer* for POS orders in the plentysystems back end. Only settings necessary to configure the default customer for POS orders are described.  For general information about creating customers, refer to the xref:crm:create-contact.adoc#[Creating a contact] page of the manual.
...
| *Country*
|Select the country in which the POS will be used from the drop-down list. The default setting is based on the option *Country* in the xref:quick-start:quick-start-initial-settings.adoc#100[Setup » Settings » Master data] menu.
...
You may find that items are to be sold for which no data record exists in plentysystems. For instance, it is often too labour intensive to create data records for miscellaneous items or remnants. Create a dummy item to be able to sell these items anyway. POS users can then add the dummy item to the POS shopping cart and modify the price and/or the name of the order item.
...
To redeem multipurpose coupons in plentyPOS, POS users enter the coupon code. A coupon code must already be saved in the plentysystems back end at the time it is redeemed.
...
* Contact data records are managed in the back end of your plentysystems system. xref:crm:create-contact.adoc#create-contact[Add new contacts] or use the xref:crm:search-contact.adoc#[search] in the *CRM » Contacts* menu to view the contact IDs for the contacts that you want to provide with loyalty cards.
...
To use additional printers, you need a printer that can be accessed via the network that is connected to one or more printers on which you want to print the POS receipts. On this computer, you install the plugin xref:automation:installing-plentybase.adoc#[plentyBase]. plentyBase establishes a connection between the plentymarkets app and the local computer. This allows the POS to access any printers that the local computer can also access. Go to the plentysystems back end menu *System » System settings » Settings » Printers* to configure these printers.
...
* The printers must be configured in the plentysystems back end menu *System » Settings » Printers*.
