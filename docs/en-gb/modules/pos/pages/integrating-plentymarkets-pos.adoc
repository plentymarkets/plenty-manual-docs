...
:author: team-mobile
:keywords: plentyPOS, set up POS, POS setup,POS user, POS payment method, POS booking account, POS pickup, prices POS orders, rounding behaviour POS orders, POS order, dummy item, miscellaneous item, POS coupon, POS return, link POS, send receipt, print receipt, customer card, loyalty card, cash journal, change POS order, delete POS, unlink POS, POS user rights, POS rights
:table-event-procedure: <<table-POS-event-procedure-outgoing-items>>, <<table-POS-event-procedure-send-receipt>>
:position: 10
:url: pos/integrating-plentymarkets-pos
:id: LLRIR57

The mobile POS solution plentyPOS allows you to integrate your brick and mortar store seamlessly both with your online presence and your plentymarkets system. The point of sale system plentyPOS is provided as a component of the <<app/installation#, plentymarkets app>>.
...
The point of sale system plentyPOS is provided as a component of the <<app/installation#, plentymarkets app>>. Take a look at our documentation and learn:

* Which <<app/installation#400, hardware requirements>> apply to plentyPOS.
* Where you can <<app/installation#500, download the free plentymarkets app>>.
* How you can <<app/installation#600, grant your employees access to the app>>.
* How to <<app/installation#1100, log into the app and find your way around>>.

plentyPOS can be operated in Euro (EUR), Swiss Franks (CHF) and Pound Sterling (GBP). However, you can only operate the POS in the <<payment/currencies#30, system currency>>. The system currency is set in the *System » System settings » Orders » Payment » Currencies* menu.

[IMPORTANT]
====
.Book a POS
To use plentyPOS, you need to link:https://www.plentymarkets.com/product/pricing/[book one or more POS for your plentymarkets edition^]. Visit our link:https://www.plentymarkets.com/product/modules/point-of-sale/[plentyPOS product page^]{nbsp}icon:external-link[] to get further information on prices and terms and conditions.
====
...
Option activated = Only variations are available in this POS for which the option <<item/managing-items#230, Active>> is activated in the **Settings** tab of the variation. +
...
*_Tip:_* Click the arrow key of your keyboard twice to show a list of storage locations.
...
*_Note:_* Existing POS cannot be made tamper-proof. Create a new POS and activate the old POS to set up the signature unit.
...
a|Select if a coupon receipt is printed and saved in the document archive. +
...
|Select if the <<glossary#grand-total, grand total>> should be included in the z report. +
Not activated (default) = The grand total is not included in the z report. *_Note:_* In this case, the grand total can only be displayed at the point of sale itself by users with admin rights. +
Activated = The grand total is included in the z report.

| **List discount codes on z report individually**
|Select if discount codes redeemed in connection with <<glossary#promotional-coupon, promotional coupons>> should be listed individually on the z report. +
...
| **Image**
|Activate the option and select a PNG file to display on the receipts generated by the POS. The file must have been uploaded to the **CMS » Image gallery** menu beforehand.
...
In case of a <<pos/plentymarkets-pos-for-pos-users#175, return>> or a <<pos/plentymarkets-pos-for-pos-users#450, Click & Collect order>>, customers usually bring their receipts to the store. If a barcode was printed on the receipt, the POS user only has to <<pos/plentymarkets-pos-for-pos-users#520, scan the barcode>> to access the corresponding order. +
...
* Create a user account of the user class **Back end**
...
The settings you need to create a user account for a POS user are described below. For general information about user accounts and user rights, refer to the <<business-decisions/user-accounts-access#, User accounts and access>> page of the manual.

[TIP]
.Tip: Create a role for POS users
====
To simplify rights management for POS users, create a <<business-decisions/user-accounts-access#70, role>> with those rights your POS users need, and link this role to the user accounts of POS users.
====

[#140]
=== Creating an account for POS users

To give new POS users access to plentyPOS, create a <<business-decisions/user-accounts-access#10, user account>> of the user class **Back end** in the plentymarkets back end. To do so, proceed as described below.
...
For POS users to edit prices, rename order items and/or give discounts, you need to assign the appropriate rights to POS users. Click <<business-decisions/user-accounts-access#70, here>> to learn how to create roles and assign them to users. You also activate additional app functions for users.
...
| **TSE unit**
|Allows POS users to open the *TSE unit* menu in the plentymarkets app. This menu displays <<pos/pos-legal-compliance#900, status information on the TSE unit>>. POS users with this right are also authorised to create <<pos/pos-legal-compliance#700, TSE exports>>.
...
====
Pay close attention to activating **all** POS order referrers for which the POS user should be able to carry out checkout activities, such as ringing up customers. If, for example, two POS are used in your store, activate both POS order referrers for the user. If you do not do so, the user will not be able to ring up a customer at POS 1 and later process a return for the same order using POS 2.
====
...
. Install the plugin link:https://marketplace.plentymarkets.com/en/pospaymentmethodrenaming_6813[plentyPOS payment methods^]{nbsp}icon:external-link[] +
...
Note that payment methods must also be permitted for customer classes in another menu. For further information, refer to <<payment/managing-payment-methods#30, Permitting the payment method in a customer class>>.
...
* link:https://marketplace.plentymarkets.com/en/plugins/payment/ingenicozvt_5140[Ingenico devices via ZVT interface^]nbsp}icon:external-link[]
* link:https://marketplace.plentymarkets.com/plugins/payment/sumup_5141[SumUp Air^]{nbsp}icon:external-link[]
* link:https://marketplace.plentymarkets.com/en/plugins/payment/VerifoneZVT_5504[Verifone devices via ZVT interface^]nbsp}icon:external-link[]
...
* link:https://marketplace.plentymarkets.com/en/plugins/payment/PosInvoice_5590[POS Invoice^]nbsp}icon:external-link[]
...
Orders generated in plentyPOS are assigned to the <<orders/accounting#800, revenue accounts>> saved in plentymarkets. Because deposits, withdrawals and till count discrepancies are not orders, you can save booking accounts for these POS events. This helps you to manage which booking accounts to associate with different POS deposits and withdrawals. In addition, you can save booking accounts for discrepancies resulting from till counts.
...
Do you want to activate the shipping profile for a large number of items at the same time? Then use the <<item/import-export-create/create/mass-processing#100, item group function>>.
...
For more detailed information and additional rounding examples, refer to the <<orders/accounting#150, Accounting>> page of the manual.

[#95]
== Creating a default customer for POS orders

For POS orders to be saved in plentymarkets correctly, every order must be associated with a contact. As such, you need to create an anonymous default user for casual customers. Proceed as described to create a **default customer** for POS orders. Only settings necessary to configure the default customer for POS orders are described.  For general information about creating customers, refer to the <<crm/managing-contacts#, Managing contacts>> page of the manual.
...
| **Country**
|Select the country in which the POS is to be used from the dropdown list. The default setting is based on the option **Country** in the <<quick-start/quick-start-initial-settings#100, Setup » Settings » Master data>> menu.
...
* <<orders/coupons#prepare-items-for-purchase-coupons, Decide which items>> customers should be allowed to purchase with coupons.
* <<orders/coupons#create-coupon-item, Create coupon items>> with 0% VAT, to add the coupons to the products that your business sells.
* <<orders/coupons#20, Create a campaign>> for coupons. Use the option *Coupon variety* to specifiy whether it is a multipurpose coupon or a promotional coupon.
* <<orders/coupons#link-coupon-item-to-campaign, Link your coupon items to the campaign>>.
* *_Optional:_* <<pos/integrating-plentymarkets-pos#850, Generate codes>> or save your own coupon codes in the back end.
...
| **Minimum order value**
|Do not enter a value. This is because multipurpose coupons are to be treated as a payment method. As such, they cannot be subject to a minimum order value.
...
| **Item**
|Do not enter a value. This is because multipurpose coupons are to be treated as a payment method. As such, they cannot be limited to specific items.

| **Clients (stores)**
|Select the clients (stores) for the coupons of the campaign.

| **Customer classes**
|Select the customer class for the coupons of the campaign.

| **Customer types**
|Select the customer type for the coupons of the campaign.
|====
...
. In the **Generate gift card codes manually** area to the left, enter the number of coupon codes you want to generate.
. *_Optional:_* In the **Value** field, enter the value to be assigned to the coupon code. +
...
*_Tip:_* Use the data format **CampaignCoupon** to import several coupon codes at once. The **Discount value** is imported as the coupon value. As such, you need to create one campaign for each coupon value if you want to use the data format.
. Provide POS users with the coupon codes and their value. +
*_Note:_* Do not circulate duplicate coupon codes. If you operate more than one POS, either split the coupon codes or create separate campaigns for each POS.
...
Now, create one or more coupon items. Because the campaign ID is linked at the item level, you cannot use variations to create coupons. For further information about creating items, refer to the <<item/import-export-create/create/new-item#, Managing items>> page of the manual. Only settings needed for creating coupon items are described here.
...
* Create a <<pos/integrating-plentymarkets-pos#830, suitable campaign>> in plentymarkets.
* Set up a <<pos/integrating-plentymarkets-pos#2600, coupon item with a flexible value>> for this coupon type in plentymarkets.
* Add this coupon item to the <<pos/integrating-plentymarkets-pos#60, favourites list>> of your POS.
* Order link:http://lmgtfy.com/?q=barcode-cards+printing[barcode cards^]{nbsp}icon:external-link[] with pre-printed codes. Most providers of barcode cards expect you to send a CSV file containing the individual codes that should be placed on the cards. Also, you should provide them a layout template in Photoshop or Adobe Illustrator format for the frame design.
...
These steps are described below. Find more detailed information on creating campaigns and coupons on the <<orders/coupons#, Coupons>> page of the manual.
...
| *Discount value*
|Enter a coupon value, for example 1.00.
...
| **Fiscalisation**
| IP address of the << pos/pos-legal-compliance#500, technical security unit>> that is linked to the POS.

| **Receipt printer**
|Enter the IP address of the receipt printer. +
*_Tip:_* Tap **Print test receipt** to print a receipt with test data.
...
(new POS with location in Austria only)
|Select to <<pos/integrating-plentymarkets-pos#1600, create the start receipt (Startbeleg)>>. This will activate the tamper-proof signature unit for the POS. After the start receipt is created, the location country of the POS cannot be changed.

| **Create end receipt** +
(POS with location in Austria and activated signature unit only)
|Select to <<pos/integrating-plentymarkets-pos#2300, create the end receipt (Endbeleg)>>. This will deactivate the tamper-proof signature unit for the POS. Once an end receipt has been created, the POS can no longer be operated.
|====
...
. Carry out the additional settings. Note <<crm/sending-emails#1200, creating a new email template>>.
. **Save** (icon:save[role="darkGrey"]) the settings.
...
In addition to automatically emailing receipts, you can offer existing customers an additional service.  Customer cards with barcodes allow you to scan the barcode directly while <<pos-plentymarkets-pos-for-pos-users#30, ringing up customers>>. This way, a contact is automatically selected. Just like with <<#2700, coupon cards>>, the contact is identified using the barcode printed on the customer card. Depending on the method used, the contact is identified using either the contact ID or the customer number. <<table-pos-loyalty-cards>> explains both methods and describes the preparatory steps for creating loyalty cards.
...
* Contact data records are managed in the back end of your plentymarkets system. <<crm/managing-contacts#100, Add new contacts>> or use the search in the *CRM » Contacts* to view the contact IDs for the contacts that you want to provide with customer cards.
...
To use additional printers, you need a printer that can be accessed via the network that is connected to one or more printers on which you want to print the POS receipts. On this computer, you install the plugin <<automation/processes/installing-plentybase#, plentyBase>>. plentyBase establishes a connection between the plentymarkets app and the local computer. This allows the POS to access any printers that the local computer can also access. Go to the plentymarkets back end menu *System » System settings » Settings » Printers* to configure these printers.
...
→ Download the installation file in the link:https://marketplace.plentymarkets.com/en/plugins/integration/plentyBase_5053[plentyMarketplace^]{nbsp}icon:external-link[]. 
* plentyBase must be configured on the computer. +
→ Refer to the <<automation/processes/installing-plentybase#, plugin description>> for instructions on how to set up plentyBase. +
...
→ To learn how to save plentyBase connection data in the app, refer to the <<automation/processes/installing-plentybase#, plugin description>>. 
...
For further information about creating label templates and printing the labels in the item search of the plentymarkets app, refer to the <<app/functions/item-search#, Searching for items, customers and orders>> page of the manual.

[IMPORTANT]
.plentyBase einrichten
====
To print labels, you need the free plugin plentyBase. plentyBase establishes an interface that allows you to use the printers that are connected to the computer in the plentymarkets back end and/or the plentymarkets app. Refer to the <<automation/processes/installing-plentybase#, plugin description>> for instructions on how to set up plentyBase.
====
...
→ The POS is deleted. +
*_Note:_* If the POS is located in Austria and the signature unit is activated, an <<pos/integrating-plentymarkets-pos#2300, end receipt>> must be created in the app. This deletes the POS automatically.
