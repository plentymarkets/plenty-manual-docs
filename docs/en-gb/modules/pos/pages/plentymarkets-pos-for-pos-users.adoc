...
:description: On this manual page, you will find detailed information about how to operate plentyPOS.

On this manual page, you will find detailed information about how to operate plentyPOS.
...
| *System (base URL)*
|What’s the URL for your plentysystems system?
Enter the domain without https:// or http://. In case you don’t know the URL, log into your plentysystems back end. Move the cursor over the shop symbol with your plentyID on the upper right. The second row in the tooltip that is displayed contains the URL. +
...
| *User*; *Password*
|Enter the user name and password for the plentysystems system.
|====

[#310]
=== Switching to a different user account

Logged in user accounts do not need to log out before you can log in. Switching users is only possible if one user account currently is logged in. The user symbol at the top right shows the user who currently is logged in. The user symbol shows the initials of the real name of the person that is logged in. The icon’s background colour is the one that was chosen for this user account in the plentysystems back end. You can switch users in the main menu, the POS menu, when creating POS orders and when booking returns. Once you switch users, the same area of the app opens. If the new user does not have rights for this area of the app, the main menu of the app opens instead. Proceed as described below to switch the user account.
...
*_Note:_*If your name is not listed, update the view by pulling the window down. If your name is still not shown, this means that no app PIN is saved for your user account in the plentysystems back end.
...
| *Select background*
|Tap on an image to use it as a POS background.

2+^| *Use full screen mode*

| *Full screen mode* +
(Android devices only)
|Activate the option to hide the control elements of the operating system and use plentyPOS in full screen mode.
|====
...
* Variations with xref:stock-management:managing-bbd-batch.adoc#[best before dates or batch numbers] are not found via the item search.
...
====
Do customers buy variations with order properties in your online shop and pick them up in your store afterwards? For this use case, the setting described above is not relevant. For these so-called xref:pos:pos-online-orders.adoc#[Click & Collect orders], order properties are already fully considered and displayed in plentyPOS. This also applies to returns that you create via plentyPOS when customers return variations from Click & Collect orders. +
...
*_Tip:_* Items with no net stock are shown in red but can be sold anyway.

[#60]
==== Opening the list of favourites

Quickly add favourite items to the POS shopping basket by accessing them through the favourites list. plentysystems administrators can create a list of favourite items for the POS. However, you can also mark frequently bought items as favourites yourself in order to speed up the checkout process. Proceed as described below to add items to the POS shopping cart by using the list of favourites.
...
If the plugin link:https://marketplace.plentymarkets.com/en/plugins/payment/PosInvoice_5590[plentyPOS Invoice^] is provisioned in the plentysystems back end, you can also sell items on account to existing customers. However, you cannot save new customer details using plentyPOS. Customers can only pay for items on account if they are existing customers and a corresponding contact data record exists in your system. 
...
====
The following instruction describes how to handle down payments for orders that have been created directly at the POS. Handling down payments for other order referrers, such as xref:pos:plentymarkets-pos-for-pos-users.adoc#450[Click & Collect] is not possible.
====
...
Enter the order ID or the receipt number into the search field. Tap *Search* to access the order data. Now you can <<#370, finalise the return>> or hand over the xref:pos:pos-online-orders.adoc#[items that were purchased online].
...
Scan the barcode on the receipt. Now you can <<#370, finalise the return>> or hand over the xref:pos:pos-online-orders.adoc#[items that were purchased online].
...
. Tap *Generate*. +
*_Tip:_* You are not prompted to enter a coupon code if you enter a quantity of more than one coupon.
...
====
You can email receipts to customers automatically when a receipt is generated. To do so, you need to xref:pos:integrating-plentymarkets-pos.adoc#70[create an event procedure] in the plentysystems back end. Note that for POS orders completed in the xref:pos:integrating-plentymarkets-pos.adoc#1100[offline mode], the receipt will only be sent after the POS orders have been uploaded.
====
...
You can also create an entirely new contact when ringing up customers. This new contact is then automatically used for the current order and saved in the plentysystems back end. After you have finished ringing up the customer, the default customer is automatically set for the next order.

Proceed as described below to create a new contact.

. In the POS shopping cart, tap on the customer symbol at the bottom right. +
→ The data record of the default customer or of the currently selected contact is displayed.
. Tap on the plus icon. +
→ The *New contact* view opens.
. Enter the data for the new contact.
. Tap on *Save*. +
→ The newly created contact is automatically selected for ringing up.
*_Important:_* If you have already placed items in the POS shopping cart, you are asked if you want to *Update prices?*. If you select *Yes*, the discounts that apply to the selected contact’s customer class are applied to all items in the shopping cart. +
*_Tip:_* You can use the undo symbol to return to the default customer, if needed.
. Tap on *X* on the upper right or tap next to the window to return to the POS shopping cart. +
→ The selected contact is displayed in the POS shopping cart.
. Finish processing the order as usual. +
→ After you have finished ringing up the customer, the default customer is set again automatically.

[#100]
==== Adding a note to the order

To save additional information on the order, save a note before completing the order. Notes are imported to the plentysystems back end and saved with the order.
...
→ The note is displayed in the list of notes and is imported to plentysystems.
...
| *Change quantity*
|Open the *Change quantity* window. Enter the new quantity and tap on *OK* to edit the item quantity in the shopping cart.

| *Rename order item*
|Enter the new name and tap on *OK* to change the name of the order item. The name is changed only in this order. The item's name in the system remains unchanged.

| *Change single price*
|Enter the new price and tap on *OK* to change the single price of all items of an order item. The price is changed only in this order. The item's price in the system remains unchanged.

| *Change total price*
|Enter the new price and tap on *OK* to change the total price of all order items. The price is changed only in this order. The item's price in the system remains unchanged.
...
|Enter the discount in percent and tap on *OK* to offer a discount on the order item. The price is changed only in this order. The item's price in the system remains unchanged. +
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping card.

| *Add fixed discount*
|Enter a fixed amount and tap on *OK* to offer a discount of a fixed amount on the order item. The price is changed only in this order. The item's price in the system remains unchanged. +
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping card.
...
Go to *POS » Cancel order* to cancel orders created for this POS since the last Z report. Since the data is stored in a cache, the cancellation is also possible if the POS has been operated in the xref:pos:integrating-plentymarkets-pos.adoc#1100[offline mode] in the meantime. Proceed as described below to cancel a POS order.

[WARNING]
.Only cancel POS orders in the plentymarkets app
====
Only cancel POS orders in the plentymarkets app and not in the plentysystems back end. Orders cancelled in the plentysystems back end are not synchronised with the app. This means that the POS data used for reporting (z reports, cash on hand, etc.) are no longer correct when you cancel POS orders in the plentysystems back end.
====

[IMPORTANT]
.Cancelling Click & Collect orders
====
xref:pos:pos-online-orders.adoc#[Click & Collect orders] are not created in plentyPOS but are loaded into plentyPOS from other referrers, such as your online shop. Thus, you can only cancel those orders in your plentysystems back end.
====
...
*_Note:_* If you carried out the cancellation while plentyPOS was in the xref:pos:integrating-plentymarkets-pos.adoc#1100[offline mode], the cancellation is only visible in the plentysystems back end after the data has been synchronised. This process is carried out automatically in the background as soon as the internet connection is re-established.
...
====
When working with the following instructions, note that xref:pos:plentymarkets-pos-for-pos-users.adoc#385[refunding returns via card terminal] is only possible in app version 1.11.7 and higher.
====

[#370]
=== Booking a return by linking it to the main order

If you know the order ID or the receipt number (external order number) of the original main order, you can open this main order and book the return directly to this order. Proceed as described below to open the main order and to either xref:pos:plentymarkets-pos-for-pos-users.adoc#330[scan] or xref:pos:plentymarkets-pos-for-pos-users.adoc#340[manually select the return items].
...
→ A return in created in the plentysystems back end and linked to the main order. +
→ A credit note is created in the plentysystems back end and linked to the return. +
...
→ xref:pos:plentymarkets-pos-for-pos-users.adoc#350[Remove those items from the return] that the customer does not want to return. +
...
. Tap *Complete and pay out cash* or *Complete and create coupon*. +
→ A return without main order is created in the plentysystems back end. +
→ A credit note without a main order is created in the plentysystems back end. +
→ If you opted for a coupon, a new order for the amount of the coupon is created.
. Pay out the refund amount in cash or hand out the coupon.
...
From version 1.11.7 of the plentymarkets app, you can not only pay out refunds for returns as coupons or cash, but also use a card terminal integrated at your plentyPOS to transfer the amount to the customer’s debit card. This method of refunding returns is available for returns <<#370, with>> or <<#380, without>> a link to the main order.
...
→ The order data is displayed. +
. Add the items to be returned by scanning them or using the item search. +
. *_Optional:_* Change xref:pos:plentymarkets-pos-for-pos-users.adoc#140[the item prices].
. *_Optional:_* Add a xref:pos:plentymarkets-pos-for-pos-users.adoc#100[note to the return].
. After you have added all returned items, tap the green arrow at the bottom of the POS shopping cart. +
→ The total amount of the return is displayed.
...
→ The receipt is imported to plentysystems and saved in the *Data » Documents » Document archive* menu.
...
During your daily business, certain situations might require that you need to print an additional copy of a receipt. plentyPOS does not offer an option to reprint a receipt via the regular receipt printer. However, you can search for the receipt in the xref:orders:document-archive.adoc#[Document archive] in the plentysystems back end to print it from there.

[IMPORTANT]
.Information on the printing process
====
If you print a receipt from the document archive, the receipt will be downloaded to your computer as a PDF file. After downloading it, you can print the receipt using a regular printer, but *not* on the receipt printer that is connected to your plentyPOS. Thus, make sure that you have xref:automation:printer.adoc#[set up an appropriate printer that is connected to your computer via plentyBase].
====
...
Use x reports to gain insights into the revenue generated for this POS since the last z report during the day. You can create any number of x reports. X reports are not imported to plentysystems. As such, the creation of x reports is not documented permanently. .
...
→ The *POS menu* is displayed.
. Tap *X report*. +
*_Note:_* If the option is greyed out, you need additional xref:pos:integrating-plentymarkets-pos.adoc#170[rights] to create x reports. +
→ The x report is printed.

[#230]
=== Creating a z report

A z report documents the revenue accrued since the last z report. By generating a z report, the daily revenue is reset to 0. Z reports are assigned a sequential number and saved in plentysystems. Depending on local law, the saving of z reports may be required for fiscal purposes. .

[TIP]
.Reprinting a z report
====
Z reports are saved in the plentysystems back end. Go to xref:orders:document-archive.adoc#100[Data » Documents » Document archive]. Filter by the document type *Z report*. You can download the report as a PDF file from the *File* column.
====
...
. Tap *Create Z report*. +
*_Note:_* If the option is greyed out, you need additional xref:pos:integrating-plentymarkets-pos.adoc#170[rights] to create z reports. +
→ The z report is imported to plentysystems. +
→ The z report is imported and saved in the *Data » Documents » Document archive* menu. +
...
icon:map-signs[] *_Or:_* To enter the total amount counted instead of the quantity per denomination, tap the on *Overwrite* field and enter the total amount you counted.
. Tap *Save till count*. +
→ A till count report is created and saved in the *Data » Documents » Document archive* menu.
. *_Optional:_* Tap on *Print till count report* to print the report.
...
→ A deposit or a withdrawal is created for the discrepancy with the reason *Till count discrepancy* and saved in the *Data » Documents » Document archive » POS receipt* menu. +
