...
. On the start screen, tap on the plentymarkets app icon. +
...
====
You remain logged in until you tap on *Logout* or *Lock* in the main menu or the list of user accounts. That means that you can open and close the app without having to log in again.
====
...
. To unlock access to the app, tap on the user icon at the top right of the screen.
. Tap on your user account.
...
. Tap on the *Logout* icon at the bottom right of the window. +
→ The *Logout* window is displayed. +
icon:map-signs[] *_Or:_* Tap on the menu icon at the top left corner of the app and then on *Log out*. +
...
Once you open plentyPOS, the POS menu is displayed. Tap on **POS** to open the POS shopping cart and add items.
...
====
Do customers buy variations with order properties in your online shop and pick them up in your store afterwards? For this use case, the setting described above is not relevant. Fort these so-called xref:pos:pos-online-orders.adoc#[Click & Collect orders], order properties are already fully considered and displayed in plentyPOS. This also applies to returns that you create via plentyPOS when customers return variations from Click & Collect orders. +
...
. In the POS shopping cart, tap on *Search*. +
...
*_Tip:_* Items with no net stock are shown in red but can be sold.
...
. In the POS shopping cart,  on the star. +
...
. In the POS shopping cart, tap on the label symbol. +
...
*_Tip:_*  If you can see the *X* button in the bottom row of the POS’s number pad, you can tap on this button, enter the quantity and then scan the item or select a favourites item. The item is placed in the POS shopping cart with the quantity you entered. An admin user must activate this feature for the POS.
...
.Cancelling an order
====
You can cancel a POS order directly after you accepted and booked the payment. To do so, tap on *Cancel order* in the *Order created* view. Go to xref:pos:plentymarkets-pos-for-pos-users.adoc#173[POS » Cancel order] to cancel any completed orders created for this POS since the last z report.
====
...
*_Tip:_* If a customer pays the exact amount in cash, you can tap on the *Exact cash amount* button in the bottom row of the POS’s number pad instead of entering the amount. The exact amount is then automatically inserted in the *Received* field.
. Tap on *Payment received*. +
→ The *Order created* view opens. +
→ The order is created. +
→ A receipt is printed. Otherwise, tap on *Print receipt* to print the receipt. +
...
*_Tip:_* (version 1.8.5 and higher) If the customer wants to pay the whole amount with just one card payment, you do not enter the amount before you tap on *Payment terminal*. The total amount is automatically inserted.
. Tap on *POS terminal*. +
...
*_Tip_* (version 1.8.5 and higher) If the customer wants to pay the whole amount with just one card payment, you do not enter the amount before you tap on *Payment terminal*. The total amount is automatically inserted.
. Tap on *POS terminal*. +
...
→ The order is created. +
→ The receipts are printed. Otherwise, tap on *Print merchant receipt* and/or *Print customer receipt* to start printing manually. +
*_Tip:_* Tap on *Show order* to view the order details.
...
→ The *Order created* view opens. +
→ The order is created. +
→ The invoice is printed. Otherwise, tap on *Print invoice* to print the invoice.
. Tap on *New order* to continue with ringing up the next customer at the POS.
...
. Enter the amount that the customer wants to pay with the first payment method.
...
. Enter the amount that the customer wants to pay with the next payment method.
. Tap on the button for the payment method you received. +
...
→ The *Order created* view opens. +
→ The order is created. +
→ The receipts are printed. Otherwise, tap on *Print receipts* to print the receipts. +
...
*_Important:_* Ensure that the customer really is the person saved in the customer data. +
. Add the items <<40, to the POS basket>>.
. Enter the amount of the down payment you received from the customer.
...
*_Tip:_* You can revert to the default customer by tapping on the **Reset** icon.
. Tap on the **X** in the top right corner or tap on the screen outside the window to return to the POS shopping cart. +
...
. In the POS shopping cart, tap on **Note**. +
...
. In the POS shopping cart, tap on the vertical ellipse at the top right of the screen. +
→ Additional options are displayed.
. Tap on **Park receipt**. +
...
. In the POS shopping cart, tap on the order item. +
→ Additional options are displayed.
...
| **Change total price**
|Enter the new price and tap on **OK** to change the total price of all items of an order item. The price is changed only in this order. The item's price in the system remains unchanged.
...
| **Add fixed discount**
|Enter the fixed amount and tap on **OK** to offer a discount of a fixed amount on the order item. The price is changed only in this order. The item's price in the system remains unchanged. +
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping card.
...
====
If the discount is applied to multiple items, the total discount will be split up among all xref:item:managing-items.adoc#40[discountable items]. An item can _not be discounted_ if the option *Promotional coupons/POS discount* was set to *Not permitted* in the *Global* tab of the item.
...
[TIP]
.Why can I not grant a discount?
====
...
. In the menu bar, tap on the *menu symbol*.
. Tap on *POS*.
. Tap on *Cancel order*. +
...
. In the row for the order, tap on the *Cancellation* icon. +
...
*_Tip:_* If the customer wants to rerturnn all items of the order, just tap on *Create return* straight away.
 ...
 *_Tip:_* Tap on the menu to the top right and then on the *Change target storage locations* option to change the target storage locations of all returned items at once.
 ...
 . *_Optional:_* Add a xref:pos:plentymarkets-pos-for-pos-users.adoc#100[note to the return].
. After you have added all returned items, tap on the green arrow at the bottom of the POS shopping cart. +
...
[TIP]
.Why do I not see the expected cash on hand?
====
...
icon:map-signs[] *_Or:_* To enter the total amount counted instead of the quantity per denomination, tap on the **Overwrite** field and enter the total amount you counted.
. Tap on **Save till count**. +
→ A till count report is created and saved in the **Orders » Document archive** menu.
. **__Optional:__** Tap on **Print till count report** to print the report.
...
. After saving the till count, tap on **Book difference** to save the actual cash on hand as the new expected cash on hand. +
