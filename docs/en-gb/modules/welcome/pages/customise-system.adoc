...
image::welcome:General-functions-customising-window.png[Customise the layout]
...
|The xref:online-store:cms.adoc#web-design[store CSS] will be loaded in the editor.

| *Standard*
|A simple standard CSS will be loaded in the editor. The background colour is white.

| *User-defined*
|The simple standard CSS will be displayed in an editing window and can be customised with the xref:online-store:cms.adoc#_syntax_editor[syntax editor]. +
...
You can create tasks in the *plentymarkets logo (Start) » Tasks* menu and in the xref:crm:using-the-ticket-system.adoc#4000[ticket system]. This allows you to directly create a task during a customer phone call, for example. If a task is created in the *CRM » Ticket system » [Open ticket] » Tab: Tasks*, it is linked to the ticket. This link is also visible in the overview of tasks under *plentymarkets logo (Start) » Tasks*.
...
When opening the *plentymarkets logo (Start) » Tasks* menu, all tasks with a progress below 100% of the currently active user are shown as standard. If you have created a task via the xref:crm:using-the-ticket-system.adoc#4000[ticket system], the ticket ID as well as the ticket title are shown in the overview. By clicking on the ticket ID, the ticket opens directly.
...
You can have your uncompleted xref:welcome:customise-system.adoc#110[tasks shown on the dashboard]. This way you can see at a glance which tasks still need to be done when starting your working day.
...
*_Important:_* Only those tags that were created in the *Setup » Settings » Tags* menu are displayed in the list. For further information, refer to the xref:welcome:tags.adoc#[Tags] page of the manual.
...
For further information about creating event procedures, refer to the chapter xref:crm:using-the-ticket-system.adoc#[Ticket system] page of the manual.