...
IMPORTANT: Have you already set up a warehouse and created a supplier? Not yet? Return to xref:welcome:quick-start-stock-management.adoc#[Setting up a warehouse and supplier].
...
There are no plugins installed on your system by default. Go to *Setup » Assistants* and complete the *Integrations Assistant* to install some basic extensions.
...
include::plugins:partial$plugin-set-verknuepfung-aufheben-en_gb.adoc[]
...
TIP: Continue with xref:welcome:quick-start-setting-up-payment-processing.adoc#[payment processing].
