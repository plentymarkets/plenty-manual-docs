...
:description: Learn how to set up the plentymarkets app on the appropriate hardware.

Do you want your business to xref:app:app.adoc#[profit from the plentymarkets app]? Don’t lose any time! Start setting up the app on the appropriate hardware.

This page guides you through the installation process step-by-step and helps you complete the tasks that need to be done in advance. The settings described on this page go into effect when you begin xref:app:functions.adoc#[using the app functions].
...
In addition, the plentymarkets app allows you to perform stock management tasks and to use the point of sale system plentyPOS. We recommend that you book a link:https://marketplace.plentymarkets.com/en/services/hardware[hardware package^] if you want to use these additional functions.

[#200]
=== Case 1: Managing company data

include::partial$hardware-key-figures.adoc[]
...
Do all of your employees have a plentysystems account? If not, log into plentysystems and xref:business-decisions:user-accounts-access.adoc#10[create the necessary accounts].

Your employees will later xref:app:installation.adoc#1200[log into the app] with the user name and password you’ve created for them.

[TIP]
.Which rights do app users have?
====
Admin users automatically have the right to access all plentysystems functions without limitation. Think twice before you create admin accounts because they will also have the right to create, manage and edit other user accounts. +
If you want to decide for yourself which functions and menus should be visible, we recommend that you create user accounts without activating one of the options *Administrator* or *Only API user*. Later on, you can use roles to assign the required rights to those user accounts.
====
...
In plentysystems, you can decide which menus and key figures should be visible for each user account.
...
. Click on *Rights*.
. Go to the option *plentymarkets App / plentyPOS*.
. Activate the functions and key figures that the role should have access to in the app. Pay attention to the explanations given in <<table-app-rights>>.
...
| *Visible functions*
|Which xref:app:functions.adoc#[app functions] should your employees see? Place check marks to specify which exact menus this user should see.

| *Incoming items*
|Should your employees be able to see and edit the purchase price when xref:app:receiving-rebooking.adoc#[booking in items]? Place check marks to grant the rights. +
This area is not activated unless you’ve checked the *Incoming items* option under *Visible functions*.

| *POS functions*
|Which xref:pos:pos.adoc#[POS functions] should your employees be able to use? Place check marks to specify the exact functions the back end user should work with. +
This area is not activated unless you’ve checked the *POS* option under *Visible functions*.

| *Visible key figures*
|Which xref:app:key-figures.adoc#[key performance indicators] should your employees see? Place check marks to specify the exact data the user should be able to see.
|====

[#900]
=== Assigning an app PIN

Will several employees share one device? Constantly logging in and out can quickly waste a lot of time. In such situations, we recommend having employees xref:app:installation.adoc#1700[use a PIN to switch from one account to another] instead of having them log in and out with their user name and password. If all of your users have their own PIN, they can quickly switch to a different user account in the plentymarkets app.
...
Your link:https://www.plentymarkets.com/product/pricing/[plentysystems edition^] specifies how many users can log into plentysystems at the same time. This limit applies to your plentysystems back end and to the plentymarkets app.

You can do the following if you reach the user limit:

* Ask users to log out manually.
* xref:business-decisions:user-accounts-access.adoc#170[Log out users in the session management menu].
* xref:business-decisions:user-accounts-access.adoc#190[Increase the user limit of your plentysystems contract].
...
The plentymarkets app will only work if you’ve link:https://www.plentymarkets.com//[booked a plentysystems ERP system^]. In other words, you’ll log in with the access data that you use for your plentysystems system.
...
. Enter the login details for your plentysystems system. Pay attention to the explanations given in <<table-app-login>>.
. Tap **Login**. +
→ The start screen is displayed. +
→ You will remain logged in until you either xref:app:installation.adoc#1500[log out] or xref:app:installation.adoc#1600[lock] your user account. That means that you can open and close the app without having to log in again.
...
| *User*
|Which user account do you want to log in with? xref:business-decisions:user-accounts-access.adoc#30[Enter the user name] that you were initially given.
...
| *Password*
|xref:business-decisions:user-accounts-access.adoc#30[Enter the password] that you were initially given.
|====
...
* The pencil icon (icon:pencil[role="blue"]) in the top right corner. Tap on this symbol to set up xref:app:key-figures.adoc#[key performance indicators like revenue and sales figures] on the start screen.

* The menu icon (icon:bars[role="blue"]) in the top left corner. Tap on this symbol to access the main menu. From here, you can view your xref:app:item-search.adoc#[item, customer and order data]. You can use the app as an xref:app:warehouse-management.adoc#[inventory management system]. Or you can xref:pos:pos.adoc#[ring up customers in your brick and mortar store].
...
* The user icon (icon:user[role="blue"]). This symbol shows who is currently logged in. Tap on this symbol to xref:app:installation.adoc#1600[lock the screen], xref:app:installation.adoc#1700[switch to a different user account] or xref:app:installation.adoc#1500[log out of the app altogether]. +
This symbol displays the initials of whoever is currently logged in. It is whatever colour was chosen for this user account in the plentysystems back end.
...
|Use the toggle button to tell plentysystems whether you want to re-enter your access data the next time you log in. +
...
→ You will see a list of users who have xref:app:installation.adoc#900[an app PIN].
...
You can create templates for several different app functions. For example, you can decide how your xref:app:item-search.adoc#[item and order data] should be displayed in the app or what your mobile pick lists should look like.
...
Scroll up and down through xref:app:installation.adoc#1900[the newly created template]. Do you see both of the headings? Each heading corresponds to a different view. For example, one area might control how information is displayed in a list of search results. And the other area might control the detailed view.
...
plentyBase is a tool that enables your local computer to communicate with plentysystems.
In other words, plentyBase establishes an interface that allows you to use the printers that are connected to the computer in the plentysystems back end and/or the plentymarkets app.

For example, you need to connect the app to plentyBase if you want to:

* xref:app:item-search.adoc#400[Print item labels with the plentymarkets app]
* xref:pos:integrating-plentymarkets-pos.adoc#1000[Print receipts with your local printer instead of with a special receipt printer]
...
. Install plentyBase onto your computer. Carry out the settings as described in the xref:automation:installing-plentybase.adoc#[plentyBase documentation]. +
...
. Carry out the settings as described in the xref:automation:installing-plentybase.adoc#[plentyBase documentation].
...
If your internet connection is down, POS orders are saved on your device and uploaded to plentysystems once connection returns. If you uninstall the app on your device before all orders were uploaded to plentysystems, the orders saved on your device are deleted permanently and cannot be recovered by you or the plentysystems support team. As such, check that no offline orders exist on the device before you uninstall the app.
...
--
Make sure that you’ve correctly entered the xref:app:installation.adoc#1200[base URL for your plentysystems system].
Enter the domain without https:// or http://. If you don’t know the URL, log into your plentysystems back end. Then, move the cursor over the shop symbol with your plentyID on the upper right. The URL is listed in the second line of the tooltip that is displayed. +
