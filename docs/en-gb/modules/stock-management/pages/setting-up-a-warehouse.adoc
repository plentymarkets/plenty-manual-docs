...
|Determine whether you want to split by xref:fulfilment:preparing-the-shipment.adoc#1000[shipping profile]. In case you are working with xref:orders:managing-orders.adoc#300[Delivery orders] select *Yes* here.
...
| *Value*
|Select a value. This field is only available, if the property is a xref:item:properties.adoc#property-details[selection property].
|====
...
Box storage racks are used as interim storage for <<glossary#_m, multi order picking>>. Box storage racks are only available in xref:automation:processes.adoc#[processes].
...
The following sub-chapters describe how to xref:stock-management:setting-up-a-warehouse.adoc#900[create], xref:stock-management:setting-up-a-warehouse.adoc#950[link], <<#group-properties, group>> and <<#search-property, search>> and how to <<#delete-property, delete>>, <<#import-properties, import>> and <<#export-properties, export>> properties.
