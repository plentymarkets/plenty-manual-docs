...
|In case you are working with xref:orders:managing-orders.adoc#300[Delivery orders] select *Yes* here.
...
The calculation methods are described in depth in the chapter xref:stock-management:new-incoming-items.adoc#250[Identifying the purchase price].
...
Box storage racks are used as interim storage for multi order picking. Box storage racks are only available in xref:automation:processes.adoc#[Processes].
