...
| *Text to speech*
|image:stock-management:icon_speaker.png[width=24, height=24]
|Activate (icon:toggle-on[role="green"]) this option so that the app provides additional spoken instructions while using the functions of plentyWarehouse. The text to speech functionality contains warnings, instructions and confirmation messages. Depending on the workflow or the use case, this can be useful help for your warehouse staff. +
To adjust the text to speech function to your requirements, use the setting *Item information* to determine which item information should be included in the text to speech output. If you do not adjust this setting, the option *Variation name* is preselected by default. Alternatively, select one of the options *Item name*, *Variation name* or *Variation ID*. If no item information should be included in the text to speech output, select the option *No selection*. +
*_Note:_* This setting takes effect for the text to speech output of all plentyWarehouse functions.
