...
|How do you want to flag orders that are currently being picked? As soon as the picking process is started in the app, orders are moved to this status so that it's not possible to accidentally start picking the same order twice. +
*_Tip:_* You can manage the xref:orders:managing-orders.adoc#1200[order statuses] displayed in this drop-down list in the *Setup » Orders » Status* menu in the back end of your plentymarkets system.

| [#intable-success-status]*Success status*
|Select the status into which orders should be moved after all the order items have been picked successfully, meaning the picking process is finished.
