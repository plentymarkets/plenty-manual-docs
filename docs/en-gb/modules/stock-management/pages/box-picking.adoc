...
[#300]
=== Carrying out settings in the plentymarkets back end
...
.Settings in the plentyWarehouse app
[cols="1,3"]
|====
|Setting |Explanation
...
|Each box represents one order whose items are picked. Select the number of orders you typically pick at once. When you use the function later, this number will be suggested by default. You can manually increase or decrease this number before starting box picking.
...
For such cases, the view on each item contains shortcuts that allow you to carry out these tasks quickly. <<table-box-picking-shortcuts>> contains additional information on the available shortcuts.
...
Tap on icon:done[set=material] to finish box picking.
