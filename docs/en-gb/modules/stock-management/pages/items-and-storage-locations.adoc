...
After you have <<#600, searched>> for storage locations, a shortcut is displayed for each entry in the list of results. This shortcut allows you to directly print a label for the storage location. Tap on icon:print[set=material] in the entry for the desired storage location to start printing a storage location label.
