...
| [#intable-incoming-items-reason]*Reason for incoming items*
...
| [#intable-default-behaviour-reorders]*Standard behaviour when loading reorders* area
...
2+^| [#intable-finish-incoming-items]*Finish incoming items* area
...
| [#intable-show-summary]*Show summary*
|Activate this option to display an overview of all important data after you have finished booking incoming items. If you tap on this option, you are forwarded to an additional settings menu. There, you select if the overview should include *Target storage locations*, *Incoming items receipts* or both information types. For further information, refer to the chapter <<#500, Displaying an incoming items summary>> below.
...
* Incoming items receipts are generated based on the xref:orders:document-builder.adoc#[DocumentBuilder]. Thus, you need to create a template for the document type *Incoming items receipt* beforehand. This step is carried out in the *Setup » Documents » DocumentBuilder* menu in the plentysystems back end. For instructions on how to create DocumentBuilder templates, please refer to xref:orders:documentbuilder.adoc#create-template[this manual page].
...
[#500]
== Displaying an incoming items summary

Do you want to display a summary of all important data regarding the incoming items process after you have finished booking incoming items? In that case, you only need to carry out a few settings to make sure that information on the affected storage locations, reorders and much more are provided as part of a central view directly in the plentyWarehouse app. To do so, proceed as described below.
...
The settings menu opens.
. Tap on *Incoming items* image:stock-management:icon_incoming_items.png[width=24, height=24].
. Scroll down until you see the *Finish incoming items* area.
. Tap on *Show summary*. +
→ Another settings menu opens. +
. Determine if information regarding storage locations and incoming items receipts should be displayed by activating the respective options. +
*_Note:_* If you do not activate any of the options *Storage locations* and *Incoming items receipts*, the summary will not be displayed.

After you have carried out the settings described above, the following information is displayed in a separate view as soon as you have tapped on *Finish incoming items* for an incoming items process:

* Number of reorders affected (if applicable)
* Number of storage locations
* Number of items booked in, including a shortcut for xref:stock-management:printing-labels.adoc#[printing barcode labels] for those items (material:print[role="green"])
* Names of the storage locations (if activated in the settings), including shortcuts to the functions xref:stock-management:items-and-storage-locations.adoc#500[Inspect storage location] and xref:stock-management:reshelving-items.adoc#[Reshelving items] for the respective storage location
* Number of generated incoming items receipts (if activated in the settings), including a shortcut for printing the receipts (material:print[role="green"])
