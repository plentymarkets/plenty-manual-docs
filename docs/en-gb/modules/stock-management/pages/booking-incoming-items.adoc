...
Now, assign rights so that the function for incoming items is visible and usable for the respective users of the plentyWarehouse app. If needed, also assign the rights for displaying and editing purchase prices and procurement costs and for displaying order information for reorders.

:rights-purpose: use the *Incoming items* function in plentyWarehouse

:right-one: pass:quotes[*plentyWarehouse > Incoming items > Incoming items > Edit*]
:right-two: pass:quotes[*plentyWarehouse > Incoming items > Purchase price > See*]
:right-three: pass:quotes[*plentyWarehouse > Incoming items > Purchase price > Edit*]
:right-four: pass:quotes[*plentyWarehouse > Incoming items > Procurement costs > Edit*]
:right-five: pass:quotes[*plentyWarehouse > Incoming items > Procurement costs > Edit*]
:right-six: pass:quotes[*plentyWarehouse > Order information > See*]
...
Typically, reorders whose items should be booked in are in specific xref:orders:managing-orders.adoc#1200[order statuses]. Which statuses are used for this depends on the individual workflow of your company.

To register incoming items without any errors and access all reorders, you have to assign rights for displaying and editing those exact order statuses in the plentymarkets back end.
...
| *Order status*
|Activate the order statuses that you normally assign to reorders in your plentymarkets system. This is necessary to make sure that orders in this status are loaded into the plentyWarehouse app properly when searching for reorders. Also, keep in mind to <<#160, assign rights for those order statuses>>.  In case you have not yet created statuses for reorders, refer to the xref:orders:managing-orders.adoc#1200[Managing orders] page of the manual.
