...
If an item is returned by a customer, you can create a xref:orders:managing-orders.adoc#600[warranty] for one or more items by opening the original order. These warranties are treated as individual orders with a distinct order number. Stock is reserved until either variations are booked out or until the warranty itself is cancelled again.
...
For further info on the order status see xref:orders:managing-orders.adoc#1200[here].
====
