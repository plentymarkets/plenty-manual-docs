...
Within the font group *Extended alphabets* you can choose between the options *East Asian*, *Arabic* and *Cyrillic* if you wish to set up document templates for <<data/internal-IDs#10, languages >> such as Chinese or Russian.
...
Activate the option *Title* if the title saved in the *Addresses* in the <<crm/managing-contacts#100, contact data>> are to be positioned in front of the contact name. +
Activate the option *Contact person* if the information on the contact person saved in the <<crm/managing-contacts#100, contact data>> are to be included in the delivery address.
...
*Attribute values*: Select for the attribute values whether they are displayed in the language just chosen or whether the description given in the <<item/settings/attributes#100, Name (internal)>> field is to be used for the reorder document.
...
You can find more information about document templates, for example linking a PDF template, in the manual section <<orders/order-documents#1700, Order documents>>.
...
====
You can only enter a reorder level if you have assigned a warehouse to the variation. If you have not yet <<item/managing-items#312, assigned a warehouse>>, do so beforehand.
====
...
====
You can only enter a minimum order quantity if you have assigned a supplier to the variation. If you have not yet <<item/managing-items#310, assigned a supplier>>, do so now.
====
...
*Back end* type users <<business-decisions/user-accounts-access#105, can only see certain areas and menus>> in plentymarkets. This means they only have limited access to the system. If you want *back end* type users to create and process reorders, you have to grant them the following rights:
...
For further information about the messenger, refer to the <<crm/messenger#, Messenger>> page of the manual.
...
|When you select this option, only <<item/online-store/checklist-items-visibility#100, active items>> are displayed in the search results.
...
.Setting up an email template
====
Before using the email service for processing reorders, you have to set up an email template for this purpose in the *Setup » Client » [Select client] » Email » Templates* menu.  For further information, refer to the <<crm/sending-emails#1200, Emails>> page of the manual.
====
...
No dates are copied, you can set these in the newly created reorder. The *owner* is not copied as well. The current user is set as the owner of the reorder. The *status* is set to the <<stock-management/managing-reorders/working-with-reorders#120, default status for reorders>> defined by you.

The behaviour of the new reorder is the same as that of other reorders. You can edit the reorder as you are used to until you set the <<stock-management/managing-reorders/working-with-reorders#1000, finish date>>.

[#1100]
== Searching for reorders

When opening the *Stock » Reorder management » Purchase order* menu, all existing reorders and <<stock-management/working-with-redistributions#, redistributions>> are displayed. Carry out a search (icon:search[set=material]) to limit the search results to the required redistributions.
