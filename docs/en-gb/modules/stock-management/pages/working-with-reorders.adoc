...
. Move (icon:drag_sort_handle[set=plenty]) the columns so they are displayed in the order you need them.
...
Activate the option *Title* if the title saved in the *Addresses* in the xref:crm:edit-contact.adoc#details-individual-areas[contact data] are to be positioned in front of the contact name. +
Activate the option *Contact person* if the information on the contact person saved in the xref:crm:edit-contact.adoc#details-individual-areas[contact data] are to be included in the delivery address.
...
*Attribute values*: Select for the attribute values whether they are displayed in the language just chosen or whether the description given in the xref:item:attributes.adoc#100[Name (internal)] field is to be used for the reorder document.
...
_Important:_ For internal reasons this field is marked as a mandatory field. But displaying it on the documents is *optional*. When you set the position to *Pos*, this field is not displayed on the document.
...
====
You can only enter a reorder level if you have assigned a warehouse to the variation. If you have not yet xref:item:managing-items.adoc#312[assigned a warehouse], do so beforehand.
====
...
====
You can only enter a minimum order quantity if you have assigned a supplier to the variation. If you have not yet xref:item:managing-items.adoc#310[assigned a supplier], do so now.
====
...
For further information about the messenger, refer to the xref:crm:messenger.adoc#[Messenger] page of the manual.
...
. *_Optional_*: Select one or several *Tags* to mark the new reorder.
. *_Optional_*: Select an *Owner*, meaning the user responsible for the reorder.
...
icon:map-signs[] *_Or_*: If not all order items should be reordered, select the order items that you wish to reorder and click on *Reorder selected items*. +
→ The reorder is created and is displayed and editable in the *Stock » Purchase order* menu.
...
In chapter 3, several methods for creating a reorder in your plentymarkets system have been described. Now, you can enter further variations to the reorder by adding order items. Carry out a search (icon:search[set=material]) to find the required order items and to add them to the reorder.
...
Remove the chip if you want to delete a set filter. Available filters are explained in <<table-filter-item-search-reorder>>.
...
|When you select this option, only items with stock are displayed.
...
_Note_ that all delivery dates in the table are deleted if you delete an already set delivery date in the order details. It is not possible to set a delivery date that is earlier than the order initiated date. +
...
* Procurement costs in total = The total of procurement costs is displayed. Grey means that the total is zero. Red means that the total is above zero.
...
Alternatively, you can manually set a date with the date picker (icon:calendar[]). After selecting a date, click on *Initiate order*. The selected date is set as the order date and the reorder is initialised.
...
icon:map-signs[] *_Or_*: Select the option *Upload reorder document* to upload a delivery note generated by and sent to you by your supplier. +
...
. Click on the *eye* (icon:eye[role="blue"]) to open the preview.
...
Click on *Add cost* when everything is set and the costs are correct. Note that the costs are *not saved yet*. Click on *save* (icon:save[role="green"]) in the reorder to save the added costs and thus to update the reorder.
...
Alternatively, you can manually set a date with the date picker (icon:calendar[]). After selecting a date, click on *Finish reorder*. The selected date is set as the finish date and the reorder cannot be edited anymore. +
...
.Unlocking a reorder
====
In case you have set the finish date by mistake, unlock the reorder by clicking on *Unlock purchase order* (icon:undo[role="orange"]). This way, the reorder is unlocked and can be edited again.
====
...
No dates are copied, you can set these in the newly created reorder. The *owner* is not copied as well. The current user is set as the owner of the reorder. The *status* is set to the xref:stock-management:working-with-reorders.adoc#120[default status for reorders] defined by you.
...
When opening the *Stock » Reorder management » Purchase order* menu, all existing reorders and xref:stock-management:working-with-redistributions.adoc#[redistributions] are displayed. Carry out a search (icon:search[set=material]) to limit the search results to the required redistributions.

There are several ways of using the search. You can enter a value in the search field and then select the required filter. When you enter a number or a letter, available filters are suggested, e.g. order ID or variation number. Enter the value in full and then select the required filter from the suggestions. Repeat this to combine filters. Click on *Search* (icon:search[set=material]) to carry out the search. +
If you first want to select a filter from the filter list, click on *Filter* l(icon:tune[set=material]). The available filters are displayed. Enter a value in the required filter. Click on *Search* once you have set all needed filters. +
Remove the chip if you want to delete a set filter. Available filters are explained in <<table-reorder-search>>.
...
| *Tags*
| Select a xref:stock-management:working-with-reorders.adoc#130[tag] to search for reorders to which this tag was assigned. You can select several tags. This is an "or" search. This means that all reorders with at least one of the selected tags assigned are shown in the search results if you select more than one tag.
