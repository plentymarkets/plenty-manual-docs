...
A certain number of statuses are saved in your system by default. Those default order statuses can also be used for redistributions. You can create statuses that are exclusively used for this particular order type. Creating new order statuses is described in detail on the xref:orders:managing-orders.adoc#1200[Managing orders] page of the manual.
