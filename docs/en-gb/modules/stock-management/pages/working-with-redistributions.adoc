...
:description: Learn how to work with redistributions from warehouse to warehouse in your plentymarkets system.
:keywords: redistribution, warehouse redistribution, PO delivery note, create PO delivery note, redistribution id, purchase order
:position: 50
:id: A5QIHYW
...
| *Default status*
|Select a Default status that should be preselected when creating a redistribution. How to create statuses for redistributions is explained in chapter <<stock-management/working-with-redistributions#200, Creating statuses for redistributions>>.
...
|Activate this option for it to be preselected in the item search of the redistribution. It means that only <<item/online-store/checklist-items-visibility#100, activated items>> are displayed in the search results.
...
A certain number of statuses are saved in your plentymarkets system by default. Those default order statuses can also be used for redistributions. You can create statuses that are exclusively used for this particular order type. Creating new order statuses is described in detail on the <<orders/managing-orders#1200, Managing orders>> page of the manual.
...
If you want to individualise the *PO delivery note*, for example by displaying your company logo or further information, proceed according to the instructions provided on the <<orders/order-documents#300, order documents>> page of the manual.
...
. Decide whether this is part of a service and is to be billed by selecting *Billable* and entering the corresponding minutes.
...
.Selecting an owner
====
If the required owner is missing, check if the user has been activated as an owner. For general information about user accounts, refer to the <<business-decisions/user-accounts-access#40, User accounts and access>> page of the manual. In addition, check the <<#400, default settings>> for owners described above.
====

[#700]
=== Adding order items

The next step consists of adding the items that should be redistributed from one warehouse to another. Carry out a search (icon:search[set=material]) to find the required order items based on the items in stock.
...
. Click on *Add* (icon:add[set=material]) in the *Order item* area. +
...
. Click on the *Add to cart* (icon:add_shopping_cart[set=material]). +
...
In the *Item name* field, you can change the name of the item. The adaptations are only valid for the redistribution they are done in. If you want to adapt the <<item/settings#, item setting>> in general, you have to do so in the *Item » Edit item* menu.
...
====
Redistributions can be deleted in the detail view as long as the redistribution has not been initiated by setting the order initiated date. To delete a redistribution, click on *Delete purchase order* (icon:delete[set=material]) in the right-hand corner above the info boxes.
====
...
. Click on *Add document* (icon:add[set=material]).
...
. Click on *Create stock movements* (icon:execute[set=plenty]). +
...
. Click on *Add* (icon:add[set=material]). +
...
When you go back to the redistribution after this, the created package content list is displayed in the corresponding area of the redistribution. In case you want to add further pallets or packages, click on *Edit* (icon:edit[set=material]) and you are redirected again to the package content list. If a redistribution is already finished, the package content list cannot be edited anymore. You can find more information about <<orders/package-content-list#, package content lists>> on the corresponding manual page.
...
hen opening the *Stock » Reorder management » Purchase order* menu, all existing redistributions and <<stock-management/managing-reorders/working-with-reorders#, reorders>> are displayed. Carry out a search (icon:search[set=material]) to limit the search results to the required redistributions and reorders.
