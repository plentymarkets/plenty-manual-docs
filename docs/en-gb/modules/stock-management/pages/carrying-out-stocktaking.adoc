...
In addition to using the functionality in the xref:stock-management:taking-stock.adoc#[plentymarkets back end], you can also take stock with the plentyWarehouse app. This manual page contains all information on the necessary preparatory settings and the process of taking stock with plentyWarehouse.
...
Do you want to check all stock changes at once after completing stocktaking? Then start by using the export format xref:stock-managemen:taking-stock.adoc#200[catalogue format *Stocks*] to export your current stock.
...
*_Note:_* This setting only takes effect if the *Mandatory item scan* setting is _not_ activated.
...
*Deactivated* (icon:toggle-off[role="darkGrey"]) = The counted stock for a storage location is saved automatically if you scan the xref:stock-management:setting-up-a-warehouse.adoc#800[storage location label] once again after counting all items.
