...
:description: In addition to using the functionality in the plentymarkets back end, you can also take stock using the plentyWarehouse app. This page shows you how it’s done.

include::partial$note-requirements.adoc[]

In addition to using the functionality in the xref:stock-management:taking-stock.adoc#[plentymarkets back end], you can also take stock using the plentyWarehouse app. This manual page contains all information on the necessary preparatory settings and the process of taking stock with plentyWarehouse.
...
. Tap on *Stocktaking* (image:stock-management:icon_stocktaking.png[width=24, height=24]).
...
|Use this setting to determine how many copies of the item have to be scanned or registered manually until the quantity dialogue opens. Then, you can use the input field within the quantity dialogue to enter the total quantity of all counted copies of this item and thus register it for stocktaking. +
*_Example:_* If you select the value *5* here, the quantity dialogue is automatically opened after the fifth copy of the item is registered.
...
Before carrying out stocktaking in plentyWarehouse, make sure that all users without admin accounts have been assigned the appropriate rights. Otherwise, the counted stock cannot be registered and saved correctly both in the plentyWarehouse app and in the plentymarkets back end.
