...
Within the catalogue, use the filter for the <<data/exporting-data/catalogues-stock-movements#30\. sum of stock movements>> to group the entries for a variation. Use an additional filter to select the xref:data:catalogues-stock-movements.adoc#30[date range] in which the stocktaking process took place.
