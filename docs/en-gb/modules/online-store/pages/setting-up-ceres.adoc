...
:position: 10
:url: online-store/setting-up-ceres
:id: GW5QHSI

link:https://marketplace.plentymarkets.com/plugins/templates/Ceres_4697[plentyShop LTS]{nbsp}icon:external-link[] is the official template plugin for the default online store of plentymarkets: plentyShop. In plentyShop, design and logic are separated from each other. The shop is integrated into your plentymarkets system via 2 plugins.
The *plentyShop LTS* plugin contains plentyShop's standard design and can be customised to meet your needs. The link:https://marketplace.plentymarkets.com/plugins/templates/IO_4696[IO^]{nbsp}icon:external-link[] plugin contains the logic of plentyShop, is a general basis for all design plugins and can also be used by other plugins.
...
[.instruction]
<<welcome/quick-start#540, purchased>> in the plentyMarketplace
- <<plugins#90, installed>> in the *Plugins » Plugin overview* menu
- <<plugins#60, deployed>> in the *Plugin » Plugin overview* menu.

You can find detailed instructions in the link:https://marketplace.plentymarkets.com/plugins/templates/Ceres_4697[plugin guides^]{nbsp}icon:external-link[].
...
Set up the default shipping method for your plentyShop. This shipping method will be used for calculating the shipping costs in the shopping cart before a customer registers in your plentyShop. The default shipping method is preselected in the checkout. For further information about shipping methods and shipping profiles, refer to the <<fulfillment/preparing-the-shipment#1000, Creating a shipping profile>> manual chapter.
...
Set up the default payment method for your plentyShop. Only link:https://marketplace.plentymarkets.com/plugins/payment[Payment method plugins^]{nbsp}icon:external-link[] can be used in *plentyShop LTS*. The payment method plugins will be available as default payment methods after the setup. Please refer to the corresponding *User guide* of the payment method plugin in the plentyMarketplace.
...
| *B2B customer class*
|Enter the ID for the default customer class for business customers. For further information, refer to the <<crm/managing-contacs#15, Creating a customer class>> chapter of the manual.

| *Enable Callisto route pattern for items*
|Activate to use the Callisto URL route pattern in plentyShop LTS. This is required, for example, when using the <<omni-channel/online-store/best-practices#checkout, plentyShop LTS checkout in Callisto>>. We advise you not to activate this setting if you use an unaltered plentyShop plentyShop LTS.

| *Contact form email address*
|Enter the email address, to which customers can send the contact form.

| *Show contact data*
|Select one, multiple or *ALL* contact information options to be displayed in the contact data in the plentyShop. The contact information activated here are loaded from the <<basics/first-steps/master-data, master data>> of your plentymarkets system.

| *Google reCAPTCHA Website key*
|Enter the Google reCAPTCHA Website key in order to integrate a Google reCAPTCHA into your contact form. You receive this key when registering your website for link:https://www.google.com/recaptcha/admin#list[Google reCAPTCHA^]{nbsp}icon:external-link[]. Note that you have to select a reCAPTCHA of the type *Hidden reCAPTCHA* during setup. The reCAPTCHA validates the visitor of your plentyShop in the background and remains invisible for your customers. Please note that the Google reCAPTCHA necessitates the inclusion of a reference in your privacy policy.

| *Google Maps API key*
| Enter the API key from Google Maps, in order to display the location of your store on a Google map. For this, you need an link:https://developers.google.com/maps/documentation/javascript/get-api-key[API key^]{nbsp}icon:external-link[] for Google Maps. Alternatively, enter 0 to hide the map.
...
You customise the homepage of your plentyShop with the <<omni-channel/online-store/shop-builder#10, ShopBuilder>>. Deactivate the default homepage to replace it with the ShopBuilder homepage.
...
Please refer to the ShopBuilder <<omni-channel/online-store/shop-builder#10, manual page>> for further information on how to set up the ShopBuilder.
...
[NOTE]
====
The settings for header and footer are only applicable for Ceres versions up to and including 2.14. Beginning with Ceres 2.15, header and footer are set up via the <<omni-channel/online-store/shop-builde#10, ShopBuilder>>.
====
...
*Technical data* = Activate to display the *Technical data* tab. If you activate the display of *Technical data* without saving a text in the *Technical data* field in the <<item/managing-items#50, Texts>> tab of an item, the tab will not be displayed. +
*Description* = Activate to display the *Description* tab in the item overview in the shopping cart. If you activate the *Description* without saving a text in the *Description* field in the <<item/managing-items#50, Texts>> tab of an item, the tab will not be displayed. +
...
*Dynamically* = Select to display variations dynamically. The listed variations include both active main and child variations. The display of variations in the item list is dependent on the selected sorting option in the front end.
...
* Store specials: Lists with new items, special offers, etc. are set up in plentyShop LTS via tags. For further information about setting up tags, refer to the <<welcome/general-functions/tags#, Tags>> page of the manual. For further information on tagging items, refer to the <<item/managing-items#150, Adding tags to items>> chapter of the manual.
* Cross-selling: Cross-selling items are displayed in plentyShop via *relations*. For further information, refer to the <<item/managing-items#950, Setting up cross-selling>> chapter of the manual.
...
Please find a complete list and explanation of the available template containers in our english link:https://developers.plentymarkets.com/dev-doc/template-containers[developers documentation^]{nbsp}icon:external-link[]. This documentation is primarily targeted at experienced users and is not necessary for the basic setup of Ceres.
...
*Item ID* = Activate to display the item's ID. +
*Item text* = Activate to display the item description. If you activate the *Item text* without saving a text in the *Item text* field in the <<item/managing-item#50, Texts>> tab of an item, the item description will not be displayed. +
*Preview text* = Activate to display the preview text of the item. +
...
|Enter the default status for returns. For further information about order statuses, refer to <<order/managing-orders#1200, Order status>>.
...
|Activate currencies available to customers in the header of plentyShop. For further information about supported currencies, refer to the <<payment/currencies#10, Supported currencies>> manual page.
...
You can carry out settings pertaining to trailing slashes of the URLs of your plentyShop in the *Setup » Client » Select client » SEO » URL structure* menu.

[#2041.instruction]
Editing trailing slashes:

. Go to *Setup » Client » Select client » SEO » URL structure » Other*.
...
For further information about *attributes*, refer to the <<item/managing-items#390, Managing attributes>> chapter.
...
For further information about *facets*, refer to the <<omni-channel/online-store/setting-up-ceres/extras/faceted-search#100, Creating facets>> chapter.

You can find detailed instructions about setting up filters in our <<omni-channel/online-store/best-practices#filter, Best Practice>>.

[#340]
=== Setting up order characteristics

With order properties, you allow your customers to enter a text when ordering an item that is linked to the order characteristic. For example, customers choose an individual text that will be printed on an item. Set up order characteristics first and link the characteristics with your items. For further information about characteristics, refer to the <<item/use-cases/personalised-items#240, Creating an order characteristic>> chapter.

You can find detailed instructions about setting up order characteristics in our <<omni-channel/online-store/best-practices#orderproperties, Best Practice>>.

[#650]
=== Automatically sending order confirmations

Set up an <<automation/event-procedures#, event procedure>> to automatically send your customers an order confirmation.
...
You want to certify your plentyShop with Trusted Shops? plentyShop LTS has been pre-certified by Trusted Shops, so your Trusted Shops certification will be handled as fast and easy as possible. Find all necessary information link:https://www.trustedshops.de/shopbetreiber/bestellen.html?shopsoftware=PLENTYMARKETS[here^]{nbsp}icon:external-link[].

[#960]
== Activating the ShopBooster 

link:https://www.plentymarkets.eu/blog/Volle-Kraft-voraus-ShopBooster-bringt-deinen-Ceres-Shop-auf-Hochtouren/b-2125/[ShopBooster^]{nbsp}icon:external-link[] is an additional module, via which online shop content of plentyShops are written into the cache in order to achieve better loading times. The ShopBooster is available for Ceres versions 2.14 and higher.

[IMPORTANT]
.Additional costs
====
Activating the ShopBooster entails additional costs in the amount of 0,004€ per 100 page accesses and changes (per client). A page access is the process in which a visitor or web crawler accesses a URL of your online shop; a page change is effected by one of the following processes:

- Updating category of item data
- Updating ShopBuilder pages
- Saving and deploying plugin sets
- Saving settings in the plentyShop LTS configuration
- Invalidating the cache by deactivating the ShopBooster

The additional costs of the ShopBooster module amount to a *maximum* of 100 € per month per client.
====

[.instruction]
Activating the ShopBooster:

. Go to the *Setup » Client » Select client» Webshop » ShopBooster* menu.
- Click on *Activate ShopBooster* +
→ A window is opened, informing you about additional costs.
. Click on *Confirm* to activate the ShopBooster. +
→ The message *ShopBooster has been activated* is displayed.

You can deactivate the ShopBooster in the same way.

[.instruction]
Deactivating the ShopBooster:

. Go to the *Setup » Client » Select client» Webshop » ShopBooster* menu.
- Click on *Deactivate ShopBooster* +
→ A window is opened, informing you about additional costs.
. Click on *Confirm* to deactivate the ShopBooster. +
→ The message *ShopBooster has been deactivated* is displayed.

=== Deactivating the invalidation

In the *Deactivate invalidation*, you can suspend the invalidation of the ShopBooster until the chosen date and time. As a result, your item data will be exclusively loaded from the cache until the chosen time, without the cache being deleted or updated. Therefore, your plentyShop will be able to achieve better loading times. +

Note that deactivating the invalidation will also affect the visibility of items in your online shop: The visibility of your items will no longer be automatically updated as long as the invalidation is inactive; it is therefore possible, that items appear as available even though they are already sold out. However, it is not possible for your customers to purchase items that are not available. + 

[.instruction]
Deactivate the invalidation:

. Go to the *Setup » Client » Select client» Webshop » ShopBooster* menu.
. Enter the date until which you want to deactivate the invalidation.
. Enter the time until which you want to deactivate the invalidation.
. *Save* your inputs.
...
*plentyShop LTS* is a public project and available on link:https://github.com/plentymarkets/plugin-ceres[GitHub]{nbsp}icon:external-link[]. You can use the plugin code to create your own template plugin. Please note the following when creating your own template on the basis of *plentyShop LTS*:

* Register with link:https://github.com/join?source=header-home[GitHub^]{nbsp}icon:external-link[]
* Open the link:https://github.com/plentymarkets/plugin-ceres[plentyShop LTS project^]{nbsp}icon:external-link[]
* Create your own fork of the project
* Customise the source code

For further information about changing the source code, refer to our link:https://developers.plentymarkets.com/dev-doc/template-plugins[template guide^]{nbsp}icon:external-link[].
...
Theme plugins are an easy way of customising the looks of *plentyShop LTS*. Various link:https://marketplace.plentymarkets.com/plugins/themes[themes^]{nbsp}icon:external-link[] for *plentyShop LTS* are available on plentyMarketplace. Changes made to the theme plugin have no impact on future updates of the template plugin *plentyShop LTS*.

[#1300]
=== Creating an individual theme

Learn how to create your own theme plugin and how to fully customise *plentyShop LTS* in our link:https://developers.plentymarkets.com/dev-doc/theme-plugins[theme guide^]{nbsp}icon:external-link[].

[#1400]
=== Integrating additional plugins

In addition to theme plugins, which serve to modify the design of plentyShop, the link:https://marketplace.plentymarkets.com/[marketplace^] offers a multitude of different plugins which can be used to tailor the functionality of plentyShop to your personal needs. Apart from the integration of link:https://marketplace.plentymarkets.com/plugins/markets[marketplaces^], you can find link:https://marketplace.plentymarkets.com/plugins/widgets[widgets^], such as the GoogleAnalytics and customer feedback plugins, link:https://marketplace.plentymarkets.com/plugins/payment[payment plugins^] for the implementation of additional payment methods and link:https://marketplace.plentymarkets.com/plugins/integration[integration plugins]{nbsp}icon:external-link[], such as the plugins of shipping providers DHL and DPD. +
...
. Open the link:http://marketplace.plentymarkets.com/de/[plentyMarketplace^]{nbsp}icon:external-link[].
. Click on *Sign in*.
