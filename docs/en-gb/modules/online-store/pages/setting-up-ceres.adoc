...
The link:https://www.plentymarkets.eu/blog/Volle-Kraft-voraus-ShopBooster-bringt-deinen-Ceres-Shop-auf-Hochtouren/b-2125/[ShopBooster^] is an additional module, via which online shop content of plentyShops is written into the cache in order to achieve better loading times.
...
Using cache blocks ensures that contents that have already been cached are not invalidated unnecessarily. Cache blocks divide the page contents into blocks which can be invalidated separately. Contents that are identical on several pages (e.g. the footer) can thus be updated once and replaced on all cached contents without the need to completely invalidate all pages.

This leads to a better cache coverage and on average also to a better Time-to-first-Byte (TTFB) which can have a strong impact on PSI.

[NOTE]
.Adaptations for additional theme
====
Cache blocks are directly compatible with plentyShop LTS and plentyShop LTS Modern. If you are using an additional theme, you need to adjust it so that cache blocks have an effect. Further information on that can be found in our link:https://developers.plentymarkets.com/en-gb/developers/main/plentyshop-plugins/shopbooster-cache-blocks.html#_using_the_plentyshop_lts_cache_blocks_in_your_own_theme[developer documentation^].
====

[.instruction]
Activating ShopBooster cache blocks:

. Go to the *Setup » Client » [Select client] » Online store » ShopBooster* menu.
...
=== Immediately invalidating the cache

You can manually remove cached contents from the cache. There are 5 invalidations available per day which will be reset each night exactly at midnight. Note that after an invalidation, the loading times of your shop will take longer until your shop pages are accessed for the first time and thus written to the cache once again.

[.instruction]
Manually invalidating the cache:

. Go to *Setup » Client » [select client] » Online store » ShopBooster*.
. In the section *Immediately invalidate cache*, click on the *Invalidate* button. +
→ The cache is deleted and the message *Cache was invalidated* is displayed. +
→ The number of *Remaining invalidations for today* is reduced by 1.

[#check-cache-status]
=== Checking the cache status of a page

To ensure that the ShopBooster is effective and that a page is loaded from the cache, you can use the developer tools of your browser. The following instruction is based on the Google Chrome browser.

[.instruction]
checking the cache status of a page:

. Open an item page or a category page in your shop.
. Open the developer tools by right-clicking and selecting *Inspect*. +
→ The developer tools open.
. At the top bar, click on the *Network* tab.
. In the *Filter* bar, click on *Doc*.
. Under *Name*, click on the name of the page.
. In the detail view of the page, open the *Headers* tab.
. Check whether the entry *x-plenty-cache: hit* appears in the *Response Headers* section.

If the Response Header contains the entry *x-plenty-cache: hit*, the page is in the ShopBooster cache. If you cannot find the entry, the checked page is not in the cache.

[NOTE]
====
Note that shop pages need to be opened once so they are written to the cache. If the inspected page does not contain an *x-plenty-cache: hit* entry, reload the page. If the page is still not loaded from the cache, contact us in the link:https://forum.plentymarkets.com/c/plentyshop/125[forum^].
====
