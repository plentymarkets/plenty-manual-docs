...
First, create an <<omni-channel/online-store/setting-up-clients/cms-syntax#email-email, email template>>. Once the item is back in stock, this email will be sent to all of the customers who saved their email addresses. Below, you will find a list of template variables and template functions that can be used for this email template. We also provide you with an example text, which you can customize as needed.
...
.  Create a new <<omni-channel/online-store/setting-up-clients/cms-syntax#email-email, email template>> in HTML format.
...
.  Select the email template from the drop-down list *Notification when item is in stock*.
