...
|*Tag*
|This filter only applies to reorders, redistributions and subscriptions because these are the only order types where xref:item:flags.adoc#400[tags] can be assigned. By using this filter, orders of these types are filtered based on the tags assigned to them. Use the option *Orders with exactly these tags* or *Orders with at least these tags* to determine exactly how tags will be filtered.
