...
Use the **Packing list** procedure to generate a list of items that have already been picked and are now ready for shipping. The xref:orders:generating-packing-list#[generating packing list] can be printed or saved via a sub-procedure.
...
| **Return reason**
|Select a reason. Returns will receive this reason when they are first created. Go to xref:orders:order-type-return.adoc#[Setup » Orders » Order types » Return] to create reasons for returns.
...
| **Item status**
|Decide which item status should be used by default when the return is created. Go to xref:orders:order-type-return.adoc#enter-return-reasons[Setup » Orders » Order types » Return] to create item statuses for returns.

| **Item value**
|Enter a percentage. This percentage will be applied to returned items. This value will also be displayed once a return has been created. Open the xref:orders:order-type-return.adoc#[return] and click on the **Overview** tab. You will find the value next to the order items.
...
*[INV]* = Invoice number +
*[DBT]* = Debtor account +
The placeholders can be entered next to each other without separators, such as :[OID][CID][INV][DBT].
