...
*_Note:_* Dunning letters can only be selected in ascending order, that means that level 2 can only be selected if a first dunning letter has already been generated etc. Moreover, a valid invoice needs to exist that was not cancelled. Otherwise, no dunning letter can be generated.
