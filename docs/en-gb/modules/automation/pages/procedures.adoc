...
| **Label template**
|Select a xref:item:managing-items#760[label template] from the drop-down list.
...
| **Label template**
|Select a xref:item:managing-items.adoc#760[label template] from the drop-down list.
|====
...
Use the **Finance export** procedure to create a CSV file that can be imported into several financial accounting solutions. You can choose between two formats: **One data record per stock unit** (default) or **One data record per invoice and VAT rate**. The xref:data:exporting-data-adoc#10[Finance export] can be saved via a sub-procedure.
...
Use the *Uploaded documents* procedure to automatically print or save uploaded documents of your choice via the corresponding sub-procedure. This procedure is suitable for documents that are not related to orders.
...
<<table-procedure-uploaded-docs>> explains the additional settings for the procedure. The general settings for procedures are described in <<table-procedures-general-settings>>.

[[table-procedure-uploaded-docs]]
.*Uploaded documents* procedure
[cols="1,3"]
|====
|Setting |Explanation

| *All* (default setting)
|All uploaded documents are selected and will be passed on to the sub-procedure. For now, only files in PDF format can be selected.

| *Select*
|Select the desired documents that should be passed on to the sub-procedure. For now, only files in PDF format can be selected.
|====
...
| *Label template*
|Select a xref:item:managing-items.adoc#3300[label template] from the drop-down list.
