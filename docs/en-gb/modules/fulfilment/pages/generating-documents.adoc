...
How you set up all other documents that are available in plentymarkets, such as invoice, order confirmation or delivery note, can be found in the xref:orders:orders.adoc#[Orders] area.
...
. Click on the gear-wheel icon (material:settings[]). +
...
Besides the convenient option to import package numbers into your system via the <<#package-numbers-elastic-sync, import tool>>, you can also import the package numbers <<#manually-import-package-numbers, manually>> or with the help of a <<#package-numbers-barcode-scanner, barcode scanner>>.

[#package-numbers-elastic-sync]
=== Importing package numbers via the import tool

Using the import tool is the easiest way to import package numbers in your plentymarkets system. To do so, create an import of the type *Orders*. Note our xref:data:practical-example-elasticsync-package-numbers-fulfilment.adoc#[Best Practice: Importing package numbers from shipping service providers].
...
. Create a packing list from these invoices for all orders that need to be packed this time. For further information, refer to the explanations on the xref:orders:generating-packing-list.adoc#[Packing list] page of the manual.
...
→ By clicking on *Save* (material:save[]), the package numbers is saved for all orders in the *Settings* tab.
...
.Settings in the *Setup » Orders » Documents » Address label* menu
[cols="1,3"]
|====
|Setting |Explanation
...
| *Customer properties*
|Enter coordinates to position the customer properties. Select the xref:crm:preparatory-settings.adoc#properties-introduction[property] from the drop-down list. The text that was saved in the *CRM » Contacts » [Open contact] » View: Properties* menu is displayed on the address labels.
...
. Click on the gear-wheel icon (material:settings[]) to generate the delivery list. +
