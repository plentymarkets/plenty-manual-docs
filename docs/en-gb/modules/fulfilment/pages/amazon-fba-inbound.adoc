...
→ The shipment is imported to plentymarkets, when its status in Amazon Seller Central is *_Working_*. +
...
[NOTE]
Only shipments with order status *_Working_* in Amazon Seller Central are imported to plentymarkets.
