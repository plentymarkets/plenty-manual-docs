...
--
You can find the following improvements in the package content list 2.0 that haven’t been available in the “old” package content list:
...
:rights-purpose: work with the package content list 2.0
:right-one: pass:quotes[*Orders > Package content list 2.0*]
...
Open all orders in a list or select filters (icon:tune[set=material]) to narrow down the search results in the overview of the package content list 2.0.

[.instruction]
Searching for orders in the package content list:

. Go to *Orders » Package content list 2.0*.
.. *_Option 1:_* Click on icon:search[set=material] to see a list of all orders.
.. *_Option 2_*: Enter an Order ID into the search field and click on one of the automatic suggestions.
Click on icon:search[set=material] to carry out the search.
.. *_Option 3_*: Click on icon:tune[set=material] to narrow down the search results with filters. Note the explanations about the filters in <<#table-search-orders>>.
...
. Click on icon:arrow_downward[set=material] or icon:arrow_upward[set=material] in order to change the sorting order.
|====

[#use-group-function]
== Using group functions

When you select an order (icon:check_box[set=material, role=skyBlue]) in the overview of the package content list 2.0, you can see the further options (icon:more_vert[set=material]) which contain several group functions. When you open an order and select several packages or pallets, more group functions become available in the detail view. <<#table-group-functions>> lists the elements from the group functions as well as their explanations.

[[table-group-functions]]
.Using group functions in the package content list 2.0
[cols="2,5"]
|====
|Element |Explanation
...
|icon:receipt_long[set=material] *Export CSV file*
|Exports the contents of the selected orders as CSV file.

|icon:lock[set=material] *Close all pallets*
|Closes all pallets of the selected orders. +
*_Note:_* Once a pallet is closed, it cannot be edited anymore.

|icon:lock[set=material] *Close all packages*
|Closes all packages of the selected orders. +
*_Note:_* Once a package is closed, it cannot be edited anymore.

|icon:delete[set=material] *Delete all pallets*
|Deletes all pallets of the selected orders.

|icon:delete[set=material] *Delete all packages*
|Deletes all packages of the selected orders.

2+^|*Group functions in the detail view of an order*

|icon:dynamic_feed[set=material] *Quick packing*
|Allows to pack several packages for an order at once.

|icon:settings[set=material] *Configure package(s)*
|Configures the selected packages of the order.

|icon:settings[set=material] *Configure pallet(s)*
|Configures the selected pallets of the order.

|icon:table_rows[set=material] *Put package(s) on pallet*
|Puts the selected packages of the order on a pallet.

|icon:lock[set=material] *Close package(s)*
|Closes the selected packages of the order.

|icon:lock[set=material] *Close pallet(s)*
|Closes the selected pallets of the order.

|icon:delete[set=material] *Delete package(s)*
|Deletes the selected packages of the order.

|icon:delete[set=material] *Delete pallet(s)*
...
For further information on the settings for using the Amazon FBA service in combination with the package content list 2.0, take a look at the manual page xref:markets:amazon-fba.adoc#120[Amazon: Using the FBA service].

[#export-order-content-as-csv]
== Exporting order contents as CSV file

In the package content list 2.0, you can export the contents of your orders as CSV file. You can also do this for several orders at once. In order to export the order contents in a CSV file, proceed as follows.
...
. At the top, click on icon:receipt_long[set=material] *Export order contents as CSV file*. +
→ The order contents are exported as CSV file.

[TIP]
.Exporting the contents of several orders as CSV file
======
If you want to export the contents of several orders as CSV file, place a check mark (icon:check_box[set=material, role=skyBlue] next to all desired orders in the overview. Afterwards, at the top, click on icon:receipt_long[set=material] *Export order contents as CSV file*. The contents of all selected orders are then exported all together in one CSV file.
======

[#work-with-pallets]
== Working with pallets

Pallets are the biggest packing unit in an order of the package content list and can contain packages and items. However, in the package content list 2.0, pallets are not required anymore in order to hand over a package to a shipping service provider. The image shows the different levels of an order. The following chapters describe how to work with pallets.
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. At the top, click on icon:add[set=material] *Add content*.
. Select the option *Create pallet*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) next to the order and select the option icon:add[set=material] *Create transport unit* > icon:add[set=material] *Create pallet*. +
...
. Click on icon:add[set=material] *Create pallet*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the pallet that you want to configure, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:settings[set=material] *Configure pallet*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:settings[set=material] *Configure pallet*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the pallet that you want to close, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:lock[set=material] *Close pallet*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:lock[set=material] *Close pallet*. +
→ The window *Close pallet* opens.
. If you are sure that you want to close the pallet, click on icon:lock[set=material] *Close pallet*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the pallet that you want to delete, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:delete[set=material] *Delete pallet*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:delete[set=material] *Delete pallet*. +
→ The window *Delete pallet* opens.
. If you are sure that you want to delete the pallet, click on icon:delete[set=material] *Delete pallet*. +
...
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) next to the order and select the option icon:add[set=material] *Create transport unit* > icon:add[set=material] *Create package*.
. Carry out the settings. Pay attention to the information given in <<#table-create-new-package>>.
. Click on icon:add[set=material] *Create package*. +
...
. On the left in the *Unpacked items* section, place a check mark (icon:check_box[set=material, role=skyBlue]) next to the items that you want to add.
. Click on *Add item to the package* (icon:add[set=material, role=green]). +
→ The items are moved to the *Packed items* section and thus added to the package.

[TIP]
.Remove packed items from a package
======
If you want to remove items from a package, place a check mark (icon:check_box[set=material, role=skyBlue]) next to the items in the *Packed items* section and click on *Remove the item from the package* (icon:delete[set=material]). Then click on icon:delete[set=material] *Remove* in order to remove the items from the package.
======
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the package that you want to put on a pallet, place a check mark (icon:check_box[set=material, role=skyBlue]). +
*_Tip:_* You can also select several packages and collectively put them on a pallet.
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:table_rows[set=material] *Put package on pallet*. +
...
. Click on icon:table_rows[set=material] *Put package on pallet*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. Place a check mark (icon:check_box[set=material, role=skyBlue]) next to the package or pallet you want to use the quick packing for. +
...
. Click on the further actions (icon:more_vert[set=material]).
. Select the option icon:dynamic_feed[set=material] *Quick packing*. +
→ The window *Quick packing* opens.
. Expand (icon:chevron_right[set=material]) the item settings on the left.
...
. Click on icon:add[set=material, role=green] *Add*. +
...
. Click on a package. +
→ The items that exist in the package are shown.
. At the top right, click on *Change scan type* (icon:settings[set=material]).
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the package that you want to configure, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:settings[set=material] *Configure package*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:settings[set=material] *Configure package*. +
...
. Click on icon:settings[set=material] *Configure package*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the package that you want to close, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:lock[set=material] *Close package*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:lock[set=material] *Close package*. +
→ The window *Close package* opens.
. If you are sure that you want to close the package, click on icon:lock[set=material] *Close package*. +
...
→ The orders that correspond to the entered search criteria are shown.
. Click in the line of the order to open the order.
. On the left of the package that you want to delete, place a check mark (icon:check_box[set=material, role=skyBlue]).
. Click on the further actions (icon:more_vert[set=material]) at the top.
. Select the option icon:delete[set=material] *Delete package*. +
*_Or:_* In the navigation on the left, click on the further options (icon:more_vert[set=material]) and select the option icon:delete[set=material] *Delete package*. +
→ The window *Delete package* opens.
. If you are sure that you want to delete the package, click on icon:delete[set=material] *Delete package*. +
...
. Click on the further actions (icon:more_vert[set=material]).
. Select the option icon:lock[set=material] *Close order*. +
→ The window *Close order* opens.
. If you are sure that you want to close the order, click on icon:lock[set=material] *Close order*. +
→ The order is closed.
