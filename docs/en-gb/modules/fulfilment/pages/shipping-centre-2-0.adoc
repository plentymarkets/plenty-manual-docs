...
* *_Option 3_*: Click on material:tune[] to narrow down the search results with filters.
...
Click on the button *Configure columns* (material:settings[]) on the top right to deactivate the columns that are already shown in the overview.
...
. Click in the line of the order material:shopping_cart[] *Order ID* to open the order in the detail view.
...
Click on the button *Configure columns* (material:settings[]) to deactivate the columns that are already shown in the overview.
...
→ The orders that correspond to the search criteria entered are shown in the overview.
. Open the order.
. Click on *Cancel order* (material:cancel[]) in the toolbar on the top. +
