...
You can configure individual packages by clicking on the line of the package. A window opens for you to configure the values for package type, packing unit type, package weight and package volume. Your settings are saved and applied to the package when you click on the button *Configure package* (material:settings[]).
