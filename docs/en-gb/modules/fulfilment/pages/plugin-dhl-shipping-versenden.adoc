...
====
You need a plugin to link plentymarkets to DHL Shipping (Versenden). Visit the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] to download the DHL Shipping (Versenden) plugin. +
Two DHL Shipping (Versenden) assistants that allow an easy setup are then available in the *Setup » Assistants » Plugins* menu in the relevant plugin set.

Before completing the two DHL Shipping (Versenden) assistants, you need to complete the *Shipping settings* assistant. This assistant is available in your plentymarkets system by default. You can find it in the *Setup » Assistants » Integration* menu. With this assistant, carry out the following settings:
...
Afterwards, complete the two DHL Shipping (Versenden) assistants:
 ...
 ** In this assistant, link the settings that you saved in the *Shipping settings* and *DHL Shipping (Versenden): Basic settings* assistants and select the product and the appropriate services.
====
...
As soon as you have <<plugins/installing-added-plugins#installing-plugins, installed>> the plugin in a plugin set, the *DHL Shipping (Versenden): Basic settings* and the *DHL Shipping (Versenden): Settings combination* assistants are available in the *Setup » Assistants » Plugins* menu in the relevant plugin set. The *Shipping settings* assistant is available in your plentymarkets system by default. You can find it in the *Setup » Assistants » Integration* menu.
 ...
 ** Link this name later in the *DHL Shipping (Versenden): Settings combination* assistant.

* Select the plugin of the shipping service provider for which you want to set up shipping costs. +
 ** You have to download the DHL Shipping (Versenden) plugin in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] before you can see it here.
...
You find the *DHL Shipping (Versenden): Basic settings* assistant in the *Setup » Assistants » Plugins* menu in the relevant plugin set. This assistant has several steps to complete. These steps are explained in the following.

[discrete]
==== Login details

In this step, save your login details for the DHL business customer portal. You can save multiple login details when you have multiple clients (shops) and/or concluded more than one contract with DHL. In the second assistant *DHL Shipping (Versenden): Settings combination*, you can link this data.

[IMPORTANT]
.Deleting linked login details
====
If you want to delete a login again, click on icon:minus-square[role="red"]. Note that the login details can only be successfully deleted in this assistant when they are _no longer_ used in the *DHL Shipping (Versenden): Settings combination* assistant. +
If you linked the login details in the *DHL Shipping (Versenden): Settings combination* assistant and you delete them only here, these login details will still be displayed when you open the assistant again. This means that first you have to remove the link in the *DHL Shipping (Versenden): Settings combination* assistant. Afterwards, you can delete the login details in this assistant.
====
...
If the option *Force label generation* is _not_ activated – this means the button is grey (icon:toggle_off[set=material, role=darkGrey]) –, non-codable shipping orders are rejected by DHL when trying to register them with DHL Versenden and an error message is displayed. The order is not registered and no shipping label is generated in your plentymarkets system.
...
In this step, enter your shipper data. The data is shown as sender address on the shipping label. Furthermore, you can show the bank details and polling data by activating the buttons (icon:toggle_on[set=material, role=skyBlue]).
...
Refer to this <<#dhl-produkte-services, list>> to see all services related to the products.
...
You find the *DHL Shipping (Versenden): Settings combination* assistant in the *Setup » Assistants » Plugins* menu in the relevant plugin set.

[discrete]
==== Settings combination

In this step, link the settings that you saved in the *Shipping settings* and *DHL Shipping (Versenden): Basic settings* assistants.
...
[TIP]
If you select a DHL billing number that is used for international shipments, the step *International shipments* automatically appears in the assistant. <<#international-shipments, Further down in this chapter>>, it is described which settings you can carry out in this step.

[IMPORTANT]
.DHL Preferred Delivery services only visible after having downloaded the DHL Preferred Delivery plugin
====
The DHL Preferred Delivery services *Preferred day*, *Preferred location* and *Preferred neighbour* are only available in this area once you have downloaded the DHL Preferred Delivery plugin in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlwunschpaket_5435[plentyMarketplace^]{nbsp}icon:external-link[] and installed it in a plugin set. +
Also for this plugin, there is an assistant available in the *Setup » Assistants » Plugins* menu in the relevant plugin set allowing you to simply and quickly set up the services.
====
...
Expand (icon:expand_more[set=material]) the following area to see a list of all DHL products and the corresponding services. Note that the products and services available in your system are based on your DHL contract. It might be that you may need to contact DHL to obtain additional activation for certain products and services.

[.collapseBox]
.DHL products and services
--

The following table lists all product and services that are currently available for the *DHL Shipping (Versenden)* plugin.
...
If you ship your parcels to countries that do _not_ belong to the European Union, activate (icon:toggle_on[set=material, role=skyBlue]) the option *Export delivery*. Two additional fields appear where you can select the franking (see <<#table-franking-incoterms>>) and the type of shipment for export deliveries. These two values are mandatory for export deliveries. +
...
| *DAP (Free delivery incl. customs clearance, duties and taxes)*
...
[discrete]
==== Completed *DHL Shipping (Versenden): Settings combination* assistant

[.collapseBox]
.Which data is shown when the *DHL Shipping (Versenden): Settings combination* assistant has been completed?
--
...
*_Note:_* The *DHL Shipping (Versenden): Settings combination* assistant has an additional setting for the service *Retail Outlet Routing* for <<#service-retail-outlet-routing, transferring the email address>> or an alternative email address.
...
* Go to the *DHL Shipping (Versenden): Settings combination* assistant and activate the service *Retail Outlet Routing*.
...
image::fulfilment/assets/dhl-shipping-label-ident-check.png[width=640, height=360]
...
* Go to the *DHL Shipping (Versenden): Settings combination* assistant and select the DHL billing number for Warenpost and the appropriate services that you want to link with this shipping setting.
...
image::fulfilment/assets/dhl-shipping-warenpost-label-size-german.png[width=640, height=360]
...
[IMPORTANT]
.*Ship* function in DHL business customer portal
====
...
[[image-dhl-cn23-form]]
.CN23 form
image::fulfilment/assets/dhl-shipping-cn23-form.png[width=640,height=360]

For further information on the customs declaration, refer to the link:https://www.dhl.de/en/toolbar/footer/informationen/zollinformationen.html[DHL website^]{nbsp}icon:external-link[].
