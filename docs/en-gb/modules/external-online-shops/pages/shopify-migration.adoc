...
*_Scenario 3:_* You had an active Shopify connection in the past. However, the Shopify plugin is not installed now. If you want to migrate this connection, follow the steps in the migration assistant to update the plugin. +
...
NOTE: You do not need to migrate all connections at the same time. However, note the information regarding event procedures if you only migrate some of your connections now.
...
You have more than one Shopify connection. If you only migrate one of your connections now, you need the old and the new event procedures. To ensure that only one of the two event procedures is triggered for the same order, you set filters for the client.
...
Outer order leads to inner calm. As such, remove the Shopify plugin from your plentymarkets back end after you have migrated all Shopify connections. Find out how xref:plugins:removing-installed-plugins.adoc[here].