...
| Activate the function after you have <<#2862608906, installed the WooCommerce plugin "Germanized">>. 
...
material:check_box[] = When you have set up the *Slug value for price label 'Lowest price of the last 30 days'* correctly, the lowest price of the last 30 days is exported to WooCommerce.
...
| *Slug value for price label 'Lowest price of the last 30 days'*
...
. Enter a value in the custom slug field, for example `lowest30days`.

Now, save the value of the custom slug in your plentymarkets system:

. Go to *Setup » Markets » WooCommerce » Tab: Settings*.
. Navigate to the area *Germanized*.
. In the field *Slug value for price label 'Lowest price of the last 30 days'*, enter the value of the custom slug. +
