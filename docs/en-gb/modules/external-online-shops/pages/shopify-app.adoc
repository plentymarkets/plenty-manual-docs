...
. Open your Shopify back end.
. In the navigation bar on the left, click on *Add apps*.
. In the search bar *Apps*, enter the search term `plentymarkets`. +
→ You are asked to search the term in the Shopify App Store. +
*_Tip:_* If the plentymarkets app is found in your back end, the app is already installed. Then you can ignore the installation steps.
. To open the Shopify App Store, use the enter key or click on the magnifying glass. +
→ The Shopify App Store opens in a new browser tab.
. Click on the plentymarkets app. +
*_Or:_* Open the plentymarkets app directly in the link:https://apps.shopify.com/plentymarkets-1?locale=en[Shopify App Store].
. Click on *Add app*. +
→ You are directed back to your Shopify back end.
. Click on *Install app*. +
...
→ You can connect a client and activate the data exchange.

[#BL4N7F]
=== Connecting client and setting up data exchange
...
*_Important:_* You can only connect one client. +
→ The data exchange options become available for the client you selected.
...
. Save the settings. +
*_Important:_* The data exchange only starts after you have also set up Shopify in the plentymarkets back end.
