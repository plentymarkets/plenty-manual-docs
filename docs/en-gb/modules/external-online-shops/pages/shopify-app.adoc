...
. Click on *Add app*. +
. Enter you credentials for Shopify. +
...
*_Important:_* You need a separate client for each Shopify shop. As such, only select one client. +
→ The data exchange options can be selected.
. Set the desired options for the data exchange. Pay attention to <<#QVFOWVID>>. +
*_Important:_* Prices, stock and item data can only be sent in one direction at a time. The direction you choose depends on where your item data is stored. Are your items stored in plentymarkets or in Shopify? Check the tabs below for different scenarios.
. Save the settings.
