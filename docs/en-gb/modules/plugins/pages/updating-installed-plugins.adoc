...
Marketplace plugins cannot be downgraded to an older version. If you want to use an older version of the plugin, you have to <</plugins/removing-installed-plugins#deleting-plugin-from-plugin-set, delete>> and then xref:plugins:installing-added-plugins.adoc[reinstall] it.
...
When your system has been switched to Plugin Build 2, you'll be informed about the changes when you deploy a plugin set that contains Git plugins for the first time. If you want to prevent the deployment process from updating a Git plugin automatically, <<switching-commit, switch to a commit>>
====
