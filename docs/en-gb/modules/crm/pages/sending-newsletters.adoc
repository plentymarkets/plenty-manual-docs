...
If you would like to use the xref:crm:sending-newsletters.adoc#800[double opt-in method], a further step is required. You have to create two templates and select them for the events *Newsletter opt-in* and *Newsletter opt-in confirmation* in the assistant *Email accounts* » Step: *Automatic despatch*. You can find the assistant in the *Setup » Client » Global » Email accounts* menu.
