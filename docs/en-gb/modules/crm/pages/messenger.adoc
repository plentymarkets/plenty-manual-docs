...
On this page of the manual, you learn how to <<#create-message, create>> new messages for your contact, <<#display-messages, display all messages>> that are available for your contact as well as <<#edit-message, edit>> and <<#delete-message, delete>> existing messages.
...
=== Searching for tags for the messenger
...
. Go to *CRM » Contacts*.
. Search (icon:search[role="blue"]) the contact by means of the filter settings as described on the xref:crm:search-contact.adoc#[Searching for a contact] page. +
→ The contacts that correspond to the search criteria entered are displayed.
. Open the context menu by clicking on material:more_vert[] in the line of the contact and then select material:message[] *Messenger*. +
icon:map-signs[] *_Or:_* Click in the line of the contact to open the contact data record and then on the left side on *Messenger*. +
→ The *Messenger* area opens.
. Click on *New message* (material:add[]).
. Create the message. Note <<#table-new-message-messenger>>.
. Click on *Send* (material:outgoing_mail[]). +
...
| *Stop watch*
|The stop watch in the top right starts to count when you open the *Messenger* area. +
Click on material:pause_circle[] to stop the timer and on material:play_circle[] to start the timer again. Click on terra:reset[] to reset the stop watch to 0.
...
| *Whisper*
|If you click on the button *Whisper* (material:visibility[] material:toggle_off[]), only those employees who have rights for the messenger can read the message. Your contacts are _not_ able to see the message. The *Whisper* function is activated when the button looks like this: material:visibility_off[] material:toggle_on[role=skyBlue]. +
*_Note:_* When the *Whisper* function is activated (material:visibility_off[] material:toggle_on[role=skyBlue]), the *Minutes* field and the *billable* button are deactivated.
...
| *Attach files*
|Click on material:attach_file[] to attach as many files as you like to the message. +
...
If you want to bill the minutes entered here, you have to select a tag of the type *Message (billable)* and activate the button terra:pay_cash[] material:toggle_off[].

| *Chargeable*
|Activate the button terra:pay_cash[] material:toggle_off[] to be able to bill the minutes that you entered in the *Minutes* field. +
*_Important:_* If you activated the button (terra:pay_cash[] material:toggle_on[role=skyBlue]), it is necessary to link a <<#tags-messenger, tag>> of the type *Message (billable)* to the message. These tags have to be <<#tags-messenger, created>> in advance in the *Setup » Settings » Tags* menu. Otherwise, the message _cannot_ be created.

| *Send*
|Click on material:outgoing_mail[] to create the message.
...
|Click on material:toggle_off[] material:done[] next to the contact ID (material:person[]) to close the message. +
In the message overview, the message is displayed with a material:done[role=green]. This tells other employees that this message has already been completed. +
Click on material:toggle_on[role=skyBlue] material:done[] to open the message again.

|Contact ID
|Click on material:person[] _Contact ID_ to open the detail view of the contact.

| *Change visibility*
|Click on material:edit[] to change the visibilities. You can remove recipients and add new recipients. +
...
| *Delete*
|Click on material:delete[] to delete the entire message stream. +
...
| *Edit tags*
|Click on *Edit tags* (material:edit[]) directly in the message to delete existing tags or add new tags. +
Click on *Save tags* (material:save[]) to save the changes.
...
Click on material:delete[] in the open message to delete the entire message stream. +
