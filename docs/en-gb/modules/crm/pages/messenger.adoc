...
Click on material:pause_circle[] to stop the timer and on material:play_circle[] to start the timer again. Click on icon:reset[set=plenty] to reset the stop watch to 0.
...
If you want to bill the minutes entered here, you have to select a tag of the type *Message (billable)* and activate the button icon:pay_cash[set=plenty] material:toggle_off[].

| *Chargeable*
|Activate the button icon:pay_cash[set=plenty] material:toggle_off[] to be able to bill the minutes that you entered in the *Minutes* field. +
*_Important:_* If you activated the button (icon:pay_cash[set=plenty] material:toggle_on[role=skyBlue]), it is necessary to link a <<#tags-messenger, tag>> of the type *Message (billable)* to the message. These tags have to be <<#tags-messenger, created>> in advance in the *Setup » Settings » Tags* menu. Otherwise, the message _cannot_ be created.
