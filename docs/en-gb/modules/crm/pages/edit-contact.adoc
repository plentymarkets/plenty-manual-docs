...
| *Active*
|By default, the contact is active (icon:toggle_on[set=material, role=blue]). This means that the contact is _not_ blocked for the selected client and is able to log into your plentyShop. +
When you click on the button (icon:toggle_off[set=material, role=grey]), the contact is no longer active. This means that the contact is _blocked_ for the selected client and is _not_ able to log into your plentyShop.
...
*_Note:_* The country that you saved as default location in the *Setup » Client » [Select client] » Settings* menu is automatically preselected.  You can of course adjust the setting before saving the address.
...
|Starts the search. The account balance data found is listed in the overview.
...
Proceed as described below to delete a contact in the overview.
...
Proceed as described below to delete a contact in the detail view.
