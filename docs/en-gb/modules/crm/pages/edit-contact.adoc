...
In the *Messenger* area, you can see all messages that belong to the contact. You can xref:crm:messenger-testphase.adoc#intable-whisper-mode[whisper] messages so that they are only internally visible and _not_ for your customers. You can xref:crm:messenger-testphase.adoc#create-message[send] messages and you can xref:crm:messenger-testphase.adoc#capture-minutes[capture minutes for the message] and save these minutes as free minutes or paid minutes. 
...
. Click on material:visbility_off[role=skyBlue] *CAPTURE MINUTES* to capture the minutes for the message. +
...
*_Note:_* material:warning[] When clicking on material:close[], the option will be directly deleted without another security question that you need to confirm.
...
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact to open the data record.
. In the toolbar on the top, click on *Download contact as vCard* (icon:address-card-o[]). +
...
//*_Note_*: If you select contacts here and do _not enter a value_, the values that have been saved so far are _deleted_ for the selected contacts. + You can find a detailed <<#step-by-step, step-by-step guide>> on how to save the payment due date using the group function below this table.
