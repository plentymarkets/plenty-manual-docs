...
Click on *New address* (material:add[]) to create a <<#addresses, new address>>. Click on material:more_vert[] to edit or delete the address. Next to the email address or next to the phone number, click on material:content_copy[] to copy these values to the clipboard.
...
Click on *Configure columns* (material:settings[]) on the top right and decide which columns to show in the overview of the *Addresses* area. Click on material:drag_indicator[] to change the order of the columns with drag-and-drop. The following columns are available:
...
Here, you can see all messages that are saved for the contact. Click on *New message* (material:add[]) to open the messenger and to create a message. In the recipient field, the contact ID is already preselected. You can add further recipients.
