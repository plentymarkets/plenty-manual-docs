...
====
If a user without admin rights is not able to see the available templates in your system, you might have to activate the view in the *Setup » Account management » Roles* menu in the *Role-defined views* area. Note the explanations on the xref:crm:preparatory-settings.adoc#activate-view-other-users[Carrying out the preparatory settings] page.
====

[IMPORTANT]
.Default view “Starter UI” in all new systems starting from December 2022
====
In all new systems that are booked as of the middle of December 2022, the 3 above mentioned <<#default-views-templates, default views>> are _no longer_ available. Only the default view *Starter UI* will be available. The starter UI contains the following areas and fields:
...
You have to save a template in the *Email accounts* assistant in the xref:crm:emailbuilder-preparatory-settings.adoc#email-automatic-despatch[Automatic despatch] step. +
...
Further information about the download file can be found in this link:https://forum.plentymarkets.com/t/dsgvo-button-fuer-den-datenexport-zu-einem-kunden-integriert/486915[changelog^] thread in the forum.
...
| *Owner*
|Select the owner for the contact. Enter at least one letter of the owner name. Your system then suggests owners that have already been saved. If you do not want to select an owner, select the empty option. +
...
Click on the following link and note the explanations on the *Creating a contact* page, if the delivery address is a xref:crm:create-contact.adoc#dhl-packstation-dhl-post-office[DHL Packstation or DHL post office].
...
*_Note:_* After saving, the address data record is given a consecutive ID that _cannot_ be changed. The ID is, however, not consecutive for each contact, but for all address data records that you create in your system.
...
*_Tip:_* Click on the following link and note the explanations on the *Creating a contact* page, if the delivery address is a xref:crm:create-contact.adoc#dhl-packstation-dhl-post-office[DHL Packstation or DHL post office].
...
*_Note 1:_* The country that you saved as default location in the *Setup » plentyShop » (Select client) » Client Settings* menu is automatically preselected. You can of course adjust the setting before saving the contact data record. +
...
| *Buyer reference*
|Is the recipient of the electronic invoice a public authority of the federal administration? If so, the buyer reference has always to be transmitted as mandatory information. For further information, refer to this link:https://www.e-rechnung-bund.de/faq/leitweg-id/#:~:text=Die%20Leitweg%2DID%20ist%20eine,Rechnungen%20an%20diesen%20zu%20adressieren[page^].
...
If the contact has more than 2 bank data records, this data is shown in a table. You can then click on *Configure columns* (material:settings[]) to determine the order of the shown columns or hide certain columns.
...
*_Note:_* After saving, the bank data record is given a consecutive ID that _cannot_ be changed. The ID is, however, not consecutive for each contact, but for all bank data records that you create in your system.
...
|Date on which payment is due for the order To have a payment due date displayed here, the xref:orders:basic-settings.adoc#[payment terms] of the corresponding order have to be filled in and an invoice has to be created.
...
Click on the already created message to open the conversation in the messenger. You can carry out further settings there. For further information, refer to the xref:crm:messenger-testphase.adoc#[Messenger] page.
...
====
Go to *Setup » Settings » Hosting » Cleansing* and determine the time period after which unused addresses are automatically deleted from your system. Unused addresses are those addresses that do _not_ have any relation to a contact, an order, a POS and a warehouse.
...
.Group function
image::crm:contacts-group-function-overview.png[width=500, height=300, alt=Group function]
...
Checking the VAT number in the plentysystems back end is not possible. You can, however, download a plugin in the plentyMarketplace, e.g. the plugin link:https://marketplace.plentymarkets.com/en/plugins/integration/vatidcheck_6023[VAT ID Check^] to carry out the VAT ID check.
