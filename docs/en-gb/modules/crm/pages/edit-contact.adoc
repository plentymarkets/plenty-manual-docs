...
[cols="1,3,3,3"]
|====
|Section |Standard - All | Standard - B2B | Standard - B2C
...
* <<#edit-bank-details, Editing bank details>>

[TIP]
If you click in the contact data record on the left side on the corresponding area that you want to edit, this area will shortly be highlighted in blue and you can directly start editing it.

[#general]
== General

<<#table-contact-general>> lists all fields that are available in the *General* area of the contact data record.
...
| *Class*
| Select a class. Classes serve as an internal distinction between customers and determine, for example, which xref:crm:preparatory-settings.adoc#create-customer-class[Customer classes] are created in the *Setup » CRM » Customer classes* menu. +
...
| *External number*
|Enter an external number for the contact, if required. External numbers can be assigned for internal purposes. They are _not_ saved automatically.
...
| *Newsletter*
|Shows if the contact receives the newsletter. It is _not_ possible to activate this option manually. To deactivate the newsletter subscription for the contact, click on icon:delete[set=material].
...
Here, the contact’s primary addresses are shown if you selected *Yes, primary* for the invoice address and/or the delivery address in the <<#addresses, Addresses>> area.

Click on *New address* (icon:add[set=material]) to create a new address. Click on icon:more_vert[set=material] to edit or delete the address. 
...
. Search the contact by means of the filter settings. Note xref:crm:search-contact.adoc#[Searching for a contact] page. +
...
. Go to *CRM » Contacts (Test phase)*.
. Search for the contact that you want to edit by means of the filter settings. Note xref:crm:search-contact.adoc#[Searching for a contact] page. +
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact to open the data record.
...
Here, you can see all orders of the contact. Click in the line of the order to open the order in the *Orders » Edit order* menu.
...
Click on icon:refresh[set=material] to reload the *Addresses* area.

Click in the line of the address to open the address for further editing.
...
When selecting this option, the address will then be shown in the <<#primary-address, Primary address>> area in the overview as icon:attach_money[set=material] *Invoice*.

| *Delivery address*
a| Which type does the new address have? Select one of the following options: +

* *No* (set by default) = The new address is _not_ a delivery address. +
* *Yes* = The new address is the delivery address. +
* *Yes, primary* = The new address is the primary delivery address. +

When selecting this option, the address will be shown in the <<#primary-address, Primary address>> area in the overview as icon:local_shipping[set=material] *Delivery*.
...
In the line of the company, click on icon:more_vert[set=material] and then on icon:edit[set=material] *Edit company* to edit the company details.
...
Click on icon:refresh[set=material] to reload the *Company* area.
...
. At the top right in the *Account* area, click on *Configure columns* (icon:cog[]). +
...
Here, you can see all options that are saved for the contact. Click on *Add option* (icon:add[set=material]) to add more options. To do so, select the appropriate option from the context menus and sub-menus. Options that are already in use are disabled and cannot be used a second time. Click on icon:close[set=material] to directly delete the option.
...
Click on *Configure columns* (icon:settings[set=material]) on the top right and decide which columns to show in the *Tickets* area. Click on icon:sort[set=material] to change the order of the columns with drag-and-drop. The following columns are available:
...
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact to open the data record.
. Click on *Commission & Cost units* on the left side.
...
*_Tip_*: You can find a detailed <<#step-by-step, step-by-step guide>> on how to save the payment due date using the group function below this table.
...
. Go to *CRM » Contacts (Test phase)*.
. Search the contacts by means of the filter settings. Note xref:crm:search-contact.adoc#[Searching for a contact] page. +
→ The contacts that correspond to the search criteria entered are displayed.
...
For further information, refer to the xref:data:change-history.adoc#[Tracking changes] page.
...
Import or update the data of your contacts including address options, contact options and account data via the import tool. To do so, use the import type xref:data:ElasticSync.adoc#[Using the import tool] page of the manual.
