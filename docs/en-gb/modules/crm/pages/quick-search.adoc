...
→ Click on *New message* (icon:add[set=material]). The stop watch in the top right corner starts automatically once you open the message. Click next to the *Minutes* field on *Billable* (icon:toggle_off[set=material] icon:pay_cash[set=plenty]) to bill the time. +
...
→ Click on *Add event* (icon:add[set=material]) to save this time. +
...
[TIP]
When you open a data record in the *CRM » Quick search* menu, you can click on *Start* (icon:play_circle[set=material]) to start the timer. Click on *Pause* (icon:pause_circle[set=material]) to stop the timer. Click on *Reset* (icon:reset[set=plenty]) to set the time to 0.
...
Click on icon:add[set=material] *New order* right next to *ORDERS* to create a new order in the *Orders » Edit orders* menu. +
...
Click on icon:add[set=material] *New offer* right next to *OFFERS* to create a new offer in the *Orders » Edit orders* menu. +
*_Note:_* If the data record is a guest order, these fields are not available.

| *MESSENGER*
|Opens the *Messenger* area further down in this view. +
Click on icon:add[set=material] *New message* right next to *MESSENGER* to open the editing window for creating a new message further down in this view. +
*_Note:_* If the data record is a guest order, these fields are not available.

| *TICKETS*
|Opens the *CRM » Ticket system* menu. The ID of the contact is automatically selected as filter. +
Click on icon:add[set=material] *New ticket* right next to *TICKETS* to open the editing window for creating a new ticket in the *CRM » Ticket system* menu. +
*_Note:_* If the data record is a guest order, these fields are not available.

| *Download all data of the contact*
|Click on icon:file_download[set=material] to download all data that is saved for this data record. +
...
|Click on icon:outgoing_mail[set=material] to send an email for the newsletter opt-in respectively the confirmation for the newsletter opt-in. +
...
|Click on icon:swap_horiz[set=material] to convert the address data from a guest order into a regular contact data record. In the *History* area of the newly created contact data record, the entry *Converted by* as well as the name of the person who converted the data record are added. +
...
| *Pause* / *Start*
|Click on icon:pause_circle[set=material] to stop the timer. Click on icon:play_circle[set=material] to start the timer again.

| *Reset*
|Click on icon:reset[set=plenty] to reset the timer to 0.
...
* In case of a guest order via the plentyShop, the saved data record only consists of the address and the corresponding order; a contact ID is _not_ saved. However, the symbol icon:done[set=material] is shown in the overview’s *Guest* column.

* In case of a guest order via a marketplace, a contact ID is assigned and the symbol icon:done[set=material] is shown in the overview’s *Guest* column.
...
. In the header, click on *Convert guest to contact* (icon:swap_horiz[set=material]]). +
...
====
plentyShop guest orders get the symbol icon:done[set=material] in the overview’s *Guest* column, but _no_ contact ID. Marketplace guest orders get the symbol icon:done[set=material] in the overview’s *Guest* column and in addition a contact ID.
...
. Click in the line of the data record. +
→ The overview with the areas *Invoice address*, *Master data* and *Info* appears.
. Click on *Download all data of the contact* (icon:file_download[set=material]) in the upper line. +
