...
→ Click on *New message* (material:add[]). The stop watch in the top right corner starts automatically once you open the message. Click next to the *Minutes* field on *Billable* (material:toggle_off[] icon:pay_cash[set=plenty]) to bill the time. +
...
[TIP]
When you open a data record in the *CRM » Quick search* menu, you can click on *Start* (material:play_circle[]) to start the timer. Click on *Pause* (material:pause_circle[]) to stop the timer. Click on *Reset* (icon:reset[set=plenty]) to set the time to 0.
