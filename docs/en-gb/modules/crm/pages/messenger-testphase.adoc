...
* *Navigation*: Here you can navigate through different parts of the menu. The currently selected area (blue background) is displayed in the main window on the right. Click on icon:menu[set=material, role=darkGrey] to expand or collapse the navigation bar. +
** Available areas in the navigation: <<#create-message, New message>>, <<#group-function, Inbox>>, <<#new-messages, New messages>>, <<#my-messages, My messages>>, <<#trash, Trash>>.
...
** *Overview*: When you start a search, you’ll see the results in an overview. The overview provides information about the conversations found at a glance. You see, for example, the email addresses of the senders, the subject, the date when the message was received, the assigned tags and by means of the icon icon:attach_file[set=material, role=darkGrey], you can see whether the message has an attachment. +
...
:right-five: pass:quotes[*Setup > Tag > Show*]
:right-six: pass:quotes[*Setup > Tag > Create*]
...
====
If you want to create messages that should be <<#bill-message, billed>> via the Messenger, you have to create tags for the availability *Message (billable)* in the same way. You have to add these tags to the <<#create-message, message>>.
====
...
In this step, activate the Messenger service for this inbox. If the button is blue (icon:toggle_on[set=material, role=skyBlue]), the service is activated. If the button is grey (icon:toggle_off[set=material, role=darkGrey]), the service has _not_ been activated yet.
...
In the lower part, enter all email addresses that are allowed to send messages to the cryptic, forwarding email address stated above. You can add as many email addresses as you like by clicking on *Add* (icon:plus-square[role="green"]).
...
In this step, save the login details of the email inbox that you want to use to send emails from. This is necessary to make sure that your plentymarkets system is able to send and receive emails via the messenger. 
...
====
In the *CRM » EmailBuilder (Test phase)* menu, you can find a default template for the messenger. You can, of course, adjust this template according to your needs. The template contains the following variables:

* Subject of the message in the subject line, shown as `{{ messenger.title }}`
...
* Conversation ID
** The variable *Conversation ID* serves as unique identifier to make sure that the message is always correctly assigned. This is why this variable should _not_ be removed from the template.
====

[#settings-email-receipt]
==== Settings for receiving emails

In this step, select one or multiple tags that should be assigned to the messages when these messages are received from and sent to the forwarded addresses.
...
[#test-mode-inactive]
==== Setting test mode to inactive

To be able to send messages with the messenger, the test mode in the *Setup » Client » [Select client] » Email » Login details* menu has to be set to *inactive*.

Setting the test mode to *inactive* can be done in this step without having to leave the assistant. To do so, select (icon:check_box[set=material, role=skyBlue]) the option *Activate live mode*.
...
. Click in the top left on icon:add[set=material, role=darkGrey] *New message*.
...
. Click on *Add attachments* (icon:attach_file[set=material, role=darkGrey]) to add attachments to the message.
. Click on *Assign tags to message* (icon:local_offer[set=material, role=darkGrey]) to assign tags for the message.
. Select further options, if needed, by clicking on icon:more_vert[set=material, role=darkGrey]. Note <<#table-create-new-message>>.
. Click on icon:forward_to_inbox[set=material, role=skyBlue] *SEND*.
...
|[#intable-inbox]*Inbox*
|Select an inbox from the drop-down list. The list shows the names of the inboxes that you set up by <<#complete-assistant, completing the assistant>>.

|[#intable-text-field]*Text field*
|Enter the content of the message here.

|[#intable-add-attachments]*Add attachments* (icon:attach_file[set=material, role=darkGrey])
|Click on icon:attach_file[set=material, role=darkGrey] to add attachments to your message. Select as many attachments as you like from your computer. +
The following file types can be sent as an attachment: bmp, csv, gif, jpeg, json, pdf, png, tiff, txt, xlsx, zip.

|[#intable-whisper-mode]*Whisper mode* (icon:visibility[set=material, role=darkGrey] / icon:visibility_off[set=material, role=skyBlue])
|You can whisper messages to make these messages only visible for internal purposes but _not_ for your contacts. To do so, click on *Whisper mode* (icon:visibility[set=material, role=darkGrey]). +
You can see in this view that a message is whispered by means of the button icon:visibility_off[set=material, role=skyBlue]. The button changes from icon:forward_to_inbox[set=material, role=skyBlue] *SEND* to icon:visibility_off[set=material, role=skyBlue] *WHISPER*. +
...
|[#intable-assign-tags-message]*Assign tags to message* (icon:local_offer[set=material, role=darkGrey])
|Click on icon:local_offer[set=material, role=darkGrey] to search for tags and select already existing tags for the message (icon:check_box[set=material, role=skyBlue]), add new tags (icon:add[set=material, role=darkGrey]) and edit existing tags (icon:edit[set=material, role=darkGrey]). +
...
|[#intable-bill-message]*Bill message* (icon:more_vert[set=material, role=darkGrey])
|You can always bill a message as long as it is a <<#intable-whisper-mode, whispered message>>. Click on icon:more_vert[set=material, role=darkGrey] first. If you then activate the button *Bill message* (icon:toggle_on[set=material, role=skyBlue]), this message will be billed. The button changes from icon:forward_to_inbox[set=material, role=skyBlue] *SEND* to icon:visibility_off[set=material, role=skyBlue] *BILL AND SEND*. +
...
|You can start a timer for every message. Click on icon:more_vert[set=material, role=darkGrey] first. If you activate the *Timer* button (icon:toggle_on[set=material, role=skyBlue]), the timer next to the icon:forward_to_inbox[set=material, role=skyBlue] *SEND* button will start running automatically. +
Click on icon:pause_circle[set=material, role=darkGrey] to stop the timer and on icon:play_circle[set=material, role=darkGrey] to start the timer again.

|[#intable-manage-inboxes]*Manage inboxes* (icon:more_vert[set=material, role=darkGrey])
|Click on icon:more_vert[set=material, role=darkGrey] first and afterwards on icon:edit[set=material, role=darkGrey] *Manage inboxes*. You will be redirected to the *Setup » Assistants » Basic setup* menu. Here, you can edit the inboxes via the available *Messenger configuration* <<#complete-assistant, assistants>>.

|[#intable-send]*SEND*
|By clicking on icon:forward_to_inbox[set=material, role=skyBlue] *SEND*, the message will be sent. +
*_Note:_* If you bill the message, the button changes to icon:visibility_off[set=material, role=skyBlue] *BILL AND SEND*. If you whisper the message, this button is changed to icon:visibility_off[set=material, role=skyBlue] *WHISPER*.
...
Open all conversations in a list or select filters (icon:tune[set=material, role=darkGrey]) to narrow down the search results in the Messenger overview.
...
. Go to *CRM » Messenger (Test phase)*. +
* *_Option 1:_* Click on icon:search[set=material, role=darkGrey] to see a list of all conversations.
...
* *_Option 3:_* Click on icon:tune[set=material, role=darkGrey] to narrow down the search results with filters.
...
|Enter a date or select the dates from the calendars (icon:today[set=material, role=darkGrey]) to search for conversations within a certain time period.
...
. Click on *SAVE*. +
→ The filter now appears under *Saved filters* (icon:bookmarks[set=material, role=darkGrey]).

[TIP]
Use drag-and-drop to arrange the filters in a specific order by clicking on *Move* (icon:drag_indicator[set=material, role=darkGrey]). Click on icon:delete[set=material, role=darkGrey] to delete a filter.
...
In the line of the saved filter, click on icon:star[set=material, role=darkGrey] *Set as default*. If you want to set another filter as default filter, deactivate the currently selected default filter by clicking on icon:star_outline[set=material, role=darkGrey] *Do not use as default*.

[#configure-columns]
== Configuring columns

By clicking on icon:settings[set=material, role=darkGrey] on the top right, you can have the following columns displayed in the overview of the Messenger:
...
|Opens the available relations that are linked with the conversation by clicking on icon:chevron_right[set=material, role=darkGrey].

|[#intable-overview-selection]*Selection*
|Shows the checkbox for the <<#group-function, group function>> (icon:check_box_outline_blank[set=material, role=darkGrey]) in the overview.

|[#intable-overview-status]*Status*
|Shows with the symbol icon:circle[set=material, role=skyBlue] (Message unread) that there are unread messages within the conversation. Shows with the symbol icon:circle[set=material, role=green] (Message completed) that the conversation is closed.

|[#intable-overview-short-id]*Short ID*
|Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] or icon:arrow_downward[set=material, role=darkGrey] to sort the table in ascending or descending order by the ID.
...
* icon:person[set=material, role=darkGrey] _Name of the contact (contact ID)_ 
* icon:shopping_cart[set=material, role=darkGrey] _Order ID_
...
* icon:mail[set=material, role=darkGrey] _Email address_
* icon:credit_card[set=material, role=darkGrey] _Payment ID_
* subscribers of the conversation.

In the line of the conversation, click on icon:chevron_right[set=material, role=darkGrey] to see all available relations at a glance. Click on icon:expand_more[set=material, role=darkGrey] to close the relation area again. +
...
|Shows the date and the time when the conversation was created. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] or icon:arrow_downward[set=material, role=darkGrey] to sort the table in ascending or descending order by the date.

|[#intable-overview-updated-at]*Updated at*
|Shows the date and the time when the conversation was updated. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] or icon:arrow_downward[set=material, role=darkGrey] to sort the table in ascending or descending order by the date.
...
|The symbol icon:attach_file[set=material, role=darkGrey] in the *Number of attachments* (icon:attach_file[set=material, role=darkGrey]) column shows whether the conversation has attachments. When you hover with your cursor over the symbol, you can see how many attachments the conversation has. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] to sort the table by the number of attachments.

|[#intable-overview-billed-messages]*Billed messages*
|The symbol icon:credit_card[set=material, role=darkGrey] in the column *Billed messages* (icon:credit_card[set=material, role=darkGrey]) shows whether the conversation has billed messages. When you hover with your cursor over the symbol, you can see the number of billed messages. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] to sort the table by the number of billed messages.

|[#intable-overview-billed-minutes]*Billed minutes*
|The symbol icon:av_timer[set=material, role=darkGrey] in the column *Billed minutes* (icon:av_timer[set=material, role=darkGrey]) shows whether the conversation has billed minutes. When you hover with your cursor over the symbol, you can see the number of billed minutes. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] to sort the table by the number of billed minutes.

|[#intable-overview-subscribed]*Subscribed*
|The column icon:notifications[set=material, role=darkGrey] shows with the symbol icon:done[set=material, role=darkGrey] whether you have <<#subscribe-for-conversation, subscribed for>> the conversation.
...
Click on the button *Configure columns* (icon:settings[set=material, role=darkGrey]) on the top right to deactivate the columns that are already shown in the overview.

[#reply-to-message]
== Replying to a message

Search the <<#search-conversation, conversation>> that contains the message you want to reply to. Click on the message to reply to it.

The only difference to <<#create-message, creating a message>> is that you can show and hide the subject line when replying to an existing message by clicking on the symbol icon:more_vert[set=material, role=darkGrey] (see <<#image-messenger-reply-to-message>>). 

[[image-messenger-reply-to-message]]
.Replying to a message
image::crm:messenger-reply-to-message.png[width=640, height=360]

You can adjust the subject, when the option *Subject* is activated (icon:toggle_on[set=material, role=skyBlue]). If you have _not_ activated the option *Subject* (icon:toggle_off[set=material, role=darkGrey]), the *Subject* field is not visible when replying to the message.
...
It is possible to change the view in the Messenger from a table to a list. To do so, click on *Change view* (icon:list[set=material, role=darkGrey]) in the top right of the overview. +
...
The following functions are available in the Messenger overview when clicking in the line of the conversation on icon:more_vert[set=material, role=darkGrey].
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to mark as read, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:mark_email_read[set=material, role=darkGrey] *Mark conversation as read*. +
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to mark as unread, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:mark_email_unread[set=material, role=darkGrey] *Mark conversation as unread*. +
...
. Go to *CRM » Messenger (Test phase)*.
. Click in the line of the conversation on icon:more_vert[set=material, role=darkGrey].
. Click on icon:local_offer[set=material, role=darkGrey] *Assign tags to conversation* to see a list of all tags that you have already <<#tags-messenger, created>> for the areas *Messages* and *Messages (billable)*. +
...
If you could not find the tag you were looking for as described in the <<#context-menu-tags-conversation, Assigning tags>> chapter, you can create a new tag from the Messenger overview. To do so, proceed as described below.
...
. Go to *CRM » Messenger (Test phase)*.
. Click in the line of the conversation on icon:more_vert[set=material, role=darkGrey].
. Click on icon:local_offer[set=material, role=darkGrey] *Assign tags to conversation*.
. Click on icon:add[set=material, role=darkGrey] *Add tag* to create a new tag. +
...
. Click in the line of the conversation on icon:more_vert[set=material, role=darkGrey].
. Click on icon:local_offer[set=material, role=darkGrey] *Assign tags to conversation*.
. Click on icon:edit[set=material, role=darkGrey] *Edit tag* to edit an existing tag. +
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to mark as done, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:circle[set=material, role=darkGrey] *Mark conversation as done*. +
→ At the beginning of the conversation’s line, the symbol icon:circle[set=material, role=green] is shown.
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to mark as not done, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:radio_button_unchecked[set=material, role=darkGrey] *Mark conversation as not done*. +
→ At the beginning of the conversation’s line, the symbol icon:circle[set=material, role=green] is removed.
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to subscribe for, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:notifications_active[set=material, role=darkGrey] *Subscribe for conversation*. +
→ The column icon:notifications[set=material, role=darkGrey] *Subscribed* does not show the symbol icon:done[set=material, role=darkGrey] any longer.
...
. Go to *CRM » Messenger (Test phase)*.
. Click in the line of the conversation that you want to unsubscribe from on icon:more_vert[set=material, role=darkGrey].
. Click on icon:notifications_off[set=material, role=darkGrey] *Unsubscribe from conversation*. +
→ The column icon:notifications[set=material, role=darkGrey] *Subscribed* does not show the symbol icon:done[set=material, role=darkGrey] any longer.
...
. Go to *CRM » Messenger (Test phase)*.
. In the line of the conversation that you want to delete, click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:delete[set=material, role=darkGrey] *Delete*. +
...
. Enter the subject of the message in the *Subject* field.
. Enter the message in the text field.
. Click on icon:visibility[set=material, role=darkGrey] to whisper the message. +
*_Note:_* Only whispered messages can be billed. The symbol icon:visibility_off[set=material, role=skyBlue] shows you that a message is whispered.
. Click on icon:local_offer[set=material, role=darkGrey] to assign tags for the message. +
...
. Click on icon:more_vert[set=material, role=darkGrey].
. Activate the button *Bill message* (icon:toggle_on[set=material, role=skyBlue]).
. Click on icon:visibility_off[set=material, role=skyBlue] *BILL AND SEND*. +
...
Once you selected a message within a conversation, you can display the messages’ content in plain text format by clicking on icon:chevron_right[set=material, role=darkGrey] at the beginning of the message. This way, you are able to see a preview of the message content already without <<#open-single-message, opening>> the entire message.

[#open-single-message]
=== Opening a message

To open a message, you can either directly click on the message. Or you open the context menu in the line of the message by clicking on icon:more_vert[set=material, role=darkGrey] and then on icon:open_in_new[set=material, role=darkGrey] *Open message*.
...
. Click on icon:local_offer[set=material, role=darkGrey] *Assign tags to message* to see a list of all tags that you have already <<#tags-messenger, created>> for the areas *Messages* and *Messages (billable)*. +
*_Tip:_* If you have created a lot of tags for the *Messages* or *Messages (billable)* areas, enter the name of the tag in the search field *Search for tag*.
...
. Open the message by clicking in the line of the message.
. Click on icon:more_vert[set=material, role=darkGrey].
. Click on icon:attach_file[set=material, role=darkGrey] *Attachments*.
. Click on one attachment or on icon:file_download[set=material, role=darkGrey] *Download all files*. +
...
Once you have selected a conversation in the overview in the *Inbox* area (icon:check_box[set=material, role=skyBlue]), the different buttons become visible (see <<#image-messenger-group-functions-conversations>>). <<#table-group-functions-messenger>> lists the elements from the group functions as well as their explanations.
...
In the navigation on the left side of the messenger, you can find the folder icon:mark_chat_unread[set=material, role=darkGrey] *New messages* below the folder icon:inbox[set=material, role=darkGrey] *Inbox*.
...
|The symbol icon:attach_file[set=material] in the *Number of attachments* (icon:attach_file[set=material, role=darkGrey]) column shows whether the conversation has attachments. When you hover with your cursor over the symbol, you can see how many attachments the conversation has. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] to sort the table by the number of attachments.
...
In the navigation on the left side of the messenger, you can find the folder icon:3p[set=material, role=darkGrey] *My messages* below the folder icon:inbox[set=material, role=darkGrey] *Inbox*. You can see all conversations here that you <<#subscribe-for-conversation, subscribed for>>. 
...
In the navigation on the left side of the Messenger, you can find the folder icon:delete[set=material, role=darkGrey] *Trash*. Here you can find all <<#context-menu-delete-conversation, deleted conversations>> and all<<#delete-single-message, deleted messages>>.
