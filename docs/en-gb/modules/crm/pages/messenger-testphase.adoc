...
In the top right, click on material:fullscreen[] to show the full-screen mode of the message and hide the overview. Click on material:fullscreen_exit[] afterwards to leave the full-screen mode and get back to the overview. +
When replying to an existing message, you can see all recipients that will receive this message. You can add more recipients. If desired, select a template from the drop-down list *Template* and a template language.
...
.Replying to a message
image::crm:messenger-reply-to-message-context-menu.png[width=640, height=360]
...
. Click on material:folder[] *Move to folder*.
...
====
If you want to remove the conversation from all folders, click on material:remove[] *Remove from all folders*.
====
...
. Click on *Save*. +
→ The folder is created. You can find the folder in the material:3p[] *My messages* > material:3p[] *My folders* area.

The sub-folder material:3p[] *My folders* is only visible when you have created at least one custom folder. If you do not create custom folder, the sub-folder material:3p[] *My folders* is _not_ visible.
...
. Go to *CRM » Messenger (Test phase)*.
. On the left side in the material:3p[] *My messages* > material:3p[] *My folders* area, click in the line of the folder that you want to delete, on material:more_vert[].
