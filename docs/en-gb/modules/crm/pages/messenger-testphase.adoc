...
====
In the *Email accounts* assistant in the xref:crm/:mailbuilder.adoc#email-server-login-details[Login details] step, the option *Activate live mode* in the area *Would you like to activate the live mode?* has to be activated (material:check_box[role=skyBlue]). This ensures that the test mode will be deactivated and all emails will be sent to the corresponding recipients. 
