...
====
We recommend to use the default template for the Messenger. This template is already available in your system. 
====
...
*_Note:_* If you select the option *Paid minutes* instead of the option *Free minutes*, the button changes to *BILL AND SAVE*.
...
→ If it’s a <<#example-captured-minutes-paid, message with paid minutes>>, a credit card symbol is shown here.
...
|The symbol material:notifications[] in the column material:done[] shows whether you have <<#subscribe-conversation, subscribed for>> the conversation.
