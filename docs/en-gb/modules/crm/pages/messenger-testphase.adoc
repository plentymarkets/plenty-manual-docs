...
====
In the *CRM » EmailBuilder* menu, you can find the *Messenger* folder that contains a default template for the messenger. You can, of course, adjust this template according to your needs. The template contains the following variables:

* Full name
* Message content
* Signature in HTML format
* Conversation ID
** The variable *Conversation ID* serves as unique identifier to make sure that the message is always correctly assigned.
====
...
Setting the test mode to *inactive* can be done in this step. To do so, select (icon:check_box[set=material, role=skyBlue]) the option *Activate live mode*.
...
|You can always bill a message as long as it is a <<#intable-whisper-mode, whispered message>>. Click on icon:more_vert[set=material] first. If you then activate the button *Bill message* (icon:toggle_on[set=material, role=skyBlue]), this message will be billed. The button changes from icon:forward_to_inbox[set=material, role=skyBlue] *SEND* to icon:visibility_off[set=material, role=skyBlue] *BILL AND SEND*. +
...
Available options: *Back end*, *REST*, *Plugin* and *PayPal plugin* (icon:pay_5_paypal[set=plenty]).
...
|Enter an ID to only search for conversations that are linked with this payment ID.
...
*_Note:_* The xref:crm:contacts.adoc#[contact] has a direct link to the corresponding data record in the *CRM » Contacts* menu. +
...
|Shows with the symbol the referrer of the message. +
Available options: *Back end*, *REST*, *Plugin* and *PayPal plugin* (icon:pay_5_paypal[set=plenty, role=darkGrey]).
