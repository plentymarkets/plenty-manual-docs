...
====
If you want to create messages that should be <<#bill-message, billed>> via the Messenger, you have to create tags for the availability _Message (billable)_ in the same way. You have to add these tags to the <<#create-message, message>>.
====
...
* Message conversation ID
** The variable *Message conversation ID* serves as unique identifier to make sure that the message is always correctly assigned. This is why this variable should _not_ be removed from the template.
====

[#settings-email-receipt]
==== Settings for receiving emails

In this step, you select one or multiple tags that should be assigned to the messages when these messages are received from and sent to the forwarded addresses.
...
To be able to send messages with the messenger, you need to set the test mode in the *Setup » Client » [Select client] » Email » Login details* to *inactive*.

You can set the test mode to *inactive* in this step without having to leave the assistant. To do so, select (icon:check_box[set=material, role=skyBlue]) the option *Activate live mode*.
...
|Click on icon:edit[set=material, role=darkGrey] *Manage inboxes* to be redirected to the *Setup » Assistants » Basic settings* menu. Here, you can edit the inboxes by using the *Messenger configuration* <<#complete-assistant, assistant>>.

|[#intable-name-inbox]_Name of the currently selected inbox_
| Click on icon:arrow_drop_down[set=material, role=darkGrey] to select another inbox from the drop-down list. The list shows the names of the inboxes that you set up by <<#complete-assistant, completing the assistant>>.
...
a|Shows with the symbol the referrer of the message. +
Possible values: 
...
|Shows the date and the time when the message was created. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] or icon:arrow_downward[set=material, role=darkGrey] to sort the table in ascending or descending order by the date.

|[#intable-overview-updated-at]*Updated at*
|Shows the date and time when the message was updated. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] or icon:arrow_downward[set=material, role=darkGrey] to sort the table in ascending or descending order by the date.
...
In the Messenger overview the following functions are available when clicking in the line of the conversation on icon:more_vert[set=material, role=darkGrey].
...
. Enter the subject of the message in the *Subject* field.
. Enter the message in the text field.
. Click on icon:visibility_on[set=material, role=darkGrey] to whisper the message. +
...
|The symbol icon:credit_card[set=material, role=darkGrey] in the *Billed messages* (icon:credit_card[set=material, role=darkGrey]) column shows whether the conversation has billed messages. When you hover with your cursor over the symbol, you can see the number of billed messages. Right next to the title bar, click on icon:arrow_upward[set=material, role=darkGrey] to sort the table by the number of billed messages.
