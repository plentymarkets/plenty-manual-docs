...
** Available areas in the navigation: <<#create-message, New message>>, <<#group-function, Inbox>>, <<#new-messages, New messages>>, <<#my-messages, My messages>>, <<#trash, Trash>>. The number behind the names of the individual areas shows the number of unread messages.
...
[IMPORTANT]
.Live mode has to be activated in *Email accounts* assistant
====
In the *Email accounts* assistant in the xref:crm:emailbuilder.adoc#email-server-login-details[Login details] step, the option *Activate live mode* in the area *Would you like to activate the live mode?* has to be activated (material:check_box[role=skyBlue]). This ensures that the test mode will be deactivated and all emails will be sent to the corresponding recipients. 
...
[TIP]
.Use the default template *Messenger default*
====
We recommend to use the default template *Messenger default* for the Messenger. This template is already available in your system in the *CRM » EmailBuilder (Test phase)* menu in the *Default templates* folder. You can, of course, adjust this template according to your needs.
...
If you do _not_ want to use the default template for the messenger in this step, but a custom email template, make sure that the email template that you create in the *CRM » EmailBuilder (Test phase)* menu, contains at least the following variables:

* Message subject
** Enter `{{ messenger.title }}` in the template’s subject line.
* Message content
** Insert the variable `Message content` into the template’s text field.
* Conversation ID
** Insert the variable `Message conversation ID` into the template’s text field.
...
To be able to send messages with the messenger, it is required that the live mode in the *Email accounts* assistant in the *Login details* step is activated (material:check_box[role=skyBlue]).
...
* Orders » Orders » Orders (Test phase)
* Group function in the order

You can find the *Email accounts* assistant in the *Setup » Assistants » Basic setup* menu. For further information about collecting the emails, refer to the xref:crm:emailbuilder.adoc#email-messenger-integration[Messenger integration] chapter on the xref:crm:emailbuilder.adoc#[EmailBuilder (Test phase)] page.
...
// *_Note:_* If you click on <<#intable-refresh-template, Refresh template>>, the content of the template and the subject saved in this template are shown.

// |[#intable-template-language]*Template language*
// |Select the template language.

// |[#intable-refresh-template]*Refresh template*
// |Click on material:refresh[] to refresh the content of the template. For example, the content of the selected template and its subject will be shown.
...
//| Click on material:arrow_drop_down[] to select another inbox from the drop-down list. The list shows the names of the inboxes that you set up by <<#complete-assistant, completing the assistant>>.

|[#intable-send]*SEND*
|By clicking on material:forward_to_inbox[role=skyBlue] *SEND*, the message will be sent. +
*_Note:_* If you capture minutes for the message, the button changes to material:visibility_off[role=skyBlue] *CAPTURE AND SEND*. If you whisper the message, this button is changed to material:visibility_off[role=skyBlue] *WHISPER*.
...
You can have the following columns displayed in the overview of the Messenger by clicking on the material:settings[] symbol in the top right:
...
Search the <<#search-conversation, conversation>> that contains the message you want to reply to. Click on the message to reply to it. When replying to an existing message, you can see all recipients that will receive this message. You can add more recipients.

// If desired, select a template from the drop-down list *Template* and a template language.

The only difference to <<#create-message, creating a message>> is that you can show and hide the subject line when replying to an existing message by clicking on the symbol material:more_vert[] (see <<#image-messenger-reply-to-message>>). 

[[image-messenger-reply-to-message]]
.Replying to a message
image::crm:messenger-reply-to-message.png[width=640, height=360]

You can adjust the subject line when the option *Subject* is activated (material:toggle_on[role=skyBlue]). If you have _not_ activated the *Subject* option (material:toggle_off[]), the *Subject* field is not visible when replying to the message. The currently selected subject is then automatically transferred when replying to the message.
...
The selected conversation with all the messages contained therein is shown in the lower left section and the editor to reply to the messages is shown in the lower right section. The selected conversation with all the messages contained therein is shown in the upper right section and the editor to reply to the messages is shown in the lower right section.
////
...
. Select tags from the list, if needed.
...
In the navigation on the left-hand side of the messenger, you can find the folder material:3p[] *My messages* below the folder material:inbox[] *Inbox*. You can see all conversations here that you <<#subscribe-conversations, subscribed for>>.
