...
Data records from guest orders get the same customer class as B2C contacts after registration if a B2C customer class is available in your system. +
If no customer class at all is available in your system, all data records get the customer class with the ID 0.
====
...
*_Tip:_* If you want to activate multiple payment methods all at once, Wenn du mehrere Zahlungsarten auf einmal aktivieren möchtest, hold down the cmd key (or Strg key) and select the desired payment methods from the list using the mouse.
