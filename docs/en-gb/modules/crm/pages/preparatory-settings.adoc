...
At a later pointy, you might probably set up a xref:crm:create-contact.adoc#[a new view to create a new contact] or a <<crm/contacts/edit-contact#edit-view#, new view to edit an existing contact>>. It is possible to activate these views also for other users in your plentymarkets system to allow them to work with these views. +
...
The terms of payment are saved in the xref:orders:managing-orders.adoc#intable-payment-terms-order[order settings]. The values in the contact data record have also priority over the values in the order settings. The values can be manually adjusted or entered in the order if no values are saved in the contact data record or in the customer class. The terms of payment saved in the order are then shown on the xref:orders:order-documents.adoc#intable-payment-terms-documents[documents]. Note the example xref:crm:companies.adoc#example-terms-of-payment[Terms of payment with valuta, payment due date, days for early payment discount and percentage for early payment discount] on the xref:crm:companies.adoc#[Companies] manual page.
...
. Go to *CRM » Contacts*.
. Search the contact by means of the filter settings. Note the explanations about the filters on the xref:crm:search-contact.adoc#[Searching for a contact] page. +
