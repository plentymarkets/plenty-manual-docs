...
. Click on *New layout* (icon:add[set=material]).
...
|Do you want to use this layout as standard layout? If so, activate the toggle button (icon:toggle_on[set=material, role=skyBlue]). In the overview on the left side, the standard address layout is marked with an asterisk.
...
Click on icon:sync_alt[set=material] to switch to the layout design. The areas *Countries*, *Layout* and *Fields* appear.
...
In order that the address layouts are automatically displayed on delivery notes, you have to carry out a setting in the xref:orders:generating-delivery-notes.adoc#[delivery note]. Your plentymarkets system automatically checks the country of delivery saved in the order and displays the delivery note in the layout listed in the *Setup » CRM » Address layout* menu. To do so, proceed as described below.
...
| Companies, xref:crm:edit-contact.adoc#company-not-linked-yet[delete relation to contact], xref:crm:edit-contact.adoc#delete-company-link[delete relation to company]
|CRM > Company > Create +
CRM > Company > Update +
CRM > Company > Delete
...
Go to the user account and open in the *Views* area the *CRM > Contacts > New contact > Custom views* area. Select (icon:check_box[set=material, role=skyBlue]) the desired views from the list.

* Activating views for users to edit an existing contact data record: +
Go to the user account and open in the *Views* area the *CRM > Contact details > Custom views* area. Select (icon:check_box[set=material, role=skyBlue]) the desired views from the list.
...
. Click on *Add type* (icon:add[set=material, role=green]).
. Enter a name.
. *Save* (icon:save[role="green"]) the settings.
