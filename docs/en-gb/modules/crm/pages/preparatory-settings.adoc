...
:description: Learn which preparatory settings you have to carry out in order to work with contact data records in plentysystems.
:page-pagination:
:author: team-crm
...
In order that the address layout is automatically displayed on your xref:orders:order-documents.adoc#[location-based order documents], you have to carry out a setting in the corresponding order document. Your system automatically checks the country of delivery saved in the order and displays the corresponding order document in the layout listed in the *Setup » CRM » Address layout* menu. To do so, proceed as described below.
...
At a later point, you might probably set up a xref:crm:create-contact.adoc#[new view to create a new contact] or a xref:crm:edit-contact.adoc#edit-view[new view to edit an existing contact]. It is possible to activate these views also for other users in your system to allow them to work with the views you created. +
...
Use the import tool and the import type *Properties* to import properties into your system. The fields that you can use for mapping and matching are described on the page of the manual of the import type xref:data:elasticSync-properties.adoc#[Properties].

For general information about importing data into your system, refer to the xref:data:ElasticSync.adoc#[Using the import tool] page of the manual.
