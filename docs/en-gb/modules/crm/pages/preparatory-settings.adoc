...
The terms of payment are saved in the xref:orders:managing-orders.adoc#intable-payment-terms-order[order settings]. The values in the contact data record have also priority over the values in the order settings. The values can be manually adjusted or entered in the order if no values are saved in the contact data record or in the customer class. The terms of payment saved in the order are then shown on the xref:orders:order-documents.adoc#intable-payment-terms-documents[documents]. Note the example <<crm/companies#example-terms-of-payment, Terms of payment with valuta, payment due date, days for early payment discount and percentage for early payment discount>> on the xref:crm:companies.adoc#[Companies] manual page.
