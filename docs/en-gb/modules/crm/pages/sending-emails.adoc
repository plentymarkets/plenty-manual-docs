...
An order is automatically created in plentymarkets after a contact has purchased an item on eBay. If you have xref:crm:sending-emails.adoc#1200[created] the email template "Order confirmation eBay" for example and selected this template from the drop-down list for the event *New order from eBay* in the *Automatic despatch* step of the *Email accounts* assistant in the *Setup » Assistants » Basic setup* menu, an email is automatically sent to the contact.
