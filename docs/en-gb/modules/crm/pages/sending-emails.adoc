...
You can adapt your email templates to the design of your online shop, e.g. by inserting your company logo in the HTML design. First, you have to upload the logo to the webspace. After you created the email template, you insert the <<#1750, logo in the HTML design>> and <<#1100, link>> the design with this email template.
...
The *Automatic despatch* step of the *Email accounts* assistant contains various events that can be linked with an email template. You can find the assistant in the *Setup » Assistants » Basic setup* menu. Select the email template that you have previously xref:crm:sending-emails.adoc#1200[created] in the from the drop-down list. This ensures that the email template is sent to your contact once the event occurs.

[IMPORTANT]
.Removing the link before deleting the email template
====
Deleting an email template that is linked with an event in the *Automatic despatch* step of the *Email accounts* assistant may lead to undesired effects. Therefore, first go to *Setup » Assistants » Basic setup* and remove the link by selecting the option *Do not send* from the drop-down list in the line of the event. Afterwards, delete the email template in the *Setup » Client » [Select client] » Email » Templates* or *CRM » EmailBuilder* menu.
====
...
*_Note:_* If you want that your customer can access the order processing in your online shop with this email to see the order status, you have to insert the template function `{%Link_Checkout()%}` in the email template that you select for this automatic event.
...
An order is automatically created in plentymarkets after a contact has purchased an item on eBay. If you have xref:crm:sending-emails.adoc#1200[created] the email template "Order confirmation eBay" for example and selected this template from the drop-down list for the event *New order from eBay* in the *Automatic despatch* step of the *Email accounts* assistant, an email is automatically sent to the contact. You can find the assistant in the *Setup » Assistants » Basic setup* menu.
