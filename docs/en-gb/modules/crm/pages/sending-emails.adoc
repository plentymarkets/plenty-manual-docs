...
icon:map-signs[] *_Or:_* Create a new folder (icon:create_new_folder[set=material]).
. Click on *Upload file* (icon:file_upload[set=material]) or use drag-and-drop. +
→ The logo is uploaded.
. In the line of the image, click on *Copy to clipboard* (icon:content_copy[set=material]) to copy the image URL.
...
You can send coupons and coupon codes via email. Simply attach them to emails as PDF files. For further information, refer to the chapters xref:orders:coupons.adoc#send-email-coupon-codes[Inserting coupon codes directly into emails] and xref:orders:coupons.adoc#set-up-coupon-template[Configuring coupon templates] on the xref:orders:coupons.adoc#[Coupons] page of the manual.
