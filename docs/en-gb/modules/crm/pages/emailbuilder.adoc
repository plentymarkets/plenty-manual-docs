...
[#code-example-item-table]
=== Code example: Items of the order shown as table

In the <<#intable-widget-code, code widget>>, insert a code that causes the item IDs and quantities of all items that are contained in the order to be shown in a table.

Click on link:https://raw.githubusercontent.com/plentymarkets/plenty-manual-docs/main/docs/en-gb/modules/crm/examples/emailbuilder-code-example-item-table.txt[this link^] to see the code example.
