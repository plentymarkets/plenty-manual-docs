...
:lang: en
:keywords: EmailBuilder, E-Mail Builder, E-Mail templates, email templates, Widgets, attach documents email, document attachment email, construction kit email templates, email variables, e-mail variables, twig expressions, email despatch, code widget
...
The header, the footer and the settings in the template such as the name, the subject etc. form the framework of your email, so to say. The content of the template is formed with xref:crm:emailbuilder-testphase.adoc#variables-emailbuilder[variables]. In the emails that are sent to your customer, the variables will then be replaced with the correct content. For example: The variable `Delivery address First name and last name` will be replaced in the email with the first name and the last name of the contact from the delivery address.
...
====
Go to *Setup » Client » [Select client] » Email » Newsletter*. Enter the login details to use the newsletter service that is integrated in plentymarkets. For further information, refer to the xref:crm:sending-newsletters.adoc#[Newsletters] page of the manual.
====
...
To see which variables are available for your email templates, refer to the lists on the bottom of this page in the <<#variables-emailbuilder, Available variables and twig expressions for the code widget>> chapter.
...
The email templates use the multipart format. This means that the emails are sent in an HTML and in a text version at the same time.
...
|This drop-down list provides all xref:orders:order-documents.adoc#100[location-based documents] that you set up in the *Setup » Client » [Select client] » Locations » [Select location] » Documents* menu. Select (icon:check-square[role="blue"]) up to 3 dynamic attachments from the drop-down list.
...
*_Background colour:_* Enter `background-color:` in this field followed by the desired link:https://htmlcolorcodes.com/[HTML colour code^]{nbsp}icon:external-link[], e.g. `background-color:&#35;A9F5D0;`. +
*_Font colour:_* Enter `color:` in this field followed by the desired link:https://htmlcolorcodes.com/[HTML colour code^]{nbsp}icon:external-link[], e.g. `color:&#35;A0A1B;`. +
...
*_Tip:_* Use the *Preview HTML* (icon:preview[set=material] in the EmailBuilder to check your entries and to make further adjustments, if needed.
...
|In this widget, you can enter text and use variables. Note that you can only use variables from the sub-category <<#variables-order-order-items, Order / Order items>> in this widget.
...
| Use this widget to upload images such as your company logo to show it in your email templates. Click on icon:settings[set=material] to open the settings. +
You can upload the image, enter the width and height of the image in px or %, enter an alternative text, adjust the styling in CSS format and save the image link. +
*_Note:_* Always use `https` for the image link. +
*_Tip:_* If you want to place the image to a certain position, you can adjust the CSS styling in the field *Custom styling*, e.g. `margin-left: auto; margin-right: 0; display: block;` or `float:right;display:block;`. Further information can be found link:https://www.w3schools.com/css/css3_images.asp[here^]{nbsp}icon:external-link[] and link:https://www.caniemail.com/[here^]{nbsp}icon:external-link[].
...
== Available variables and twig expressions for the code widget and the subject line
...
Do you want to know which variables are available for the individual categories and sub-categories and how the twig expressions are called that you can use in the code widget in the template’s subject line? Simply expand the corresponding areas below by clicking on icon:expand_more[set=material].
...
Do you want to know which variables are available for the individual categories and sub-categories and how the twig expressions are called that you can use in the code widget? Simply expand the following areas by clicking on icon:expand_more[set=material].
...
| `Number of packages`
|Returns the number of packages contained in the order.
| `order.package`
...
| `Item type`
|Returns the item types as saved in the item’s *Global* tab. Possible values: stocked item, production item, colli, special order item.
...
|First name of the person who has subscribed for the newsletter.
| `newsletter.firstName`

| `Newsletter last name`
|Last name of the person who has subscribed for the newsletter.
| `newsletter.lastName`
...
Go to *Setup » Orders » Events* to set up an xref:automation:event-procedures.adoc#[event procedure] that will send an email to your customers and employees when the event that you determined occurs.
...
Click on link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/crm/assets/emailbuilder-code-example-shipping-profile.txt[this link^]{nbsp}icon:external-link[] to see the code example.
...
Click on link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/crm/assets/emailbuilder-code-example-bank-details.txt[this link^]{nbsp}icon:external-link[] to see the code example.
...
Click on link:https://raw.githubusercontent.com/plentymarkets/manual/master/en/crm/assets/emailbuilder-code-example-order-items.txt[this link^]{nbsp}icon:external-link[] to see the code example.
