...
The header, the footer and the settings in the template such as the name, the subject etc. form the framework of your email, so to say. The content of the template is formed with <<crm/emailbuilder-testphase#insert-widget, widgets>> and <<crm/emailbuilder-testphase#variables-emailbuilder, variables>>. In the emails that are sent to your customer, the variables will then be replaced with the correct content. For example: The variable `Delivery address First name and last name` will be replaced in the email with the first name and the last name of the contact from the delivery address.
...
*_Note:_* To see the entire list of the order types and their IDs, refer to this <<data/internal-IDs#80, page of the manual>>.
| `order.typeId`
