...
You can create as many <<#create-folder, folders>> and <<#create-sub-folder, sub-folders>> as you like in the EmailBuilder. After you created a folder, you can create new sub-folders, new templates in general and new header and footer templates by clicking on material:more_vert[].
...
. Click in the bottom left on material:add[] *New main folder*. +
...
[TIP]
Once you have carried out all settings as described in the following – so you have created sub-folders, templates, headers and footers –, you can open the main folder by clicking on material:chevron_right[]. You will see there at a glance all sub-folders, templates, headers and footers that you created directly in the main folder.
...
→ A window opens.
. Click on material:add[] *New sub-folder*. +
→ The *Create new folder* window opens.
...
. Go to *CRM » EmailBuilder (Test phase)*.
. In the line of the folder or sub-folder in which you want to create a header, click on the context menu (material:more_vert[]). +
→ A window opens.
. Click on material:add[] *New header*.
...
→ A window opens.
. Click on material:add[] *New footer*. +
→ A new view opens.
...
====
If you later edit multiple templates in the editor at the same time, you can use the navigation on the left to switch between the individual templates. By clicking on material:account_tree[] *Configuration*, you will be routed directly to the folder tree. The template that you have last edited will be opened there automatically. Click on material:close[] to close the templates.
====
...
2+^|Click on material:edit[] to edit the content of the template and to use <<#insert-widget, widgets>> and <<#variables-emailbuilder, variables>>. +
...
|This drop-down list provides all attachments that you uploaded before in the *CMS » Documents* menu. These are, for example, installation guides or product data sheets. Select (material:check_box[role=skyBlue]) up to 3 static attachments from the drop-down list.
...
*_Tip:_* Save the settings. Afterwards, the button *Preview* (material:preview[]) is visible. Use the preview to check your entries and to make further adjustments, if needed.
...
Click on *Close settings* (icon:times[]) to close the settings. Click on *Delete widget* (material:delete[]) to remove the widget.
...
Click on material:settings[] to open the settings. Here you can select from the drop-down lists which contents should be shown. By clicking on *Add new entry* (icon:plus-square[role="green"]) in the settings, you can add further columns. By clicking on *Remove entry* (icon:minus-square[role="red"]), the columns are deleted from the overview.
...
. Click on material:edit[] or on material:settings[] and carry out the settings. Note the explanations in <<#table-available-widgets-emailbuilder>>.
. *Save* (material:save[]) the settings.
...
You want to copy an existing template to save time when creating a new template with similar content? Proceed as follows.
...
→ A window opens.
. Click on *Copy*. +
→ The template is copied and gets the suffix `(copy)`.

Now <<#edit-template, edit>> the copied template according to your needs.
[#delete-template]
=== Deleting a template
...
*_Note:_* Have a look at our xref:crm:practical-example-send-tracking-url.adoc#[practical example about this topic]. Learn which settings you need to carry out to send the tracking URL to your customers.
| `order.formattedShipping.trackingURL`

|====

[#variablen-order-subscription]
...
| `Ticket ID`
|Returns the ID of the ticket from which the email was sent.
| `ticket.id`
...
| `Ticket priority`
|Returns the priority of the ticket as an ID. The values of the stars are shown in ascending order with numbers from `0` (grey star, lowest priority) to `5` (2 yellow stars, highest priority).
| `ticket.priorityId`
...
| `Ticket URL`
|Links to the ticket in the customer's *My account* area. Customers can read the message there and directly enter and send a reply. +
