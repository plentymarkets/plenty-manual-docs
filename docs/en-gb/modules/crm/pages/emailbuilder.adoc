...
*_Note:_* If you have already created "old" templates in the *Setup » Client » [Select client] » Email » Templates* menu, you can use this field to search for the template’s old ID. During the migration, the old ID was written in brackets behind the name.
...
The *Email accounts* assistant contains various events that you can link with a template. In the *Automatic despatch* step, select from the drop-down list the template that you <<#create-template, created>> before in the *CRM » EmailBuilder (Test phase)* menu. This ensures that the email template is sent to your contacts once the event occurs.

[IMPORTANT]
.Removing the link before deleting the email template
====
Deleting an email template that is linked with an event in the *Automatic despatch* step of the *Email accounts* assistant may lead to undesired effects. Therefore, first go to *Setup » Assistants » Basic setup* and open the *Email accounts* assistant. Remove the link by selecting the option *Do not send* from the drop-down list in the line of the event in the *Automatic despatch* step. Afterwards, delete the template in the *CRM » EmailBuilder (Test phase)* menu.
====
...
. In the *Automatic despatch* step, select a template from the drop-down list in the line of the event. Note <<#table-events-automatic-email-despatch>>.
