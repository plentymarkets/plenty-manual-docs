...
You can also use the quick search to display all data that is saved for a contact including addresses, orders, notes and tickets. How to proceed is described on the xref:crm:quick-search.adoc#[Quick search] page in the xref:crm:quick-search.adoc#search-data-record[Searching for a data record] chapter.
...
The following sub-chapters describe how to xref:crm:managing-contacts.adoc#960[create], xref:crm:managing-contacts.adoc#970[link], <<#search-properties, search for >> and <<#delete-properties, delete>> properties and how to <<#group-properties, group>> as well as <<#import-properties, import>> and <<#export-properties, export>> them.
...
====
Click on *New order (beta)* to generate orders with the new logic. For further information, refer to the xref:orders:manually-create-orders.adoc#create-order-via-contact[Creating an order or an offer via contacts] chapter on the xref:orders:manually-create-orders.adoc#[Manually creating orders] page.
====
...
Refer to the xref:orders:manually-create-orders.adoc#create-order-via-contact[Creating an order or an offer via contacts] chapter to learn how to create an individual offer for your contact.
...
*_Important:_* This option needs to be selected before pain.008 formats can be retrieved for xref:payment:managing-bank-details.adoc#220[SEPA payments].
