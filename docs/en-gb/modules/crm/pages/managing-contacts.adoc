...
Go to *Setup » CRM* to carry out the preparatory settings such as creating xref:crm:managing-contacts.adoc#15[customer classes] and xref:crm:managing-contacts.adoc#20[types]. Furthermore, you set up xref:crm:managing-contacts.adoc#25[discounts] for your customers.
...
How to create delivery notes is described on the xref:orders:generating-delivery-notes.adoc#[Generating delivery notes] page of the manual.
...
In plentymarkets, you have various possibilities for offering discounts. You may give your customers a xref:crm:managing-contacts.adoc#26[customer class discount], a xref:crm:managing-contacts.adoc#28[discount based on the method of payment] or a xref:crm:managing-contacts.adoc#27[discount scale based on the net value of items]. Furthermore, you can xref:crm:managing-contacts.adoc#29[save sales prices as discounts] and xref:crm:managing-contacts.adoc#30[manually enter discounts]. Go to *Setup » CRM » Customer classes* to carry out the settings for the types of discounts. The following sub-chapters describe which settings you need to carry out in order to give discounts.
...
You find information about how to create customer classes in the xref:crm:managing-contacts.adoc#15[Creating a customer class] chapter.
...
====
Instead of configuring the settings in this menu, it is also possible to directly save a global discount for a payment method. An example is a discount in percent for the payment method *Cash in advance*. For further information, refer to the xref:payment:managing-payment-methods.adoc#20[Managing payment methods] page of the manual. You are not able to completely customise the settings if you save a discount directly for a payment method. It is only possible to completely customise the settings when using the option *Discount based on the method of payment* in the *Setup » CRM » Customer classes* menu.
====
...
For further information about linking a contact to a company at a later point in time and creating a new company, refer to the xref:crm:managing-contacts.adoc#210[Linking an existing contact to a company] and xref:crm:managing-contacts.adoc#220[Creating a company] chapters.

[#200]
=== Searching for a contact

In the *Filter* view, you can search for contacts according to specific criteria. The contacts found are listed in the *Overview* view. The overview currently displays the following information: ID, guest, actions (icon:bars[]), company, rating, first name, last name, email address, telephone number, customer class, number of orders, postcode and country. How you edit contact data records is described in the xref:crm:managing-contacts.adoc#300[Editing a contact] chapter.

You can also use the quick search to display all data that is saved for a contact including addresses, orders, notes and tickets. How to proceed is described on the xref:crm:quick-search.adoc#[Quick search] page in the xref:crm:quick-search.adoc#find-data-records[Finding data records via the quick search] chapter.
...
*_Important:_* Back end users can only select those clients that they have a right to access. Go to *Setup » Settings » User » Accounts* and set the rights in the user account in the xref:business-decisions:user-accounts-access.adoc#105[Rights] tab.
...
Search the contact by means of the xref:crm:managing-contacts.adoc#200[filter settings]. Open the contact by clicking in the line of the contact to edit the contact's data such as the date of birth, telephone number and email address, for example, or to add further information. Click on *Delete contact option* (icon:minus-square[role="red"]) to delete the contact option.
...
|Select a type. The *Customer*, *Sales representative*, *Supplier*, *Partners*, *Manufacturers* and *Interested party* types can be selected by default. Create xref:crm:managing-contacts.adoc#20[further types] in the *Setup » CRM » Types* menu. +
...
Necessary settings: +
- The payment method xref:payment:invoice.adoc#[Invoice] has to be configured in your plentymarkets system so it is available in the order. +
...
icon:map-signs[] *_Or:_* Click into the field and enter the first 3 letters of the tag name. For further information about tags, refer to the xref:crm:managing-contacts.adoc#150[Creating tags for contacts] chapter.

2+|[#intable-edit-contact-account]*Company*

|[#intable-edit-contact-account-name]*Name*
|Enter the name of the xref:crm:managing-contacts.adoc#220[created company] in the suggestion list to assign this company to the contact. Or click into the field and enter the first 3 letters of the company name. +
...
How to create a new company is described in the xref:crm:managing-contacts.adoc#220[Creating a company] chapter. How you edit an existing company is described in the xref:crm:managing-contacts.adoc#230[Editing a company] chapter.
...
After having entered the VAT number here, go to *Setup » Client » [Select client] » Locations » [Select location] » Accounting* and select the setting xref:orders:accounting.adoc#400[Net invoice] for the option *VAT number*. This causes invoices to be issued without VAT for contacts who have a VAT number saved in their data record. +
...
For further information, refer to the xref:crm:managing-contacts.adoc#500[Showing the history] chapter.
...
For further information about data cleansing, refer to the xref:data:data-cleansing.adoc#[Data cleansing] page of the manual.
====
...
. Go to *CRM » Contacts*.
. Search (icon:search[]) the contact(s) by means of the filter settings as described in the xref:crm:managing-contacts.adoc#200[Searching for a contact] chapter. +
→ The contacts that correspond to the search criteria entered are displayed.
...
*_Important:_* The email template has to be xref:crm:sending-emails.adoc#1200[created] in the *Setup » Client » [Select client] » Email » Templates* menu in advance.
...
This chapter explains how to add addresses to an already existing contact data record. How to add an address directly when creating a new contact data record is described in the xref:crm:managing-contacts.adoc#100[Creating a contact] chapter.
...
You can edit addresses as well as address options at any time. Note the explanations in the xref:crm:managing-contacts.adoc#600[Adding an address] chapter. Click on *Delete address option* (icon:minus-square[role="red"]) to delete the address options.
...
. Carry out the desired changes. Note <<#table-add-address>> in the xref:crm:managing-contacts.adoc#700[Adding an address] chapter and the information about the toolbar given in <<#table-edit-address>>.
. *Save* (icon:save[role="green"]) the settings.
...
The following sub-chapters describe how to xref:crm:managing-contacts.adoc#960[create], xref:crm:managing-contacts.adoc#970[link], <<#search-property, search for >> and <<#delete-property, delete>> properties and how to <<#group-properties, group>> as well as <<#import-properties, import>> and <<#export-properties, export>> them.
...
====
Click on *New order (beta)* to generate orders with the new logic. For further information, refer to the xref:orders:managing-orders.adoc#245[beta: Creating an order or an offer in the contact overview] chapter on the xref:orders:managing-orders.adoc[Managing orders] page.
====
...
You can also edit the order. Orders can be edited as long as no document has been generated for this order. Further information about orders can be found on the xref:orders:managing-orders.adoc#[Managing orders] page of the manual.
...
*_Note:_* If an invoice address and/or delivery address were defined as xref:crm:managing-contacts.adoc#600[primary], these addresses are automatically transferred to the new order.
. Carry out the desired settings. Note the information on the xref:orders:managing-orders.adoc[Managing orders] page.
. *Save* (icon:save[role="green"]) the settings.
...
Refer to the xref:orders:managing-orders.adoc#260[beta: Creating an order or an offer in the contact overview] chapter to learn how to create an individual offer for your contact.

[#1107]
=== Creating a subscription

Refer to the xref:orders:subscription.adoc[Subscription] page to learn how to create a subscription for your contact.
...
====
Click on *New subscription (beta)* to use the new subscription module. For further information, refer to the xref:orders:subscription.adoc[Subscription] page.
====
...
For further information, refer to the xref:orders:scheduler.adoc#[Scheduler] page of the manual. The filter options are also described there. You can use the filter options both in the open contact data record in the *Scheduler* area and in the *Orders » Scheduler » Scheduled orders* menu.
...
Use the xref:crm:using-the-ticket-system.adoc#[ticket system] in plentymarkets to manage your customer communication. The following chapter explains how you can create a new ticket directly from the open contact.
...
. Carry out the desired settings. Pay attention to the information given on the xref:crm:using-the-ticket-system.adoc#1600[Ticket system] page of the manual.
. *Save* (icon:save[role="green"]) the settings.
...
You have the possibility to edit and delete tickets. For further information, refer to the xref:crm:using-the-ticket-system.adoc#[Ticket system] page of the manual.
...
The revenues of the contact are displayed in detail above the table. Apart from *Balance* also information about invoices and credit notes. Note <<#table-amounts-contact>>. You can also go directly to the xref:crm:ar-list.adoc#[AR list] by clicking on *Open AR list* (icon:open_external_link[]).
...
*_Important:_* This option needs to be selected before pain.008 formats can be retrieved for xref:payment:managing-bank-details.adoc#200[SEPA payments].
...
For further information about sending emails automatically, refer to the xref:crm:sending-emails.adoc#2300[Emails] page of the manual.
...
Create contact-specific statistics in the *Statistics* area. This allows you to analyse data related to the contact and display this data as a statistic. For example, you could create a statistic that displays the contact's total revenue development during a particular time period. You can also edit existing statistics in this area. For further information, refer to the xref:business-decisions:statistics.adoc#70[Customer statistics] chapter.
...
For further information, refer to the page of the manual xref:data:change-history.adoc#[Change history].
...
Import or update the data of your contacts including address options, contact options and company data via the import tool. For further information, refer to the xref:data:ElasticSync.adoc#[Using the import tool] page.
