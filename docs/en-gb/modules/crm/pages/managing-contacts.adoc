...
Go to *Setup » CRM* to carry out the preparatory settings such as creating xref:crm:managing-contacts.adoc#25[discounts] for your customers.
...
The terms of payment are saved in the xref:orders:managing-contacts.adoc#300[Editing a contact] chapter. You can find an example regarding the terms of payment there.
...
Not only is it possible to automatically assign discounts, but it is also possible to manually assign a discount to some or all of the variations contained in an order. There are 2 ways to manually enter discounts: in the list view of the xref:orders:managing-orders.adoc#1530[order].
...
For further information about linking a contact to a company at a later point in time and creating a new company, refer to the xref:crm:managing-contacts.adoc#220[Creating a company] chapters.
...
You can also use the quick search to display all data that is saved for a contact including addresses, orders, notes and tickets. How to proceed is described on the xref:crm:quick-search.adoc#find-data-records[Finding data records via the quick search] chapter.
...
How to create a new company is described in the xref:crm:managing-contacts.adoc#230[Editing a company] chapter.
...
Further information about converting plentyShop guest orders into regular contact data records can be found on the xref:crm:quick-search.adoc#convert-guest-account[Converting a guest account into a contact data record] chapter.
...
In this case, you have, however, the possibility to anonymise the data record. How to proceed is described on the xref:crm:quick-search.adoc#anonymise-data-record[Anonymising a data record] chapter.
====
...
The following sub-chapters describe how to xref:crm:managing-contacts.adoc#970[link>>, <<#search-property, search for >> and <<#delete-property, delete>> properties and how to <<#group-properties, group>> as well as <<#import-properties, import>> and <<#export-properties, export] them.
...
====
Click on *New order (beta)* to generate orders with the new logic. For further information, refer to the xref:orders:managing-orders.adoc[Managing orders] page.
====
...
For further information, refer to the xref:orders:managing-orders.adoc#[Managing orders] page of the manual.
