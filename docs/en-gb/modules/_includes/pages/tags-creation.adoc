...
. Click on icon:add[set=material, role=darkGrey]. +
...
Click on icon:square[set=material] and choose the colour.
...
Select all of the appropriate availabilities (icon:check_box[set=material, role=skyBlue]).
...
Select all of the appropriate webshops (icon:check_box[set=material, role=skyBlue]).
...
. Click on icon:add[set=material, role=darkGrey] and select a language version from the drop-down list.
