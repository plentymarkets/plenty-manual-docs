...
Users with roles that are not of the type *Administrator* xref:business-decisions:user-accounts-access.adoc#105[can only see certain areas and menus] in plentymarkets. This means they only have limited access to the system. Should coworker {rights-purpose} they need at least one of the following rights:
...
. Make sure that you are using the simple mode. This means that the option *Advanced* (material:toggle_off[role=darkGrey]) is *_not_* activated.
