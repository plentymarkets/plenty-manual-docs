...
. Go to *{menu-path}*.
.. *_Option 1:_* Click on material:search[] to see a list of all {search-for-plural}.
.. *_Option 2:_* Enter {value} in the search field and select the appropriate filter from the suggestion list.
Click on material:search[] to carry out the search.
.. *_Option 3_*: Click on material:tune[] to narrow down the search results with filters. Note the explanations about the filters in {table-filter}. +
