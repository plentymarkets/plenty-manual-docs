...
This manual page describes how to import data using the import tool. Importing data is recommended if you want to add new items, customer data or stock etc. to your plentymarkets system and you already have this data available in a digital format. You can import data manually or automate the process.
...
. Click on *Add import* (icon:add[set=material]). +
→ The editing area *New import* opens.
. Carry out the settings. Pay attention to the explanations given in <<tabelle-sync-erstellen>>. +
...
* xref:data:elasticSync-manufacturers.adoc#[Manufacturers] = Imports manufacturer data
...
icon:check_box_outline_blank[set=material] = All data of the file is reimported during the import process. +
icon:check_box[set=material] = Only changed records are imported.
...
! *Manual upload*
!
. In the field *File name*, click on *Select file* (icon:more_horiz[set=material]). +
→ The files saved in your webspace are shown.
. Click on *Upload file* (icon:file_upload[set=material]). +
. Select a local file. +
...
! *FTP*
! *_From 21.07.2022, plentymarkets will no longer support FTP servers. As such, do not select this option._*
...
. Determine the import options. Pay attention to the explanations given in <<tabelle-Sync-Importoptionen>>.
...
* *Import new data, update existing* = If data can be found by means of the matching fields, a new data record is added, for example a new item or a new order. Existing records are updated.
* *Only update existing data* = Existing records are updated. If no record can be found based on the matching fields, the record is not imported.
* *Only import new data* = Only new records are imported, i.e. any records for which no matching data is found based on the matching fields.

| *Decimal number*
| How are decimal numbers formatted in the import files?

* *German spelling* = Decimal numbers are formatted with commas. A period is used as thousands separator. +
  *_Examples:_* 13,95 Pound, 15.000 pieces
* *English spelling* = Decimal numbers are formatted with periods. A comma is used as thousands separator. +
...
====
Matching fields are optional settings. However, if you do not select a matching field, the data in your file cannot be assigned to existing data in your system. So new records are saved during the import. This can result in duplicate records in your system.
====
...
*_Tip:_* Every xref::page$import-types.adoc#[import type has different matching fields].
. Save (icon:save[set=material]) the settings.
...
Mappings allow you to specify which columns of your file are imported to which plentymarkets data fields. So without any mappings, no data can be imported. You can create new mappings in the view *Mapping*. A position is automatically assigned to each new mapping. You can change this position in the view *Settings*.

[#understand-mappings]
=== Understanding mappings

*_What is a mapping?_*

With your mappings, you specify what data of the import file is saved in which plentymarkets data fields.

As such, you "map" the sources of the import file to the plentymarkets data fields in which you want to save the data.

You do not need to map all sources of the import file. Instead, only map the sources that you want to import.

*_What is a source?_*

In CSV files, each column is a source. TXT files are structured differently. However, for simplicity think of the data in TXT files also as columns.

image::daten:import-mapping-colour.png[]

*_Workflow:_*

. Create the mapping. (<<#create-mappings, How?>>)
...
. Click on *Add mapping* (icon:add[set=material, role=green]). +
→ The window *New mapping* opens.
. Enter a name for the mapping.
. Save the setting. +
→ The mapping is created and opened. +
→ New mappings are active automatically.
...
* The column *Import data* is a list of all columns in the import file.
* The column *Preview* shows the first 6 values in each column of the file.
...
. Save (icon:save[set=material]) the mappings.
...
. Click on icon:add[set=material] *FIELD*. +
→ A new line is added. +
...
. Save (icon:save[set=material]) the mapping.
...
====
Regular expressions are a powerful tool. However, they can also be tricky. As such, use regular expressions carefully and only if you are confident that they will work as expected. Some examples for resources are:
...
* link:https://regexr.com/[https://regexr.com/^] +
  On this page, you need to change the RegEx engine setting to PCRE first. To do so, use the drop-down list at the top right. This is preset to *Javascript*.
====
...
* icon:toggle_off[set=material, role="darkGrey"] = The source is inactive. The source is ignored during the export. +
* icon:toggle_on[set=material, role="blue"] = The source is active. During the import, the data of this source is imported.
. Save (icon:save[set=material, role="green"]) the settings.
--
...
* icon:toggle_off[set=material, role="darkGrey"] = The mapping is inactive. During the import, the settings of this mapping are not applied. +
* icon:toggle_on[set=material, role="blue"] = The mapping is active. During the import, the settings of this mapping are applied. +
*_Important:_* If more than one mapping is active, the mapping with the lowest ID is imported first. All other active mappings are also imported in ascending order based on the position.
...
. Click on icon:add[set=material] *Filter*. +
...
. If errors occur and you need help in the forum:
  * Click on *Copy log to clipboard* (icon:content_copy[set=material]).
...
In the menu *Data » Status*, you can cancel an import that has already started. That means that this function is only available while the import is being processed.

*_Important:_* When you cancel a running import, the records that were already imported are not deleted. If necessary, you need to delete these records yourself.
...
. At the very right in the line of the import that you want to cancel, click on *Cancel import* (icon:close[set=material]). +
...
. In the *File name* column, click on *Upload file* (icon:upload[set=material]). +
...
[#1730]
=== Restoring older/deleted imports
...
. In the row of the backup that you want to import, click on *Restore backup* (icon:open_in_browser[set=material]) on the right. +
...
. In the toolbar, click on *Copy import* (icon:content_copy[set=material]). +
...
. Click on *Export format* (icon:system_update_alt[set=material]). +
...
. Go to *Data » Import*. +
→ The import overview opens.
. Click on *Import format* (icon:open_in_browser[set=material]). +
→ A list of the files in your webspace is shown.
. Click on *Upload file* (icon:upload[set=material]).
...
. Go to *Data » Data import » Status*. +
. In the toolbar, expand the filters (icon:tune[set=material]).
. Select the filters you need.
. Click on *Search*. +
→ The list is filtered.

*_Tip:_* The filters that you applied are shown in the toolbar as individual "chips". Delete one or more of these chips to quickly delete filters. Click on *Search* (icon:search[set=material]) to update the list.
...
At the top left of the toolbar, click on *Status* (icon:playlist_play[set=material]).
. Click on the import. +
→ The detail view opens.
. At the top left of the toolbar, click on *Log* (icon:playlist_play[set=material]). +
→ The log is shown in a new window.
. *_Optional:_* Click on *Copy to clipboard* (icon:content_copy[set=material]). Add the information in the forum when you post a support request.
