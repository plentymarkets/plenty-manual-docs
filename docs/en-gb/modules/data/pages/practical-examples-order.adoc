...
Select the following xref:data:managing-catalogues.adoc#120[basic settings] when you want to create a new catalogue for the export of order data:
...
Add the following xref:data:managing-catalogues.adoc#160[data fields] to combine the listed data in one catalogue. You add a data field by clicking on *Add data field* (icon:add[set=material]). Save (icon:save[set=material]) the catalogue after you added the data fields.
...
While you're at it, also check the xref:data:managing-catalogues.adoc#150[Export settings] of your catalogue. You can use the setting *Transfer* to decide when the export is started. Note that catalogues are only saved for 15 days after exporting them. Take this into consideration if you want to use a catalogue more than once. Therefore, factor in the xref:data:managing-catalogues.adoc#260[catalogue export] into your workflow.
...
Therefore, xref:data:managing-catalogues.adoc#190[copy] the just created catalogue so that we can extend the copy for this example. +
...
If needed, change the xref:data:managing-catalogues.adoc#165[order of data fields]. In addition, you could also adapt the xref:data:managing-catalogues.adoc#162[Export keys]. Save (icon:save[set=material]) the catalogue after you added the data fields.
...
When you are done with all the changes and saving the catalogue, you can use the xref:data:managing-catalogues.adoc#255[Preview] again to check if everything is working out the way you need. After checking the settings, the catalogue is ready for xref:data:managing-catalogues.adoc#260[export].
...
Add the following xref:data:managing-catalogues.adoc#160[data fields] to combine the listed data in one catalogue. You add a data field by clicking on *Add data field* (icon:add[set=material]). +
