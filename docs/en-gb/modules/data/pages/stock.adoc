...
The data format **Stock** is used to exchange data that relates to your xref:data:stock-management.adoc#[stock]. Your current stock will be exported during the export process. The export of your stock is one step in the process of stocktaking for example. A stock correction is carried out during the import and the stock will be changed to reflect the value that was imported. Please note that item bundles do not have any stock.
