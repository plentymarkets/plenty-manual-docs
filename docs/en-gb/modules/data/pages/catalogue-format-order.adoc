...
| *Status*
| Exports the xref:orders:order-statuses.adoc#[order status] of the order at the time of the export. The automatically generated export key for this data field is *order.statusId*. +
In the export file, the value is given in form of the xref:data:internal-IDs.adoc#120[ID of the order status].

| *Status name*
| Exports the xref:orders:order-statuses.adoc#[order status] of the order at the time of the export. The automatically generated export key for this data field is *order.statusname*. +
...
* *Grouped* = When an order was created by xref:orders:working-with-orders.adoc#order-group-functions[grouping] other orders, several references of the type *Grouped* exist. These are references to the original orders. By this, you can easily see of an order was created by grouping several original orders. If it is a grouped order, the order IDs of the original orders are exported. +
* *Split* = When an order is xref:orders:working-with-orders.adoc#splitting-orders[split] into two new orders, each of the two new orders has a reference to the original order from which they were created. In this case, the order ID of the original order which was split is exported.
...
| *Round total only*
| Exports the xref:orders:accounting.adoc#150[rounding settings] for the order. +
0 = No rounding. +
1 = Rounding. +
The automatically generated export key for this data field is *orderConfig.roundTotalsOnly*.

| *Number of decimals*
| Exports the selected setting for the xref:orders:accounting.adoc#150[number of decimals for the order]. +
...
[#130]
==== Payment terms
In this subfolder data fields for xref:orders:working-with-orders.adoc#area-payment-terms[payment terms] are grouped.
...
//This revenue account is saved under *Item » Items » [Open item] » Element: Settings » Entry field: Revenue account*.
