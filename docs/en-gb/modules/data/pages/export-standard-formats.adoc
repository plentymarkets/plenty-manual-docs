...
* <<#optional-recalculate, Recalculate numerical values (icon:calculate[set=material] *Formula*)>>
...
→ The catalogue overview opens.
. Click on the catalogue. +
→ The view *Data fields* (icon:order_return_create_edit[set=plenty]) of the catalogue opens. +
...
*_Tip:_* A number tells you how many assignments are necessary for the data field.
    .. Click on the icon icon:more_vert[set=material]. +
...
→ The tab *Data fields* (icon:order_return_create_edit[set=plenty]) with all available data fields opens.
...
→ The catalogue overview opens.
. Open the catalogue.
. In the navigation on the left, click on *Filters* (icon:filter_alt[set=material]). +
→ The filter overview opens.
. Select a filter from the drop-down list. +
*_Tip:_* Different filters are available for different standard formats.
. On the right, click on *Add filter* (icon:add[set=material]). +
→ The filter is added.
