...
*_Recommendation:_* Enter the port `21`.
. Enter the *user name* for the server.
. Enter the *password* for the server.
. Enter the *path* to a storage location for the export.
. Click on *Test connection*. +
→ The server settings are checked:
...
Some examples for protocol prefixes are `ftp://` and `ftps://`.
. Enter a *port* or use the default setting. +
*_Recommendation:_* Enter the port `22`.
. Enter the *user name* for the server.
. Enter the *password* for the server.
. Enter the *path* to a storage location for the export. +
*_Note:_* Enter the complete path.
. Click on *Test connection*. +
→ The server settings are checked:
  * Can plentymarkets connect to the server?
  * Can an export file be saved at the storage location? +
...
! *Google Sheets*
! The export is saved in Google Sheets.

[.collapseBox]
.Setting up provisioning
--
. Select the option *Google Sheets*. +
→ The setting *Spreadsheet URL* is shown.
...
ifdef::marketplace-export[*_Note:_* This only tests if a file export is possible with the current export settings. The option does _not_ check if a connection to a market can be established.]

| Spreadsheet URL +
(for Google Sheets)
|

. Enter the URL of the Google Sheets file in which the export is to be saved.
. Click on *Connect with Google*. +
→ The connection is established.
...
. Click on *Download data* (material:file_download[]). +
→ The export starts. +
*_Important:_* The export runs in the background. Note that there will be _no_ window for downloading the file straight away. Even exporting small amounts of data will take around 30 minutes. +
Once the export is finished, you will be notified (material:notifications_none[]) that the export file can be downloaded.
. In the notification centre, click on material:link[] *Download*. +
