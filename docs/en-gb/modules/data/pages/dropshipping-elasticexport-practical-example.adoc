...
:description: Best Practice: How to send order data to a local shipping provider without a separate interface for data transmission.
:author: team-plenty-channel

Thanks to the combination of our plugin link:https://marketplace.plentymarkets.com/plugins/markets/ElasticExport_4763[ElasticExport^] with the xref:data:ElasticSync.adoc#[import tool] and the xref:data:FormatDesigner.adoc#[FormatDesigner], data exchange between plentymarkets and additional systems or service providers can be easily automated.
