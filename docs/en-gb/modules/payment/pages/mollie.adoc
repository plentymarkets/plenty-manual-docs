...
====
There are some parameters set by payment providers, among them usually also supported currencies. Which currencies are supported by Mollie can be found in the Mollie documentation on the page link:https://docs.mollie.com/payments/multicurrency[Multicurrency^]{nbsp}icon:external-link[].
====
...
The containers needed by default on your online shop are automatically linked during setup with the Mollie assistant. In case you want to link containers manually, proceed as follows.
...
Pay attention to the explanations given in <<table-mollie-template-container>>.
...
Event procedures are part of automations you can set up in plentymarkets. When you use event procedures, a specific event triggers an action for a selection of orders. For further information, refer to the <<automation/event-procedures#, Event procedures>> page of the manual.
...
| If you selected *Status change*, then select the <<orders/managing-orders#1200, order status>> you are using for cancelled orders from the drop-down list. By default, these are *8 - Cancelled* and *8.1 - Cancelled by customer*.
...
Make sure that the order items which are to be refunded are contained in the credit note order. You can find more information about credit note orders in general in the chapter <<orders/managing-orders#500, Working with credit note orders>> of the manual.
...
| *Procedure*
| *Order > Change status*
|Select the order status from the drop-down list that should be assigned to orders with Mollie payment methods for which an error occurred in the refund process. For example, you could create an own <<orders/managing-orders#1200, order status>> for this. This way, you detect the orders which require further processing faster.
...
| *Procedure*
| *Order > Change status*
|Select the order status from the drop-down list that should be assigned to orders with Mollie payment methods for which the shipment could not be registered with Mollie. For example, you could create an own <<orders/managing-orders#1200, order status>> for this. This way, you detect the orders which require further processing faster.
