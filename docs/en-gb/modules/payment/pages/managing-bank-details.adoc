...
. Click on *Bank data* on the left side.
...
The email template is sent by using the group function in the *CRM » Contacts* menu.
...
. Select the previously crated SEPA email template from the drop-down list.
. Click on *Execute*.

For further information, refer to the xref:crm:edit-contact.adoc#[Editing a contact] page of the manual. Specifically, refer to the chapter xref:crm:edit-contact.adoc#group-function[Carrying out the group function].
...
→ The detail view of the contact opens.
. Click on *Bank data* on the left side.
. Activate (icon:toggle_on[set=material, role=skyBlue]) *SEPA direct debit mandate*.
. *Save* (icon:save[set=material]) the settings.
