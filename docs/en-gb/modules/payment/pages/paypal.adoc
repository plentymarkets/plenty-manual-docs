...
* In case you selected new payment methods in the assistant, you need to xref:payment:papal.adoc#80 [grant permission in the customer classes]. Also check your shipping profiles and the settings for event procedures, the procedure manager and processes. Changes in document templates may also be required. All this depends on your workflows. +
Once the assistant has been finished, you can optionally set widgets in the xref:online-store:shop-builder.adoc#widgets[ShopBuider].
======
...
Also pay attention to xref:payment:paypal.adoc#50[Additional information about the PayPal PLUS Wall]. Additional information as well as restrictions are described there.
...
======
Make sure that the PayPal plugin is already activated and deployed in the xref:installing-installed-plugins.adoc#[plugin set] of the main client when you are linking your account with plentymarkets. If this is not the case, malfunctions can occur. +
...
* Check the xref:payment:paypal.adoc#intable-paypal-plus-wall[container links]. They should be created automatically by completing the assistant but can also be set manually.
